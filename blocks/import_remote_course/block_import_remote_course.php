    ...
    public function get_content() {
        ...
        if (!$coursetemplate) {
            ...
            if (count(get_fast_modinfo($COURSE->id)->cms) <= 1) {
                ...
                $form .= '<input id="restorebutton" type="button" value="'.get_string("restore", 'block_import_remote_course').'" ' .
                    'onclick="Y.one(\'form#restoreremotecourse\').append(\''."<br>" .
                get_string('courseisnotempty', 'block_import_remote_course') . '\');Y.one(\'input#restorebutton\').remove();">';
            }
            $form .= '</form>';
            $form .= '<a id="importsite" target="_blank" href="' . get_config('block_import_remote_course', 'testenv') . '" class="btn">' .
            get_string('trytemplates', 'block_import_remote_course') . '</a>';
            ...
            $context->newitems = get_string('newact', 'block_import_remote_course',
                    notification_helper::count_records_select("type = ? and courseid= ? ", array('new', $COURSE->id)));
            $context->updatesstr = get_string('updateact', 'block_import_remote_course',
