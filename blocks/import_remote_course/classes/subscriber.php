...
class subscriber {
    ...
    public static function  subscribe () {
        ...
        $params = array('username' => $remoteusername, 'name' => $name,
                'url' => $url, 'user' => $local_user, 'token' => $local_token );
            ...
            case 'ua':
                ...
                foreach ($coursewithtamplayte as $course) {
                    ...
                    if ($existasnew = notification_helper::get_records_select($sql,
                            ['courseid' => (int)$course->course_id, 'cm' => (int)$cm, 'type' => 'new'])) {
    ...
    /**
     * remove ctivity backup from the server.
     *
     * @param int $cmid -  moudle cmid id.
     * @return void
     * @throws dml_exception A DML specific exception is thrown for any errors.
     * */
