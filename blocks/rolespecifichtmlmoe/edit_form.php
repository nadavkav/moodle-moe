...
class block_rolespecifichtmlmoe_edit_form extends block_edit_form {
    ...
    protected function specific_definition($mform) {
        ...
        $mform->addElement('text', 'config_title', get_string('configtitle', 'block_rolespecifichtmlmoe'));
        $mform->setType('config_title', PARAM_MULTILANG);

        $contextopts['course'] = get_string('course', 'block_rolespecifichtmlmoe');
        $contextopts['system'] = get_string('system', 'block_rolespecifichtmlmoe');
        $mform->addElement('select', 'config_context', get_string('configcontext', 'block_rolespecifichtmlmoe'), $contextopts);
        $mform->setType('config_context', PARAM_TEXT);
        $mform->setDefault('config_context', 'course');
        $mform->addHelpButton('config_context', 'context', 'block_rolespecifichtmlmoe');

        $displayopts['allmatches'] = get_string('showallmatches', 'block_rolespecifichtmlmoe');
        $displayopts['highest'] = get_string('showhighest', 'block_rolespecifichtmlmoe');
        $mform->addElement('select', 'config_display', get_string('configdisplay', 'block_rolespecifichtmlmoe'), $displayopts);
        $mform->setDefault('config_display', 'highest');

        $editoroptions = array('maxfiles' => EDITOR_UNLIMITED_FILES, 'noclean' => true, 'context' => $this->block->context);
        $mform->addElement('editor', 'config_text_all', get_string('configcontentforall', 'block_rolespecifichtmlmoe'), null, $editoroptions);
        ...
        */
        ...
        foreach ($roles as $r) {
            ...
            $mform->addElement('editor', 'config_text_'.$r->id, get_string('configcontent', 'block_rolespecifichtmlmoe', $r->localname), null, $editoroptions);
    ...
    function set_data($defaults, &$files = null) {
        ...
        if (!empty($this->block->config) && is_object($this->block->config)) {
            ...
            $defaults->config_text_all['text'] = file_prepare_draft_area($draftid_editor, $this->block->context->id, 'block_rolespecifichtmlmoe', 'content', 0, array('subdirs' => true), $currenttext);
            ...
            if (!empty($roles)) {
                foreach ($roles as $r) {
                    ...
                    $defaults->{$configtextvar}['text'] = file_prepare_draft_area($draftid_editor, $this->block->context->id, 'block_rolespecifichtmlmoe', 'content', 0, array('subdirs' => true), $currenttext);
