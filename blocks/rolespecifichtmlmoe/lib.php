...
function block_rolespecifichtmlmoe_pluginfile($course, $birecord_or_cm, $context, $filearea, $args, $forcedownload) {
    global $SCRIPT;
    ...
    if (!$file = $fs->get_file($context->id, 'block_rolespecifichtmlmoe', 'content', 0, $filepath, $filename) or $file->is_directory()) {
        send_file_not_found();
    }
 ...
 * @return void
 */
function block_rolespecifichtmlmoe_global_db_replace($search, $replace) {
    global $DB;

    $instances = $DB->get_recordset('block_instances', array('blockname' => 'rolespecifichtmlmoe'));
