...
$PAGE->set_url('/course/format/moetabs/duplicate.php', array('courseid' => $courseid, 'section' => $section));
...
if (!empty($sectioninfo)) {
    ...
    if (is_object($modinfo) && isset($modinfo->sections[$section])) {
        ...
        if (is_array($section_mods)) {
            ...
            $k = 0;
            $pbar->update_full(40, get_string('progress_counter', 'format_moetabs', $data_progress));
            foreach ($section_mods as $modnumber) {
                ...
                $pbar->update_full($percent, get_string('progress_counter', 'format_moetabs', $data_progress));
            }
        }
    }
    else {
