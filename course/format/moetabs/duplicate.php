...
$PAGE->set_url('/course/format/moetabs/duplicate.php', array(
    'courseid' => $courseid,
    'section' => $section
));
...
$PAGE->set_title(get_string('coursetitle', 'moodle', array(
...
if (! empty($sectioninfo)) {
    ...
    $pbar = new progress_bar('moetabs_duplicate_bar', 500, true);
    $pbar->update_full(1, get_string('duplicating', 'format_moetabs'));

    $courseformat = course_get_format($course);

    $lastsectionnum = $DB->get_field('course_sections', 'MAX(section)', array(
        'course' => $courseid
    ), MUST_EXIST);

    $numnewsection = $lastsectionnum + 1;

    if (! $courseformat->update_course_format_options(array(
        'numsections' => $numnewsection
    ))) {
        print_error('cantcreatesection', 'error', null, $course->fullname);
        return;
    }

    $pbar->update_full(5, get_string('creating_section', 'format_moetabs'));
    ...
    $event = \core\event\course_section_updated::create(array(
        ...
        'context' => $context,
        'other' => array(
            'sectionnum' => $numnewsection
        )
    ));
    $event->trigger();

    $pbar->update_full(10, get_string('rebuild_course_cache', 'format_moetabs'));
    ...
    if (is_object($modinfo) && isset($modinfo->sections[$section])) {
        ...
        if (is_array($sectionmods)) {
            ...
            $k = 0;
            $pbar->update_full(40, get_string('progress_counter', 'format_moetabs', $dataprogress));
            foreach ($sectionmods as $modnumber) {
                ...
                $pbar->update_full($percent, get_string('progress_counter', 'format_moetabs', $dataprogress));
            }
        }
    } else {
        $pbar->update_full(100, get_string('progress_full', 'format_moetabs'));
    }

    $sectiontogo = $numnewsection;
} else {
    $sectiontogo = $section;
    echo get_string('error_nosectioninfo', 'format_moetabs');
