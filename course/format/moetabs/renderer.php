...
class format_moetabs_renderer extends format_section_renderer_base {
    ...
    public function print_single_section_page($course, $sections, $mods, $modnames, $modnamesused, $displaysection) {
        ...
        echo html_writer::start_tag('div', array('class' => 'single-section moetabs'));
                        ...
                        if($format_options['level'] == 0 || count($tabs) == 0) {
                            ...
                            if (!is_array($tabs[$parent_index]->subtree)) {
                                ...
                                if ($parentformat_options['firsttabtext']) {
                                    ...
                                    $firsttab_text = get_string('index', 'format_moetabs');
                                }
        ...
        if ($PAGE->user_is_editing() && has_capability('moodle/course:update', $context)) {
            ...
            if (isset($USER->moetabs_da[$course->id]) && $USER->moetabs_da[$course->id]) {
                ...
                $text_button_disableajax = get_string('enable', 'format_moetabs');
            }
            else {
                $url->param('moetabs_da', 1);
                $text_button_disableajax = get_string('disable', 'format_moetabs');
            }
            ...
            if (has_capability('moodle/course:manageactivities', $context)) {
                $url_duplicate = new moodle_url('/course/format/moetabs/duplicate.php', array('courseid' => $course->id, 'section' => $displaysection, 'sesskey' => sesskey()));

                $link = new action_link($url_duplicate, get_string('duplicate', 'format_moetabs'));
