...
$courseformat->course_create_sections_if_missing($course, range(0, $course->numsections));
...
if ((!empty($displaysection)) && ($course->coursedisplay == COURSE_DISPLAY_MULTIPAGE)) {
    ...
    $defaulttogglepersistence = clean_param(get_config('format_moetopcoll', 'defaulttogglepersistence'), PARAM_INT);
    ...
    $defaultuserpreference = clean_param(get_config('format_moetopcoll', 'defaultuserpreference'), PARAM_INT);
    $renderer->set_default_user_preference($defaultuserpreference);

    $PAGE->requires->js_init_call('M.format_moetopcoll.init', array(
        $course->id,
        $userpreference,
        $course->numsections,
    ...
    $topcollsidewidth = get_string('topcollsidewidthlang', 'format_moetopcoll');
    ...
    $tcborderradiustl = clean_param(get_config('format_moetopcoll', 'defaulttoggleborderradiustl'), PARAM_TEXT);
    $tcborderradiustr = clean_param(get_config('format_moetopcoll', 'defaulttoggleborderradiustr'), PARAM_TEXT);
    $tcborderradiusbr = clean_param(get_config('format_moetopcoll', 'defaulttoggleborderradiusbr'), PARAM_TEXT);
    $tcborderradiusbl = clean_param(get_config('format_moetopcoll', 'defaulttoggleborderradiusbl'), PARAM_TEXT);
    echo '.course-content ul.ctopics li.section .content .toggle, .course-content ul.ctopics li.section .content.sectionhidden {';
...
$PAGE->requires->js('/course/format/moetopcoll/format.js');
