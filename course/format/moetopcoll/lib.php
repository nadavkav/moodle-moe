...
class format_moetopcoll extends format_base {
    ...
    public function get_section_name($section) {
        ...
        return $this->get_moetopcoll_section_name($course, $section, false);
    }
     ...
     * @return string The section name.
     */
    public function get_moetopcoll_section_name($course, $section, $additional) {
        ...
        if ((string) $thesection->name !== '') {
            ...
            $o = get_string('section0name', 'format_moetopcoll');
        } else {
            if (($tcsettings['layoutstructure'] == 1) || ($tcsettings['layoutstructure'] == 4)) {
                $o = get_string('sectionname', 'format_moetopcoll') . ' ' . $thesection->section;
            } else {
                $o .= $this->get_section_dates($section, $course, $tcsettings);
            }
        }
        ...
        if (($additional == true) && ($thesection->section != 0)) {
            switch ($tcsettings['layoutelement']) {
                ...
                case 4:
                    // The word 'Toggle'.
                    $o .= '<div class="cttoggle"> - ' .get_string('topcolltoggle', 'format_moetopcoll') . '</div>';
                    break;
            }
        }
    ...
    public function get_section_dates($section, $course, $tcsettings) {
        ...
        if ($tcsettings['layoutstructure'] == 5) {
            $day = $this->format_moetopcoll_get_section_day($section, $course);

            $weekday = userdate($day, $dateformat);
            $o = $weekday;
        } else {
            $dates = $this->format_moetopcoll_get_section_dates($section, $course);
    ...
    public function ajax_section_move() {
        ...
        if ($sections = $modinfo->get_section_info_all()) {
            foreach ($sections as $number => $section) {
                $titles[$number] = $this->get_moetopcoll_section_name($course, $section, true);
    ...
    public function section_format_options($foreditform = false) {
        ...
        if ($foreditform && !isset($sectionformatoptions['donotshowdate']['label'])) {
            $sectionformatoptionsedit = array(
                'donotshowdate' => array(
                    'label' => new lang_string('donotshowdate', 'format_moetopcoll'),
                    'help' => 'donotshowdate',
                    'help_component' => 'format_moetopcoll',
    ...
    public function course_format_options($foreditform = false) {
        ...
        if ($courseformatoptions === false) {
            ...
            $defaulttgfgcolour = get_config('format_moetopcoll', 'defaulttgfgcolour');
            if ($defaulttgfgcolour[0] == '#') {
                $defaulttgfgcolour = substr($defaulttgfgcolour, 1);
            }
            $defaulttgfghvrcolour = get_config('format_moetopcoll', 'defaulttgfghvrcolour');
            if ($defaulttgfghvrcolour[0] == '#') {
                $defaulttgfghvrcolour = substr($defaulttgfghvrcolour, 1);
            }
            $defaulttgbgcolour = get_config('format_moetopcoll', 'defaulttgbgcolour');
            if ($defaulttgbgcolour[0] == '#') {
                $defaulttgbgcolour = substr($defaulttgbgcolour, 1);
            }
            $defaulttgbghvrcolour = get_config('format_moetopcoll', 'defaulttgbghvrcolour');
            if ($defaulttgbghvrcolour[0] == '#') {
                $defaulttgbghvrcolour = substr($defaulttgbghvrcolour, 1);
            }
            $readme = new moodle_url('/course/format/moetopcoll/Readme.md');
            ...
            $courseformatoptions = array(
                ...
                'coursedisplay' => array(
                    'default' => get_config('format_moetopcoll', 'defaultcoursedisplay'),
                    'type' => PARAM_INT,
                ),
                'displayinstructions' => array(
                    'default' => get_config('format_moetopcoll', 'defaultdisplayinstructions'),
                    'type' => PARAM_INT,
                ),
                'layoutelement' => array(
                    'default' => get_config('format_moetopcoll', 'defaultlayoutelement'),
                    'type' => PARAM_INT,
                ),
                'layoutstructure' => array(
                    'default' => get_config('format_moetopcoll', 'defaultlayoutstructure'),
                    'type' => PARAM_INT,
                ),
                'layoutcolumns' => array(
                    'default' => get_config('format_moetopcoll', 'defaultlayoutcolumns'),
                    'type' => PARAM_INT,
                ),
                'layoutcolumnorientation' => array(
                    'default' => get_config('format_moetopcoll', 'defaultlayoutcolumnorientation'),
                    'type' => PARAM_INT,
                ),
                'togglealignment' => array(
                    'default' => get_config('format_moetopcoll', 'defaulttogglealignment'),
                    'type' => PARAM_INT,
                ),
                'toggleiconposition' => array(
                    'default' => get_config('format_moetopcoll', 'defaulttoggleiconposition'),
                    'type' => PARAM_INT,
                ),
                'toggleiconset' => array(
                    'default' => get_config('format_moetopcoll', 'defaulttoggleiconset'),
                    'type' => PARAM_ALPHA,
                ),
                'toggleallhover' => array(
                    'default' => get_config('format_moetopcoll', 'defaulttoggleallhover'),
                    'type' => PARAM_INT,
                ),
                ...
                'showsectionsummary' => array(
                    'default' => get_config('format_moetopcoll', 'defaultshowsectionsummary'),
                    'type' => PARAM_INT,
                ),
                'readme' => array(
                    'default' => get_string('readme_desc', 'format_moetopcoll', array('url' => $readme)),
                    'type' => PARAM_ALPHA,
                )
            );
        }
        if ($foreditform && !isset($courseformatoptions['coursedisplay']['label'])) {
            ...
            $courseformatoptionsedit = array(
                'numsections' => array(
                    'label' => new lang_string('numbersections', 'format_moetopcoll'),
                    'element_type' => 'select',
                ...
                'displayinstructions' => array(
                    'label' => new lang_string('displayinstructions', 'format_moetopcoll'),
            ...
            if (has_capability('format/moetopcoll:changelayout', $context)) {
                $courseformatoptionsedit['layoutelement'] = array(
                    'label' => new lang_string('setlayoutelements', 'format_moetopcoll'),
                    ...
                    'element_attributes' => array( // In insertion order and not numeric for sorting purposes.
                        array(
                            ...
                            3 => new lang_string('setlayout_toggle_word_section_x', 'format_moetopcoll'),
                            // Toggle word and section number.
                            2 => new lang_string('setlayout_toggle_word_section_number', 'format_moetopcoll'),
                            // Toggle section x and section number.
                            5 => new lang_string('setlayout_toggle_section_x_section_number', 'format_moetopcoll'),
                            // Toggle word.
                            4 => new lang_string('setlayout_toggle_word', 'format_moetopcoll'),
                            // Toggle section x.
                ...
                $courseformatoptionsedit['layoutstructure'] = array(
                    'label' => new lang_string('setlayoutstructure', 'format_moetopcoll'),
                    ...
                    'element_attributes' => array(
                        array(
                            // Topic.
                            1 => new lang_string('setlayoutstructuretopic', 'format_moetopcoll'),
                            // Week.
                            2 => new lang_string('setlayoutstructureweek', 'format_moetopcoll'),
                            // Current Week First.
                            3 => new lang_string('setlayoutstructurelatweekfirst', 'format_moetopcoll'),
                            // Current Topic First.
                            4 => new lang_string('setlayoutstructurecurrenttopicfirst', 'format_moetopcoll'),
                            // Day.
                            5 => new lang_string('setlayoutstructureday', 'format_moetopcoll'))
                    )
                );
                $courseformatoptionsedit['layoutcolumns'] = array(
                    'label' => new lang_string('setlayoutcolumns', 'format_moetopcoll'),
                    'help' => 'setlayoutcolumns',
                    'help_component' => 'format_moetopcoll',
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(1 => new lang_string('one', 'format_moetopcoll'),   // Default.
                              2 => new lang_string('two', 'format_moetopcoll'),   // Two.
                              3 => new lang_string('three', 'format_moetopcoll'), // Three.
                              4 => new lang_string('four', 'format_moetopcoll'))  // Four.
                    )
                );
                $courseformatoptionsedit['layoutcolumnorientation'] = array(
                    'label' => new lang_string('setlayoutcolumnorientation', 'format_moetopcoll'),
                    'help' => 'setlayoutcolumnorientation',
                    'help_component' => 'format_moetopcoll',
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(1 => new lang_string('columnvertical', 'format_moetopcoll'),
                              2 => new lang_string('columnhorizontal', 'format_moetopcoll')) // Default.
                    )
                );
                $courseformatoptionsedit['toggleiconposition'] = array(
                    'label' => new lang_string('settoggleiconposition', 'format_moetopcoll'),
                    'help' => 'settoggleiconposition',
                    'help_component' => 'format_moetopcoll',
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(1 => new lang_string('left', 'format_moetopcoll'),   // Left.
                              2 => new lang_string('right', 'format_moetopcoll'))  // Right.
                    )
                );
                $courseformatoptionsedit['showsectionsummary'] = array(
                    'label' => new lang_string('setshowsectionsummary', 'format_moetopcoll'),
                ...
                $courseformatoptionsedit['layoutelement'] = array(
                    'label' => get_config('format_moetopcoll', 'defaultlayoutelement'), 'element_type' => 'hidden');
                $courseformatoptionsedit['layoutstructure'] = array(
                    'label' => get_config('format_moetopcoll', 'defaultlayoutstructure'), 'element_type' => 'hidden');
                $courseformatoptionsedit['layoutcolumns'] = array(
                    'label' => get_config('format_moetopcoll', 'defaultlayoutcolumns'), 'element_type' => 'hidden');
                $courseformatoptionsedit['layoutcolumnorientation'] = array(
                    'label' => get_config('format_moetopcoll', 'defaultlayoutcolumnorientation'), 'element_type' => 'hidden');
                $courseformatoptionsedit['toggleiconposition'] = array(
                    'label' => get_config('format_moetopcoll', 'defaulttoggleiconposition'), 'element_type' => 'hidden');
                $courseformatoptionsedit['showsectionsummary'] = array(
                    'label' => get_config('format_moetopcoll', 'defaultshowsectionsummary'), 'element_type' => 'hidden');
            }

            if (has_capability('format/moetopcoll:changetogglealignment', $context)) {
                $courseformatoptionsedit['togglealignment'] = array(
                    'label' => new lang_string('settogglealignment', 'format_moetopcoll'),
                    'help' => 'settogglealignment',
                    'help_component' => 'format_moetopcoll',
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(1 => new lang_string('left', 'format_moetopcoll'),   // Left.
                              2 => new lang_string('center', 'format_moetopcoll'), // Centre.
                              3 => new lang_string('right', 'format_moetopcoll'))  // Right.
                    )
                );
            } else {
                $courseformatoptionsedit['togglealignment'] = array(
                    'label' => get_config('format_moetopcoll', 'defaulttogglealignment'), 'element_type' => 'hidden');
            }

            if (has_capability('format/moetopcoll:changetoggleiconset', $context)) {
                $courseformatoptionsedit['toggleiconset'] = array(
                    'label' => new lang_string('settoggleiconset', 'format_moetopcoll'),
                    'help' => 'settoggleiconset',
                    'help_component' => 'format_moetopcoll',
                    'element_type' => 'select',
                    'element_attributes' => array(
                        array(
                            'arrow' => new lang_string('arrow', 'format_moetopcoll'),               // Arrow icon set.
                            'bulb' => new lang_string('bulb', 'format_moetopcoll'),                 // Bulb icon set.
                            'cloud' => new lang_string('cloud', 'format_moetopcoll'),               // Cloud icon set.
                            'eye' => new lang_string('eye', 'format_moetopcoll'),                   // Eye icon set.
                            'groundsignal' => new lang_string('groundsignal', 'format_moetopcoll'), // Ground signal set.
                            'led' => new lang_string('led', 'format_moetopcoll'),                   // LED icon set.
                            'point' => new lang_string('point', 'format_moetopcoll'),               // Point icon set.
                            'power' => new lang_string('power', 'format_moetopcoll'),               // Power icon set.
                            'radio' => new lang_string('radio', 'format_moetopcoll'),               // Radio icon set.
                            'smiley' => new lang_string('smiley', 'format_moetopcoll'),             // Smiley icon set.
                            'square' => new lang_string('square', 'format_moetopcoll'),             // Square icon set.
                            'sunmoon' => new lang_string('sunmoon', 'format_moetopcoll'),           // Sun / Moon icon set.
                            'switch' => new lang_string('switch', 'format_moetopcoll'))             // Switch icon set.
                    )
                );
                $courseformatoptionsedit['toggleallhover'] = array(
                    'label' => new lang_string('settoggleallhover', 'format_moetopcoll'),
                ...
                $courseformatoptionsedit['toggleiconset'] = array(
                    'label' => get_config('format_moetopcoll', 'defaulttoggleiconset'), 'element_type' => 'hidden');
                $courseformatoptionsedit['toggleallhover'] = array(
                    'label' => get_config('format_moetopcoll', 'defaulttoggleallhover'), 'element_type' => 'hidden');
            }

            if (has_capability('format/moetopcoll:changecolour', $context)) {
                $courseformatoptionsedit['toggleforegroundcolour'] = array(
                    'label' => new lang_string('settoggleforegroundcolour', 'format_moetopcoll'),
                ...
                $courseformatoptionsedit['toggleforegroundhovercolour'] = array(
                    'label' => new lang_string('settoggleforegroundhovercolour', 'format_moetopcoll'),
                ...
                $courseformatoptionsedit['togglebackgroundcolour'] = array(
                    'label' => new lang_string('settogglebackgroundcolour', 'format_moetopcoll'),
                ...
                $courseformatoptionsedit['togglebackgroundhovercolour'] = array(
                    'label' => new lang_string('settogglebackgroundhovercolour', 'format_moetopcoll'),
            ...
            $courseformatoptionsedit['readme'] = array(
                    'label' => get_string('readme_title', 'format_moetopcoll'),
                    'element_type' => 'static'
                );
    ...
    public function create_edit_form_elements(&$mform, $forsection = false) {
        global $CFG, $OUTPUT;
        MoodleQuickForm::registerElementType('tccolourpopup', "$CFG->dirroot/course/format/moetopcoll/js/tc_colourpopup.php",
        ...
        if ($forsection == false) {
            ...
            $changelayout = has_capability('format/moetopcoll:changelayout', $context);
            $changecolour = has_capability('format/moetopcoll:changecolour', $context);
            $changetogglealignment = has_capability('format/moetopcoll:changetogglealignment', $context);
            $changetoggleiconset = has_capability('format/moetopcoll:changetoggleiconset', $context);
            $resetall = is_siteadmin($USER); // Site admins only.

            $elements[] = $mform->addElement('header', 'ctreset', get_string('ctreset', 'format_moetopcoll'));
            $mform->addHelpButton('ctreset', 'ctreset', 'format_moetopcoll', '', true);

            $resetelements = array();
            $checkboxname = get_string('resetdisplayinstructions', 'format_moetopcoll').
                $OUTPUT->help_icon('resetdisplayinstructions', 'format_moetopcoll');
            $resetelements[] =& $mform->createElement('checkbox', 'resetdisplayinstructions', '', $checkboxname);

            if ($changelayout) {
                $checkboxname = get_string('resetlayout', 'format_moetopcoll').$OUTPUT->help_icon('resetlayout', 'format_moetopcoll');
                $resetelements[] =& $mform->createElement('checkbox', 'resetlayout', '', $checkboxname);
            }

            if ($changecolour) {
                $checkboxname = get_string('resetcolour', 'format_moetopcoll').$OUTPUT->help_icon('resetcolour', 'format_moetopcoll');
                $resetelements[] =& $mform->createElement('checkbox', 'resetcolour', '', $checkboxname);
            }

            if ($changetogglealignment) {
                $checkboxname = get_string('resettogglealignment', 'format_moetopcoll').
                    $OUTPUT->help_icon('resettogglealignment', 'format_moetopcoll');
                $resetelements[] =& $mform->createElement('checkbox', 'resettogglealignment', '', $checkboxname);
            }

            if ($changetoggleiconset) {
                $checkboxname = get_string('resettoggleiconset', 'format_moetopcoll').
                    $OUTPUT->help_icon('resettoggleiconset', 'format_moetopcoll');
                $resetelements[] =& $mform->createElement('checkbox', 'resettoggleiconset', '', $checkboxname);
            }
            $elements[] = $mform->addGroup($resetelements, 'resetgroup', get_string('resetgrp', 'format_moetopcoll'), null, false);

            if ($resetall) {
                $resetallelements = array();

                $checkboxname = get_string('resetalldisplayinstructions', 'format_moetopcoll').
                    $OUTPUT->help_icon('resetalldisplayinstructions', 'format_moetopcoll');
                $resetallelements[] =& $mform->createElement('checkbox', 'resetalldisplayinstructions', '', $checkboxname);

                $checkboxname = get_string('resetalllayout', 'format_moetopcoll').
                    $OUTPUT->help_icon('resetalllayout', 'format_moetopcoll');
                $resetallelements[] =& $mform->createElement('checkbox', 'resetalllayout', '', $checkboxname);

                $checkboxname = get_string('resetallcolour', 'format_moetopcoll').
                    $OUTPUT->help_icon('resetallcolour', 'format_moetopcoll');
                $resetallelements[] =& $mform->createElement('checkbox', 'resetallcolour', '', $checkboxname);

                $checkboxname = get_string('resetalltogglealignment', 'format_moetopcoll').
                    $OUTPUT->help_icon('resetalltogglealignment', 'format_moetopcoll');
                $resetallelements[] =& $mform->createElement('checkbox', 'resetalltogglealignment', '', $checkboxname);

                $checkboxname = get_string('resetalltoggleiconset', 'format_moetopcoll').
                    $OUTPUT->help_icon('resetalltoggleiconset', 'format_moetopcoll');
                ...
                $elements[] = $mform->addGroup($resetallelements, 'resetallgroup',
                    get_string('resetallgrp', 'format_moetopcoll'), null, false);
            }
        }
    ...
    public function edit_form_validation($data, $files, $errors) {
        ...
        if ($this->validate_colour($data['toggleforegroundcolour']) === false) {
            $retr['toggleforegroundcolour'] = get_string('colourrule', 'format_moetopcoll');
        }
        if ($this->validate_colour($data['toggleforegroundhovercolour']) === false) {
            $retr['toggleforegroundhovercolour'] = get_string('colourrule', 'format_moetopcoll');
        }
        if ($this->validate_colour($data['togglebackgroundcolour']) === false) {
            $retr['togglebackgroundcolour'] = get_string('colourrule', 'format_moetopcoll');
        }
        if ($this->validate_colour($data['togglebackgroundhovercolour']) === false) {
            $retr['togglebackgroundhovercolour'] = get_string('colourrule', 'format_moetopcoll');
        }
    ...
    public function is_section_current($section) {
        ...
        if (($tcsettings['layoutstructure'] == 2) || ($tcsettings['layoutstructure'] == 3)) {
            ...
            $timenow = time();
            $dates = $this->format_moetopcoll_get_section_dates($section, $this->get_course());
            ...
            $timenow = time();
            $day = $this->format_moetopcoll_get_section_day($section, $this->get_course());
            $onedayseconds = 86400;
     ...
     */
    private function format_moetopcoll_get_section_dates($section, $course) {
     ...
     */
    private function format_moetopcoll_get_section_day($section, $course) {
        $onedayseconds = 86400;
        /* Hack alert. We add 2 hours to avoid possible DST problems. (e.g. we go into daylight
           savings and the date changes. */
        $startdate = $course->startdate + 7200;
     ...
     */
    public function reset_moetopcoll_setting($courseid, $displayinstructions, $layout, $colour, $togglealignment, $toggleiconset) {
        global $DB, $USER;
        ...
        if ($displayinstructions && $resetallifall) {
            $updatedata['displayinstructions'] = get_config('format_moetopcoll', 'defaultdisplayinstructions');
            $updatedisplayinstructions = true;
        }
        if ($layout && has_capability('format/moetopcoll:changelayout', $context) && $resetallifall) {
            $updatedata['coursedisplay'] = get_config('format_moetopcoll', 'defaultcoursedisplay');
            $updatedata['layoutelement'] = get_config('format_moetopcoll', 'defaultlayoutelement');
            $updatedata['layoutstructure'] = get_config('format_moetopcoll', 'defaultlayoutstructure');
            $updatedata['layoutcolumns'] = get_config('format_moetopcoll', 'defaultlayoutcolumns');
            $updatedata['layoutcolumnorientation'] = get_config('format_moetopcoll', 'defaultlayoutcolumnorientation');
            $updatedata['toggleiconposition'] = get_config('format_moetopcoll', 'defaulttoggleiconposition');
            $updatedata['showsectionsummary'] = get_config('format_moetopcoll', 'defaultshowsectionsummary');
            $updatelayout = true;
        }
        if ($togglealignment && has_capability('format/moetopcoll:changetogglealignment', $context) && $resetallifall) {
            $updatedata['togglealignment'] = get_config('format_moetopcoll', 'defaulttogglealignment');
            $updatetogglealignment = true;
        }
        if ($colour && has_capability('format/moetopcoll:changecolour', $context) && $resetallifall) {
            $updatedata['toggleforegroundcolour'] = get_config('format_moetopcoll', 'defaulttgfgcolour');
            $updatedata['toggleforegroundhovercolour'] = get_config('format_moetopcoll', 'defaulttgfghvrcolour');
            $updatedata['togglebackgroundcolour'] = get_config('format_moetopcoll', 'defaulttgbgcolour');
            $updatedata['togglebackgroundhovercolour'] = get_config('format_moetopcoll', 'defaulttgbghvrcolour');
            $updatecolour = true;
        }
        if ($toggleiconset && has_capability('format/moetopcoll:changetoggleiconset', $context) && $resetallifall) {
            $updatedata['toggleiconset'] = get_config('format_moetopcoll', 'defaulttoggleiconset');
            $updatedata['toggleallhover'] = get_config('format_moetopcoll', 'defaulttoggleallhover');
     ...
     * in the old 'format_moetopcoll_settings' table.
     * @param int $courseid If not 0, then a specific course to reset.
     ...
     */
    public function restore_moetopcoll_setting($courseid, $layoutelement, $layoutstructure, $layoutcolumns, $tgfgcolour,
        ...
        $lco = get_config('format_moetopcoll', 'defaultlayoutcolumnorientation');
        if (empty($lco)) {
            ...
            $data['displayinstructions'] = get_config('format_moetopcoll', 'defaultdisplayinstructions');
            $data['layoutcolumnorientation'] = get_config('format_moetopcoll', 'defaultlayoutcolumnorientation');
            $data['showsectionsummary'] = get_config('format_moetopcoll', 'defaultshowsectionsummary');
            $data['togglealignment'] = get_config('format_moetopcoll', 'defaulttogglealignment');
            $data['toggleallhover'] = get_config('format_moetopcoll', 'defaulttoggleallhover');
            $data['toggleiconposition'] = get_config('format_moetopcoll', 'defaulttoggleiconposition');
            $data['toggleiconset'] = get_config('format_moetopcoll', 'defaulttoggleiconset');
        }
     ...
     */
    public function update_moetopcoll_columns_setting($layoutcolumns) {
        // Create data array.
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = true,
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_moetopcoll');
        }
        if (empty($editlabel)) {
            $course = $this->get_course();
            $title = $this->get_moetopcoll_section_name($course, $section, false);
            $editlabel = new lang_string('newsectionname', 'format_moetopcoll', $title);
        }
 ...
 * @return \core\output\inplace_editable
 */
function format_moetopcoll_inplace_editable($itemtype, $itemid, $newvalue) {
    global $CFG;
    ...
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        ...
        $section = $DB->get_record_sql(
            ...
            array($itemid, 'moetopcoll'), MUST_EXIST);
 ...
 */
function callback_moetopcoll_definition() {
    return get_string('sectionname', 'format_moetopcoll');
}
 ...
 * CONTRIB-3520.
 */
function format_moetopcoll_delete_course($courseid) {
    global $DB;
