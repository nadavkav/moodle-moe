 ...
 * @namespace
 */
M.format_moetopcoll = M.format_moetopcoll || {};
...
M.format_moetopcoll.thesparezeros = "00000000000000000000000000";
M.format_moetopcoll.togglestate = false;
M.format_moetopcoll.courseid = 0;
M.format_moetopcoll.togglePersistence = 1; // Toggle persistence - 1 = on, 0 = off.
M.format_moetopcoll.ourYUI = false;
M.format_moetopcoll.numSections = 0;
M.format_moetopcoll.ie8 = false;

// Namespace constants:....
M.format_moetopcoll.TOGGLE_6 = 1;
M.format_moetopcoll.TOGGLE_5 = 2;
M.format_moetopcoll.TOGGLE_4 = 4;
M.format_moetopcoll.TOGGLE_3 = 8;
M.format_moetopcoll.TOGGLE_2 = 16;
M.format_moetopcoll.TOGGLE_1 = 32;
 ...
 */
M.format_moetopcoll.init = function(Y, theCourseId, theToggleState, theNumSections, theTogglePersistence,
    ...
    M.format_moetopcoll.ie8 = bodyNode.hasClass('ie8');
...
M.format_moetopcoll.toggleClick = function(e) {
    var toggleIndex = parseInt(e.currentTarget.get('id').replace("toggle-", ""));
    e.preventDefault();
    this.toggle_topic(e.currentTarget, toggleIndex);
};

M.format_moetopcoll.allOpenClick = function(e) {
    e.preventDefault();
    M.format_moetopcoll.ourYUI.all(".toggledsection").addClass('sectionopen');
    M.format_moetopcoll.ourYUI.all(".toggle span.the_toggle").addClass('toggle_open').removeClass('toggle_closed')
        .setAttribute('aria-pressed', 'true');
    M.format_moetopcoll.resetState(M.format_moetopcoll.get_max_digit());
    M.format_moetopcoll.save_toggles();
};

M.format_moetopcoll.allCloseClick = function(e) {
    e.preventDefault();
    M.format_moetopcoll.ourYUI.all(".toggledsection").removeClass('sectionopen');
    M.format_moetopcoll.ourYUI.all(".toggle span.the_toggle").addClass('toggle_closed').removeClass('toggle_open')
        .setAttribute('aria-pressed', 'false');
    M.format_moetopcoll.resetState(M.format_moetopcoll.get_min_digit());
    M.format_moetopcoll.save_toggles();
};

M.format_moetopcoll.resetState = function(dchar) {
    M.format_moetopcoll.togglestate = "";
    var numdigits = M.format_moetopcoll.get_required_digits(M.format_moetopcoll.numSections);
    for (var i = 0; i < numdigits; i++) {
        M.format_moetopcoll.togglestate += dchar;
    }
};

// Toggle functions
// Args - targetNode that initiated the call, toggleNum the number of the toggle.
M.format_moetopcoll.toggle_topic = function(targetNode, toggleNum) {
    "use strict";
    ...
    if (M.format_moetopcoll.ie8) {
        M.format_moetopcoll.ourYUI.all(".toggle span.the_toggle").addClass('ie8_hackclass_donotuseincss')
...
M.format_moetopcoll.to2baseString = function(thirtysix) {
    "use strict";
...
M.format_moetopcoll.save_toggles = function() {
    "use strict";
    if (this.togglePersistence == 1) { // Toggle persistence - 1 = on, 0 = off.
        M.format_moetopcoll.set_user_preference('topcoll_toggle_' + this.courseid, this.togglestate);
    }
};

// New base 64 code:....
M.format_moetopcoll.is_old_preference = function(pref) {
    "use strict";
    var retr = false;
...
M.format_moetopcoll.convert_to_new_preference = function() {
    "use strict";
 ...
 */
M.format_moetopcoll.set_toggle_state = function(togglenum, state) {
    "use strict";
...
M.format_moetopcoll.get_required_digits = function(numtoggles) {
    "use strict";
    return this.get_toggle_pos(numtoggles);
};

M.format_moetopcoll.get_toggle_pos = function(togglenum) {
    "use strict";
    return Math.ceil(togglenum / 6);
};

M.format_moetopcoll.get_min_digit = function() {
    "use strict";
    return ':'; // ':' is 58.
};

M.format_moetopcoll.get_max_digit = function() {
    "use strict";
    return 'y'; // 'y' is 121.
};

M.format_moetopcoll.get_toggle_flag = function(togglenum, togglecharpos) {
    "use strict";
...
M.format_moetopcoll.decode_character_to_value = function(character) {
    "use strict";
    return character.charCodeAt(0) - 58;
};

M.format_moetopcoll.encode_value_to_character = function(val) {
    "use strict";
 ...
 */
M.format_moetopcoll.set_user_preference = function(name, value) {
    YUI().use('io', function(Y) {
        var url = M.cfg.wwwroot + '/course/format/moetopcoll/settopcollpref.php?sesskey=' + M.cfg.sesskey + '&pref=' + encodeURI(name) + '&value=' + encodeURI(value); // jshint ignore:line
        ...
        if (M.cfg.developerdebug) {
            cfg.on.failure = function() {
                console.log("Error updating moetopcoll preference '" + name + "' using AJAX.  Almost certainly your session has timed out.  Clicking this link will repeat the AJAX call that failed so you can see the error: "); // jshint ignore:line
            };
        }
