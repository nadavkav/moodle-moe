...
Adder.template = [
    ...
    '  <button type="button">' + M.util.get_string('annotate', 'local_notes') + '</button>',
    '</div>'
].join('\n');
...
var Editor = exports.Editor = Widget.extend({
    ...
    constructor: function (options) {
        ...
        if (this.options.defaultFields) {
            this.addField({
                type: 'textarea',
                label: M.util.get_string('comments', 'local_notes') + '\u2026',
...
Editor.template = [
    ...
    '     <a href="#cancel" class="annotator-cancel">' + M.util.get_string('cancel','local_notes') + '</a>',
    '      <a href="#save"',
    '         class="annotator-save">' + M.util.get_string('save', 'local_notes') + '</a>',
    '    </div>',
    '  </form>',
    '</div>'
].join('\n');
...
var Filter = exports.Filter = function Filter(options) {
    ...
    if (this.options.addAnnotationFilter) {
        this.addFilter({label: M.util.get_string('annotation', 'local_notes'), property: 'text'});
    }
};
...
Filter.prototype.addFilter = function (options) {
    ...
    if (!hasFilterForProp) {
        ...
        filter.element.find('input')
            .attr({
                id: filter.id,
                placeholder: M.util.get_string('filterby', 'local_notes') + filter.label + '\u2026'
            });
...
Filter.html = {
    element: [
        ...
        '  <strong>' + M.util.get_string('navigate', 'local_notes') + '</strong>',
        '  <span class="annotator-filter-navigation">',
        '    <button type="button"',
        '            class="annotator-filter-previous">' +
            M.util.get_string('previous', 'local_notes') +
            '</button>',
        '    <button type="button"',
        '            class="annotator-filter-next">' + M.util.get_string('next', 'local_notes') + '</button>',
        '  </span>',
        '  <strong>' + M.util.get_string('filterby', 'local_notes') + '</strong>',
        '</div>'
    ].join('\n'),
    ...
    filter: [
        ...
        '  <input/>',
        '  <button type="button"',
        '          class="annotator-filter-clear">' + M.util.get_string('clear', 'local_notes') + '</button>',
        '</span>'
    ].join('\n')
};
...
function addPermissionsCheckboxes(editor, ident, authz) {
    function createLoadCallback(action) {
        return function loadCallback(field, annotation) {
            ...
            if (authz.permits(action, annotation, null) && action != 'update') {
    ...
    editor.addField({
        type: 'checkbox',
        label: M.util.get_string('alloweveryoneview', 'local_notes'),
    ...
    editor.addField({
        type: 'checkbox',
        label: M.util.get_string('alloweveryoneedit', 'local_notes'),
    ...
    constructor: function (options) {
        ...
        this.render = function (annotation) {
            if (annotation.text) {
                ...
                return "<i>" + M.util.get_string('nocomment', 'local_notes') + "</i>";
            }
        };
        ...
        if (this.options.defaultFields) {
            this.addField({
                load: function (field, annotation) {
                	$(field).parent().prepend('<div class="annotator-item_part text span7"><span>' +self.render(annotation)+ '</span></div>');
                	var date = new Date(annotation.created * 1000);
                	$(field).parent().prepend('<div class="annotator-item_part image span4">'+
                			'<div class="row-fluid">'+
                				'<div class="span6">'+
                			    	'<img src="' + annotation.userpicture +'" class="anottatepicture img-responsive"/>'+
                			    '</div>'+
                			    '<div class="span6">'+
...
Viewer.template = [
    ...
    '     <a href="#cancel" class="annotator-cancel span3 offset1">' + M.util.get_string('cancel', 'local_notes') + '</a>',
    '     <a href="#resolved" class="annotator-resolved span4">' + M.util.get_string('resolved' , 'local_notes') + '</a>',
    '     <a href="#repay" class="annotator-reply span3 offset1">' + M.util.get_string('repaly', 'local_notes') +'</a>',
    '  </div>',
    '</div>'
].join('\n');
...
Viewer.itemTemplate = [
    ...
    '				<div class="span12">',			
    '    				<button',
    '            			title="' + M.util.get_string('edit', 'local_notes') + '"',
    '            			class="annotator-edit"></button>',
    '				</div>',
    '				<div class="span12">',
    '    				<button',
    '            			title="' + M.util.get_string('delete', 'local_notes') + '"',
