...
class local_remote_backup_provider_external extends external_api {
    ...
    public static function subscribe($name, $url, $user, $token, $username) {
        ...
        if (publisher::subscribe($name, $url, $user, $token)){
            $sql = "select CO.id, CA.idnumber as tag, CO.fullname as course_name from {course} as CO inner join mdl_course_categories as CA on CA.id = CO.category where CA.idnumber<>''";
