...
class tour {
     ...
     */
    const TOUR_LAST_COMPLETED_BY_USER   = 'local_usertours_tour_completion_time_';
     ...
     */
    const TOUR_REQUESTED_BY_USER        = 'local_usertours_tour_reset_time_';
    ...
    protected function fetch($id) {
        ...
        return $this->reload_from_record(
            $DB->get_record('usertours_tours', array('id' => $id), '*', MUST_EXIST)
        );
    }
    ...
    public function persist($force = false) {
        ...
        if ($this->id) {
            ...
            $DB->update_record('usertours_tours', $record);
        } else {
            $this->calculate_sortorder();
            $record = $this->to_record();
            unset($record->id);
            $this->id = $DB->insert_record('usertours_tours', $record);
        }
    ...
    public function remove() {
        ...
        $DB->delete_records('usertours_tours', array('id' => $this->id));
    ...
    public function reset_step_sortorder() {
        global $DB;
        $steps = $DB->get_records('usertours_steps', array('tourid' => $this->id), 'sortorder ASC', 'id');

        $index = 0;
        foreach ($steps as $step) {
            $DB->set_field('usertours_steps', 'sortorder', $index, array('id' => $step->id));
            $index++;
        }
    ...
    public function get_tour_key() {
        ...
        return sprintf('local_usertours_%d_%s', $this->get_id(), $tourtime);
    }
    ...
    protected function add_config_field_to_form(\MoodleQuickForm &$mform, $key) {
        ...
        $mform->addElement('select', $key, get_string($key, 'local_usertours'), $options);
        $mform->setDefault($key, configuration::get_default_value($key));
        $mform->addHelpButton($key, $key, 'local_usertours');
