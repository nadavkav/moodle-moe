...
require_once($CFG->dirroot.'/mod/moewiki/annotate_form.php');
...
$url = new moodle_url('/mod/moewiki/annotate.php', array('id' => $id));
$PAGE->set_url($url);
...
if ($save) {
    if (!$lockok) {
        ...
        print_error('cannotlockpage', 'moewiki', 'view.php?'.moewiki_display_wiki_parameters($pagename,
                $subwiki, $cm, MOEWIKI_PARAMS_URL));
    }
    ...
    $stored = moewiki_get_annotations($pageversion);
    $updated = $stored;
    ...
    if ($deleteorphaned) {
        foreach ($stored as $annotation) {
            if ($annotation->orphaned) {
                $DB->delete_records('moewiki_annotations', array('id' => $annotation->id));
    ...
    foreach ($edited_annotations as $key => $value) {
        if ($value == '') {
            $DB->delete_records('moewiki_annotations', array('id' => $key));
            ...
            $DB->update_record('moewiki_annotations', $dataobject);
        }
    }
    ...
    foreach ($new_annotations as $key => $value) {
        if ($value != '') {
            ...
            $dataobject->timemodified = time();
            $dataobject->content = $value;
            $newannoid = $DB->insert_record('moewiki_annotations', $dataobject);
            $updated[$newannoid] = '';
    ...
    $neednewversion = (moewiki_cleanup_annotation_tags($updated, $pageversion->xhtml)) ? true : $neednewversion;
    ...
    if (has_capability('mod/moewiki:lock', $context)) {
        moewiki_lock_editing($pageversion->pageid, $lockunlock);
    }
    ...
    if ($neednewversion) {
        ...
        if ($contenttag = strpos($pageversion->xhtml, '<div class="moewiki_content">') !== false) {
        ...
        moewiki_save_new_version($course, $cm, $moewiki, $subwiki, $pagename, $pageversion->xhtml);
    }
...
if ($save || $cancel) {
    moewiki_release_lock($pageversion->pageid);
    redirect('view.php?'.moewiki_display_wiki_parameters($pagename, $subwiki, $cm, MOEWIKI_PARAMS_URL), '', 0);
}
// OK, not redirecting...
...
print get_string('advice_annotate', 'moewiki');
$data = $moewikioutput->moewiki_print_page($subwiki, $cm, $pageversion, false, 'annotate', $moewiki->enablewordcount);
echo $data[0];
$annotations = $data[1];
...
$customdata[3] = $userid;
$customdata[4] = has_capability('mod/moewiki:lock', $context);
echo html_writer::start_div('ouw-annotation-list');
echo html_writer::tag('h2', get_string('annotations', 'moewiki'));
echo html_writer::end_div();

$annotateform = new mod_moewiki_annotate_form('annotate.php?id='.$id, $customdata);
...
echo '        <div class="hd">'.get_string('addannotation', 'moewiki').'</div>';
echo '        <div class="bd">';
echo '            <form method="POST" action="post.php">';
echo '                <label for="annotationtext">'.get_string('addannotation', 'moewiki').':</label>';
...
// init JS module
$stringlist[] = array('add', 'moewiki');
$stringlist[] = array('cancel', 'moewiki');
$jsmodule = array('name'     => 'mod_moewiki_annotate',
