...
class mod_moewiki_annotate_form extends moodleform {
    ...
    public function definition() {
        ...
        if (count($annotations != 0)) {
            usort($annotations, array('mod_moewiki_annotate_form', 'moewiki_internal_position_sort'));
            $editnumber = 1;
        ...
        if ($orphaned) {
            $mform->addElement('checkbox', 'deleteorphaned', get_string('deleteorphanedannotations', 'moewiki'));
        }

        if ($canlock) {
            $mform->addElement('checkbox', 'lockediting', get_string('lockediting', 'moewiki'));
            if (moewiki_is_page_editing_locked($pageid)) {
    ...
    private function moewiki_internal_position_sort($a, $b) {
