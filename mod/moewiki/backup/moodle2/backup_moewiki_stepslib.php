...
/**
 * Define all the backup steps that will be used by the backup_moewiki_activity_task
 */

/**
 * Define the complete moewiki structure for backup, with file and id annotations
 */
class backup_moewiki_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $moewiki = new backup_nested_element('moewiki', array('id'), $els);
        ...
        $annotation->annotate_ids('user', 'userpage');

        // Define file annotations
        $moewiki->annotate_files('mod_moewiki', 'intro', null); // This file area hasn't itemid
        if (!$userinfo) {
            $moewiki->annotate_files('mod_moewiki', 'template', 'id');
        }
        $version->annotate_files('mod_moewiki', 'attachment', 'id');
        $version->annotate_files('mod_moewiki', 'content', 'id');

        // Return the root element (wiki), wrapped into standard activity structure
        return $this->prepare_activity_structure($moewiki);
