...
/**
 * Structure step to restore one moewiki activity
 */
class restore_moewiki_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $paths[] = new restore_path_element('moewiki', '/activity/moewiki');
        $paths[] = new restore_path_element('moewiki_subwiki', '/activity/moewiki/subs/subwiki');
    ...
    protected function process_moewiki($data) {
        global $DB;
        ...
        // insert the moewiki record
        $newitemid = $DB->insert_record('moewiki', $data);
        // immediately after inserting "activity" record, call this
        $this->apply_activity_instance($newitemid);
        // Get MOE Wiki id for later use.
        $this->moewikiid = $newitemid;
        $this->set_mapping('moewiki', $oldid, $newitemid);
    }

    protected function process_moewiki_subwiki($data) {
        global $DB;

        $data = (object)$data;
        $oldid = $data->id;

        $data->wikiid = $this->get_new_parentid('moewiki');
        ...
        $newitemid = $DB->insert_record('moewiki_subwikis', $data);
        $this->set_mapping('moewiki_subwiki', $oldid, $newitemid);
    }

    protected function process_moewiki_page($data) {
        global $DB;

        $data = (object)$data;
        $oldid = $data->id;

        $data->subwikiid = $this->get_new_parentid('moewiki_subwiki');

        $newitemid = $DB->insert_record('moewiki_pages', $data);

        $this->set_mapping('moewiki_page', $oldid, $newitemid);

        // Flush out any unsaved versions.
        $this->flush_versions();
    }

    protected function process_moewiki_version($data) {
        global $CFG;
        ...
        if (!isset($data->wordcount)) {
            ...
            $wordcount = moewiki_count_words($data->xhtml);
        ...
        // it from the process_moewiki_link function.
        $data->links = array();
    ...
    private function flush_versions() {
        ...
        foreach ($this->versions as $data) {
            // Insert version.
            $oldid = $data->id;
            $newversionid = $DB->insert_record('moewiki_versions', $data);
            $this->set_mapping('moewiki_version', $oldid, $newversionid, true);
    ...
    protected function process_moewiki_link($data) {
        $data = (object)$data;
    ...
    protected function process_moewiki_annotation($data) {
        global $DB;
    ...
    protected function after_execute() {
        global $DB, $CFG;
        $transaction = $DB->start_delegated_transaction();
        $moewikiid = $this->get_task()->get_activityid();

        // Flush out any unsaved versions.
        $this->flush_versions();

        // Add moewiki related files, no need to match by itemname (just internally handled context).
        $this->add_related_files('mod_moewiki', 'intro', null);
        $this->add_related_files('mod_moewiki', 'template', 'moewiki');

        // Add post related files.
        $this->add_related_files('mod_moewiki', 'attachment', 'moewiki_version');
        $this->add_related_files('mod_moewiki', 'content', 'moewiki_version');
        ...
        $sql = 'SELECT v.pageid,
                ...
                FROM
                    {moewiki} o
                    JOIN {moewiki_subwikis} s ON s.wikiid = o.id
                    JOIN {moewiki_pages} p ON p.subwikiid = s.id
                    JOIN {moewiki_versions} v ON v.pageid = p.id
                WHERE
                    o.id = ?
                GROUP BY v.pageid, p.id
                ORDER BY v.pageid';
        $rs = $DB->get_recordset_sql($sql, array($moewikiid));
        foreach ($rs as $entry) {
        ...
        $sql = 'SELECT l.id AS linkid, l.topageid
                FROM
                    ...
                    JOIN {moewiki_pages} p ON p.subwikiid = s.id
                    JOIN {moewiki_versions} v ON v.pageid = p.id
                    JOIN {moewiki_links} l ON l.fromversionid = v.id
                WHERE
                    o.id = ? AND l.topageid IS NOT NULL';
        $rs = $DB->get_recordset_sql($sql, array($moewikiid));
        $errors = array();
        foreach ($rs as $entry) {
            $newpageid = $this->get_mappingid('moewiki_page', $entry->topageid, null);
            if (!$newpageid && empty($errors[$entry->topageid])) {
                $errors[$entry->topageid] = true;
                $this->get_logger()->process('MOE wiki: link to missing pageid ' .
                        $entry->topageid . ' not restored properly.', backup::LOG_WARNING);
            }
            $DB->set_field('moewiki_links', 'topageid', $newpageid, array('id' => $entry->linkid));
        }
        $rs->close();

        // Update xhtml field with correct annotations.
        // Get all table entries in moewiki_versions table for this wiki.
        ...
        foreach ($rs as $entry) {
            ...
            if ($found) {
                ...
                $DB->set_field('moewiki_versions', 'xhtml', $entry->xhtml, array('id' => $entry->id));
            }
        }
        ...
        if ($this->get_setting_value('userinfo') && moewiki_search_installed()) {
            moewiki_ousearch_update_all(false, $this->get_courseid());
        }
    }
}
