...
function xmldb_moewiki_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2008100600) {
        ...
        // Define field deletedat to be added to moewiki_versions.
        $table = new xmldb_table('moewiki_versions');
        ...
        upgrade_mod_savepoint(true, 2008100600, 'moewiki');
    }

    if ($oldversion < 2009120801) {
        // Launch create table for moewiki_annotations - if it does not already exist (extra precaution).
        $table = new xmldb_table('moewiki_annotations');
        if (!$dbman->table_exists($table)) {
            $dbman->install_one_table_from_xmldb_file($CFG->dirroot.'/mod/moewiki/db/install.xml', 'moewiki_annotations');
        }

        // Define field locked to be added to moewiki_pages.
        $table = new xmldb_table('moewiki_pages');
        ...
        upgrade_mod_savepoint(true, 2009120801, 'moewiki');
    }
    ...
    if ($oldversion < 2010022300) {
        ...
        upgrade_mod_savepoint(true, 2010022300, 'moewiki');
    }

    if ($oldversion < 2010122001) {

        // Drop the old comments table
        $table = new xmldb_table('moewiki_comments');
        if ($dbman->table_exists($table)) {
            $dbman->drop_table($table);
        }

        // Drop the old commenting field in moewiki
        $table2 = new xmldb_table('moewiki');
        $field = new xmldb_field('commenting');
        ...
        // Define field annotation to be added to moewiki
        ...
        // Define field introformat to be added to moewiki
        ...
        // Define field introformat to be added to moewiki
        $table3 = new xmldb_table('moewiki_versions');
        ...
        if ($CFG->texteditors !== 'textarea') {
            // introformat
            $rs = $DB->get_recordset('moewiki', array('introformat' => FORMAT_MOODLE), '', 'id,intro,introformat');
            foreach ($rs as $r) {
                $r->intro       = text_to_html($r->intro, false, false, true);
                $r->introformat = FORMAT_HTML;
                $DB->update_record('moewiki', $r);
                upgrade_set_timeout();
            }
            $rs->close();

            // xhtmlformat
            $rs = $DB->get_recordset('moewiki_versions', array('xhtmlformat' => FORMAT_MOODLE), '', 'id,xhtml,xhtmlformat');
            foreach ($rs as $r) {
                $r->xhtml       = text_to_html($r->xhtml, false, false, true);
                $r->xhtmlformat = FORMAT_MOODLE;
                $DB->update_record('moewiki_versions', $r);
                upgrade_set_timeout();
            }
            $rs->close();
        }

        upgrade_mod_savepoint(true, 2010122001, 'moewiki');
    }

    if ($oldversion < 2011031800) {
        upgrade_mod_savepoint(true, 2011031800, 'moewiki');
    }

    if ($oldversion < 2011060100) {

        // Define field enablewordcount to be added to moewiki
        $table = new xmldb_table('moewiki');
        ...
        // Define field wordcount to be added to moewiki_versions
        $table = new xmldb_table('moewiki_versions');
        ...
        // we need to update all moewiki_versions wordcounts
        $count = $DB->count_records_sql('SELECT COUNT(*) FROM {moewiki_versions}');
        $rs = $DB->get_recordset_sql('SELECT id, xhtml FROM {moewiki_versions} ORDER BY id');
        if ($rs->valid()) {
            $pbar = new progress_bar('countwordsmoewikiversionsxhtml', 500, true);

            $i = 0;
            foreach ($rs as $entry) {
                $i++;
                upgrade_set_timeout(60); // set up timeout, may also abort execution
                $pbar->update($i, $count, "Counting words of moewiki version entries - $i/$count.");

                // retrieve wordcount
                require_once($CFG->dirroot.'/mod/moewiki/locallib.php');
                $wordcount = moewiki_count_words($entry->xhtml);
                $entry->wordcount = $wordcount;
                $DB->update_record('moewiki_versions', $entry);
            }
        }
        $rs->close();

        upgrade_mod_savepoint(true, 2011060100, 'moewiki');
    }

    if ($oldversion < 2011071300) {

        // Define field grade to be added to moewiki
        $table = new xmldb_table('moewiki');
        ...
        // we need to update all moewiki_versions scales
        $rs = $DB->get_recordset_sql('SELECT id FROM {moewiki} ORDER BY id');
        if ($rs->valid()) {
            foreach ($rs as $entry) {
                $entry->grade = 0;
                $DB->update_record('moewiki', $entry);
            }
        }
        $rs->close();

        upgrade_mod_savepoint(true, 2011071300, 'moewiki');
    }

    if ($oldversion < 2011072000) {

        // Define field firstversionid to be added to moewiki_pages
        $table = new xmldb_table('moewiki_pages');
        ...
        // Define field previousversionid to be added to moewiki_versions
        $table = new xmldb_table('moewiki_versions');
        ...
        // update all moewiki_versions previousversionids and moewiki_pages firstversionids
        // firstversionid
        $sql = 'SELECT v.pageid,
                    (SELECT MIN(id)
                        FROM {moewiki_versions} v3
        ...
        if ($rs->valid()) {
            foreach ($rs as $entry) {
                if (isset($entry->firstversionid)) {
                    $DB->set_field('moewiki_pages', 'firstversionid', $entry->firstversionid,
        ...
        $rs = $DB->get_recordset_sql($sql);
        if ($rs->valid()) {
            $pbar = new progress_bar('moewikifirstandpreviousversions', 500, true);
            ...
            foreach ($rs as $entry) {
                ...
                if (isset($entry->previousversionid)) {
                    $DB->set_field('moewiki_versions', 'previousversionid',
        ...
        upgrade_mod_savepoint(true, 2011072000, 'moewiki');
    }
...
FROM
   {moewiki_pages} p
   LEFT JOIN {moewiki_pages} p2 ON p2.subwikiid = p.subwikiid AND p2.title IS NULL
...
WHERE
   p2.id IS NOT NULL");
        ...
        if ($ids) {
            ...
            $DB->execute("DELETE FROM {moewiki_pages} WHERE id $sql", $params);
        }

        // Set all the null values to blank
        $DB->execute("UPDATE {moewiki_pages} SET title='' WHERE title IS NULL");

        // Also in ousearch table if installed
        $table = new xmldb_table('local_ousearch_documents');
        if ($dbman->table_exists($table)) {
            $DB->execute("UPDATE {local_ousearch_documents} SET stringref='' WHERE stringref IS NULL AND plugin='mod_moewiki'");
        }

        // Changing nullability of field title on table moewiki_pages to not null
        $table = new xmldb_table('moewiki_pages');
        ...
        // Define index subwikiid-title (unique) to be added to moewiki_pages
        ...
        // moewiki savepoint reached
        upgrade_mod_savepoint(true, 2011102802, 'moewiki');
    }
    ...
    if ($oldversion < 2013060600) {
        ...
        $params[] = '<div class="moewiki_content"%';
        $params[] = 0;
        $sql = "select l.id as lid, l.tourl, v.id as vid, v.xhtml, p.id as pageid, p.subwikiid
            from {moewiki_links} l
            inner join {moewiki_versions} v on v.id = l.fromversionid
            inner join {moewiki_pages} p on v.id = p.currentversionid
            where ($likes)
        ...
        foreach ($links as $linkdetails) {
            ...
            if (!in_array($linkdetails->pageid, $pagesprocessed)) {
                ...
                for ($i = 0; $i < count($matches[0]); $i++) {
                    ...
                    if (strip_tags($text) == $text) {
                        ...
                        $newtag = str_replace("{$CFG->wwwroot}/view.php",
                            "{$CFG->wwwroot}/mod/moewiki/view.php", $tag);
                ...
                if ($newxhtml != $linkdetails->xhtml) {
                    ...
                    $DB->update_record('moewiki_versions', $update);
                }
            ...
            if (!isset($subwikipages[$linkdetails->subwikiid])) {
                ...
                $subwikipages[$linkdetails->subwikiid] = $DB->get_records('moewiki_pages',
            ...
            $DB->update_record('moewiki_links', $update);
        }
        $links = null;
        $subwikipages = null;
        // moewiki savepoint reached.
        upgrade_mod_savepoint(true, 2013060600, 'moewiki');
    }

    if ($oldversion < 2014012700) {

        // Define field allowimport to be added to moewiki.
        $table = new xmldb_table('moewiki');
        ...
        // Define field importversionid to be added to moewiki_versions.
        $table = new xmldb_table('moewiki_versions');
        ...
        // Moewiki savepoint reached.
        upgrade_mod_savepoint(true, 2014012700, 'moewiki');
    }

    if ($oldversion < 2014031100) {

        // Define index moewiki_subwikis_unique_group (unique) to be added to moewiki_subwikis.
        $table = new xmldb_table('moewiki_subwikis');
        $index = new xmldb_index('moewiki_subwikis_unique_group', XMLDB_INDEX_UNIQUE, array('wikiid', 'groupid'));

        // Conditionally launch add index moewiki_subwikis_unique_group.
        if (!$dbman->index_exists($table, $index)) {
            $dbman->add_index($table, $index);
        }

        // Define index moewiki_subwikis_unique_user (unique) to be added to moewiki_subwikis.
        $table = new xmldb_table('moewiki_subwikis');
        $index = new xmldb_index('moewiki_subwikis_unique_user', XMLDB_INDEX_UNIQUE, array('wikiid', 'userid'));

        // Conditionally launch add index moewiki_subwikis_unique_user.
        if (!$dbman->index_exists($table, $index)) {
            $dbman->add_index($table, $index);
        }
