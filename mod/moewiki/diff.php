...
$url = new moodle_url('/mod/moewiki/diff.php', array('id' => $id, 'v1' => $v1, 'v2' => $v2, 'page' => $pagename));
$PAGE->set_url($url);
...
$pageversion1 = moewiki_get_page_version($subwiki, $pagename, $v1);
$pageversion2 = moewiki_get_page_version($subwiki, $pagename, $v2);
...
echo $moewikioutput->moewiki_print_start($moewiki, $cm, $course, $subwiki, $pagename, $context,
    array(
        ...
        array('name' => get_string('changesnav', 'moewiki'), 'link' => null)),
    true, true
);

// Obtain difference between two versions
list($diff1, $diff2, $numchanges) = moewiki_diff_html($pageversion1->xhtml, $pageversion2->xhtml);

$fs = get_file_storage();
$files1 = ($files1 = $fs->get_area_files($context->id, 'mod_moewiki', 'attachment', $pageversion1->versionid, "timemodified", false)) ? $files1 : null;
$files2 = ($files2 = $fs->get_area_files($context->id, 'mod_moewiki', 'attachment', $pageversion2->versionid, "timemodified", false)) ? $files2 : null;

list($attachdiff1, $attachdiff2, $attachnumchanges) = moewiki_diff_attachments($files1, $files2, $context->id,
...
if ($numchanges == 0) {
    $annotations = moewiki_get_annotations($pageversion2);
    if (count($annotations) === 0) {
        $advice = get_string('diff_nochanges', 'moewiki');
    } else {
        $advice = get_string('diff_someannotations', 'moewiki');
    }
} else {
    $advice = get_string('advice_diff', 'moewiki');
}

print get_accesshide(get_string('changedifferences', 'moewiki'), 'h1');
print '<p class="ouw_advice">'.
     $advice.' '.
     get_string('returntohistory', 'moewiki',
    'history.php?'.moewiki_display_wiki_parameters($pagename, $subwiki, $cm)).'</p>';

// Obtain pluginfile urls.
$pageversion1->xhtml = file_rewrite_pluginfile_urls($pageversion1->xhtml, 'pluginfile.php',
    $context->id, 'mod_moewiki', 'content', $pageversion1->versionid);
$pageversion2->xhtml = file_rewrite_pluginfile_urls($pageversion2->xhtml, 'pluginfile.php',
    $context->id, 'mod_moewiki', 'content', $pageversion2->versionid);

// Obtain difference between two versions
list($diff1, $diff2) = moewiki_diff_html($pageversion1->xhtml, $pageversion2->xhtml);

// To make it look like a user object
$pageversion1->id = $pageversion1->userid;
$v1name = moewiki_display_user($pageversion1, $course->id);
$pageversion2->id = $pageversion2->userid;
$v2name = moewiki_display_user($pageversion2, $course->id);

// Disply the two versions
$v1 = new StdClass;
$v1->version = get_string('olderversion', 'moewiki');
$v1->date = userdate($pageversion1->timecreated);
$v1->savedby = get_string('savedby', 'moewiki', $v1name);
$v1->content = $diff1;
$v1->attachments = $attachdiff1;

$v2 = new StdClass;
$v2->version = get_string('newerversion', 'moewiki');
$v2->date = userdate($pageversion2->timecreated);
$v2->savedby = get_string('savedby', 'moewiki', $v2name);
$v2->content = $diff2;
$v2->attachments = $attachdiff2;

echo $moewikioutput->moewiki_print_diff($v1, $v2);
