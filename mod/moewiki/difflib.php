...
function moewiki_diff_internal($file1, $file2) {
    // Basic variables
    $n = count($file2);
    $m = count($file1);
    ...
    for ($i = 1; $i <= $m; $i++) {
        ...
        $pos[$i] = moewiki_diff_find_last($vhs, $eqc, crc32($file1[$i]));
    }
    ...
    for ($i = 1; $i <= $m; $i++) {
        if ($pos[$i] !== 0) {
            moewiki_diff_merge($karr, $k, $i, $eqc, $pos[$i], $candidates);
        }
    }
...
function moewiki_diff_merge(&$karr, &$k, $i, &$eqc, $p, &$candidates) {
    $r = 0;
    $c = $karr[0];
...
// From Step 2
function moewiki_diff_sort_v($a, $b) {
    if ($a->hash < $b->hash) {
        return -1;
...
// From Step 4
function moewiki_diff_find_last(&$vhs, &$eqc, $hash) {
...
class moewiki_line {
    ...
    public function __construct($data, $linepos) {
        ...
        while (true) {
            ...
            if ($space2 === false) {
                ...
                if (!empty($word)) {
                    $this->words[] = new moewiki_word($word, $pos + $linepos);
                }
                break;
            } else {
     ...
     * @param array $lines Array of moewiki_line
 ...
 * @return array Array of moewiki_line objects
 */
function moewiki_diff_html_to_lines($content) {
    ...
    while ($pos < strlen($content)) {
        ...
        $line = new moewiki_line($linestr, $pos);
        if (!$line->is_empty()) {
            $lines[$index++] = $line;
        }
        $pos = $nextline+1;
 ...
 * two source files.
 */
class moewiki_change_range {
    public $file1start, $file1count;
    public $file2start, $file2count;
}

/**
 * A more logical representation of the results from moewiki_internal_diff()
 */
class moewiki_changes {
    ...
    public function __construct($diff, $count2) {
        ...
        $this->adds = self::internal_find_deletes(
            moewiki_diff_internal_flip($diff, $count2), count($diff));
        ...
        foreach ($diff as $index1 => $index2) {
            ...
            if ($index2 === 0 && !in_array($index1, $this->deletes)) {
                if ($inrange === -1) {
                    ...
                    $this->changes[$inrange] = new moewiki_change_range;
...
/**
 * Flips around the array returned by moewiki_diff_internal
 ...
 */
function moewiki_diff_internal_flip($diff, $count2) {
    $flip = array();
    for ($i = 1; $i <= $count2; $i++) {
        $flip[$i] = 0;
    }
 ...
 * differ.
 * @param array $lines1 Array of moewiki_line
 * @param array $lines2 Array of moewiki_line
 * @return array (deleted,added); deleted and added are arrays of moewiki_word with
 *   position numbers from $lines1 and $lines2 respectively
 */
function moewiki_diff_words($lines1, $lines2) {
    // Prepare arrays
    $deleted = array();
    $added = array();
    // Get line difference
    $linediff = moewiki_diff(
        moewiki_line::get_as_strings($lines1),
        moewiki_line::get_as_strings($lines2));
    ...
    foreach ($linediff->changes as $changerange) {
        ...
        // Run diff on strings
        $worddiff = moewiki_diff($file1strings, $file2strings);
...
/**
 * Runs diff and interprets results into moewiki_changes object.
 * @param array $file1 Array of lines in file 1. The first line in the file
 *   MUST BE INDEX 1 NOT ZERO!!
 * @param array $file2 Array of lines in file 2, again starting from 1.
 * @return moewiki_changes Object describing changes
 */
function moewiki_diff($file1, $file2) {
    return new moewiki_changes(moewiki_diff_internal($file1, $file2), count($file2));
}

/**
 * Adds HTML span elements to $html around the words listed in $words.
 * @param string $html HTML content
 * @param array $words Array of moewiki_word to mark
 * @param string $markerclass Name of class for span element
 * @return HTML with markup added
 */
function moewiki_diff_add_markers($html, $words, $markerclass, $beforetext, $aftertext) {
    ...
    $spanstart = "<moewiki_diff_add_markers>";
    $pos = 0;
    $result = '';
    foreach ($words as $word) {
        ...
        // Add word
        $result .= $spanstart.$word->word.'</moewiki_diff_add_markers>';
        // Update position
    ...
    $result = preg_replace('^</moewiki_diff_add_markers>(\s*)<moewiki_diff_add_markers>^', '$1', $result);

    // Turn markers into proper span
    $result = preg_replace('^<moewiki_diff_add_markers>^', $beforetext.'<span class="'.$markerclass.'">', $result);
    $result = preg_replace('^</moewiki_diff_add_markers>^', '</span>'.$aftertext, $result);
 ...
 */
function moewiki_diff_html($html1, $html2) {
    global $OUTPUT;

    $lines1 = moewiki_diff_html_to_lines($html1);
    $lines2 = moewiki_diff_html_to_lines($html2);
    list($deleted, $added) = moewiki_diff_words($lines1, $lines2);
    $changes = count($deleted) + count($added);
    $result1 = moewiki_diff_add_markers($html1, $deleted, 'ouw_deleted',
        '<img src="'.$OUTPUT->pix_url('diff_deleted_begins', 'moewiki').'" alt="'.get_string('deletedbegins', 'moewiki').'" />',
        '<img src="'.$OUTPUT->pix_url('diff_added_ends', 'moewiki').'" alt="'.get_string('deletedends', 'moewiki').'" />');
    $result2 = moewiki_diff_add_markers($html2, $added, 'ouw_added',
        '<img src="'.$OUTPUT->pix_url('diff_added_begins', 'moewiki').'" alt="'.get_string('addedbegins', 'moewiki').'" />',
        '<img src="'.$OUTPUT->pix_url('diff_added_ends', 'moewiki').'" alt="'.get_string('addedends', 'moewiki').'" />');
    return array($result1, $result2, $changes);
}

// compare attachments between versions

function moewiki_diff_attachments($a1, $a2, $modcontextid, $p1versionid, $p2versionid) {
    global $PAGE;
    ...
    if (empty($a1) && empty($a2)) {
        ...
        $result1 = get_string('noattachments', 'moewiki');
        $result2 = get_string('noattachments', 'moewiki');
    } else if (!empty($a1) && empty($a2)) {
        // all attachments removed in version 2; only in version 1
        $result1 = '<ul>';
        foreach ($a1 as $file) {
            $result1 .= $moewikioutput->moewiki_print_attachment_diff($file, 'delete');
        }
        $result1 .= '</ul>';
        $result2 = get_string('noattachments', 'moewiki');
        $changes = count($a1);
        ...
        $result2 = '<ul>';
        foreach ($a2 as $file) {
            $result2 .= $moewikioutput->moewiki_print_attachment_diff($file, 'add');
        }
        $result2 .= '</ul>';
        $result1 = get_string('noattachments', 'moewiki');
        $changes = count($a2);
        ...
        foreach ($a1 as $f1) {
            ...
            foreach ($a2 as $f2) {
                ...
                if ($f1hash === $f2hash) {
                    $result1 .= $moewikioutput->moewiki_print_attachment_diff($f1);
                    $result2 .= $moewikioutput->moewiki_print_attachment_diff($f2);
                }
            }
        }
        ...
        foreach ($a2 as $f2) {
            ...
            if (!array_key_exists($f2hash, $f1hashes)) {
                $result2 .= $moewikioutput->moewiki_print_attachment_diff($f2, 'add');
                $changes++;
            }
        }
        ...
        foreach ($a1 as $f1) {
            ...
            if (!array_key_exists($f1hash, $f2hashes)) {
                $result1 .= $moewikioutput->moewiki_print_attachment_diff($f1, 'delete');
                $changes++;
            }
        }
        $result1 .= '</ul>';
