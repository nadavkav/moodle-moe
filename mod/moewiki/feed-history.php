...
// Get history
$changes = moewiki_get_page_history($pageversion->pageid, false, 0, MOEWIKI_FEEDSIZE);
...
$pagetitle = $pageversion->title === '' ? get_string('startpage', 'moewiki') :
...
$feeddescription = get_string('feeddescriptionhistory', 'moewiki');

// Domain name, used for IDs (we assume this is owned by site operator in 2007)
$domainname = preg_replace('/^.*\/\/(www\.)?(.*?)\/.*$/', '$2', $CFG->wwwroot);
$id = 'tag:'.$domainname.',2007:moewiki/'.$moewiki->id.'/wikihistory/changes/'.$pageversion->pageid;

$wikiparams = moewiki_display_wiki_parameters($pagename, $subwiki, $cm);
$pagelink = $CFG->wwwroot .'/mod/moewiki/history.php?'.$wikiparams;
...
$pageparams = moewiki_display_wiki_parameters($pagename, $subwiki, $cm);
...
foreach ($changes as $change) {
    ...
    if ($change->versionid == $pageversion->versionid) {
        ...
        $itemlink = $CFG->wwwroot.'/mod/moewiki/viewold.php?'.$pageparams.'&amp;version='.$change->versionid;
    }
    $itemtitle = $moewiki->name.' - '.$pagetitle.' ('.moewiki_nice_date($change->timecreated).')';
    ...
    $itemdescription = get_string('feeditemdescriptionnodate', 'moewiki', $a);
    if ($rss) {
...
<item>
  ...
  <guid>'.$CFG->wwwroot.'/mod/moewiki/history.php?'.$pageparams.'#v'.$change->versionid.'</guid>
</item>';
    } else {
        print '
<entry>
