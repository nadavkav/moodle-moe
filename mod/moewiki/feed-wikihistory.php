...
global $moewiki_nologin;
$moewiki_nologin = true;
...
// Get changes
if ($newpages) {
    $changes = moewiki_get_subwiki_recentpages($subwiki->id, 0, MOEWIKI_FEEDSIZE);
    $wikiparams = $wikiparams . '&amp;type=pages';
} else {
    $changes = moewiki_get_subwiki_recentchanges($subwiki->id, 0, MOEWIKI_FEEDSIZE);
}
...
$a->name = htmlspecialchars($moewiki->name);
$a->subtitle = get_string($newpages ? 'tab_index_pages' : 'tab_index_changes', 'moewiki');
$feedtitle = get_string('feedtitle', 'moewiki', $a);
$feedlink = 'http://'.htmlspecialchars($_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
$feeddescription=get_string('feeddescription'. ($newpages ? 'pages' : 'changes'), 'moewiki');

// Domain name, used for IDs (we assume this is owned by site operator in 2007)
$domainname = preg_replace('/^.*\/\/(www\.)?(.*?)\/.*$/', '$2', $CFG->wwwroot);
$id = 'tag:'.$domainname.',2007:moewiki/'.$moewiki->id.'/wikihistory/'.($newpages ? 'pages' : 'changes');

$pagelink = $CFG->wwwroot . '/mod/moewiki/wikihistory.php?' . $wikiparams;

print '<?xml version="1.0"?>
<?xml-stylesheet type="text/xsl" href="'.$CFG->wwwroot.'/mod/moewiki/feed.xsl"?>';
if ($rss) {
    print '
<rss version="2.0">
  <channel>
...
foreach ($changes as $change) {
    ...
    $a->date = moewiki_nice_date($change->timecreated);

    $itemtitle = $moewiki->name . ' - ' . ($change->title === ''
            ? get_string('startpage', 'moewiki') : htmlspecialchars($change->title));
    $itemlink = $CFG->wwwroot.'/mod/moewiki/view.php?'.$pageparams;
    if (!empty($change->previousversionid)) {
        $a->url = $CFG->wwwroot.'/mod/moewiki/diff.php?'.$pageparams.'&amp;v1='.
            $change->previousversionid.'&amp;v2='.$change->versionid;
        $a->main = get_string('feedchange', 'moewiki', $a);
    } else {
        $a->main = get_string('feednewpage', 'moewiki', $a);
    }
    $itemdescription = get_string('feeditemdescriptiondate', 'moewiki', $a);
    if ($rss) {
...
<item>
  ...
  <guid>'.$CFG->wwwroot.'/mod/moewiki/wikihistory.php?'.$wikiparams.'#v'.$change->versionid.'</guid>
</item>';
    } else {
        print '
<entry>
