...
$url = new moodle_url('/mod/moewiki/history.php', array('id' => $id));
$PAGE->set_url($url);
...
if ($compare) {
    ...
    if (count($versions) != 2) {
        print_error(get_string('mustspecify2', 'moewiki'));
    }
    sort($versions, SORT_NUMERIC);
    $wikiurlparams = html_entity_decode(moewiki_display_wiki_parameters($pagename, $subwiki, $cm), ENT_QUOTES);
...
$wikiinputs = moewiki_display_wiki_parameters($pagename, $subwiki, $cm, MOEWIKI_PARAMS_FORM);

// Do header
$atomurl = $CFG->wwwroot.'/mod/moewiki/feed-history.php?'.$wikiparams.
    '&amp;magic='.$subwiki->magic;
$rssurl = $CFG->wwwroot.'/mod/moewiki/feed-history.php?'.$wikiparams.
...
$title = get_string('historyfor', 'moewiki');
if ($pagename) {
...
echo $moewikioutput->moewiki_print_start($moewiki, $cm, $course, $subwiki, $pagename, $context, null, false, false, $meta, $title);

// Get history
$changes = moewiki_get_page_history($pageversion->pageid, $candelete);
moewiki_print_tabs('history', $pagename, $subwiki, $cm, $context, true, $pageversion->locked);

print_string('advice_history', 'moewiki', "view.php?$wikiparams");
...
if ($candelete) {
    ...
    if ($found) {
        print '<p class="ouw_deletedpageinfo">'.get_string('pagedeletedinfo', 'moewiki').'</p>';
    }
}
...
<thead>
<tr><th scope='col'>".get_string('date')."</th><th scope='col'>".get_string('time')."</th><th><span class='accesshide'>".get_string('actionheading', 'moewiki')."</span>
</th>";
if ($moewiki->enablewordcount) {
    print "<th scope='col'>".get_string('words', 'moewiki')."</th>";
}
if ($overwritten) {
    print '<th scope="col">'.get_string('importedfrom', 'moewiki').'</th>';
}
print "<th scope='col'>".get_string('changedby', 'moewiki')."</th><th scope='col'><span class='accesshide'>".get_string('compare', 'moewiki')."</span></th>";
...
foreach ($changes as $change) {
    ...
    if ($nextchange) {
        $changelink = " <small>(<a href='diff.php?$wikiparams&amp;v2={$change->versionid}&amp;v1={$nextchange->versionid}'>".
                get_string('changes', 'moewiki')."<span class=\"accesshide\"> $lastdate $createdtime</span></a>)</small>";
    } else {
        $changelink = '';
    }
    $revertlink = '';
    ...
    if ($candelete) {
        ...
        if (!empty($change->deletedat)) {
            ...
            $strdelete = get_string('undelete', 'moewiki');
    ...
    $selectaccessibility = get_string('historycompareaccessibility', 'moewiki', $a);
    ...
    if ($moewiki->enablewordcount) {
        if ($change->previouswordcount) {
            $wordcountchanges = moewiki_wordcount_difference($change->wordcount, $change->previouswordcount, true);
        } else {
...
$input = '<input id="ouw_comparebutton" type="submit" value="' .
        get_string('compareselected', 'moewiki') . '" class="osep-smallbutton" />';
