...
/**
 * Import page content from other moewiki activities into this wiki.
 * @package mod
 * @subpackage moewiki
...
require($CFG->dirroot . '/mod/moewiki/import_form.php');

$pageparams = moewiki_display_wiki_parameters($pagename, $subwiki, $cm, MOEWIKI_PARAMS_ARRAY);
...
$url = new moodle_url('/mod/moewiki/import.php', array('id' => $id, 'page' => $pagename));
$PAGE->set_url($url);
...
echo $moewikioutput->moewiki_print_start($moewiki, $cm, $course, $subwiki, get_string('import', 'moewiki'), $context);

echo $OUTPUT->heading(get_string('import', 'moewiki'));

if ((!$subwiki->canedit) || (!$moewiki->allowimport)) {
...
echo '<div class="moewiki_import moewiki_import_step' . $curstep . '" id="moewiki_belowtabs">';
...
if ($curstep == 1) {
    ...
    foreach ($courses as $listcourse) {
        ...
        $allwikis = $modinfo->get_instances_of('moewiki');
        unset($allwikis[$moewiki->id]);// Don't include current activity.
        ...
        foreach ($allwikis as $wikiact) {
            ...
            if (!$wikiact->uservisible ||
                    !has_capability('mod/moewiki:view', $wikicontext)) {
                continue;
            }
            ...
            $wikiinst = $DB->get_record('moewiki', array('id' => $wikiact->instance));
            if (!$wikiinst) {
                continue;
            }
            if ($wikiinst->subwikis == MOEWIKI_SUBWIKIS_SINGLE) {
                // Course wiki, check subwiki and start page exists.
                if (!$wikisubwiki = $DB->get_record_select('moewiki_subwikis', 'wikiid = ? AND
                        groupid IS NULL AND userid IS NULL', array($wikiinst->id), 'id')) {
                    $wikiob->nocontent = true;
                } else if (!moewiki_subwiki_content_exists($wikisubwiki->id)) {
                    $wikiob->nocontent = true;
                } else {
                    $wikiob->selectordefault = $wikisubwiki->id;
                }
            } else if ($wikiinst->subwikis == MOEWIKI_SUBWIKIS_GROUPS) {
                ...
                foreach ($groups as $group) {
                    ...
                    if ($wikisubwiki = $DB->get_record_select('moewiki_subwikis', 'wikiid = ?
                            AND groupid = ? AND userid IS NULL', array($wikiinst->id, $group->id), 'id')) {
                        if (moewiki_subwiki_content_exists($wikisubwiki->id)) {
            ...
            } else if ($wikiinst->subwikis == MOEWIKI_SUBWIKIS_INDIVIDUAL) {
                ...
                if (!has_capability('mod/moewiki:viewallindividuals', $wikicontext)) {
                    if (!has_capability('mod/moewiki:viewgroupindividuals', $wikicontext)) {
    ...
    if (!$wikisfound) {
        ...
        echo $OUTPUT->notification(get_string('unabletoimport', 'moewiki'));
        unset($pageparams['step']);
        echo $OUTPUT->continue_button(new moodle_url('/mod/moewiki/view.php', $pageparams));
    }
} else if ($curstep == 2) {
    ...
    moewiki_get_wikiinfo($selectedact, $selectedsubwiki, $selectedmoewiki, $importedfromcourse);

    echo html_writer::tag('p', get_string('import_selectwiki', 'moewiki', $selectedact->get_formatted_name()));

    // Build page selector.
    $pages = '';
    $index = moewiki_get_subwiki_index($selectedsubwiki->id);
    moewiki_build_tree($index);
    // Print out in hierarchical form...
    $pages .= html_writer::start_tag('ul', array('class' => 'ouw_indextree'));
    $pages .= moewiki_tree_index('moewiki_display_wikiindex_page_in_index', reset($index)->pageid,
    ...
    foreach ($index as $indexitem) {
        if (count($indexitem->linksfrom) == 0 && $indexitem->title !== '') {
            ...
            $orphans .= moewiki_tree_index('moewiki_display_wikiindex_page_in_index',
                    $indexitem->pageid, $orphanindex, $selectedsubwiki, $selectedact, null, true);
        }
    }
    if (!empty($orphans)) {
        $pages .= $OUTPUT->heading(get_string('orphanpages', 'moewiki'), 3);
        $pages .= html_writer::start_tag('ul', array('class' => 'ouw_indextree'));
        $pages .= $orphans;
        $pages .= html_writer::end_tag('ul');
    }
    $PAGE->requires->yui_module('moodle-mod_moewiki-pageselector', 'M.mod_moewiki.pageselector.init', array($pagelist));
    // Prepare form parameters.
    $wikiinfo = array('importid' => $selectedact->id , 'subwikiid' => $selectedsubwiki->id);
    $pageparams['step']++;
    $pageparams['courseid'] = $importedfromcourse->id;
    $form = new mod_moewiki_import_pageselect_form(null, array('params' => array_merge($pageparams,
    ...
    if ($pages = optional_param('pages', null, PARAM_SEQUENCE)) {
        ...
        foreach ($_POST as $key => $value) {
            if (strpos($key, 'page') === 0) {
                ...
                moewiki_add_linkedpages($pagelist, $index, $value);
            }
        }
        $pagelist = array_unique($pagelist);
    }
    echo $OUTPUT->heading(get_string('import_confirm', 'moewiki'), 3);
    // Setup confirm form.
    $confirmdata = array(
    ...
    foreach ($pagelist as $page) {
        ...
        if ($ipagename == '') {
            ...
            array_unshift($confirmdata['pages'], get_string('startpage', 'moewiki'));
        } else {
            $confirmdata['pages'][] = $ipagename;
        }
        // Check if this page already exists, and is locked.
        if ($pageexists = moewiki_get_current_page($subwiki, $ipagename, MOEWIKI_GETPAGE_ACCEPTNOVERSION)) {
            $locked = false;
            if ($pageexists->locked || (moewiki_is_page_locked($pageexists->pageid) &&
                    !has_capability('mod/moewiki:overridelock', $context))) {
                $locked = true;
                if ($ipagename != '') {
    ...
    if (!$confirmdata['startselected'] && !empty($pagelist)) {
        ...
        $confirmdata['wikipages']['-1'] = get_string('import_confirm_linkfrom_newpage', 'moewiki');
        $thisindex = moewiki_get_subwiki_allpages_index($subwiki);
        foreach ($thisindex as $thispage) {
            if ($thispage->title == '') {
                $thispage->title = get_string('startpage', 'moewiki');
            }
            if ($thispage->locked) {
                // Prevent selection by adding a note that page is locked.
                $confirmdata['wikipages'][$thispage->pageid] = $thispage->title .
                '[' . get_string('import_lockedpage', 'moewiki') . ']';
            } else {
                $confirmdata['wikipages'][$thispage->pageid] = $thispage->title;
            }
        }
        if (empty($thisindex)) {
            ...
            $confirmdata['wikipages'][0] = get_string('startpage', 'moewiki');
        }
    }
    $wikiinfo = array('importid' => $selectedact->id , 'subwikiid' => $selectedsubwiki->id,
    ...
    $pageparams['step']++;
    $pageparams['courseid'] = $importedfromcourse->id;
    $form = new mod_moewiki_import_confirm_form(null, array('params' => array_merge($pageparams, $wikiinfo),
    ...
    $index = moewiki_get_subwiki_allpages_index($selectedsubwiki);
    ...
    echo $OUTPUT->heading(get_string('import_process', 'moewiki'), 3);
    try {
        ...
        } else if ($pageexists = moewiki_get_current_page($subwiki, '', MOEWIKI_GETPAGE_CREATE)) {
        ...
        if ($startpagemerge == 1) {
            if ($currentpageexists = moewiki_get_current_page($subwiki, get_string('importedstartpage', 'moewiki'),
                    MOEWIKI_GETPAGE_ACCEPTNOVERSION)) {
                $checkpagelist[] = $currentpageexists->pageid;
            }
        }
        if ($linkfrom == -1) {
            if ($currentpageexists = moewiki_get_current_page($subwiki, get_string('importedpages', 'moewiki'),
                        MOEWIKI_GETPAGE_ACCEPTNOVERSION)) {
                $checkpagelist[] = $currentpageexists->pageid;
            }
        }
        ...
        foreach ($checkpagelist as $page) {
            if (!isset($index[$page])) {
                $ipagename = $DB->get_field('moewiki_pages', 'title', array('id' => $page), MUST_EXIST);
            } else {
                $ipagename = $index[$page]->title;
            }
            if ($ipagename == '' && $startpagemerge == 1) {
                // If start page exists then set imported start page in conflicts array.
                if (moewiki_get_current_page($subwiki, $ipagename, MOEWIKI_GETPAGE_ACCEPTNOVERSION)) {
            ...
            // Check if this page already exists, and is locked.
            if ($currentpageexists = moewiki_get_current_page($subwiki, $ipagename, MOEWIKI_GETPAGE_ACCEPTNOVERSION)) {
                $conflicts[$page] = $ipagename;
                if (has_capability('mod/moewiki:overridelock', $context)) {
                    moewiki_override_lock($currentpageexists->pageid);
                }
                list ($lock, $by) = moewiki_obtain_lock($moewiki, $currentpageexists->pageid);
                if ($currentpageexists->locked) {
                    $lock = false;// No one can edit.
                }
                if (!$lock) {
                    throw new moodle_exception('import_process_locked', 'moewiki');
                }
            }
        }
        ...
        $pbar = new progress_bar('mod_moewiki_import', 500, true);
        for ($len = count($pagelist), $a = 0; $a < $len; $a++) {
            ...
            if (in_array($page, array_keys($conflicts))) {
                ...
                if ($pageinfo->title == '' && $startpagemerge == 1) {
                    ...
                    $importedstartpage = get_string('importedstartpage', 'moewiki');
                    $newpage = true;
                    // See whether an 'Imported start page' already exists in the target wiki.
                    if ($importedstartpageexists = moewiki_get_current_page($subwiki, $importedstartpage, MOEWIKI_GETPAGE_CREATE)) {
                        ...
                        $versionid = moewiki_save_new_version($course, $cm, $moewiki, $subwiki, $importedstartpageexists->title,
                        ...
                        $updatepagelist[] = get_string('startpage', 'moewiki');
                    }
                } else {
                    ...
                    if ($conflictmerge == 0 || ($pageinfo->title == '' && $startpagemerge == 0)) {
                        ...
                        if (!$currentpageexists = moewiki_get_current_page($subwiki, $pageinfo->title,
                                MOEWIKI_GETPAGE_REQUIREVERSION)) {
                        ...
                        // Save new version of existing page.
                        $versionid = moewiki_save_new_version($course, $cm, $moewiki, $subwiki, $currentpageexists->title,
                        ...
                        // Save new version of existing page.
                        $versionid = moewiki_save_new_version($course, $cm, $moewiki, $subwiki, $pageinfo->title, $importedcontent,
                ...
                $pageversion = moewiki_get_current_page($subwiki, $pageinfo->title, MOEWIKI_GETPAGE_CREATE);
                $versionid = moewiki_save_new_version($course, $cm, $moewiki, $subwiki, $pageinfo->title, $pageinfo->xhtml,
                        -1, -1,  -1, null, null, null,  $pageinfo->versionid);
            ...
            // Add any files.
            if ($oldfiles = $fs->get_area_files($modcontext->id, 'mod_moewiki', 'content', $prevversion)) {
            ...
            // Add any attachments.
            if ($oldfiles = $fs->get_area_files($modcontext->id, 'mod_moewiki', 'attachment', $prevversion)) {
                foreach ($oldfiles as $oldfile) {
                    try {
                        // Copy this file to the version record.
                        $fs->create_file_from_storedfile(array(
            ...
            $pbar->update($a + 1, count($pagelist), get_string('import_process', 'moewiki'));
        ...
        if (!is_null($linkfrom)) {
            ...
            if ($linkfrom == 0) {
                ...
                $updatepagelist[] = get_string('startpage', 'moewiki');
                $pageversion = moewiki_get_current_page($subwiki, null, MOEWIKI_GETPAGE_CREATE);
                //  Add links into selected page content.
                $versionid = moewiki_save_new_version($course, $cm, $moewiki, $subwiki, '', $mergexhtml);
            } else if ($linkfrom == -1) {
                ...
                $importedpages = get_string('importedpages', 'moewiki');
                // Create/update(merge) 'Imported pages'.
                $linkfrompage = moewiki_get_current_page($subwiki, $importedpages, MOEWIKI_GETPAGE_CREATE);
                ...
                $versionid = moewiki_save_new_version($course, $cm, $moewiki, $subwiki, $linkfrompage->title, $linkfrompage->xhtml);
                // Add page names to $newpagelist|$updatepagelist array as appropriate.
                if ($newpage) {
                    ...
                    moewiki_save_new_version($course, $cm, $moewiki, $subwiki, $title, $xhtmlcontent);
                } else {
                ...
                if (!$pageinfo = $DB->get_record('moewiki_pages', array('id' => $linkfrom))) {
                    throw new moodle_exception('Error. Page to import into missing.');
                }
                // Store page name so we can go there.
                $linkfrom = $pageinfo->title;
                // Add links into selected page content.
                $linkfrompage = moewiki_get_current_page($subwiki, $pageinfo->title, MOEWIKI_GETPAGE_CREATE);
                $linkfrompageid = $linkfrompage->pageid;
                // Merge links xhtml into import into page xhtml content.
                $linkfrompage->xhtml = $linkfrompage->xhtml . $mergexhtml;
                $versionid = moewiki_save_new_version($course, $cm, $moewiki, $subwiki, $linkfrompage->title, $linkfrompage->xhtml);
        ...
        echo $OUTPUT->heading(get_string('import_process_summary', 'moewiki'), 3);
        echo html_writer::tag('p', $warned ? get_string('import_process_summary_warn', 'moewiki') :
                get_string('import_process_summary_success', 'moewiki'));
        if ($startpagelocked) {
            // Start page locked.
            echo html_writer::div(get_string('import_process_startpage_locked', 'moewiki'));
        }
        if ($pagelocked) {
            // Page imported into locked.
            echo html_writer::div(get_string('import_process_locked', 'moewiki'));
        }

        // Show pages imported.
        echo html_writer::start_div();
        echo get_string('import_process_summary_imported', 'moewiki');
        echo html_writer::alist(array_unique($newpagelist));
        echo html_writer::end_div();
        if (!empty($updatepagelist)) {
            // Show pages updated.
            echo html_writer::start_div();
            echo get_string('import_process_summary_updated', 'moewiki');
        ...
        $wikiinfo = array('importid' => $selectedact->id , 'subwikiid' => $selectedsubwiki->id,
                ...
                'sesskey' => sesskey());
        $continue = new moodle_url('/mod/moewiki/import.php', array_merge($pageparams, $wikiinfo));
        $pageparams['step']++;
        $pageparams['cancel'] = get_string('cancel');
        $cancel = new moodle_url('/mod/moewiki/import.php', array_merge($pageparams, $wikiinfo));
        echo $OUTPUT->confirm($e->getMessage(), $cancel, new moodle_url('/mod/moewiki/view.php', $pageparams));
 ...
 */
function moewiki_add_linkedpages(&$pagelist, &$index, $value) {
    if (!in_array($value, $pagelist) && in_array($value, array_keys($index))) {
        ...
        foreach ($newpages as $newvalue) {
            moewiki_add_linkedpages($pagelist, $index, $newvalue);
        }
    }
}
