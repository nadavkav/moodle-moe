...
class mod_moewiki_import_wikiselect_form extends moodleform {
    protected function definition() {
        ...
        if (!empty($this->_customdata['actlink'])) {
            $actlink = '<div class="moewiki_import_link">' . $this->_customdata['actlink'] .
                '</div>';
            $mform->addElement('html', $actlink);
        }

        if ($wikiinfo->nocontent) {
            $warn = '<div class="moewiki_import_nocontent">' .
                    get_string('import_nocontent', 'moewiki') . '</div>';
        ...
        if (!empty($wikiinfo->selector)) {
            ...
            $mform->addElement('select', 'subwikiid' . $wikiinfo->cm->id,
                    get_string('import_selectsubwiki', 'moewiki'), $wikiinfo->selector);
            $mform->addHelpButton('subwikiid' . $wikiinfo->cm->id, 'import_selectsubwiki', 'moewiki');
        ...
        $this->add_action_buttons(false, get_string('import_selectwiki', 'moewiki',
                $wikiinfo->cm->name));
...
class mod_moewiki_import_pageselect_form extends moodleform {
    protected function definition() {
        global $CFG;
        ...
        $this->add_action_buttons(true, get_string('import', 'moewiki'));
    }
}

class mod_moewiki_import_confirm_form extends moodleform {
    protected function definition() {
        global $CFG, $OUTPUT;
        ...
        $mform->addElement('header', 'infohead', get_string('import_confirm_infoheader', 'moewiki'));
        $mform->setExpanded('infohead');

        // Create confirm form layout.
        $mform->addElement('static', 'importfrom', get_string('import_confirm_from', 'moewiki'),
                $confirmdata['importfrom']);

        if (!empty($confirmdata['pages'])) {
            $mform->addElement('static', 'importpages', get_string('import_confirm_pages', 'moewiki'),
                html_writer::alist($confirmdata['pages']));
            $mform->addHelpButton('importpages', 'import_confirm_pages', 'moewiki');
        } else {
            $mform->addElement('html', html_writer::tag('p',
                    get_string('import_confirm_pages_none', 'moewiki'),
                        array('class' => 'moewiki_import_warn')));
        }

        $conflicts = $confirmdata['conflicts'];
        if (!empty($conflicts)) {
            $mform->addElement('header', 'conflicthead', get_string('import_confirm_mergeheader', 'moewiki'));
            ...
            foreach ($conflicts as $pagename => $locked) {
                ...
                $pagelink = new moodle_url('/mod/moewiki/view.php', array('id' => $params['id'], 'page' => $pagename));
                $conflictlist .= html_writer::tag('li', html_writer::link($pagelink, $pagename) . ' ' . $img);
            }
            $conflictlist .= html_writer::end_tag('ul');
            $mform->addElement('html', html_writer::tag('p',
                    get_string('import_confirm_conflicts_instruct', 'moewiki'), array('class' => 'moewiki_import_info')));
            $mform->addElement('static', 'conflictpages', get_string('import_confirm_conflicts', 'moewiki'),
                    $conflictlist);
            if (!$confirmdata['lockedpage']) {
                // Get merge strategy.
                $radioarr = array();
                $radioarr[] = $mform->createElement('radio', 'conflictmerge', '',
                        get_string('import_confirm_conflicts_option1', 'moewiki'), 0);
                $radioarr[] = $mform->createElement('radio', 'conflictmerge', '',
                        get_string('import_confirm_conflicts_option2', 'moewiki'), 1);
                $mform->addGroup($radioarr, 'conflictmergegrp',
                        get_string('import_confirm_conflicts_label', 'moewiki'), array('<br />'), false);
            }
        }
        ...
        if ($confirmdata['lockedpage']) {
            ...
            $mform->addElement('html', html_writer::tag('p',
                    get_string('import_confirm_conflicts_lockedwarn', 'moewiki'), array('class' => 'moewiki_import_warn')));
        } else {
            $mform->addElement('header', 'linkhead', get_string('import_confirm_linkheader', 'moewiki'));
            ...
            if ($confirmdata['startselected']) {
                ...
                $radioarr[] = $mform->createElement('radio', 'startpagemerge', '',
                        get_string('import_confirm_linkfrom_startpage1', 'moewiki'), 0, $disabled);
                $radioarr[] = $mform->createElement('radio', 'startpagemerge', '',
                        get_string('import_confirm_linkfrom_startpage2', 'moewiki'), 1);
                $mform->addGroup($radioarr, 'startpagemergegrp',
                        get_string('import_confirm_linkfrom_startpage', 'moewiki'), array('<br />'), false);
                ...
                $select = $mform->createElement('select', 'linkfrom', get_string('import_confirm_linkfrom', 'moewiki'));
                foreach ($confirmdata['wikipages'] as $id => $name) {
                    $disabled = strpos($name, get_string('import_lockedpage', 'moewiki')) ?
                        array('disabled' => 'disabled') : null;
                    $select->addOption($name, $id, $disabled);
                }
                $mform->addElement($select);
                $mform->addHelpButton('linkfrom', 'import_confirm_linkfrom', 'moewiki');
                ...
                } else if (in_array(get_string('startpage', 'moewiki'), $confirmdata['wikipages'])) {
                    $mform->setDefault('linkfrom', array_search(get_string('startpage', 'moewiki'), $confirmdata['wikipages']));
                }
            }
        }
