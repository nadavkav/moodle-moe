...
function moewiki_update_instance($data, $mform) {
    global $CFG, $DB;
    ...
    foreach ($nullvalues as $nullvalue) {
        if (empty($data->{$nullvalue})) {
            ...
            $DB->set_field('moewiki', $nullvalue, null, array('id' => $data->id));
        }
    }
    if (strlen(preg_replace('/(<.*?>)|(&.*?;)|\s/', '', $data->intro)) == 0) {
        unset($data->intro);
        $DB->set_field('moewiki', 'intro', null, array('id' => $data->id));
    }

    moewiki_grade_item_update($data);

    if (!$cm = get_coursemodule_from_id('moewiki', $data->coursemodule)) {
    ...
    if ($filename = $mform->get_new_filename('template_file')) {
        ...
        $context = context_module::instance($cmid);
        $fs = get_file_storage();
        $fs->delete_area_files($context->id, 'mod_moewiki', 'template', $data->id);
        // Add template file.
        $file = $mform->save_stored_file('template_file', $context->id, 'mod_moewiki', 'template', $data->id, '/', $filename);
        $DB->set_field('moewiki', 'template', '/'.$file->get_filename(), array('id' => $data->id));
        // Check for empty wikis (i.e. wikis without a start page already created).
        $subwikis = moewiki_get_subwikis($data->id);
        $moewiki = $DB->get_record_select('moewiki', 'id = ?', array($data->id));
        foreach ($subwikis as $subwiki) {
            if (!moewiki_subwiki_content_exists($subwiki->id)) {
...
function moewiki_delete_instance($id) {
    global $DB, $CFG;

    require_once($CFG->dirroot.'/mod/moewiki/locallib.php');

    $cm = get_coursemodule_from_instance('moewiki', $id, 0, false, MUST_EXIST);

    // Delete associated template data.
    $context = context_module::instance($cm->id);
    $fs = get_file_storage();
    $fs->delete_area_files($context->id, 'mod_moewiki', 'template', $id);
    ...
    // Delete grade
    $moewiki = $DB->get_record('moewiki', array('id' => $cm->instance));
    moewiki_grade_item_delete($moewiki);
    ...
    $versions = $DB->get_records_sql("SELECT DISTINCT v.id
                        FROM {moewiki_subwikis} s
                        INNER JOIN {moewiki_pages} p ON p.subwikiid = s.id
    ...
    if (!empty($versions)) {
        ...
        $DB->delete_records_select('moewiki_links', "fromversionid $vsql", $vparams);
    }
    ...
    if (!empty($pages)) {
        ...
        $DB->delete_records_select('moewiki_versions', "pageid $psql", $pparams);
        $DB->delete_records_select('moewiki_locks', "pageid $psql", $pparams);
        $DB->delete_records_select('moewiki_sections', "pageid $psql", $pparams);
    }
    ...
    if (!empty($subwikis)) {
        ...
        $DB->delete_records_select('moewiki_pages', "subwikiid $swsql", $swparams);
    }

    $DB->delete_records_select('moewiki_subwikis', 'wikiid = ?', array($id));
    $DB->delete_records('moewiki', array('id' => $id));
    return true;
}
 ...
 */
function moewiki_get_extra_capabilities() {
 ...
 */
function moewiki_ousearch_update_all($feedback = null, $courseid = 0) {
    global $CFG, $DB;
    require_once($CFG->dirroot.'/mod/moewiki/locallib.php');
    ...
    $sql = "SELECT cm.id, cm.course, cm.instance, w.subwikis
                                            ...
                                            INNER JOIN {moewiki} w ON cm.instance = w.id
                                        WHERE m.name = 'moewiki' {$coursecriteria}";
    ...
    foreach ($coursemodules as $coursemodule) {
        ...
        $sql = "SELECT p.id, p.title, v.xhtml, v.timecreated, sw.groupid, sw.userid
            ...
            INNER JOIN {moewiki_pages} p ON p.subwikiid = sw.id
            INNER JOIN {moewiki_versions} v ON v.id = p.currentversionid
        ...
        foreach ($rs as $result) {
            ...
            $doc->init_module_instance('moewiki', $coursemodule);
            if ($result->groupid) {
 ...
 */
function moewiki_ousearch_get_document($document) {
    global $CFG, $DB;
    ...
    $sql = "SELECT w.name AS activityname, p.title AS title, v.xhtml AS content
        ...
        INNER JOIN {moewiki} w ON cm.instance = w.id
        INNER JOIN {moewiki_subwikis} sw ON sw.wikiid = w.id
        INNER JOIN {moewiki_pages} p ON p.subwikiid = sw.id
        INNER JOIN {moewiki_versions} v ON v.id = p.currentversionid
            WHERE cm.id = ?
            $titlecondition
    ...
    if ($result->title == '') {
        $result->title = get_string('startpage', 'moewiki');
    }
    $result->activityurl = new moodle_url('/mod/moewiki/view.php', array('id' => $document->coursemoduleid));
...
/**
 * Indicates API features that the moewiki supports.
 *
 ...
 */
function moewiki_get_completion_state($course, $cm, $userid, $type) {
    global $CFG, $DB;
    ...
    $countsql = "SELECT COUNT(1)
            FROM {moewiki_versions} v
                ...
                INNER JOIN {moewiki_subwikis} s ON s.id = p.subwikiid
            WHERE v.userid = ? AND v.deletedat IS NULL AND s.wikiid = ?";

    $result = $type; // Default return value

    if ($moewiki->completionedits) {
        $value = $moewiki->completionedits <= $DB->get_field_sql($countsql, array($userid, $moewiki->id));
 ...
 */
function moewiki_print_overview($courses, &$htmlarray) {
    global $USER, $CFG, $DB;
    ...
    if (!$wikis = get_all_instances_in_courses('moewiki', $courses)) {
        return;
    }

    // get all moewiki logs in ONE query (much better!)
    $params = array();
    ...
    $sql .= ") AND l.module = 'moewiki' AND action = 'edit' "
    ...
    $strwikis = get_string('modulename', 'moewiki');
    $strnumrespsince1 = get_string('overviewnumentrysince1', 'moewiki');
    $strnumrespsince = get_string('overviewnumentrysince', 'moewiki');
    ...
    foreach ($wikis as $wiki) {
        ...
        if (array_key_exists($wiki->id, $new) && !empty($new[$wiki->id])) {
            ...
            if ($count > 0) {
                ...
                $viewurl = new moodle_url('/mod/moewiki/view.php', array('id' => $wiki->coursemodule));
 ...
 */
function moewiki_user_outline($course, $user, $mod, $wiki) {
    global $DB, $CFG;

    // Get user grades.
    require_once("$CFG->libdir/gradelib.php");
    $grades = grade_get_grades($course->id, 'mod', 'moewiki', $wiki->id, $user->id);
    ...
    $vsql = "SELECT v.id AS versionid, v.timecreated
               ...
               JOIN {moewiki_subwikis} s ON s.id = p.subwikiid
               JOIN {moewiki_versions} v ON v.pageid = p.id
              WHERE v.userid = :userid
                AND s.wikiid = :moewikiid
    ...
    if (!empty($versions)) {
        $result = new stdClass();
        $result->info = get_string('numedits', 'moewiki', count($versions));
 ...
 * @param stdClass $wiki
 */
function moewiki_user_complete($course, $user, $mod, $wiki) {
    ...
    echo '<div class="moewiki_user_complete_report">';
    if (empty($usergroups)) {
        $subwiki = moewiki_get_subwiki($course, $wiki, $mod, $context, 0, $user->id, true);
        $canview = moewiki_can_view_participation($course, $wiki, $subwiki, $mod, $USER->id);
        list($newuser, $changes) = moewiki_get_user_participation($user->id, $subwiki);
        echo $moewikioutput->moewiki_render_user_participation($user, $changes, $mod, $course, $wiki,
            $subwiki, '', 0, '', $canview, $context, $fullname,
            false, '');
    } else {
        foreach ($usergroups as $group) {
            $subwiki = moewiki_get_subwiki($course, $wiki, $mod, $context, $group->id, $user->id, true);
            $canview = moewiki_can_view_participation($course, $wiki, $subwiki, $mod, $USER->id);
            list($newuser, $changes) = moewiki_get_user_participation($user->id, $subwiki);
...
/**
 * Serves the moewiki attachments. Implements needed access control ;-)
 *
 * @param object $course
 * @param object $cm
 ...
 */
function moewiki_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload) {
    global $CFG, $DB, $USER;
    ...
    if (!$version = $DB->get_record('moewiki_versions', array('id' => $versionid))) {
        return false;
    }

    $fs = get_file_storage();
    $relativepath = implode('/', $args);
    $fullpath = "/$context->id/mod_moewiki/$filearea/$versionid/$relativepath";
    if (!$file = $fs->get_file_by_hash(sha1($fullpath)) or $file->is_directory()) {
        return false;
    }

    require_capability('mod/moewiki:view', $context);

    send_stored_file($file, 0, 0, true); // download MUST be forced - security!
}

/**
 * File browsing support for moewiki module.
 ...
 * or cannot access)
 */
function moewiki_get_file_info($browser, $areas, $course, $cm, $context, $filearea,
    ...
    if (!has_capability('mod/moewiki:view', $context)) {
        return null;
    }
    if (!$pageid = $DB->get_field('moewiki_versions', 'pageid',
            array('id' => $itemid), IGNORE_MISSING)) {
        return null;
    }
    if (!$subwikiid = $DB->get_field('moewiki_pages', 'subwikiid',
            array('id' => $pageid), IGNORE_MISSING)) {
        return null;
    }
    $groupid = $DB->get_field('moewiki_subwikis', 'groupid',
    ...
    $userid = $DB->get_field('moewiki_subwikis', 'userid',
            array('id' => $subwikiid), IGNORE_MISSING);
    if ($userid) {
        if ($userid != $USER->id && !has_capability('mod/moewiki:viewallindividuals', $context)) {
    ...
    if (!($storedfile = $fs->get_file($context->id, 'mod_moewiki', $filearea, $itemid,
            $filepath, $filename))) {
        return null;
    }
...
/**
 * Create grade item for given moewiki
 *
 * @param object $moewiki object with extra cmidnumber
 * @param mixed optional array/object of grade(s); 'reset' means reset grades in gradebook
 * @return int 0 if ok, error code otherwise
 */
function moewiki_grade_item_update($moewiki, $grades = null) {
    global $CFG;
    require_once($CFG->libdir.'/gradelib.php');

    $params = array('itemname' => $moewiki->name);

    if ($moewiki->grade > 0) {
        $params['gradetype'] = GRADE_TYPE_VALUE;
        $params['grademax']  = $moewiki->grade;
        $params['grademin']  = 0;

    } else if ($moewiki->grade < 0) {
        $params['gradetype'] = GRADE_TYPE_SCALE;
        $params['scaleid']   = -$moewiki->grade;
    ...
    return grade_update('mod/moewiki', $moewiki->course, 'mod',
        'moewiki', $moewiki->id, 0, $grades, $params);
}

/**
 * Deletes grade item for given moewiki.
 *
 * @param object $moewiki object
 * @return int GRADE_UPDATE_xx constant
 */
function moewiki_grade_item_delete($moewiki) {
    global $CFG;
    require_once($CFG->libdir.'/gradelib.php');

    return grade_update('mod/moewiki', $moewiki->course, 'mod',
        'moewiki', $moewiki->id, 0, null, array('deleted' => 1));
}
 ...
 * @param cm_info $cm
 */
function moewiki_cm_info_dynamic(cm_info $cm) {
    if (!has_capability('mod/moewiki:view',
 ...
 * @param cm_info $cm
 */
function moewiki_cm_info_view(cm_info $cm) {
    global $CFG;
    if (!$cm->uservisible) {
        return;
    }
    ...
    $lastpostdate = moewiki_get_last_modified($cm, $cm->get_course());
    if (!empty($lastpostdate)) {
        $cm->set_after_link(html_writer::span(get_string('lastmodified', 'moewiki',
                userdate($lastpostdate, get_string('strftimerecent', 'moewiki'))), 'lastmodtext moewikilmt'));
    }
}
 ...
 * @return array
 */
function moewiki_get_ourecent_activity($course) {
    global $CFG;
    require_once($CFG->dirroot . '/mod/moewiki/locallib.php');
    ...
    foreach ($modinfo->get_instances_of('moewiki') as $wiki) {
        if ($wiki->uservisible) {
            $lastpostdate = moewiki_get_last_modified($wiki, $wiki->get_course());
            if (!empty($lastpostdate)) {
                $data = new stdClass();
                $data->cm = $wiki;
                $data->text = get_string('lastmodified', 'moewiki',
                        userdate($lastpostdate, get_string('strftimerecent', 'moewiki')));
 ...
 * @return array
 */
function moewiki_get_view_actions() {
 ...
 * @return array
 */
function moewiki_get_post_actions() {
