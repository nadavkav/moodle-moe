...
M.mod_moewiki_view = {
    ...
    init : function(Y, args) {
        ...
        // or show annotations is disabled in moewiki settings
        ...
        // check to see whether there anno tags to show - either because there are none
        // or show annotations is disabled in moewiki settings
        var annospans = this.Y.all('span.moewiki-annotation-tag');
        if (annospans) {
            this.moewikiShowAllAnnotations("none");
    ...
    show_annotation_icons : function(show) {
        ...
        var container = this.Y.one('.moewiki-content');
        var hideclass = 'moewiki-hide-annotations';
        if (show) {
    ...
    moewikiToggleFunction : function(target, link) {
        return function() {
    ...
    moewikiKeyFunction : function(link) {
        return function(e) {
    ...
    moewikiShowFormFunction : function(target, header, link) {
        return function() {
    ...
    moewikiSetFields : function() {
        ...
        pagename.onfocus = function() { _this.moewikiResetThisField(pagename); };
        pagename.onkeyup = function() { _this.moewikiClearDisabled(createbutton, pagename); };
        pagename.value = M.str.moewiki.typeinpagename;
        ...
        sectionname.onfocus = function() { _this.moewikiResetThisField(sectionname); };
        sectionname.onkeyup = function() { _this.moewikiClearDisabled(addbutton, sectionname); };
        sectionname.value = M.str.moewiki.typeinsectionname;
    },

    moewikiClearDisabled : function(element, field) {
    ...
    moewikiResetThisField : function(field) {
        if (field.notusedyet) {
            field.value = '';
    ...
    moewikiShowAllAnnotations : function(action) {
        annoboxes = this.Y.all('span.moewiki-annotation');
        annoboxes.each(function (annobox) {
            ...
            if (action == "block") {
                imgtag.set('alt', M.str.moewiki.collapseannotation);
                imgtag.set('title', M.str.moewiki.collapseannotation);
            } else if (action == "none") {
                imgtag.set('alt', M.str.moewiki.expandannotation);
                imgtag.set('title', M.str.moewiki.expandannotation);
            }
        });

        if(action == "block") {
            this.moewikiSwapAnnotationUrl("hide");
        } else if(action == "none") {
            this.moewikiSwapAnnotationUrl("show");
        }
    },

    moewikiSwapAnnotationUrl : function(action){
    ...
    moewikiShowHideAnnotation : function(id) {
        ...
        if (box.style.display == "block") {
            ...
            this.moewikiSwapAnnotationUrl("show");
            imgtag.alt = M.str.moewiki.expandannotation;
            imgtag.title = M.str.moewiki.expandannotation;
        } else {
            box.style.display = "block";
            imgtag.alt = M.str.moewiki.collapseannotation;
            imgtag.title = M.str.moewiki.collapseannotation;
            annoboxes = this.Y.all('span.moewiki-annotation');
    ...
    setupspans : function(span) {
        ...
        span.on('keydown', function(e) {
            ...
            if (evt.keyCode == 13 || evt.keyCode == 32) {
                _this.moewikiShowHideAnnotation("annotationbox" + span.get('id').substring(10));
...
M.mod_moewiki_annotate = {
    ...
    init : function(Y, args) {
        ...
        var _this = this;
        var save = M.util.get_string('add', 'moewiki');
        var cancel = M.util.get_string('cancel', 'moewiki');
        ...
        // setup keycodes
        var markers = this.Y.all('span.moewiki-annotation-marker');
        ...
        init : function(Y, args) {
            ...
            btns.on('click', function(e) {
                function savefail() {
                    ...
                    var content = M.util.get_string('savefailnetwork', 'moewiki');
                    var panel = new M.core.alert({
                        title: M.util.get_string('savefailtitle', 'moewiki'),
                        message: content,
                        render: true,
                    ...
                    cancel.on('click', function(e) {
                        var form = Y.one('#moewiki_belowtabs #mform1');
