...
$url = new moodle_url('/mod/moewiki/participation.php', $params);
$PAGE->set_url($url);
...
$participation = moewiki_get_participation($moewiki, $subwiki, $context, $groupid);

// is grading enabled and available for the current user
$grading_info = array();
if ($moewiki->grade != 0 && has_capability('mod/moewiki:grade', $context) &&
    ...
    $grading_info = grade_get_grades($course->id, 'mod',
        'moewiki', $moewiki->id, array_keys($participation));
}
...
// Headers
if (empty($download)) {
    echo $moewikioutput->moewiki_print_start($moewiki, $cm, $course, $subwiki, get_string('userparticipation', 'moewiki'), $context);

    // gets a message after grades updated
    if (isset($SESSION->moewikigradesupdated)) {
        $message = $SESSION->moewikigradesupdated;
        unset($SESSION->moewikigradesupdated);
        echo $OUTPUT->notification($message, 'notifysuccess');
    }
}

$moewikioutput->moewiki_render_participation_list($cm, $course, $pagename, $groupid, $moewiki,
