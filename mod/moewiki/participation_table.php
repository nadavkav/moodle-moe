...
/**
 * Class moewiki_participation_table
 * extends flexible_table to override header and download rules
 */
class moewiki_participation_table extends flexible_table {
    ...
    public function __construct($cm, $course, $moewiki, $pagename, $groupid = 0,
        $groupname, $grade = null) {

        $this->cm = $cm;
        $this->course = $course;
        $this->moewiki = $moewiki;
        $this->pagename = $pagename;
        $this->groupid = $groupid;
        $this->groupname = $groupname;
        $this->grade = $grade;
        parent::__construct('mod-moewiki-participation');
    }
    ...
    public function setup($download = '') {
        ...
        if (!empty($download)) {
            $this->extraheaders = array(
                ...
                format_string($this->moewiki->name, true),
            );
        ...
        $headers = array(
            '',
            get_string('user'),
            get_string('pagescreated', 'moewiki'),
            get_string('pageedits', 'moewiki'),
        );
        ...
        if ($this->moewiki->enablewordcount) {
            $columns[] = 'wordsadded';
            $columns[] = 'wordsdeleted';
            $headers[] = get_string('wordsadded', 'moewiki');
            $headers[] = get_string('wordsdeleted', 'moewiki');
        }

        if ($this->moewiki->allowimport) {
            $columns[] = 'importedfrom';
            $headers[] = get_string('pagesimported', 'moewiki');
        }
        ...
        $this->define_baseurl($CFG->wwwroot . '/mod/moewiki/participation.php?id=' .
    ...
    public function grade_form_header() {
        $output = '';
        $formattrs = array();
        $formattrs['action'] = new moodle_url('/mod/moewiki/savegrades.php');
    ...
    public function grade_form_footer() {
        ...
        $savegrades = html_writer::empty_tag('input', array('type' => 'submit',
            'name' => 'savegrades', 'value' => get_string('savegrades', 'moewiki')));
...
/**
 * Class moewiki_user_participation_table
 * extends flexible_table to override header and download rules
 */
class moewiki_user_participation_table extends flexible_table {
    ...
    public function __construct($cm, $course, $moewiki, $pagename,
        ...
        parent::__construct('mod-moewiki-user-participation');
    }
    ...
    public function setup($download = '') {
        ...
        $headers = array(
            get_string('date'),
            get_string('time'),
            get_string('page', 'moewiki')
        );
        if ($this->moewiki->enablewordcount) {
            if (empty($download)) {
                $columns[] = 'words';
                $headers[] = get_string('words', 'moewiki');
            } else {
                $columns[] = 'wordsadded';
                $columns[] = 'wordsdeleted';
                $headers[] = get_string('wordsadded', 'moewiki');
        ...
        if ($this->moewiki->allowimport) {
            $columns[] = 'importedfrom';
            $headers[] = get_string('importedfrom', 'moewiki');
        }
        ...
        $this->define_columns($columns);
        $this->define_headers($headers);
        $this->define_baseurl($CFG->wwwroot . '/mod/moewiki/userparticipation.php?id=' .
