...
$url = new moodle_url('/mod/moewiki/search.php', array('id'=>$id, 'user'=>$userid, 'query'=>$querytext));
$PAGE->set_url($url);
$moewikioutput = $PAGE->get_renderer('mod_moewiki');
echo $moewikioutput->moewiki_print_start($moewiki, $cm, $course, $subwiki, get_string('searchresults'), $context, null, null, null, '', '', $querytext);
...
$foundsomething = $query->display_results('search.php?'.moewiki_display_wiki_parameters('', $subwiki, $cm));
...
// Footer
moewiki_print_footer($course, $cm, $subwiki, null, 'search.php?query='.urlencode($querytext), $foundsomething ? null : 'searchfailure', $querytext);
