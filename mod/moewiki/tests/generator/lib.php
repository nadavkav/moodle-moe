...
class mod_moewiki_generator extends testing_module_generator {
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        if (!isset($record->subwikis) && !isset($options['subwikis'])) {
            $record->subwikis = MOEWIKI_SUBWIKIS_SINGLE;
        }
        ...
        if (!isset($record->name) && !isset($options['name'])) {
            $record->name = 'MOEWIKI' . $this->modcount;
    ...
    public function create_content($instance, $record = array()) {
        ...
        $cm = get_coursemodule_from_instance('moewiki', $instance->id);
        $context = context_module::instance($cm->id);
        // Setup subwiki.
        if (!isset($record['subwiki'])) {
            // Create a new sub wiki object for current user.
            $subwiki = moewiki_get_subwiki($instance->course, $instance, $cm, $context, 0, $USER->id, true);
        } else {
        ...
        if (isset($record['newversion'])) {
            ...
            return moewiki_save_new_version($instance->course, $cm, $instance, $subwiki, $newverinfo->pagename,
            ...
            moewiki_get_current_page($subwiki, $newpageinfo->pagename, MOEWIKI_GETPAGE_CREATE);
            if (!isset($newpageinfo->newpagename)) {
                $this->pagecount++;
                $newpageinfo->newpagename = 'MOE Wiki Test Page' . $this->pagecount;
            }
            if (!isset($newpageinfo->content)) {
                $newpageinfo->content = 'Test content';
            }
            moewiki_create_new_page($instance->course, $cm, $instance, $subwiki, $newpageinfo->pagename,
                    $newpageinfo->newpagename, $newpageinfo->content, $newpageinfo->formdata);
            return moewiki_get_current_page($subwiki, $newpageinfo->newpagename);
        }
    }
