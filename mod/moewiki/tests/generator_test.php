 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_moewiki_generator_testcase extends advanced_testcase {

    public function test_moewiki_generator() {
        global $DB;

        $this->resetAfterTest(true);

        $this->assertEquals(0, $DB->count_records('moewiki'));

        $course = $this->getDataGenerator()->create_course();

        $generator = $this->getDataGenerator()->get_plugin_generator('mod_moewiki');
        $this->assertInstanceOf('mod_moewiki_generator', $generator);
        $this->assertEquals('moewiki', $generator->get_modulename());

        $firstwiki = $generator->create_instance(array('course' => $course->id));
        $this->assertEquals($firstwiki->name, 'MOEWIKI1');

        $wiki = $generator->create_instance(array('course' => $course->id,
                'subwikis' => 1, 'name' => 'TEST'));

        // Test general wiki creation.
        $this->assertEquals(2, $DB->count_records('moewiki'));

        $cm = get_coursemodule_from_instance('moewiki', $wiki->id);
        $this->assertEquals($wiki->id, $cm->instance);
        $this->assertEquals('moewiki', $cm->modname);
    ...
    public function test_moewiki_create_content() {
        global $DB;
        ...
        $version = $DB->get_record('moewiki_versions', array('pageid' => $newpage->pageid));
        $this->assertInstanceOf('stdClass', $newpage);
        $this->assertEquals('MOE Wiki Test Page1', $newpage->title);
        ...
        $record = array();
        $record['subwiki'] = $DB->get_record('moewiki_subwikis', array('wikiid' => $wiki->id));
        $record['newversion'] = (object) array('content' => 'NEW');
        $updatever = $generator->create_content($wiki, $record);
        $updatever = $DB->get_record('moewiki_versions', array('id' => $updatever));
