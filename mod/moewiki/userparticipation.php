...
$url = new moodle_url('/mod/moewiki/userparticipation.php', $params);
$PAGE->set_url($url);
...
$canview = moewiki_can_view_participation($course, $moewiki, $subwiki, $cm, $USER->id);
if (($canview == MOEWIKI_NO_PARTICIPATION)
    || ($USER->id != $userid && $canview == MOEWIKI_MY_PARTICIPATION)) {
    print_error('nopermissiontoshow');
}

list($user, $changes) = moewiki_get_user_participation($userid, $subwiki);

$fullname = fullname($user, has_capability('moodle/site:viewfullnames', $context));
$cangrade = has_capability('mod/moewiki:grade', $context);
...
if (empty($download)) {
    ...
    if ($canview == MOEWIKI_USER_PARTICIPATION) {
        $nav[] = array(
            'name' => get_string('userparticipation', 'moewiki'),
            'link' => "/mod/moewiki/participation.php?id=$cm->id" .
            "&pagename=$pagename$groupparams"
        );
    }
    $nav[] = array('name' => $fullname, 'link' => null);
    echo $moewikioutput->moewiki_print_start($moewiki, $cm, $course,
        $subwiki, null, $context, $nav, null, null, '', '', $userid);
}

echo $moewikioutput->moewiki_render_user_participation($user, $changes, $cm, $course, $moewiki,
    $subwiki, $pagename, $groupid, $download, $canview, $context, $fullname,
    $cangrade, $groupname);

// Footer
if (empty($download)) {
    moewiki_print_footer($course, $cm, $subwiki, $pagename, null, 'view');
}
