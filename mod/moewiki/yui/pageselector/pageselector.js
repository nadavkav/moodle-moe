YUI.add('moodle-mod_moewiki-pageselector', function(Y) {
    M.mod_moewiki = M.mod_moewiki || {};
    M.mod_moewiki.pageselector = M.mod_moewiki.pageselector || {
            init: function(pagesselected) {
                Y.all('.ouw_indextree').each(function(list) {
                    list.all('input.moewiki_page_checkbox').each(function(checkbox) {
                        ...
                        checkbox.on('click', function (e) {
                            ...
                            if (node.get('checked')) {
                                ...
                                node.get('parentNode').all('ul li input.moewiki_page_checkbox').each(
                                        function (child) {
