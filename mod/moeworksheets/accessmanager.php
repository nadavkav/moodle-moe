...
/**
 * Classes to enforce the various access rules that can apply to a moeworksheets.
 *
 ...
 * moeworksheets, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 2.2
 */
class moeworksheets_access_manager {
    /** @var moeworksheets the moeworksheets settings object. */
    protected $moeworksheetsobj;
    /** @var int the time to be considered as 'now'. */
    protected $timenow;
    /** @var array of moeworksheets_access_rule_base. */
    protected $rules = array();

    /**
     * Create an instance for a particular moeworksheets.
     * @param object $moeworksheetsobj An instance of the class moeworksheets from attemptlib.php.
     *      The moeworksheets we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/moeworksheets:ignoretimelimits', ...)).
     */
    public function __construct($moeworksheetsobj, $timenow, $canignoretimelimits) {
        $this->moeworksheetsobj = $moeworksheetsobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($moeworksheetsobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular moeworksheets.
     * @param moeworksheets $moeworksheetsobj information about the moeworksheets in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/moeworksheets:ignoretimelimits capability.
     * @return array of {@link moeworksheets_access_rule_base}s.
     */
    protected function make_rules($moeworksheetsobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($moeworksheetsobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['moeworksheetsaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('moeworksheetsaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the moeworksheets table.
     *
     * @param mod_moeworksheets_mod_form $moeworksheetsform the moeworksheets settings form that is being built.
    ...
    public static function add_settings_form_fields(
        ...
        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($moeworksheetsform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'moeworksheets'));
        foreach (self::get_rule_classes() as $rule) {
     ...
     * @param mod_moeworksheets_mod_form $moeworksheetsform the moeworksheets form object.
     * @return array $errors the updated $errors array.
     */
    public static function validate_settings_form_fields(array $errors,
            array $data, $files, mod_moeworksheets_mod_form $moeworksheetsform) {

        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $moeworksheetsform);
        }
    ...
    /**
     * Save any submitted settings when the moeworksheets settings form is submitted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the moeworksheets table.
     *
     * @param object $moeworksheets the data from the moeworksheets form, including $moeworksheets->id
     *      which is the id of the moeworksheets being saved.
     */
    public static function save_settings($moeworksheets) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($moeworksheets);
        }
    }

    /**
     * Delete any rule-specific settings when the moeworksheets is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the moeworksheets table.
     *
     * @param object $moeworksheets the data from the database, including $moeworksheets->id
     *      which is the id of the moeworksheets being deleted.
     * @since Moodle 2.7.1, 2.6.4, 2.5.7
     */
    public static function delete_settings($moeworksheets) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::delete_settings($moeworksheets);
        }
    }
     ...
     */
    protected static function get_load_sql($moeworksheetsid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{moeworksheets} moeworksheets';
        $allparams = array('moeworksheetsid' => $moeworksheetsid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($moeworksheetsid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE moeworksheets.id = :moeworksheetsid", $allparams);
    }
     ...
     */
    public static function load_settings($moeworksheetsid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($moeworksheetsid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($moeworksheetsid);
        }
    ...
    /**
     * Load the moeworksheets settings and any settings required by the access rules.
     ...
     *
     * @param int $moeworksheetsid the moeworksheets id.
     * @return object mdl_moeworksheets row with extra fields.
     */
    public static function load_moeworksheets_and_settings($moeworksheetsid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($moeworksheetsid, $rules, 'moeworksheets.*');
        $moeworksheets = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($moeworksheetsid) as $name => $value) {
                $moeworksheets->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this moeworksheets, such
     * as is shown at the top of the moeworksheets view page. Note that not all
    ...
    /**
     * Whether or not a user should be allowed to start a new attempt at this moeworksheets now.
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_moeworksheets_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        ...
        $method = 'post';
        if (!empty($_GET['_qf__mod_moeworksheets_preflight_check_form'])) {
            $method = 'get';
        }
        return new mod_moeworksheets_preflight_check_form($url->out_omit_querystring(),
                array('rules' => $this->rules, 'moeworksheetsobj' => $this->moeworksheetsobj,
     ...
     * moeworksheets. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant moeworksheets_attempts row.
    ...
    /**
     * @return bolean if this moeworksheets should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the moeworksheets view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_moeworksheets_renderer $output the moeworksheets renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->moeworksheetsobj->view_url(), $message);
            die();
        } else {
            redirect($this->moeworksheetsobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the moeworksheets, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(moeworksheets_attempt::IN_PROGRESS, moeworksheets_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = moeworksheets_attempt_state($this->moeworksheetsobj->get_moeworksheets(), $attempt);
        $reviewoptions = mod_moeworksheets_display_options::make_from_moeworksheets(
                $this->moeworksheetsobj->get_moeworksheets(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->moeworksheetsobj->cannot_review_message($when, true));

        } else {
            return $output->review_link($this->moeworksheetsobj->review_url($attempt->id),
