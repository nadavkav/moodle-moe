...
Feature: Fault-tolerant mode admin setting
  ...
  As an admin
  I need to set a default for whether fault-tolerant mode is enabled for new moeworksheetszes.
  ...
  Scenario: The default can be changed so that fault-tolerant mode is enabled by default.
    When I navigate to "moeworksheets fault-tolerant mode" node in "Site administration > Plugins > Activity modules > moeworksheets"
