...
Feature: Fault-tolerant mode navigation without page reloads for a moeworksheets in sequential mode.
  In order to attempt moeworksheetszes with dodgy wifi
  As a student
  I need to be able to navigate between pages of the moeworksheets even if the moeworksheets uses sequential navigation.
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | moeworksheets       | moeworksheets fault-tolerant | C1     | moeworksheets1    | 1                   | sequential |
  ...
  Scenario: Clicking on a nav button has no effect.
    When I press "Attempt moeworksheets now"
    And I click on "#moeworksheetsnavbutton2" "css_element"
    Then I should see "Answer me A"
    And I should not see "Answer me B"
    And I should not see "Answer me C"
    And I should not see "Summary of attempt"
  ...
  @javascript
  Scenario: Start a moeworksheets attempt and verify that next works.
    When I press "Attempt moeworksheets now"
  ...
  @javascript
  Scenario: Start a moeworksheets attempt and verify that switching back from the summary works.
    When I press "Attempt moeworksheets now"
    And I start watching to see if a new page loads
    And I click on "Finish attempt ..." "link" in the "moeworksheets navigation" "block"
