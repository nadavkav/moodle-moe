...
$cm = get_coursemodule_from_id('moeworksheets', $cmid, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$moeworksheets = $DB->get_record('moeworksheets', array('id' => $cm->instance), '*', MUST_EXIST);
$moeworksheetsurl = new moodle_url('/mod/moeworksheets/view.php', array('id' => $cm->id));
$context = context_module::instance($cm->id);

$PAGE->set_url('/mod/moeworksheets/accessrule/offlinemode/upload.php', array('id' => $cmid));
require_login($course, false, $cm);
require_capability('moeworksheetsaccess/offlinemode:uploadresponses', $context);

$form = new \moeworksheetsaccess_offlinemode\form\upload_responses($PAGE->url);
if ($form->is_cancelled()) {
    redirect($moeworksheetsurl);

} else if ($fromform = $form->get_data()) {

    // Process submission.
    $title = get_string('uploadingresponsesfor', 'moeworksheetsaccess_offlinemode',
            format_string($moeworksheets->name, true, array('context' => $context)));
    ...
    $privatekey = null;
    $privatekeystring = get_config('moeworksheetsaccess_offlinemode', 'privatekey');
    if ($privatekeystring) {
    ...
    foreach ($files as $file) {
        ...
        echo $OUTPUT->heading(get_string('processingfile', 'moeworksheetsaccess_offlinemode', s($file->get_filename())), 3);
        ...
        try {
            ...
            // Load the attempt.
            $attemptobj = moeworksheets_attempt::create($postdata['attempt']);
            if ($attemptobj->get_cmid() != $cmid) {
                throw new coding_exception('The uploaded data does not belong to this moeworksheets.');
            }
            ...
            echo $OUTPUT->notification(get_string('dataprocessedsuccessfully', 'moeworksheetsaccess_offlinemode',
                    html_writer::link($attemptobj->review_url(), get_string('reviewthisattempt', 'moeworksheetsaccess_offlinemode'))),
                    'notifysuccess');
            ...
            echo $OUTPUT->heading(get_string('uploadfailed', 'moeworksheetsaccess_offlinemode'), 4);
    ...
    echo $OUTPUT->confirm(get_string('processingcomplete', 'moeworksheetsaccess_offlinemode', 3),
            new single_button($PAGE->url, get_string('uploadmoreresponses', 'moeworksheetsaccess_offlinemode'), 'get'),
            new single_button($moeworksheetsurl, get_string('backtothemoeworksheets', 'moeworksheetsaccess_offlinemode'), 'get'));
    echo $OUTPUT->footer();

} else {

    // Show the form.
    $title = get_string('uploadresponsesfor', 'moeworksheetsaccess_offlinemode',
            format_string($moeworksheets->name, true, array('context' => $context)));
    $PAGE->navbar->add($title);
    $PAGE->set_pagelayout('admin');
    $PAGE->set_title($title);
