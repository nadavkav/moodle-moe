YUI.add('moodle-moeworksheetsaccess_offlinemode-autosave', function (Y, NAME) {
...
/**
 * Auto-save functionality for during moeworksheets attempts.
 *
 * @module moodle-moeworksheetsaccess_offlinemode-autosave
 */

/**
 * Auto-save functionality for during moeworksheets attempts.
 *
 * @class M.moeworksheetsaccess_offlinemode.autosave
 */

M.moeworksheetsaccess_offlinemode = M.moeworksheetsaccess_offlinemode || {};
M.moeworksheetsaccess_offlinemode.autosave = {
    /**
     ...
     */
    SELECTORS: {
        moeworksheets_FORM:             '#responseform',
        VALUE_CHANGE_ELEMENTS: 'input, textarea, [contenteditable="true"]',
        CHANGE_ELEMENTS:       'input, select',
        HIDDEN_INPUTS:         'input[type=hidden]',
        NAV_BUTTON:            '#moeworksheetsnavbutton',                       // Must have slot appended.
        QUESTION_CONTAINER:    '#q',                                   // Must have slot appended.
        STATE_HOLDER:          ' .state',
        SUMMARY_ROW:           '.moeworksheetssummaryofattempt tr.moeworksheetssummary', // Must have slot appended.
        STATE_COLUMN:          ' .c1',
        FINISH_ATTEMPT_INPUT:  'input[name=finishattempt]',
        SUBMIT_BUTTON:         'input[type=submit]',
        FORM:                  'form',
        SAVING_NOTICE:         '#moeworksheets-saving',
        LAST_SAVED_MESSAGE:    '#moeworksheets-last-saved-message',
        LAST_SAVED_TIME:       '#moeworksheets-last-saved',
        SAVE_FAILED_NOTICE:    '#mod_moeworksheets_navblock .moeworksheets-save-failed'
    },
     ...
     * @type String
     * @default M.cfg.wwwroot + '/mod/moeworksheets/autosave.ajax.php'
     * @private
     */
    AUTOSAVE_HANDLER: M.cfg.wwwroot + '/mod/moeworksheets/accessrule/offlinemode/autosave.ajax.php',
     ...
     * @private
     */
    RELOGIN_SCRIPT: M.cfg.wwwroot + '/mod/moeworksheets/accessrule/offlinemode/relogin.php',
     ...
     */
    init: function(delay) {
        this.form = Y.one(this.SELECTORS.moeworksheets_FORM);
        if (!this.form) {
            Y.log('No response form found. Why did you try to set up autosave?', 'debug', 'moodle-moeworksheetsaccess_offlinemode-autosave');
            return;
        }

        M.core_question_engine.init_form(Y, this.SELECTORS.moeworksheets_FORM);
        Y.on('submit', M.mod_moeworksheets.timer.stop, this.SELECTORS.moeworksheets_FORM);
    ...
    init_tinymce: function(repeatcount) {
        if (typeof tinyMCE === 'undefined') {
            if (repeatcount > 0) {
                ...
                Y.log('Gave up looking for TinyMCE.', 'debug', 'moodle-moeworksheetsaccess_offlinemode-autosave');
            }
            return;
        }

        Y.log('Found TinyMCE.', 'debug', 'moodle-moeworksheetsaccess_offlinemode-autosave');
    ...
    init_tinymce_editor: function(e, editor) {
        Y.log('Found TinyMCE editor ' + editor.id + '.', 'debug', 'moodle-moeworksheetsaccess_offlinemode-autosave');
    ...
    value_changed: function(e) {
        ...
        Y.log('Detected a value change in element ' + name + '.', 'debug', 'moodle-moeworksheetsaccess_offlinemode-autosave');
        this.start_save_timer_if_necessary();
        this.mark_question_changed_if_necessary(name);
    },

    editor_changed: function(editor) {
        Y.log('Detected a value change in editor ' + editor.id + '.', 'debug', 'moodle-moeworksheetsaccess_offlinemode-autosave');
        this.start_save_timer_if_necessary();
    ...
    mark_question_changed_if_necessary: function(elementname) {
        ...
        if (slot) {
            this.set_question_state_string(slot, M.util.get_string('answerchanged', 'moeworksheetsaccess_offlinemode'));
    ...
    set_question_state_string: function(slot, newstate) {
        Y.log('State of question ' + slot + ' changed to ' + newstate + '.',
                'debug', 'moodle-moeworksheetsaccess_offlinemode-autosave');
    ...
    save_changes: function() {
        ...
        if (this.is_time_nearly_over()) {
            Y.log('No more saving, time is nearly over.', 'debug', 'moodle-moeworksheetsaccess_offlinemode-autosave');
            this.stop_autosaving();
            return;
        }

        Y.log('Doing a save.', 'debug', 'moodle-moeworksheetsaccess_offlinemode-autosave');
        if (typeof tinyMCE !== 'undefined') {
            tinyMCE.triggerSave();
        }
    ...
    save_done: function(transactionid, response) {
        ...
        if (result.result === 'lostsession') {
            Y.log('Session loss detected.', 'debug', 'moodle-moeworksheetsaccess_offlinemode-autosave');
        ...
        Y.log('Save completed.', 'debug', 'moodle-moeworksheetsaccess_offlinemode-autosave');
        this.save_transaction = null;
        ...
        if (this.dirty) {
            Y.log('Dirty after save.', 'debug', 'moodle-moeworksheetsaccess_offlinemode-autosave');
            this.start_save_timer();
        }
    ...
    save_failed: function() {
        Y.log('Save failed.', 'debug', 'moodle-moeworksheetsaccess_offlinemode-autosave');
        this.save_transaction = null;
    ...
    is_time_nearly_over: function() {
        return M.mod_moeworksheets.timer && M.mod_moeworksheets.timer.endtime &&
                (new Date().getTime() + 2 * this.delay) > M.mod_moeworksheets.timer.endtime;
    },
    ...
    warn_if_unsaved_data: function(e) {
        ...
        // Show a warning.
        e.returnValue = M.util.get_string('changesmadereallygoaway', 'moeworksheetsaccess_offlinemode');
    ...
    submit_and_finish_clicked: function(e) {
        ...
        var confirmationDialogue = new M.core.confirm({
            ...
            yesLabel: M.util.get_string('submitallandfinish', 'moeworksheets'),
            question: M.util.get_string('confirmclose', 'moeworksheets')
        });
    ...
    submit_and_finish: function(e) {
        ...
        submitButton.ancestor('.controls').removeClass('moeworksheets-save-failed');
        failureMessage.header.hide();
        failureMessage.message.hide();
        this.form.append('<input name="finishattempt" value="1">');

        Y.log('Trying to submit.', 'debug', 'moodle-moeworksheetsaccess_offlinemode-autosave');
        if (typeof tinyMCE !== 'undefined') {
            tinyMCE.triggerSave();
        }
        this.save_transaction = Y.io(this.AUTOSAVE_HANDLER, {
    ...
    submit_done: function(transactionid, response) {
        ...
        Y.log('Submit completed, redirecting.', 'debug', 'moodle-moeworksheetsaccess_offlinemode-autosave');
        this.save_transaction = null;
        this.dirty = false;
    ...
    submit_failed: function() {
        Y.log('Submit failed.', 'debug', 'moodle-moeworksheetsaccess_offlinemode-autosave');
        this.save_transaction = null;
        ...
        var failureMessage = this.get_submit_failed_message(submitButton.ancestor('.controls'));
        submitButton.ancestor('.controls').addClass('moeworksheets-save-failed');
    ...
    get_submit_progress: function(controlsDiv) {
        ...
        submitProgress.append(M.util.get_string('submitting', 'moeworksheetsaccess_offlinemode'));
        return submitProgress;
    },
    ...
    get_submit_failed_message: function(controlsDiv) {
        ...
        failedHeader.append('<h4>' + M.util.get_string('submitfailed', 'moeworksheetsaccess_offlinemode') + '</h4>');
        failedHeader.append('<p>' + M.util.get_string('submitfailedmessage', 'moeworksheetsaccess_offlinemode') + '</p>');

        var downloadLink = '<a href="#" class="response-download-link">' +
                M.util.get_string('savetheresponses', 'moeworksheetsaccess_offlinemode') + '</a>';
        var failedMessage = controlsDiv.appendChild('<div class="submit-failed-message">');
        failedMessage.append('<p>' + M.util.get_string('submitfaileddownloadmessage', 'moeworksheetsaccess_offlinemode', downloadLink) + '</p>');
    ...
    create_status_messages: function() {
        var downloadLink = '<a href="#" class="response-download-link">' +
                M.util.get_string('savetheresponses', 'moeworksheetsaccess_offlinemode') + '</a>';
        Y.one('#mod_moeworksheets_navblock .content').append('<div id="moeworksheets-save-status">' +
                '<div id="moeworksheets-last-saved-message">' + M.util.get_string('lastsaved', 'moeworksheetsaccess_offlinemode',
                        '<span id="moeworksheets-last-saved"></span>') + '</div>' +
                '<div id="moeworksheets-saving">' + M.util.get_string('savingdots', 'moeworksheetsaccess_offlinemode') + '</div>' +
                '<div class="moeworksheets-save-failed">' + M.util.get_string('savefailed', 'moeworksheetsaccess_offlinemode', downloadLink) + '</div>' +
                '</div>');
        this.update_status_for_successful_save();
    },
    ...
    update_status_for_successful_save: function() {
        ...
        Y.one(this.SELECTORS.SAVING_NOTICE).setHTML(M.util.get_string('savingdots', 'moeworksheetsaccess_offlinemode'));
    ...
    update_status_for_failed_save: function() {
        Y.one(this.SELECTORS.LAST_SAVED_MESSAGE).setHTML(
                M.util.get_string('lastsavedtotheserver', 'moeworksheetsaccess_offlinemode',
                Y.one(this.SELECTORS.LAST_SAVED_TIME).get('outerHTML')));
        Y.one(this.SELECTORS.SAVING_NOTICE).setStyle('visibility', 'hidden');
        Y.one(this.SELECTORS.SAVING_NOTICE).setHTML(M.util.get_string('savingtryagaindots', 'moeworksheetsaccess_offlinemode'));
    ...
    try_to_restore_session: function() {
        this.loginDialogue = new M.core.notification.info({
            id:        'moeworksheets-relogin-dialogue',
            width:     '70%',
            center:    true,
            modal:     true,
            visible:   false,
        ...
        this.loginDialogue.setStdModContent(Y.WidgetStdMod.HEADER,
                '<h1 id="moodle-moeworksheets-relogin-dialogue-header-text">' + M.util.get_string('logindialogueheader', 'moeworksheetsaccess_offlinemode') + '</h1>', Y.WidgetStdMod.REPLACE);
        this.loginDialogue.setStdModContent(Y.WidgetStdMod.BODY,
                '<iframe src="' + this.RELOGIN_SCRIPT + '?userid=' +
                Y.one('#moeworksheets-userid').get('value') + '">', Y.WidgetStdMod.REPLACE);
