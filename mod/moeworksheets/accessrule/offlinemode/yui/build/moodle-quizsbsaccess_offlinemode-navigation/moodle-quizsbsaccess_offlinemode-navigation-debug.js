YUI.add('moodle-moeworksheetsaccess_offlinemode-navigation', function (Y, NAME) {
...
/**
 * fault-tolerant mode for moeworksheets attempts.
 *
 * @module moodle-moeworksheetsaccess_offlinemode-navigation
 */

/**
 * Auto-save functionality for during moeworksheets attempts.
 *
 * @class M.moeworksheetsaccess_offlinemode.navigation
 */

M.moeworksheetsaccess_offlinemode = M.moeworksheetsaccess_offlinemode || {};
M.moeworksheetsaccess_offlinemode.navigation = {
    /**
    ...
    SELECTORS: {
        ...
        NAV_BLOCK:            '#mod_moeworksheets_navblock',
        NAV_BUTTON:           '.qnbutton',
        FINISH_LINK:          '.endtestlink',
        NEXT_BUTTON:          'input[name=next]',
        SUMMARY_TABLE:        '.moeworksheetssummaryofattempt',
        ...
        SUMMARY_PAGE_BUTTON:  '#moeworksheetsaccess_offlinemode-attempt_page--1 .submitbtns input[type=submit]',
        PAGE_DIV_ROOT:        '#moeworksheetsaccess_offlinemode-attempt_page-',
        ALL_PAGE_DIVS:        'div[id|=moeworksheetsaccess_offlinemode-attempt_page]',
    ...
    /**
     * A Node reference to the main moeworksheets form.
     *
     * @property form
     * @type Node
     * @default null
    ...
    init: function(currentpage) {
        ...
        if (!this.form) {
            Y.log('Response form not found.', 'debug', 'moodle-moeworksheetsaccess_offlinemode-navigation');
            return;
        }
        ...
        Y.all(this.SELECTORS.ALL_PAGE_DIVS).each(function(element) {
                    var pageno;
                    var matches = element.get('id').match(/moeworksheetsaccess_offlinemode-attempt_page-(\d+)/);
                    if (matches) {
                        pageno = +matches[1];
        ...
        Y.all(this.SELECTORS.ALL_PAGE_DIVS).removeClass('moeworksheets-loading-hide');
        ...
        Y.log('Initialised fault-tolerant moeworksheets mode.', 'debug', 'moodle-moeworksheetsaccess_offlinemode-navigation');
    },
    ...
    nav_button_click: function(e) {
        // Prevent the moeworksheets's own event handler running.
        e.halt();
    ...
    page_number_from_link: function(anchor) {
        var dataValue = anchor.getData('moeworksheets-page');
    ...
    navigate_to_page: function(pageno) {
        ...
        if (pageno === -1) {
            Y.one(this.SELECTORS.moeworksheets_FORM).addClass('hidden');
        } else {
            Y.one(this.SELECTORS.moeworksheets_FORM).removeClass('hidden');
        }
        ...
        if (window.history.replaceState) {
            ...
            window.history.replaceState(null, '', M.cfg.wwwroot + '/mod/moeworksheets/accessrule/offlinemode/attempt.php' + queryString);
        }
