...
$thispageurl = new moodle_url('/mod/moeworksheets/additionalcontentlist.php');
list($thispageurl, $contexts, $cmid, $cm, $moeworksheets, $pagevars) = question_edit_setup('editq', '/mod/moeworksheets/additionalcontentlist.php', true);
...
$output = $PAGE->get_renderer('mod_moeworksheets', 'contentlist');

$PAGE->set_title(get_string('editingmoeworksheetsx', 'moeworksheets', format_string($moeworksheets->name)));
$PAGE->set_heading($course->fullname);
switch ($action) {
    case 'delete':
        $content = $output->delete_content_page($thispageurl, $moeworksheetsobj);
        break;
    default:
        $content = $output->contentlist_page($moeworksheetsobj, $structure, $contexts, $thispageurl, $pagevars);
        break;
}
echo $OUTPUT->header();
