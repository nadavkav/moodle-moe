...
/**
 * Fallback page of /mod/moeworksheets/edit.php add random question dialog,
...
list($thispageurl, $contexts, $cmid, $cm, $moeworksheets, $pagevars) =
        question_edit_setup('editq', '/mod/moeworksheets/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/moeworksheets/edit.php', array('cmid' => $cmid));
}
if ($scrollpos) {
...
$mform = new moeworksheets_add_random_form(new moodle_url('/mod/moeworksheets/addrandom.php'),
...
if ($data = $mform->get_data()) {
    ...
    moeworksheets_add_random_questions($moeworksheets, $addonpage, $categoryid, $data->numbertoadd, $includesubcategories);
...
// Setup $PAGE.
$streditingmoeworksheets = get_string('editinga', 'moodle', get_string('modulename', 'moeworksheets'));
$PAGE->navbar->add($streditingmoeworksheets);
$PAGE->set_title($streditingmoeworksheets);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

if (!$moeworksheetsname = $DB->get_field($cm->modname, 'name', array('id' => $cm->instance))) {
            print_error('invalidcoursemodule');
}

echo $OUTPUT->heading(get_string('addrandomquestiontomoeworksheets', 'moeworksheets', $moeworksheetsname), 2);
$mform->display();
echo $OUTPUT->footer();
