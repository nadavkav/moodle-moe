 ...
 *
 * This is also responsible for opening the pop-up window, if the moeworksheets requires to be in one.
 *
 * @module    mod_moeworksheets/preflightcheck
...
define(['jquery', 'core/yui'], function($, Y) {
    ...
    /**
     * @alias module:mod_moeworksheets/preflightcheck
     */
    var t = {
         ...
         * @param {String} popupoptions If not null, the moeworksheets should be launced in a pop-up.
         */
            ...
            Y.use('moodle-core-notification', 'moodle-core-formchangechecker', 'io-form', function () {
                if (Y.one(confirmationForm)) {
                    t.confirmDialogue = new M.core.dialogue({
                        ...
                        modal: true,
                        width: null,
                        extraClasses: ['mod_moeworksheets_preflight_popup']
                    });
                ...
                if (popupoptions) {
                    Y.one(finalStartButton).on('click', t.launchmoeworksheetsPopup, t, popupoptions);
                }
            });
        },
        ...
        /**
         * Event handler for the moeworksheets start attempt button.
         */
        launchmoeworksheetsPopup: function(e, popupoptions) {
            e.halt();
            M.core_formchangechecker.reset_form_dirty_state();
            var form = e.target.ancestor('form');
            window.openpopup(e, {
                url: form.get('action') + '?' + Y.IO.stringify(form).replace(/\bcancel=/, 'x='),
                windowname: 'moeworksheetspopup',
