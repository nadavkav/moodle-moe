...
/**
 * This script displays a particular page of a moeworksheets attempt that is in progress.
 *
...
if ($id = optional_param('id', 0, PARAM_INT)) {
    redirect($CFG->wwwroot . '/mod/moeworksheets/startattempt.php?cmid=' . $id . '&sesskey=' . sesskey());
} else if ($qid = optional_param('q', 0, PARAM_INT)) {
    if (!$cm = get_coursemodule_from_instance('moeworksheets', $qid)) {
        print_error('invalidmoeworksheetsid', 'moeworksheets');
    }
    redirect(new moodle_url('/mod/moeworksheets/startattempt.php',
...
if ($autosaveperiod) {
    $PAGE->requires->yui_module('moodle-mod_moeworksheets-autosave',
            'M.mod_moeworksheets.autosave.init', array($autosaveperiod));
}
...
$PAGE->requires->js_init_call('M.mod_moeworksheets.init_attempt_form', null, false, moeworksheets_get_js_module());
