...
/**
 * Back-end code for handling data about moeworksheetszes and the current user's attempt.
 *
 * There are classes for loading all the information about a moeworksheets and attempts,
...
/**
 * Class for moeworksheets exceptions. Just saves a couple of arguments on the
 ...
 * @since     Moodle 2.0
 */
class moodle_moeworksheets_exception extends moodle_exception {
    public function __construct($moeworksheetsobj, $errorcode, $a = null, $link = '', $debuginfo = null) {
        if (!$link) {
            $link = $moeworksheetsobj->view_url();
        }
        parent::__construct($errorcode, 'moeworksheets', $link, $a, $debuginfo);
    }
}


/**
 * A class encapsulating a moeworksheets and the questions it contains, and making the
...
class moeworksheets {
    ...
    protected $cm;
    /** @var stdClass the moeworksheets settings from the database. */
    protected $moeworksheets;
    /** @var context the moeworksheets context. */
    protected $context;

    /** @var array of questions augmented with slot information. */
    protected $questions = null;
    /** @var array of moeworksheets_section rows. */
    protected $sections = null;
    /** @var moeworksheets_access_manager the access manager for this moeworksheets. */
    protected $accessmanager = null;
    /** @var bool whether the current user has capability mod/moeworksheets:preview. */
     ...
     *
     * @param object $moeworksheets the row from the moeworksheets table.
     * @param object $cm the course_module object for this moeworksheets.
     * @param object $course the row from the course table for the course we belong to.
     * @param bool $getcontext intended for testing - stops the constructor getting the context.
     */
    public function __construct($moeworksheets, $cm, $course, $getcontext = true) {
        $this->moeworksheets = $moeworksheets;
        $this->cm = $cm;
        $this->moeworksheets->cmid = $this->cm->id;
        $this->course = $course;
    ...
    /**
     * Static function to create a new moeworksheets object for a specific user.
     *
     * @param int $moeworksheetsid the the moeworksheets id.
     * @param int $userid the the userid.
     * @return moeworksheets the new moeworksheets object
     */
    public static function create($moeworksheetsid, $userid = null) {
        global $DB;

        $moeworksheets = moeworksheets_access_manager::load_moeworksheets_and_settings($moeworksheetsid);
        $course = $DB->get_record('course', array('id' => $moeworksheets->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('moeworksheets', $moeworksheets->id, $course->id, false, MUST_EXIST);

        // Update moeworksheets with override information.
        if ($userid) {
            $moeworksheets = moeworksheets_update_effective_access($moeworksheets, $userid);
        }

        return new moeworksheets($moeworksheets, $cm, $course);
    }

    /**
     * Create a {@link moeworksheets_attempt} for an attempt at this moeworksheets.
     * @param object $attemptdata row from the moeworksheets_attempts table.
     * @return moeworksheets_attempt the new moeworksheets_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new moeworksheets_attempt($attemptdata, $this->moeworksheets, $this->cm, $this->course);
    }

    // Functions for loading more data =========================================

    /**
     * Load just basic information about all the questions in this moeworksheets.
     */
    public function preload_questions() {
        $this->questions = question_preload_questions(null,
                'slot.maxmark, slot.id AS slotid, slot.slot, slot.page',
                '{moeworksheets_slots} slot ON slot.moeworksheetsid = :moeworksheetsid AND q.id = slot.questionid',
                array('moeworksheetsid' => $this->moeworksheets->id), 'slot.slot');
    }

    /**
     * Fully load some or all of the questions for this moeworksheets. You must call
    ...
    /**
     * Get an instance of the {@link \mod_moeworksheets\structure} class for this moeworksheets.
     * @return \mod_moeworksheets\structure describes the questions in the moeworksheets.
     */
    public function get_structure() {
        return \mod_moeworksheets\structure::create_for_moeworksheets($this);
    }
    ...
    /** @return int the moeworksheets id. */
    public function get_moeworksheetsid() {
        return $this->moeworksheets->id;
    }

    /** @return object the row of the moeworksheets table. */
    public function get_moeworksheets() {
        return $this->moeworksheets;
    }

    /** @return string the name of this moeworksheets. */
    public function get_moeworksheets_name() {
        return $this->moeworksheets->name;
    }

    /** @return int the moeworksheets navigation method. */
    public function get_navigation_method() {
        return $this->moeworksheets->navmethod;
    }

    /** @return int the number of attempts allowed at this moeworksheets (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->moeworksheets->attempts;
    }
    ...
    /** @return object the module context for this moeworksheets. */
    public function get_context() {
        return $this->context;
    }

    /**
     * @return bool wether the current user is someone who previews the moeworksheets,
     * rather than attempting it.
     */
    public function is_preview_user() {
        if (is_null($this->ispreviewuser)) {
            $this->ispreviewuser = has_capability('mod/moeworksheets:preview', $this->context);
        }
        return $this->ispreviewuser;
    }

    /**
     * @return whether any questions have been added to this moeworksheets.
     */
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'moeworksheets', $this->view_url());
            }
    ...
    /**
     * Get all the sections in this moeworksheets.
     * @return array 0, 1, 2, ... => moeworksheets_sections row from the database.
     */
    public function get_sections() {
        global $DB;
        if ($this->sections === null) {
            $this->sections = array_values($DB->get_records('moeworksheets_sections',
                    array('moeworksheetsid' => $this->get_moeworksheetsid()), 'firstslot'));
        }
        return $this->sections;
    }

    /**
     * Return moeworksheets_access_manager and instance of the moeworksheets_access_manager class
     * for this moeworksheets at this time.
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new moeworksheets_access_manager($this, $timenow,
                    has_capability('mod/moeworksheets:ignoretimelimits', $this->context, null, false));
        }
        return $this->accessmanager;
    }

    /**
     * Wrapper round the has_capability funciton that automatically passes in the moeworksheets context.
     */
    ...
    /**
     * Wrapper round the require_capability funciton that automatically passes in the moeworksheets context.
     */
    ...
    /**
     * @return string the URL of this moeworksheets's view page.
     */
    public function view_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/moeworksheets/view.php?id=' . $this->cm->id;
    }

    /**
     * @return string the URL of this moeworksheets's edit page.
     */
    public function edit_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/moeworksheets/edit.php?cmid=' . $this->cm->id;
    }
    ...
    public function attempt_url($attemptid, $page = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/moeworksheets/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
        return $url;
    ...
    /**
     * @return string the URL of this moeworksheets's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/moeworksheets/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new moodle_url('/mod/moeworksheets/review.php', array('attempt' => $attemptid));
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/moeworksheets/summary.php', array('attempt' => $attemptid));
    }
     ...
     * @deprecated since 3.1. This sort of functionality is now entirely handled by moeworksheets access rules.
     */
    public function confirm_start_attempt_message($notused) {
        debugging('confirm_start_attempt_message is deprecated. ' .
                'This sort of functionality is now entirely handled by moeworksheets access rules.');
        return '';
    }
     ...
     *
     * @param int $when One of the mod_moeworksheets_display_options::DURING,
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($when == mod_moeworksheets_display_options::DURING ||
                $when == mod_moeworksheets_display_options::IMMEDIATELY_AFTER) {
            return '';
        } else if ($when == mod_moeworksheets_display_options::LATER_WHILE_OPEN && $this->moeworksheets->timeclose &&
                $this->moeworksheets->reviewattempt & mod_moeworksheets_display_options::AFTER_CLOSE) {
            return get_string('noreviewuntil' . $langstrsuffix, 'moeworksheets',
                    userdate($this->moeworksheets->timeclose, $dateformat));
        } else {
            return get_string('noreview' . $langstrsuffix, 'moeworksheets');
        }
    }

    /**
     * @param string $title the name of this particular moeworksheets page.
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_moeworksheets_exception($this, 'questionnotloaded', $id);
        }
    }

    /**
     * Return all the question types used in this moeworksheets.
     *
     * @param  boolean $includepotential if the moeworksheets include random questions, setting this flag to true will make the function to
    ...
    public function get_all_question_types_used($includepotential = false) {
        ...
        // We must be careful with random questions, if we find a random question we must assume that the moeworksheets may content
...
/**
 * This class extends the moeworksheets class to hold data about the state of a particular attempt,
 * in addition to the data about the moeworksheets.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class moeworksheets_attempt {
    ...
    /** @var int maximum number of slots in the moeworksheets for the review page to default to show all. */
    const MAX_SLOTS_FOR_DEFAULT_REVIEW_SHOW_ALL = 50;

    /** @var moeworksheets object containing the moeworksheets settings. */
    protected $moeworksheetsobj;

    /** @var stdClass the moeworksheets_attempts row. */
    protected $attempt;

    /** @var question_usage_by_activity the question usage for this moeworksheets attempt. */
    protected $quba;
    ...
    /** @var array of moeworksheets_sections rows, with a ->lastslot field added. */
    protected $sections;
     ...
     * Constructor assuming we already have the necessary data loaded.
     *
     * @param object $attempt the row of the moeworksheets_attempts table.
     * @param object $moeworksheets the moeworksheets object for this attempt and user.
     * @param object $cm the course_module object for this moeworksheets.
     * @param object $course the row from the course table for the course we belong to.
     ...
     */
    public function __construct($attempt, $moeworksheets, $cm, $course, $loadquestions = true) {
        global $DB;

        $this->attempt = $attempt;
        $this->moeworksheetsobj = new moeworksheets($moeworksheets, $cm, $course);
        ...
        $this->slots = $DB->get_records('moeworksheets_slots',
                array('moeworksheetsid' => $this->get_moeworksheetsid()), 'slot',
     ...
     * @param array $conditions passed to $DB->get_record('moeworksheets_attempts', $conditions).
     */
    protected static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('moeworksheets_attempts', $conditions, '*', MUST_EXIST);
        $moeworksheets = moeworksheets_access_manager::load_moeworksheets_and_settings($attempt->moeworksheets);
        $course = $DB->get_record('course', array('id' => $moeworksheets->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('moeworksheets', $moeworksheets->id, $course->id, false, MUST_EXIST);

        // Update moeworksheets with override information.
        $moeworksheets = moeworksheets_update_effective_access($moeworksheets, $attempt->userid);

        return new moeworksheets_attempt($attempt, $moeworksheets, $cm, $course);
    }

    /**
     * Static function to create a new moeworksheets_attempt object given an attemptid.
     *
    ...
    /**
     * Static function to create a new moeworksheets_attempt object given a usage id.
     *
    ...
    public static function state_name($state) {
        return moeworksheets_attempt_state_name($state);
    }
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    $this->questionnumbers[$slot] = get_string('infoshort', 'moeworksheets');
                }
    ...
    // Simple getters ==========================================================
    public function get_moeworksheets() {
        return $this->moeworksheetsobj->get_moeworksheets();
    }

    public function get_moeworksheetsobj() {
        return $this->moeworksheetsobj;
    }

    /** @return int the course id. */
    public function get_courseid() {
        return $this->moeworksheetsobj->get_courseid();
    }

    /** @return int the course id. */
    public function get_course() {
        return $this->moeworksheetsobj->get_course();
    }

    /** @return int the moeworksheets id. */
    public function get_moeworksheetsid() {
        return $this->moeworksheetsobj->get_moeworksheetsid();
    }

    /** @return string the name of this moeworksheets. */
    public function get_moeworksheets_name() {
        return $this->moeworksheetsobj->get_moeworksheets_name();
    }

    /** @return int the moeworksheets navigation method. */
    public function get_navigation_method() {
        return $this->moeworksheetsobj->get_navigation_method();
    }

    /** @return object the course_module object. */
    public function get_cm() {
        return $this->moeworksheetsobj->get_cm();
    }

    /** @return object the course_module object. */
    public function get_cmid() {
        return $this->moeworksheetsobj->get_cmid();
    }
     ...
     */
    public function is_preview_user() {
        return $this->moeworksheetsobj->is_preview_user();
    }

    /** @return int the number of attempts allowed at this moeworksheets (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->moeworksheetsobj->get_num_attempts_allowed();
    }

    /** @return int number fo pages in this moeworksheets. */
     ...
     */
    public function get_access_manager($timenow) {
        return $this->moeworksheetsobj->get_access_manager($timenow);
    }
    ...
    /** @return object the row from the moeworksheets_attempts table. */
    ...
    /** @return string one of the moeworksheets_attempt::IN_PROGRESS, FINISHED, OVERDUE or ABANDONED constants. */
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/moeworksheets:viewreports')) {
            return false;
        }
    ...
    /**
     * Has the student, in this attempt, engaged with the moeworksheets in a non-trivial way?
     ...
     * To see how this works, try setting a moeworksheets to one of the CBM behaviours,
     * and then look at the extra information displayed at the top of the moeworksheets
     ...
     *
     * @param question_display_options $options the display options for this moeworksheets attempt at this time.
    ...
    public function get_overall_feedback($grade) {
        return moeworksheets_feedback_for_grade($grade, $this->get_moeworksheets(),
                $this->moeworksheetsobj->get_context());
    }
     ...
     */
    public function has_capability($capability, $userid = null, $doanything = true) {
        return $this->moeworksheetsobj->has_capability($capability, $userid, $doanything);
    }
     ...
     */
    public function require_capability($capability, $userid = null, $doanything = true) {
        return $this->moeworksheetsobj->require_capability($capability, $userid, $doanything);
    }
    ...
    public function check_review_capability() {
        if ($this->get_attempt_state() == mod_moeworksheets_display_options::IMMEDIATELY_AFTER) {
            $capability = 'mod/moeworksheets:attempt';
        } else {
            $capability = 'mod/moeworksheets:reviewmyattempts';
        }

        // These next tests are in a slighly funny order. The point is that the
        // common and most performance-critical case is students attempting a moeworksheets
        // so we want to check that permisison first.

        if ($this->has_capability($capability)) {
            // User has the permission that lets you do the moeworksheets as a student. Fine.
            return;
        }

        if ($this->has_capability('mod/moeworksheets:viewreports') ||
                $this->has_capability('mod/moeworksheets:preview')) {
    ...
    public function can_navigate_to($slot) {
        switch ($this->get_navigation_method()) {
            case moeworksheets_NAVMETHOD_FREE:
                return true;
                break;
            case moeworksheets_NAVMETHOD_SEQ:
                return false;
                break;
        }
        return true;
    ...
    public function get_attempt_state() {
        return moeworksheets_attempt_state($this->get_moeworksheets(), $this->attempt);
    }

    /**
     * Wrapper that the correct mod_moeworksheets_display_options for this moeworksheets at the
     * moment.
     *
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = moeworksheets_get_review_options($this->get_moeworksheets(),
                        $this->attempt, $this->moeworksheetsobj->get_context());
            ...
            $options = mod_moeworksheets_display_options::make_from_moeworksheets($this->get_moeworksheets(),
                    mod_moeworksheets_display_options::DURING);
            $options->flags = moeworksheets_get_flag_option($this->attempt, $this->moeworksheetsobj->get_context());
            return $options;
        }
    }
     ...
     * @return bool true if this is the last page of the moeworksheets.
     */
    ...
    /**
     * Return the list of slot numbers for either a given page of the moeworksheets, or for the
     * whole moeworksheets.
     *
    ...
    public function is_blocked_by_previous_question($slot) {
        return $slot > 1 && isset($this->slots[$slot]) && $this->slots[$slot]->requireprevious &&
                ...
                $this->get_navigation_method() != moeworksheets_NAVMETHOD_SEQ &&
    ...
    public function can_question_be_redone_now($slot) {
        return $this->get_moeworksheets()->canredoquestions && !$this->is_finished() &&
    ...
    /**
     * Return the page of the moeworksheets where this question appears.
     * @param int $slot the number used to identify this question within this attempt.
     * @return int the page of the moeworksheets this question appears on.
     */
     ...
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return string the formatted grade, to the number of decimal places specified by the moeworksheets.
     */
    public function get_question_mark($slot) {
        return moeworksheets_format_question_grade($this->get_moeworksheets(), $this->quba->get_question_mark($slot));
    }
    ...
    public function get_due_date() {
        $deadlines = array();
        if ($this->moeworksheetsobj->get_moeworksheets()->timelimit) {
            $deadlines[] = $this->attempt->timestart + $this->moeworksheetsobj->get_moeworksheets()->timelimit;
        }
        if ($this->moeworksheetsobj->get_moeworksheets()->timeclose) {
            $deadlines[] = $this->moeworksheetsobj->get_moeworksheets()->timeclose;
        }
        if ($deadlines) {
        ...
        switch ($this->attempt->state) {
            ...
            case self::OVERDUE:
                return $duedate + $this->moeworksheetsobj->get_moeworksheets()->graceperiod;
    ...
    // URLs related to this attempt ============================================
    /**
     * @return string moeworksheets view url.
     */
    public function view_url() {
        return $this->moeworksheetsobj->view_url();
    }
    ...
    public function start_attempt_url($slot = null, $page = -1) {
        ...
        return $this->moeworksheetsobj->start_attempt_url($page);
    }
    ...
    /**
     * @return string the URL of this moeworksheets's summary page.
     */
    public function summary_url() {
        return new moodle_url('/mod/moeworksheets/summary.php', array('attempt' => $this->attempt->id));
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/moeworksheets/processattempt.php');
    }
    ...
    public function cannot_review_message($short = false) {
        return $this->moeworksheetsobj->cannot_review_message(
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                    ...
                    get_string('startnewpreview', 'moeworksheets'));
        } else {
            return '';
        }
    }
     ...
     * @param mod_moeworksheets_renderer $renderer the moeworksheets renderer.
     ...
     */
    public function render_question($slot, $reviewing, mod_moeworksheets_renderer $renderer, $thispageurl = null) {
        if ($this->is_blocked_by_previous_question($slot)) {
            ...
            return html_writer::div($placeholderqa->render($displayoptions,
                    ...
                    'mod_moeworksheets-blocked_question_warning');
        }
     ...
     */
    protected function render_question_helper($slot, $reviewing, $thispageurl, mod_moeworksheets_renderer $renderer, $seq) {
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag('p',
                        get_string('redoesofthisquestion', 'moeworksheets', $renderer->render($links)));
            }
        }
    ...
    protected function make_blocked_question_placeholder($slot) {
        ...
        $question->name = '';
        $question->questiontext = get_string('questiondependsonprevious', 'moeworksheets');
        ...
        $placeholderqa->start($this->get_moeworksheets()->preferredbehaviour, 1);
     ...
     *
     * @param int $id the id of a question in this moeworksheets attempt.
     ...
     * @return string HTML for the question in its current state.
     */
    public function render_question_at_step($slot, $seq, $reviewing, mod_moeworksheets_renderer $renderer, $thispageurl = '') {
     ...
     *
     * @param $panelclass The type of panel, moeworksheets_attempt_nav_panel or moeworksheets_review_nav_panel
     * @param $page the current page number.
     * @param $showall whether we are showing the whole moeworksheets on one page. (Used by review.php)
     * @return moeworksheets_nav_panel_base the requested object.
     */
    public function get_navigation_panel(mod_moeworksheets_renderer $output,
             $panelclass, $page, $showall = false) {
        $panel = new $panelclass($this, $this->get_display_options(true), $page, $showall);

        $bc = new block_contents();
        $bc->attributes['id'] = 'mod_moeworksheets_navblock';
        $bc->attributes['role'] = 'navigation';
        $bc->attributes['aria-labelledby'] = 'mod_moeworksheets_navblock_title';
        $bc->title = html_writer::span(get_string('moeworksheetsnavigation', 'moeworksheets'), '', array('id' => 'mod_moeworksheets_navblock_title'));
        $bc->content = $output->navigation_panel($panel, $this, $page);
        return $bc;
    }

    /**
     * Return an array of variant URLs to other attempts at this moeworksheets.
     *
     * The $url passed in must contain an attempt parameter.
     *
     * The {@link mod_moeworksheets_links_to_other_attempts} object returned contains an
     ...
     * @return mod_moeworksheets_links_to_other_attempts containing array int => null|moodle_url.
     */
    public function links_to_other_attempts(moodle_url $url) {
        $attempts = moeworksheets_get_user_attempts($this->get_moeworksheets()->id, $this->attempt->userid, 'all');
        if (count($attempts) <= 1) {
            return false;
        }

        $links = new mod_moeworksheets_links_to_other_attempts();
     ...
     * @return mod_moeworksheets_links_to_other_attempts|null containing array int => null|moodle_url,
    ...
    public function handle_if_time_expired($timestamp, $studentisonline) {
        ...
        if ($this->attempt->state == self::OVERDUE) {
            ...
            $graceperiod = $this->moeworksheetsobj->get_moeworksheets()->graceperiod;
        ...
        // Otherwise, we were in moeworksheets_attempt::IN_PROGRESS, and time has now expired.
        // Transition to the appropriate state.
        switch ($this->moeworksheetsobj->get_moeworksheets()->overduehandling) {
            case 'autosubmit':
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false, $simulatedresponses = null) {
        ...
        if ($becomingoverdue) {
            ...
            $DB->update_record('moeworksheets_attempts', $this->attempt);
        }

        if (!$this->is_preview() && $this->attempt->state == self::FINISHED) {
            moeworksheets_save_best_grade($this->get_moeworksheets(), $this->get_userid());
        }
    ...
    public function process_redo_question($slot, $timestamp) {
        ...
        $qubaids = new \mod_moeworksheets\question\qubaids_for_users_attempts(
                $this->get_moeworksheetsid(), $this->get_userid());
    ...
    public function process_finish($timestamp, $processsubmitted) {
        ...
        if (!$this->is_preview()) {
            moeworksheets_save_best_grade($this->get_moeworksheets(), $this->attempt->userid);

            // Trigger event.
            $this->fire_state_transition_event('\mod_moeworksheets\event\attempt_submitted', $timestamp);
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('moeworksheets_attempts', 'timecheckstate', $time, array('id' => $this->attempt->id));
        }
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_moeworksheets\event\attempt_becameoverdue', $timestamp);

        $transaction->allow_commit();

        moeworksheets_send_overdue_message($this);
    }
    ...
    public function process_abandon($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_moeworksheets\event\attempt_abandoned', $timestamp);
    ...
    protected function fire_state_transition_event($eventclass, $timestamp) {
        global $USER;
        $moeworksheetsrecord = $this->get_moeworksheets();
        $params = array(
            'context' => $this->get_moeworksheetsobj()->get_context(),
            'courseid' => $this->get_courseid(),
            'objectid' => $this->attempt->id,
            'relateduserid' => $this->attempt->userid,
            'other' => array(
                'submitterid' => CLI_SCRIPT ? null : $USER->id,
                'moeworksheetsid' => $moeworksheetsrecord->id
            )
        );

        $event = $eventclass::create($params);
        $event->add_record_snapshot('moeworksheets', $this->get_moeworksheets());
        $event->add_record_snapshot('moeworksheets_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    /**
     * Get a URL for a particular question on a particular page of the moeworksheets.
     * Used by {@link attempt_url()} and {@link review_url()}.
     *
     * @param string $script. Used in the URL like /mod/moeworksheets/$script.php
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/moeworksheets/' . $script . '.php' . $fragment,
    ...
    /**
     * Process responses during an attempt at a moeworksheets.
     *
    ...
    public function process_attempt($timenow, $finishattempt, $timeup, $thispage) {
        ...
        // to show the student another page of the moeworksheets. Just finish now.
        $graceperiodmin = null;
        ...
        $toolate = false;
        if ($timeclose !== false && $timenow > $timeclose - moeworksheets_MIN_TIME_TO_CONTINUE) {
            $timeup = true;
            $graceperiodmin = get_config('moeworksheets', 'graceperiodmin');
        ...
        if ($timeup) {
            if ($this->get_moeworksheets()->overduehandling == 'graceperiod') {
                if (is_null($graceperiodmin)) {
                    $graceperiodmin = get_config('moeworksheets', 'graceperiodmin');
                }
                if ($timenow > $timeclose + $this->get_moeworksheets()->graceperiod + $graceperiodmin) {
        ...
        // Update the moeworksheets attempt record.
        try {
    ...
    public function check_page_access($page) {
        ...
        if ($this->get_currentpage() != $page) {
            if ($this->get_navigation_method() == moeworksheets_NAVMETHOD_SEQ && $this->get_currentpage() > $page) {
                return false;
            }
        }
        return true;
    ...
    public function set_currentpage($page) {
        ...
        if ($this->check_page_access($page)) {
            $DB->set_field('moeworksheets_attempts', 'currentpage', $page, array('id' => $this->get_attemptid()));
            return true;
        }
        return false;
    }
    ...
    public function fire_attempt_viewed_event() {
        $params = array(
            ...
            'other' => array(
                'moeworksheetsid' => $this->get_moeworksheetsid()
            )
        );
 ...
 * @since      Moodle 2.9
 */
class moeworksheets_nav_section_heading implements renderable {
 ...
 * @since      Moodle 2.1
 */
class moeworksheets_nav_question_button implements renderable {
 ...
 * @since      Moodle 2.0
 */
abstract class moeworksheets_nav_panel_base {
    /** @var moeworksheets_attempt */
    ...
    public function __construct(moeworksheets_attempt $attemptobj,
    ...
    /**
     * Get the buttons and section headings to go in the moeworksheets navigation block.
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            if ($heading = $this->attemptobj->get_heading_before_slot($slot)) {
                $buttons[] = new moeworksheets_nav_section_heading(format_string($heading));
            }

            $qa = $this->attemptobj->get_question_attempt($slot);
            $showcorrectness = $this->options->correctness && $qa->has_marks();

            $button = new moeworksheets_nav_question_button();
            $button->id          = 'moeworksheetsnavbutton' . $slot;
            ...
            if ($this->attemptobj->is_blocked_by_previous_question($slot)) {
                ...
                $button->statestring = get_string('questiondependsonprevious', 'moeworksheets');
            }
            $buttons[] = $button;
        }
        $pages = array();
    ...
    protected function get_state_string(question_attempt $qa, $showcorrectness) {
        ...
        if ($qa->get_state() == question_state::$todo) {
            return get_string('notyetviewed', 'moeworksheets');
        } else {
            return get_string('viewed', 'moeworksheets');
        }
    }

    public function render_before_button_bits(mod_moeworksheets_renderer $output) {
        return '';
    }

    abstract public function render_end_bits(mod_moeworksheets_renderer $output);
    ...
    public function user_picture() {
        global $DB;
        if ($this->attemptobj->get_moeworksheets()->showuserpicture == moeworksheets_SHOWIMAGE_NONE) {
            return null;
        }
        $user = $DB->get_record('user', array('id' => $this->attemptobj->get_userid()));
        $userpicture = new user_picture($user);
        $userpicture->courseid = $this->attemptobj->get_courseid();
        if ($this->attemptobj->get_moeworksheets()->showuserpicture == moeworksheets_SHOWIMAGE_LARGE) {
    ...
    public function get_button_container_class() {
        // moeworksheets navigation is set on 'Show all questions on one page'.
        if ($this->showall) {
            return 'allquestionsononepage';
        }
        // moeworksheets navigation is set on 'Show one page at a time'.
        return 'multipages';
    }
}
...
/**
 * Specialisation of {@link moeworksheets_nav_panel_base} for the attempt moeworksheets page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class moeworksheets_attempt_nav_panel extends moeworksheets_nav_panel_base {
    ...
    public function render_before_button_bits(mod_moeworksheets_renderer $output) {
        return html_writer::tag('div', get_string('navnojswarning', 'moeworksheets'),
                array('id' => 'moeworksheetsnojswarning'));
    }

    public function render_end_bits(mod_moeworksheets_renderer $output) {
...
/**
 * Specialisation of {@link moeworksheets_nav_panel_base} for the review moeworksheets page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class moeworksheets_review_nav_panel extends moeworksheets_nav_panel_base {
    public function get_question_url($slot) {
        ...
        if ($this->attemptobj->get_num_pages() > 1) {
            if ($this->showall) {
                $html .= html_writer::link($this->attemptobj->review_url(null, 0, false),
                        get_string('showeachpage', 'moeworksheets'));
            } else {
                $html .= html_writer::link($this->attemptobj->review_url(null, 0, true),
                        get_string('showall', 'moeworksheets'));
            }
        }
