...
/**
 * moeworksheets external functions
 *
 * @package    mod_moeworksheets
 * @category   external
 * @copyright  2016 Juan Leyva <juan@moodle.com>
 ...
 * @since      Moodle 3.1
 */
class mod_moeworksheets_external extends external_api {

    /**
     * Describes the parameters for get_moeworksheetszes_by_courses.
     *
     * @return external_external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_moeworksheetszes_by_courses_parameters() {
    ...
    /**
     * Returns a list of moeworksheetszes in a provided list of courses,
     * if no list is provided all moeworksheetszes that the user can view will be returned.
     *
     * @param array $courseids Array of course ids
     * @return array of moeworksheetszes details
     * @since Moodle 3.1
     */
    public static function get_moeworksheetszes_by_courses($courseids = array()) {
        global $USER;

        $warnings = array();
        $returnedmoeworksheetszes = array();

        $params = array(
            'courseids' => $courseids,
        );
        $params = self::validate_parameters(self::get_moeworksheetszes_by_courses_parameters(), $params);
        ...
        if (!empty($params['courseids'])) {
            ...
            // Get the moeworksheetszes in this course, this function checks users visibility permissions.
            // We can avoid then additional validate_context calls.
            $moeworksheetszes = get_all_instances_in_courses("moeworksheets", $courses);
            foreach ($moeworksheetszes as $moeworksheets) {
                ...
                $moeworksheets = moeworksheets_update_effective_access($moeworksheets, $USER->id);

                // Entry to return.
                $moeworksheetsdetails = array();
                // First, we return information that any user can see in the web interface.
                $moeworksheetsdetails['id'] = $moeworksheets->id;
                $moeworksheetsdetails['coursemodule']      = $moeworksheets->coursemodule;
                $moeworksheetsdetails['course']            = $moeworksheets->course;
                $moeworksheetsdetails['name']              = external_format_string($moeworksheets->name, $context->id);

                if (has_capability('mod/moeworksheets:view', $context)) {
                    // Format intro.
                    list($moeworksheetsdetails['intro'], $moeworksheetsdetails['introformat']) = external_format_text($moeworksheets->intro,
                                                                    $moeworksheets->introformat, $context->id, 'mod_moeworksheets', 'intro', null);

                    $viewablefields = array('timeopen', 'timeclose', 'grademethod', 'section', 'visible', 'groupmode',
                                            'groupingid');

                    $timenow = time();
                    $moeworksheetsobj = moeworksheets::create($moeworksheets->id, $USER->id);
                    $accessmanager = new moeworksheets_access_manager($moeworksheetsobj, $timenow, has_capability('mod/moeworksheets:ignoretimelimits',
                                                                $context, null, false));

                    // Fields the user could see if have access to the moeworksheets.
                    if (!$accessmanager->prevent_access()) {
                        // Some times this function returns just empty.
                        $hasfeedback = moeworksheets_has_feedback($moeworksheets);
                        $moeworksheetsdetails['hasfeedback'] = (!empty($hasfeedback)) ? 1 : 0;
                        $moeworksheetsdetails['hasquestions'] = (int) $moeworksheetsobj->has_questions();
                        $moeworksheetsdetails['autosaveperiod'] = get_config('moeworksheets', 'autosaveperiod');
                    ...
                    foreach ($viewablefields as $field) {
                        $moeworksheetsdetails[$field] = $moeworksheets->{$field};
                    }
                }
                $returnedmoeworksheetszes[] = $moeworksheetsdetails;
            }
        }
        $result = array();
        $result['moeworksheetszes'] = $returnedmoeworksheetszes;
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_moeworksheetszes_by_courses return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_moeworksheetszes_by_courses_returns() {
        return new external_single_structure(
            array(
                'moeworksheetszes' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            'id' => new external_value(PARAM_INT, 'Standard Moodle primary key.'),
                            'course' => new external_value(PARAM_INT, 'Foreign key reference to the course this moeworksheets is part of.'),
                            'coursemodule' => new external_value(PARAM_INT, 'Course module id.'),
                            'name' => new external_value(PARAM_RAW, 'moeworksheets name.'),
                            'intro' => new external_value(PARAM_RAW, 'moeworksheets introduction text.', VALUE_OPTIONAL),
                            'introformat' => new external_format_value('intro', VALUE_OPTIONAL),
                            'timeopen' => new external_value(PARAM_INT, 'The time when this moeworksheets opens. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timeclose' => new external_value(PARAM_INT, 'The time when this moeworksheets closes. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timelimit' => new external_value(PARAM_INT, 'The time limit for moeworksheets attempts, in seconds.',
                                                                VALUE_OPTIONAL),
                            ...
                            'canredoquestions' => new external_value(PARAM_INT, 'Allows students to redo any completed question
                                                                        within a moeworksheets attempt.', VALUE_OPTIONAL),
                            ...
                            'attemptonlast' => new external_value(PARAM_INT, 'Whether subsequent attempts start from the answer
                                                                    ...
                                                                    VALUE_OPTIONAL),
                            'grademethod' => new external_value(PARAM_INT, 'One of the values moeworksheets_GRADEHIGHEST, moeworksheets_GRADEAVERAGE,
                                                                    moeworksheets_ATTEMPTFIRST or moeworksheets_ATTEMPTLAST.', VALUE_OPTIONAL),
                            ...
                            'reviewattempt' => new external_value(PARAM_INT, 'Whether users are allowed to review their moeworksheets
                                                                    attempts at various times. This is a bit field, decoded by the
                                                                    mod_moeworksheets_display_options class. It is formed by ORing together
                                                                    the constants defined there.', VALUE_OPTIONAL),
                            'reviewcorrectness' => new external_value(PARAM_INT, 'Whether users are allowed to review their moeworksheets
                                                                        attempts at various times.
                                                                        A bit field, like reviewattempt.', VALUE_OPTIONAL),
                            'reviewmarks' => new external_value(PARAM_INT, 'Whether users are allowed to review their moeworksheets attempts
                            ...
                            'reviewspecificfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their
                                                                            moeworksheets attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'reviewgeneralfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their
                                                                            moeworksheets attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'reviewrightanswer' => new external_value(PARAM_INT, 'Whether users are allowed to review their moeworksheets
                                                                        attempts at various times. A bit field, like
                                                                        reviewattempt.', VALUE_OPTIONAL),
                            'reviewoverallfeedback' => new external_value(PARAM_INT, 'Whether users are allowed to review their moeworksheets
                                                                            attempts at various times. A bit field, like
                                                                            reviewattempt.', VALUE_OPTIONAL),
                            'questionsperpage' => new external_value(PARAM_INT, 'How often to insert a page break when editing
                                                                        the moeworksheets, or when shuffling the question order.',
                                                                        VALUE_OPTIONAL),
                            'navmethod' => new external_value(PARAM_ALPHA, 'Any constraints on how the user is allowed to navigate
                                                                around the moeworksheets. Currently recognised values are
                            ...
                            'sumgrades' => new external_value(PARAM_FLOAT, 'The total of all the question instance maxmarks.',
                                                                VALUE_OPTIONAL),
                            'grade' => new external_value(PARAM_FLOAT, 'The total that the moeworksheets overall grade is scaled to be
                                                            out of.', VALUE_OPTIONAL),
                            'timecreated' => new external_value(PARAM_INT, 'The time when the moeworksheets was added to the course.',
                                                                VALUE_OPTIONAL),
                            ...
                            'password' => new external_value(PARAM_RAW, 'A password that the student must enter before starting or
                                                                continuing a moeworksheets attempt.', VALUE_OPTIONAL),
                            'subnet' => new external_value(PARAM_RAW, 'Used to restrict the IP addresses from which this moeworksheets can
                            ...
                            'completionattemptsexhausted' => new external_value(PARAM_INT, 'Mark moeworksheets complete when the student has
                            ...
                            'hasfeedback' => new external_value(PARAM_INT, 'Whether the moeworksheets has any non-blank feedback text',
                                                                VALUE_OPTIONAL),
                            'hasquestions' => new external_value(PARAM_INT, 'Whether the moeworksheets has questions', VALUE_OPTIONAL),
    ...
    /**
     * Utility function for validating a moeworksheets.
     *
     * @param int $moeworksheetsid moeworksheets instance id
     * @return array array containing the moeworksheets, course, context and course module objects
     * @since  Moodle 3.1
     */
    protected static function validate_moeworksheets($moeworksheetsid) {
        global $DB;

        // Request and permission validation.
        $moeworksheets = $DB->get_record('moeworksheets', array('id' => $moeworksheetsid), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($moeworksheets, 'moeworksheets');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        return array($moeworksheets, $course, $cm, $context);
    }

    /**
     * Describes the parameters for view_moeworksheets.
     *
     * @return external_external_function_parameters
     * @since Moodle 3.1
     */
    public static function view_moeworksheets_parameters() {
        return new external_function_parameters (
            array(
                'moeworksheetsid' => new external_value(PARAM_INT, 'moeworksheets instance id'),
            )
        );
    }
     ...
     */
    public static function view_moeworksheets($moeworksheetsid) {
        global $DB;

        $params = self::validate_parameters(self::view_moeworksheets_parameters(), array('moeworksheetsid' => $moeworksheetsid));
        $warnings = array();

        list($moeworksheets, $course, $cm, $context) = self::validate_moeworksheets($params['moeworksheetsid']);
    ...
    /**
     * Describes the view_moeworksheets return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function view_moeworksheets_returns() {
        return new external_single_structure(
            array(
    ...
    public static function get_user_attempts_parameters() {
        return new external_function_parameters (
            array(
                ...
                'status' => new external_value(PARAM_ALPHA, 'moeworksheets status: all, finished or unfinished', VALUE_DEFAULT, 'finished'),
    ...
    /**
     * Return a list of attempts for the given moeworksheets and user.
     *
     * @param int $moeworksheetsid moeworksheets instance id
     * @param int $userid user id
     * @param string $status moeworksheets status: all, finished or unfinished
     * @param bool $includepreviews whether to include previews or not
     * @return array of warnings and the list of attempts
     * @since Moodle 3.1
     * @throws invalid_parameter_exception
     */
    public static function get_user_attempts($moeworksheetsid, $userid = 0, $status = 'finished', $includepreviews = false) {
        global $DB, $USER;

        $warnings = array();

        $params = array(
            'moeworksheetsid' => $moeworksheetsid,
            'userid' => $userid,
            'status' => $status,
        ...
        if ($USER->id != $user->id) {
            require_capability('mod/moeworksheets:viewreports', $context);
        }

        $attempts = moeworksheets_get_user_attempts($moeworksheets->id, $user->id, $params['status'], $params['includepreviews']);
    ...
    private static function attempt_structure() {
        return new external_single_structure(
            array(
                ...
                'moeworksheets' => new external_value(PARAM_INT, 'Foreign key reference to the moeworksheets that was attempted.',
                                                VALUE_OPTIONAL),
                'userid' => new external_value(PARAM_INT, 'Foreign key reference to the user whose attempt this is.',
                                                VALUE_OPTIONAL),
                'attempt' => new external_value(PARAM_INT, 'Sequentially numbers this students attempts at this moeworksheets.',
                                                VALUE_OPTIONAL),
                'uniqueid' => new external_value(PARAM_INT, 'Foreign key reference to the question_usage that holds the
                                                    details of the the question_attempts that make up this moeworksheets
                ...
                'timecheckstate' => new external_value(PARAM_INT, 'Next time moeworksheets cron should check attempt for
    ...
    /**
     * Get the best current grade for the given user on a moeworksheets.
     *
     * @param int $moeworksheetsid moeworksheets instance id
     * @param int $userid user id
     ...
     * @since Moodle 3.1
     */
    public static function get_user_best_grade($moeworksheetsid, $userid = 0) {
        global $DB, $USER;
        ...
        $result = array();
        $grade = moeworksheets_get_best_grade($moeworksheets, $user->id);
    ...
    public static function get_user_best_grade_returns() {
        return new external_single_structure(
            array(
                'hasgrade' => new external_value(PARAM_BOOL, 'Whether the user has a grade on the given moeworksheets.'),
     ...
     * @since Moodle 3.1
     */
    public static function get_combined_review_options($moeworksheetsid, $userid = 0) {
        global $DB, $USER;
        ...
        $attempts = moeworksheets_get_user_attempts($moeworksheets->id, $user->id, 'all', true);
    ...
    /**
     * Starts a new attempt at a moeworksheets.
     *
     * @param int $moeworksheetsid moeworksheets instance id
     ...
     */
    public static function start_attempt($moeworksheetsid, $preflightdata = array(), $forcenew = false) {
        global $DB, $USER;
        ...
        if (!$moeworksheetsobj->has_questions()) {
            throw new moodle_moeworksheets_exception($moeworksheetsobj, 'noquestionsfound');
        }
        ...
        list($currentattemptid, $attemptnumber, $lastattempt, $messages, $page) =
            moeworksheets_validate_new_attempt($moeworksheetsobj, $accessmanager, $forcenew, -1, false);
        ...
        if (!$moeworksheetsobj->is_preview_user() && $messages) {
            ...
            if ($accessmanager->is_preflight_check_required($currentattemptid)) {
                ...
                if (!empty($errors)) {
                    throw new moodle_moeworksheets_exception($moeworksheetsobj, array_shift($errors));
                }
            ...
            if ($currentattemptid) {
                if ($lastattempt->state == moeworksheets_attempt::OVERDUE) {
                    throw new moodle_moeworksheets_exception($moeworksheetsobj, 'stateoverdue');
                } else {
                    throw new moodle_moeworksheets_exception($moeworksheetsobj, 'attemptstillinprogress');
                }
            }
    ...
    protected static function validate_attempt($params, $checkaccessrules = true, $failifoverdue = true) {
        ...
        $attemptobj = moeworksheets_attempt::create($params['attemptid']);
        ...
        if ($attemptobj->get_userid() != $USER->id) {
            throw new moodle_moeworksheets_exception($attemptobj->get_moeworksheetsobj(), 'notyourattempt');
        }
        ...
        if ($checkaccessrules) {
            ...
            if (!$ispreviewuser && $messages) {
                throw new moodle_moeworksheets_exception($attemptobj->get_moeworksheetsobj(), 'attempterror');
            }
        }

        // Attempt closed?.
        if ($attemptobj->is_finished()) {
            throw new moodle_moeworksheets_exception($attemptobj->get_moeworksheetsobj(), 'attemptalreadyclosed');
        } else if ($failifoverdue && $attemptobj->get_state() == moeworksheets_attempt::OVERDUE) {
            throw new moodle_moeworksheets_exception($attemptobj->get_moeworksheetsobj(), 'stateoverdue');
        }

        // User submitted data (like the moeworksheets password).
        if ($accessmanager->is_preflight_check_required($attemptobj->get_attemptid())) {
            ...
            if (!empty($errors)) {
                throw new moodle_moeworksheets_exception($attemptobj->get_moeworksheetsobj(), array_shift($errors));
            }
            // Pre-flight check passed.
        ...
        if (isset($params['page'])) {
            ...
            if ($params['page'] != $attemptobj->force_page_number_into_range($params['page'])) {
                throw new moodle_moeworksheets_exception($attemptobj->get_moeworksheetsobj(), 'Invalid page number');
            }

            // Prevent out of sequence access.
            if (!$attemptobj->check_page_access($params['page'])) {
                throw new moodle_moeworksheets_exception($attemptobj->get_moeworksheetsobj(), 'Out of sequence access');
            }
            ...
            if (empty($slots)) {
                throw new moodle_moeworksheets_exception($attemptobj->get_moeworksheetsobj(), 'noquestionsfound');
            }
        }
    ...
    private static function question_structure() {
        return new external_single_structure(
            array(
                ...
                'page' => new external_value(PARAM_INT, 'page of the moeworksheets this question appears on'),
                ...
                'number' => new external_value(PARAM_INT, 'question ordering number in the moeworksheets', VALUE_OPTIONAL),
     ...
     *
     * @param  moeworksheets_attempt  $attemptobj  the moeworksheets attempt object
     * @param  bool  $review  whether if we are in review mode or not
     * @param  mixed  $page  string 'all' or integer page number
     * @return array array of questions including data
     */
    private static function get_attempt_questions_data(moeworksheets_attempt $attemptobj, $review, $page = 'all') {
        global $PAGE;

        $questions = array();
        $contextid = $attemptobj->get_moeworksheetsobj()->get_context()->id;
    ...
    /**
     * Returns information for the given attempt page for a moeworksheets attempt in progress.
     *
     ...
     * @since Moodle 3.1
     * @throws moodle_moeworksheets_exceptions
     */
        ...
        return new external_single_structure(
            array(
                ...
                'messages' => new external_multiple_structure(
                    ...
                    'access messages, will only be returned for users with mod/moeworksheets:preview capability,
    ...
    /**
     * Returns a summary of a moeworksheets attempt before it is submitted.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Processes save requests during the moeworksheets. This function is intended for the moeworksheets auto-save feature.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Process responses during an attempt at a moeworksheets and also deals with attempts finishing.
     *
     * @param int $attemptid attempt id
     * @param array $data the data to be saved
    ...
    protected static function validate_attempt_review($params) {
        ...
        if ($attemptobj->is_own_attempt()) {
            if (!$attemptobj->is_finished()) {
                throw new moodle_moeworksheets_exception($attemptobj->get_moeworksheetsobj(), 'attemptclosed');
    ...
    public static function get_attempt_review($attemptid, $page = -1) {
        ...
        foreach ($summarydata as $key => $data) {
            ...
            $result['additionaldata'][] = array(
                'id' => $key,
                'title' => $data['title'], $attemptobj->get_moeworksheetsobj()->get_context()->id,
                'content' => $data['content'],
            );
        }

        // Feedback if there is any, and the user is allowed to see it now.
        $grade = moeworksheets_rescale_grade($attemptobj->get_attempt()->sumgrades, $attemptobj->get_moeworksheets(), false);
    ...
    public static function get_attempt_review_returns() {
        return new external_single_structure(
            array(
                'grade' => new external_value(PARAM_RAW, 'grade for the moeworksheets (or empty or "notyetgraded")'),
                'attempt' => self::attempt_structure(),
     ...
     * @since Moodle 3.1
     */
    public static function get_moeworksheets_feedback_for_grade_parameters() {
        return new external_function_parameters (
            array(
                'moeworksheetsid' => new external_value(PARAM_INT, 'moeworksheets instance id'),
    ...
    /**
     * Get the feedback text that should be show to a student who got the given grade in the given moeworksheets.
     *
     * @param int $moeworksheetsid moeworksheets instance id
     ...
     * @throws moodle_exception
     */
    public static function get_moeworksheets_feedback_for_grade($moeworksheetsid, $grade) {
        global $DB;
        ...
        $params = self::validate_parameters(self::get_moeworksheets_feedback_for_grade_parameters(), $params);
        $warnings = array();
        ...
        $feedback = moeworksheets_feedback_record_for_grade($params['grade'], $moeworksheets);
        if (!empty($feedback->feedbacktext)) {
            list($text, $format) = external_format_text($feedback->feedbacktext, $feedback->feedbacktextformat, $context->id,
                                                        'mod_moeworksheets', 'feedback', $feedback->id);
    ...
    /**
     * Describes the get_moeworksheets_feedback_for_grade return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_moeworksheets_feedback_for_grade_returns() {
        return new external_single_structure(
            array(
    ...
    /**
     * Describes the parameters for get_moeworksheets_access_information.
     *
     * @return external_external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_moeworksheets_access_information_parameters() {
        return new external_function_parameters (
            array(
                'moeworksheetsid' => new external_value(PARAM_INT, 'moeworksheets instance id')
            )
    ...
    /**
     * Return access information for a given moeworksheets.
     *
     * @param int $moeworksheetsid moeworksheets instance id
     ...
     * @throws  moodle_moeworksheets_exception
     */
    public static function get_moeworksheets_access_information($moeworksheetsid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_moeworksheets_access_information_parameters(), $params);

        list($moeworksheets, $course, $cm, $context) = self::validate_moeworksheets($params['moeworksheetsid']);

        $result = array();
        // Capabilities first.
        $result['canattempt'] = has_capability('mod/moeworksheets:attempt', $context);;
        $result['canmanage'] = has_capability('mod/moeworksheets:manage', $context);;
        $result['canpreview'] = has_capability('mod/moeworksheets:preview', $context);;
        $result['canreviewmyattempts'] = has_capability('mod/moeworksheets:reviewmyattempts', $context);;
        $result['canviewreports'] = has_capability('mod/moeworksheets:viewreports', $context);;

        // Access manager now.
        $moeworksheetsobj = moeworksheets::create($cm->instance, $USER->id);
        $ignoretimelimits = has_capability('mod/moeworksheets:ignoretimelimits', $context, null, false);
        $timenow = time();
        $accessmanager = new moeworksheets_access_manager($moeworksheetsobj, $timenow, $ignoretimelimits);
    ...
    /**
     * Describes the get_moeworksheets_access_information return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_moeworksheets_access_information_returns() {
        return new external_single_structure(
            array(
                'canattempt' => new external_value(PARAM_BOOL, 'Whether the user can do the moeworksheets or not.'),
                'canmanage' => new external_value(PARAM_BOOL, 'Whether the user can edit the moeworksheets settings or not.'),
                'canpreview' => new external_value(PARAM_BOOL, 'Whether the user can preview the moeworksheets or not.'),
                'canreviewmyattempts' => new external_value(PARAM_BOOL, 'Whether the users can review their previous attempts
                                                                or not.'),
                'canviewreports' => new external_value(PARAM_BOOL, 'Whether the user can view the moeworksheets reports or not.'),
    ...
    /**
     * Return access information for a given attempt in a moeworksheets.
     *
     * @param int $moeworksheetsid moeworksheets instance id
     ...
     * @throws  moodle_moeworksheets_exception
     */
    public static function get_attempt_access_information($moeworksheetsid, $attemptid = 0) {
        global $DB, $USER;
        ...
        if ($unfinishedattempt = moeworksheets_get_user_attempt_unfinished($moeworksheets->id, $USER->id)) {
            ...
            if ($unfinishedattempt->state != moeworksheets_attempt::IN_PROGRESS and $unfinishedattempt->state != moeworksheets_attempt::OVERDUE) {
    ...
    /**
     * Describes the parameters for get_moeworksheets_required_qtypes.
     *
     * @return external_external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_moeworksheets_required_qtypes_parameters() {
        return new external_function_parameters (
            array(
                'moeworksheetsid' => new external_value(PARAM_INT, 'moeworksheets instance id')
            )
    ...
    /**
     * Return the potential question types that would be required for a given moeworksheets.
     ...
     * @throws  moodle_moeworksheets_exception
     */
    public static function get_moeworksheets_required_qtypes($moeworksheetsid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_moeworksheets_required_qtypes_parameters(), $params);
        ...
        // Question types used.
        $result = array();
        $result['questiontypes'] = $moeworksheetsobj->get_all_question_types_used(true);
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_moeworksheets_required_qtypes return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_moeworksheets_required_qtypes_returns() {
        return new external_single_structure(
            array(
                'questiontypes' => new external_multiple_structure(
                                    new external_value(PARAM_PLUGIN, 'question type'), 'list of question types used in the moeworksheets'),
                'warnings' => new external_warnings(),
            )
        );
    }
    ...
    public static function add_content_to_subject ($ids, $subjectid) {
        global $DB;
        $existingcontent = $DB->get_records('moeworksheets_additionalcont', array('subjectid' => $subjectid));
        foreach ($existingcontent as $additionalcontent){
            if(!in_array($additionalcontent->id, $ids)){
                $DB->set_field('moeworksheets_additionalcont', 'subjectid', null, array('id' => $additionalcontent->id));
            }
        }
        foreach ($ids as $id){
            $DB->set_field('moeworksheets_additionalcont', 'subjectid', $subjectid, array('id' => $id));
        }
    ...
    public static function get_question_preview ($id, $cmid) {
        ...
        $quba = \question_engine::make_questions_usage_by_activity('mod_quizbs', $moeworksheetsobj->get_context());
    ...
    public static function add_subject_to_content_parameters() {
        return new external_function_parameters(array(
            'id' => new external_value(PARAM_INT),
    ...
    public static function add_subject_to_content ($id, $contentid, $cmid) {
        global $DB;
        list($moeworksheets, $cm) = get_module_from_cmid($cmid);
        $additionalcontent = new additional_content($contentid);
        $additionalcontent->set_subjectid($id);
        $additionalcontent->add_entry();
        return array('status' => 1);
    }

    public static function add_subject_to_content_returns() {
        return new external_single_structure(array(
            'status' => new external_value(PARAM_INT, 'status of execution', false, 1)
        ));
    }
