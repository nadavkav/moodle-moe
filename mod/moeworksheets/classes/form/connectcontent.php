...
class connectcontent extends \moodleform {
    ...
    protected function definition() {
        ...
        $options = array(
            'multiple' => false,
            'noselectionstring' => get_string('contentlist', 'moeworksheets'),
        );
        $contents = array();
        $contents[0] = get_string('nocontent', 'moeworksheets');
        foreach ($this->_customdata->contents as $key => $value) {
            $contents[$key] = $value->name;
        }

        $select = $mform->addElement('select', 'contents', get_string('contentlist', 'moeworksheets'), $contents, $options);
