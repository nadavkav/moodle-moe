...
/**
 * Renderer outputting the moeworksheets editing UI.
 *
...
class edit_renderer extends \plugin_renderer_base {
     ...
     *
     * @param \moeworksheets $moeworksheetsobj object containing all the moeworksheets settings information.
     * @param structure $structure object containing the structure of the moeworksheets.
     * @param \question_edit_contexts $contexts the relevant question bank contexts.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @param array $pagevars the variables from {@link question_edit_setup()}.
     * @return string HTML to output.
     */
    public function edit_page(\moeworksheets $moeworksheetsobj, structure $structure,
            \question_edit_contexts $contexts, \moodle_url $pageurl, array $pagevars) {
        $output = '';

        // Page title.
        $output .= $this->heading_with_help(get_string('editingmoeworksheetsx', 'moeworksheets',
                format_string($moeworksheetsobj->get_moeworksheets_name())), 'editingmoeworksheets', 'moeworksheets', '',
                get_string('basicideasofmoeworksheets', 'moeworksheets'), 2);

        // Information at the top.
        $output .= $this->moeworksheets_state_warnings($structure);
        $output .= $this->moeworksheets_information($structure);
        $output .= $this->maximum_grade_input($structure, $pageurl);
        $output .= $this->repaginate_button($structure, $pageurl);
        $output .= $this->total_marks($moeworksheetsobj->get_moeworksheets());
        ...
        if ($structure->can_be_edited()) {
            ...
            $this->page->requires->yui_module('moodle-mod_moeworksheets-moeworksheetsquestionbank',
                    'M.mod_moeworksheets.moeworksheetsquestionbank.init',
                    array('class' => 'questionbank', 'cmid' => $structure->get_cmid()));

            $popups .= $this->random_question_form($pageurl, $contexts, $pagevars);
            $this->page->requires->yui_module('moodle-mod_moeworksheets-randomquestion',
                    'M.mod_moeworksheets.randomquestion.init');

            $output .= html_writer::div($popups, 'mod_moeworksheets_edit_forms');

            // Include the question chooser.
            $output .= $this->question_chooser();
            $this->page->requires->yui_module('moodle-mod_moeworksheets-questionchooser', 'M.mod_moeworksheets.init_questionchooser');
        }
    ...
    /**
     * Render any warnings that might be required about the state of the moeworksheets,
     * e.g. if it has been attempted, or if the shuffle questions option is
     * turned on.
     *
     * @param structure $structure the moeworksheets structure.
     * @return string HTML to output.
     */
    public function moeworksheets_state_warnings(structure $structure) {
     ...
     * @return string HTML to output.
     */
    public function moeworksheets_information(structure $structure) {
        list($currentstatus, $explanation) = $structure->get_dates_summary();

        $output = html_writer::span(
                    get_string('numquestionsx', 'moeworksheets', $structure->get_question_count()),
                    'numberofquestions') . ' | ' .
                html_writer::span($currentstatus, 'moeworksheetsopeningstatus',
    ...
    /**
     * Render the form for setting a moeworksheets' overall grade
     *
     * @param structure $structure the moeworksheets structure.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
    ...
    public function maximum_grade_input($structure, \moodle_url $pageurl) {
        ...
        $output .= html_writer::start_tag('form', array('method' => 'post', 'action' => 'edit.php',
                'class' => 'moeworksheetssavegradesform'));
        ...
        $a = html_writer::empty_tag('input', array('type' => 'text', 'id' => 'inputmaxgrade',
                ...
                'value' => $structure->formatted_moeworksheets_grade()));
        $output .= html_writer::tag('label', get_string('maximumgradex', '', $a),
                array('for' => 'inputmaxgrade'));
        $output .= html_writer::empty_tag('input', array('type' => 'submit',
                'name' => 'savechanges', 'value' => get_string('save', 'moeworksheets')));
     ...
     * @param structure $structure the structure of the moeworksheets being edited.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
     */
    protected function repaginate_button(structure $structure, \moodle_url $pageurl) {

        $header = html_writer::tag('span', get_string('repaginatecommand', 'moeworksheets'), array('class' => 'repaginatecommand'));
        ...
        $buttonoptions = array(
            ...
            'value' => get_string('repaginatecommand', 'moeworksheets'),
        );
        if (!$structure->can_be_repaginated()) {
            $buttonoptions['disabled'] = 'disabled';
        } else {
            $this->page->requires->yui_module('moodle-mod_moeworksheets-repaginate', 'M.mod_moeworksheets.repaginate.init');
        }
    ...
    protected function repaginate_form(structure $structure, \moodle_url $pageurl) {
        $perpage = array();
        $perpage[0] = get_string('allinone', 'moeworksheets');
        for ($i = 1; $i <= 50; ++$i) {
            $perpage[$i] = $i;
        }
        ...
        $formcontent = html_writer::tag('form', html_writer::div(
                    ...
                    get_string('repaginate', 'moeworksheets', $select) .
    ...
    /**
     * Render the total marks available for the moeworksheets.
     *
     * @param \stdClass $moeworksheets the moeworksheets settings from the database.
     * @return string HTML to output.
     */
    public function total_marks($moeworksheets) {
        $totalmark = html_writer::span(moeworksheets_format_grade($moeworksheets, $moeworksheets->sumgrades), 'mod_moeworksheets_summarks');
        return html_writer::tag('span',
                get_string('totalmarksx', 'moeworksheets', $totalmark),
     ...
     *
     * @param structure $structure the structure of the moeworksheets being edited.
     * @param \stdClass $section The moeworksheets_section entry from DB
     * @return string HTML to output.
     */
    protected function start_section($structure, $section) {
        ...
        if (!$structure->can_be_edited()) {
            ...
            $editsectionheadingicon = html_writer::link(new \moodle_url('#'),
                $this->pix_icon('t/editstring', get_string('sectionheadingedit', 'moeworksheets', $section->heading),
     ...
     *
     * @param structure $structure object containing the structure of the moeworksheets.
     * @param \stdClass $section data from the moeworksheets_section table.
     * @return string HTML to output.
     */
    public function section_shuffle_questions(structure $structure, $section) {
        ...
        if ($structure->is_first_section($section)) {
            $help = $this->help_icon('shufflequestions', 'moeworksheets');
        } else {
            $help = '';
        }
        ...
        $label = html_writer::label(get_string('shufflequestions', 'moeworksheets') . ' ' . $help,
    ...
    /**
     * Render an icon to remove a section from the moeworksheets.
     *
     * @param object $section the section to be removed.
     * @return string HTML to output.
     */
    public function section_remove_icon($section) {
        $title = get_string('sectionheadingremove', 'moeworksheets', $section->heading);
        $url = new \moodle_url('/mod/moeworksheets/edit.php',
    ...
    /**
     * Renders HTML to display the questions in a section of the moeworksheets.
     *
     * This function calls {@link core_course_renderer::moeworksheets_section_question()}
     *
     * @param structure $structure object containing the structure of the moeworksheets.
    ...
    public function question_row(structure $structure, $slot, $contexts, $pagevars, $pageurl) {
        ...
        $joinhtml = '';
        if ($structure->can_be_edited() && !$structure->is_last_slot_in_moeworksheets($slot) &&
    ...
    public function page_row(structure $structure, $slot, $contexts, $pagevars, $pageurl) {
        ...
        $editlink = \html_writer::link(new \moodle_url('/mod/moeworksheets/connect.php', array(
            'cmid' => $PAGE->cm->id,
            'page' => $pagenumber,
    ...
    public function add_menu_actions(structure $structure, $page, \moodle_url $pageurl,
        ...
        $menu->set_constraint('.mod-moeworksheets-edit-content');
        $trigger = html_writer::tag('span', get_string('add', 'moeworksheets'), array('class' => 'add-menu'));
        ...
        // Disable the link if moeworksheets has attempts.
        if (!$structure->can_be_edited()) {
    ...
    public function edit_menu_actions(structure $structure, $page,
        ...
        if (!isset($str)) {
            $str = get_strings(array('addasection', 'addaquestion', 'addarandomquestion',
                    'addarandomselectedquestion', 'questionbank'), 'moeworksheets');
        }
        ...
        // Add a new question to the moeworksheets.
        ...
        if ($page) {
            $title = get_string('addquestionfrombanktopage', 'moeworksheets', $page);
        } else {
            $title = get_string('addquestionfrombankatend', 'moeworksheets');
        }
        ...
        $returnurl = new \moodle_url('/mod/moeworksheets/edit.php', array('cmid' => $structure->get_cmid(), 'data-addonpage' => $page));
        $params = array('returnurl' => $returnurl, 'cmid' => $structure->get_cmid(), 'appendqnumstring' => 'addarandomquestion');
        $url = new \moodle_url('/mod/moeworksheets/addrandom.php', $params);
        ...
        if ($page) {
            $title = get_string('addrandomquestiontopage', 'moeworksheets', $page);
        } else {
            $title = get_string('addrandomquestionatend', 'moeworksheets');
        }
    ...
    /**
     * Render the form that contains the data for adding a new question to the moeworksheets.
     *
     * @param structure $structure object containing the structure of the moeworksheets.
     * @param int $page the page number that this menu will add to.
     * @param \moodle_url $pageurl the canonical URL of this page.
    ...
    public function question(structure $structure, $slot, \moodle_url $pageurl) {
        ...
        // Action icons.
        $questionicons = '';
        $questionicons .= $this->question_preview_icon($structure->get_moeworksheets(), $structure->get_question_in_slot($slot));
        if ($structure->can_be_edited()) {
     ...
     *
     * @param \stdClass $moeworksheets the moeworksheets settings from the database.
     * @param \stdClass $question data from the question and moeworksheets_slots tables.
     * @param bool $label if true, show the preview question label after the icon
     * @param int $variant which question variant to preview (optional).
     * @return string HTML to output.
     */
    public function question_preview_icon($moeworksheets, $question, $label = null, $variant = null) {
        $url = moeworksheets_question_preview_url($moeworksheets, $question, $variant);

        // Do we want a label?
        $strpreviewlabel = '';
        if ($label) {
            $strpreviewlabel = ' ' . get_string('preview', 'moeworksheets');
        }

        // Build the icon.
        $strpreviewquestion = get_string('previewquestion', 'moeworksheets');
    ...
    /**
     * Render an icon to remove a question from the moeworksheets.
     *
     * @param structure $structure object containing the structure of the moeworksheets.
     * @param int $slot the first slot on the page we are outputting.
    ...
    /**
     * Display an icon to split or join two pages of the moeworksheets.
     *
     * @param structure $structure object containing the structure of the moeworksheets.
     * @param int $slot the first slot on the page we are outputting.
     * @return string HTML to output.
    ...
    public function page_split_join_button($structure, $slot) {
        ...
        $url = new \moodle_url('repaginate.php', array('moeworksheetsid' => $structure->get_moeworksheetsid(),
                'slot' => $slot, 'repag' => $insertpagebreak ? 2 : 1, 'sesskey' => sesskey()));

        if ($insertpagebreak) {
            $title = get_string('addpagebreak', 'moeworksheets');
            $image = $this->pix_icon('e/insert_page_break', $title);
            $action = 'addpagebreak';
        } else {
            $title = get_string('removepagebreak', 'moeworksheets');
    ...
    public function question_dependency_icon($structure, $slot) {
        ...
        if ($structure->is_question_dependent_on_previous_slot($slot)) {
            $title = get_string('questiondependencyremove', 'moeworksheets', $a);
            $image = $this->pix_icon('t/locked', get_string('questiondependsonprevious', 'moeworksheets'),
                    'moodle', array('title' => ''));
            $action = 'removedependency';
        } else {
            $title = get_string('questiondependencyadd', 'moeworksheets', $a);
            $image = $this->pix_icon('t/unlocked', get_string('questiondependencyfree', 'moeworksheets'),
                    'moodle', array('title' => ''));
    ...
    /**
     * Renders html to display a name with the link to the question on a moeworksheets edit page
     *
    ...
    public function question_name(structure $structure, $slot, $pageurl) {
        ...
        $instancename = moeworksheets_question_tostring($question);
        ...
        $output .= html_writer::link($editurl, $activitylink,
                array('title' => get_string('editquestion', 'moeworksheets').' '.$title));
    ...
    public function random_question(structure $structure, $slot, $pageurl) {
        ...
        $temp = clone($question);
        $temp->questiontext = '';
        $instancename = moeworksheets_question_tostring($temp);

        $configuretitle = get_string('configurerandomquestion', 'moeworksheets');
        $qtype = \question_bank::get_qtype($question->qtype, false);
        $namestr = $qtype->local_name();
        $icon = $this->pix_icon('icon', $namestr, $qtype->plugin_name(), array('title' => $namestr,
                'class' => 'icon activityicon', 'alt' => ' ', 'role' => 'presentation'));
        ...
        $qbanklink = ' ' . \html_writer::link($qbankurl,
                get_string('seequestions', 'moeworksheets'), array('class' => 'mod_moeworksheets_random_qbank_link'));
    ...
    public function marked_out_of_field(structure $structure, $slot) {
        ...
        $output = html_writer::span($structure->formatted_question_grade($slot),
                'instancemaxmark decimalplaces_' . $structure->get_decimal_places_for_question_marks(),
                array('title' => get_string('maxmark', 'moeworksheets')));
        ...
        $output .= html_writer::span(
            html_writer::link(
                ...
                array(
                    ...
                    'title' => get_string('editmaxmark', 'moeworksheets'),
                )
            )
        );
    ...
    protected function random_question_form(\moodle_url $thispageurl, \question_edit_contexts $contexts, array $pagevars) {
        ...
        $randomform = new \moeworksheets_add_random_form(new \moodle_url('/mod/moeworksheets/addrandom.php'),
    ...
    protected function initialise_editing_javascript(structure $structure,
        ...
        $config = new \stdClass();
        $config->resourceurl = '/mod/moeworksheets/edit_rest.php';
        $config->sectionurl = '/mod/moeworksheets/edit_rest.php';
        ...
        $this->page->requires->yui_module('moodle-mod_moeworksheets-toolboxes',
                'M.mod_moeworksheets.init_resource_toolbox',
                array(array(
                        'courseid' => $structure->get_courseid(),
                        'moeworksheetsid' => $structure->get_moeworksheetsid(),
        ...
        $this->page->requires->yui_module('moodle-mod_moeworksheets-toolboxes',
                'M.mod_moeworksheets.init_section_toolbox',
                array(array(
                        'courseid' => $structure,
                        'moeworksheetsid' => $structure->get_moeworksheetsid(),
        ...
        $this->page->requires->yui_module('moodle-mod_moeworksheets-dragdrop', 'M.mod_moeworksheets.init_section_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'moeworksheetsid' => $structure->get_moeworksheetsid(),
                        'ajaxurl' => $config->sectionurl,
        ...
        $this->page->requires->yui_module('moodle-mod_moeworksheets-dragdrop', 'M.mod_moeworksheets.init_resource_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'moeworksheetsid' => $structure->get_moeworksheetsid(),
                        'ajaxurl' => $config->resourceurl,
    ...
    protected function new_page_template(structure $structure,
        ...
        $strcontexts[] = 'addonpage="';
        $strcontexts[] = get_string('addquestionfrombanktopage', 'moeworksheets', '');
     ...
     *
     * @param \mod_moeworksheets\question\bank\custom_view $questionbank the question bank view object.
     * @param array $pagevars the variables from {@link \question_edit_setup()}.
     * @return string HTML to output / send back in response to an AJAX request.
     */
    public function question_bank_contents(\mod_moeworksheets\question\bank\custom_view $questionbank, array $pagevars) {
