...
/**
 * Defines the custom question bank view used on the Edit moeworksheets page.
 *
...
namespace mod_moeworksheets\question\bank;
defined('MOODLE_INTERNAL') || die();


/**
 * Subclass to customise the view of the question bank for the moeworksheets editing screen.
 *
 * @copyright  2009 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class custom_view extends \core_question\bank\view {
    /** @var bool whether the moeworksheets this is used by has been attemptd. */
    protected $moeworksheetshasattempts = false;
    /** @var \stdClass the moeworksheets settings. */
    protected $moeworksheets = false;
     ...
     * @param \stdClass $moeworksheets moeworksheets settings.
     */
    public function __construct($contexts, $pageurl, $course, $cm, $moeworksheets) {
    ...
    protected function wanted_columns() {
        ...
        if (empty($CFG->moeworksheetsquestionbankcolumns)) {
            $moeworksheetsquestionbankcolumns = array(
                'add_action_column',
                'checkbox_column',
            ...
            $moeworksheetsquestionbankcolumns = explode(',', $CFG->moeworksheetsquestionbankcolumns);
        }

        foreach ($moeworksheetsquestionbankcolumns as $fullname) {
            if (!class_exists($fullname)) {
                if (class_exists('mod_moeworksheets\\question\\bank\\' . $fullname)) {
                    $fullname = 'mod_moeworksheets\\question\\bank\\' . $fullname;
                    ...
                    debugging('Legacy question bank column class question_bank_' .
                            $fullname . ' should be renamed to mod_moeworksheets\\question\\bank\\' .
    ...
    protected function heading_column() {
        return 'mod_moeworksheets\\question\\bank\\question_name_text_column';
    }

    protected function default_sort() {
        return array(
            'core_question\\bank\\question_type_column' => 1,
            'mod_moeworksheets\\question\\bank\\question_name_text_column' => 1,
        );
    }

    /**
     * Let the question bank display know whether the moeworksheets has been attempted,
     * hence whether some bits of UI, like the add this question to the moeworksheets icon,
     * should be displayed.
     * @param bool $moeworksheetshasattempts whether the moeworksheets has attempts.
     */
    public function set_moeworksheets_has_attempts($moeworksheetshasattempts) {
        $this->moeworksheetshasattempts = $moeworksheetshasattempts;
        if ($moeworksheetshasattempts && isset($this->visiblecolumns['addtomoeworksheetsaction'])) {
            unset($this->visiblecolumns['addtomoeworksheetsaction']);
        }
    }

    public function preview_question_url($question) {
        return moeworksheets_question_preview_url($this->moeworksheets, $question);
    }

    public function add_to_moeworksheets_url($questionid) {
        global $CFG;
        $params = $this->baseurl->params();
        $params['addquestion'] = $questionid;
        $params['sesskey'] = sesskey();
        return new \moodle_url('/mod/moeworksheets/edit.php', $params);
    }
    ...
    protected function display_bottom_controls($totalnumber, $recurse, $category, \context $catcontext, array $addcontexts) {
        ...
        $cmoptions->hasattempts = !empty($this->moeworksheetshasattempts);
        ...
        if ($canuseall) {
            ...
            // Add selected questions to the moeworksheets.
            $params = array(
                    'type' => 'submit',
                    'name' => 'add',
                    'value' => get_string('addselectedquestionstomoeworksheets', 'moeworksheets'),
            );
    ...
    protected function display_options_form($showquestiontext, $scriptpath = '/mod/moeworksheets/edit.php',
    ...
    protected function print_category_info($category) {
        ...
        $strcategory = get_string('category', 'moeworksheets');
