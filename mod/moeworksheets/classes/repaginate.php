...
/**
 * Defines the moeworksheets repaginate class.
 *
 ...
 *
 * The moeworksheets setting allows users to write moeworksheetszes with one question per page,
...
class repaginate {
    ...
    /** @var int the id of the moeworksheets being manipulated. */
    private $moeworksheetsid;
    /** @var array the moeworksheets_slots for that moeworksheets. */
    private $slots;

    /**
     * Constructor.
     * @param int $moeworksheetsid the id of the moeworksheets being manipulated.
     * @param stdClass[] $slots the moeworksheets_slots for that moeworksheets.
     */
    public function __construct($moeworksheetsid = 0, $slots = null) {
        global $DB;
        $this->moeworksheetsid = $moeworksheetsid;
        if (!$this->moeworksheetsid) {
            $this->slots = array();
        }
        if (!$slots) {
            $this->slots = $DB->get_records('moeworksheets_slots', array('moeworksheetsid' => $this->moeworksheetsid), 'slot');
        } else {
            $this->slots = $slots;
        }
    }
    ...
    public function repaginate_slots($nextslotnumber, $type) {
        ...
        foreach ($this->slots as $slot) {
            if ($slot->slot < $nextslotnumber) {
                ...
                // Update DB.
                $DB->update_record('moeworksheets_slots', $nextslot, true);
    ...
    /**
     * Repaginate the rest.
     * @param stdClass[] $moeworksheetsslots
     * @param int $slotfrom
     * @param int $type
     * @param bool $dbupdate
     * @return stdClass[]
     */
    public function repaginate_the_rest($moeworksheetsslots, $slotfrom, $type, $dbupdate = true) {
        global $DB;
        if (!$moeworksheetsslots) {
            return null;
        }
        $newslots = array();
        foreach ($moeworksheetsslots as $slot) {
            if ($type == self::LINK) {
