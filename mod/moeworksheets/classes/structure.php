...
/**
 * Defines the \mod_moeworksheets\structure class.
 *
...
namespace mod_moeworksheets;
use mod_moeworksheets\local\additional_content;

defined('MOODLE_INTERNAL') || die();

/**
 * moeworksheets structure class.
 *
 * The structure of the moeworksheets. That is, which questions it is built up
 * from. This is used on the Edit moeworksheets page (edit.php) and also when
 * starting an attempt at the moeworksheets (startattempt.php). Once an attempt
 ...
 */
class structure {
    /** @var \moeworksheets the moeworksheets this is the structure of. */
    protected $moeworksheetsobj = null;

    /**
     * @var \stdClass[] the questions in this moeworksheets. Contains the row from the questions
     * table, with the data from the moeworksheets_slots table added, and also question_categories.contextid.
     */
    protected $questions = array();

    /** @var \stdClass[] moeworksheets_slots.id => the moeworksheets_slots rows for this moeworksheets, agumented by sectionid. */
    protected $slots = array();

    /** @var \stdClass[] moeworksheets_slots.slot => the moeworksheets_slots rows for this moeworksheets, agumented by sectionid. */
    protected $slotsinorder = array();

    /**
     * @var \stdClass[] currently a dummy. Holds data that will match the
     * moeworksheets_sections, once it exists.
     */
    ...
    /**
     * Create an instance of this class representing an empty moeworksheets.
     * @return structure
     */
    ...
    /**
     * Create an instance of this class representing the structure of a given moeworksheets.
     * @param \moeworksheets $moeworksheetsobj the moeworksheets.
     * @return structure
     */
    public static function create_for_moeworksheets($moeworksheetsobj) {
        $structure = self::create();
        $structure->moeworksheetsobj = $moeworksheetsobj;
        $structure->populate_structure($moeworksheetsobj->get_moeworksheets());
        return $structure;
    }

    /**
     * Whether there are any questions in the moeworksheets.
     * @return bool true if there is at least one question in the moeworksheets.
     */
    ...
    /**
     * Get the number of questions in the moeworksheets.
     * @return int the number of questions in the moeworksheets.
     */
     ...
     * question_category.contextid, and the moeworksheets_slots data for the question in this moeworksheets.
     */
    ...
    public function get_content_name_for_slot($pagenumber) {
        global $DB;
        $additional = $DB->get_record('moeworksheets_additionalcont', array(
     ...
     * questions cannot be depended upon because of moeworksheets options.
     * @param int $slotnumber the index of the slot in question.
    ...
    public function can_finish_during_the_attempt($slotnumber) {
        if ($this->moeworksheetsobj->get_navigation_method() == moeworksheets_NAVMETHOD_SEQ) {
            return false;
        }
        ...
        if (in_array($this->get_question_type_for_slot($slotnumber), array('random', 'missingtype'))) {
            return \question_engine::can_questions_finish_during_the_attempt(
                    $this->moeworksheetsobj->get_moeworksheets()->preferredbehaviour);
        }
        ...
        try {
            $quba = \question_engine::make_questions_usage_by_activity('mod_moeworksheets', $this->moeworksheetsobj->get_context());
            $tempslot = $quba->add_question(\question_bank::load_question(
                    $this->slotsinorder[$slotnumber]->questionid));
            $quba->set_preferred_behaviour($this->moeworksheetsobj->get_moeworksheets()->preferredbehaviour);
    ...
    /**
     * Get the course id that the moeworksheets belongs to.
     * @return int the course.id for the moeworksheets.
     */
    ...
    /**
     * Get the course module id of the moeworksheets.
     * @return int the course_modules.id for the moeworksheets.
     */
    ...
    /**
     * Get id of the moeworksheets.
     * @return int the moeworksheets.id for the moeworksheets.
     */
    ...
    /**
     * Get the moeworksheets object.
     * @return \stdClass the moeworksheets settings row from the database.
     */
    ...
    /**
     * moeworksheetszes can only be repaginated if they have not been attempted, the
     * questions are not shuffled, and there are two or more questions.
     * @return bool whether this moeworksheets can be repaginated.
     */
    ...
    /**
     * moeworksheetszes can only be edited if they have not been attempted.
     * @return bool whether the moeworksheets can be edited.
     */
    public function can_be_edited() {
        if ($this->canbeedited === null) {
            $this->canbeedited = !moeworksheets_has_attempts($this->moeworksheetsobj->get_moeworksheetsid());
        }
        return $this->canbeedited;
    }

    /**
     * This moeworksheets can only be edited if they have not been attempted.
     * Throw an exception if this is not the case.
     */
    public function check_can_be_edited() {
        if (!$this->can_be_edited()) {
            $reportlink = moeworksheets_attempt_summary_link_to_reports($this->get_moeworksheets(),
                    $this->moeworksheetsobj->get_cm(), $this->moeworksheetsobj->get_context());
            throw new \moodle_exception('cannoteditafterattempts', 'moeworksheets',
                    new \moodle_url('/mod/moeworksheets/edit.php', array('cmid' => $this->get_cmid())), $reportlink);
        }
    }

    /**
     * How many questions are allowed per page in the moeworksheets.
     * This setting controls how frequently extra page-breaks should be inserted
     * automatically when questions are added to the moeworksheets.
     * @return int the number of questions that should be on each page of the
     * moeworksheets by default.
     */
    public function get_questions_per_page() {
        return $this->moeworksheetsobj->get_moeworksheets()->questionsperpage;
    }

    /**
     * Get moeworksheets slots.
     * @return \stdClass[] the slots in this moeworksheets.
     */
    ...
    /**
     * Is this slot the last one in the moeworksheets?
     * @param int $slotnumber the index of the slot in question.
     * @return bool whether this slot the last one in the moeworksheets.
     */
    public function is_last_slot_in_moeworksheets($slotnumber) {
        end($this->slotsinorder);
        return $slotnumber == key($this->slotsinorder);
    }

    /**
     * Is this the first section in the moeworksheets?
     * @param \stdClass $section the moeworksheets_sections row.
     * @return bool whether this is first section in the moeworksheets.
     */
    ...
    /**
     * Is this the last section in the moeworksheets?
     * @param \stdClass $section the moeworksheets_sections row.
     * @return bool whether this is first section in the moeworksheets.
     */
    ...
    /**
     * Get the final slot in the moeworksheets.
     * @return \stdClass the moeworksheets_slots for for the final slot in the moeworksheets.
     */
     ...
     * @return \stdClass the requested moeworksheets_slots row.
     */
    ...
    /**
     * Get all the slots in a section of the moeworksheets.
    ...
    /**
     * Get all the sections of the moeworksheets.
     * @return \stdClass[] the sections in this moeworksheets.
     */
    ...
    /**
     * Get the overall moeworksheets grade formatted for display.
     * @return string the maximum grade for this moeworksheets.
     */
    public function formatted_moeworksheets_grade() {
        return moeworksheets_format_grade($this->get_moeworksheets(), $this->get_moeworksheets()->grade);
    }
    ...
    public function formatted_question_grade($slotnumber) {
        return moeworksheets_format_question_grade($this->get_moeworksheets(), $this->slotsinorder[$slotnumber]->maxmark);
    }

    /**
     * Get the number of decimal places for displyaing overall moeworksheets grades or marks.
     * @return int the number of decimal places.
     */
    public function get_decimal_places_for_grades() {
        return $this->get_moeworksheets()->decimalpoints;
    }
    ...
    public function get_decimal_places_for_question_marks() {
        return moeworksheets_get_grade_format($this->get_moeworksheets());
    }
    ...
    public function get_edit_page_warnings() {
        ...
        if (moeworksheets_has_attempts($this->moeworksheetsobj->get_moeworksheetsid())) {
            $reviewlink = moeworksheets_attempt_summary_link_to_reports($this->moeworksheetsobj->get_moeworksheets(),
                    $this->moeworksheetsobj->get_cm(), $this->moeworksheetsobj->get_context());
            $warnings[] = get_string('cannoteditafterattempts', 'moeworksheets', $reviewlink);
        }
    ...
    /**
     * Get the date information about the current state of the moeworksheets.
    ...
    public function get_dates_summary() {
        $timenow = time();
        $moeworksheets = $this->moeworksheetsobj->get_moeworksheets();

        // Exact open and close dates for the tool-tip.
        $dates = array();
        if ($moeworksheets->timeopen > 0) {
            if ($timenow > $moeworksheets->timeopen) {
                $dates[] = get_string('moeworksheetsopenedon', 'moeworksheets', userdate($moeworksheets->timeopen));
            } else {
                $dates[] = get_string('moeworksheetswillopen', 'moeworksheets', userdate($moeworksheets->timeopen));
            }
        }
        if ($moeworksheets->timeclose > 0) {
            if ($timenow > $moeworksheets->timeclose) {
                $dates[] = get_string('moeworksheetsclosed', 'moeworksheets', userdate($moeworksheets->timeclose));
            } else {
                $dates[] = get_string('moeworksheetscloseson', 'moeworksheets', userdate($moeworksheets->timeclose));
            }
        }
        if (empty($dates)) {
            $dates[] = get_string('alwaysavailable', 'moeworksheets');
        }
        $explanation = implode(', ', $dates);

        // Brief summary on the page.
        if ($timenow < $moeworksheets->timeopen) {
            $currentstatus = get_string('moeworksheetsisclosedwillopen', 'moeworksheets',
                    userdate($moeworksheets->timeopen, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($moeworksheets->timeclose && $timenow <= $moeworksheets->timeclose) {
            $currentstatus = get_string('moeworksheetsisopenwillclose', 'moeworksheets',
                    userdate($moeworksheets->timeclose, get_string('strftimedatetimeshort', 'langconfig')));
        } else if ($moeworksheets->timeclose && $timenow > $moeworksheets->timeclose) {
            $currentstatus = get_string('moeworksheetsisclosed', 'moeworksheets');
        } else {
            $currentstatus = get_string('moeworksheetsisopen', 'moeworksheets');
        }
    ...
    /**
     * Set up this class with the structure for a given moeworksheets.
     * @param \stdClass $moeworksheets the moeworksheets settings.
     */
    public function populate_structure($moeworksheets) {
        global $DB;
        ...
        // Get moeworksheets sections in ascending order of the firstslot.
        $this->sections = $DB->get_records('moeworksheets_sections', array('moeworksheetsid' => $moeworksheets->id), 'firstslot ASC');
     ...
     * @param \stdClass[] $slots the data about the slots and questions in the moeworksheets.
    ...
    protected function populate_missing_questions($slots) {
        ...
        foreach ($slots as $slot) {
            if ($slot->qtype === null) {
                ...
                $slot->name = get_string('missingquestion', 'moeworksheets');
                $slot->slot = $slot->slot;
                $slot->maxmark = 0;
    ...
    protected function populate_question_numbers() {
        ...
        foreach ($this->slots as $slot) {
            if ($this->questions[$slot->questionid]->length == 0) {
                $slot->displayednumber = get_string('infoshort', 'moeworksheets');
            } else {
    ...
    public function move_slot($idmove, $idmoveafter, $page) {
        ...
        } else if (!$this->is_last_slot_in_moeworksheets($moveafterslotnumber) &&
        ...
        if ($moveafterslotnumber > $movingslotnumber) {
            ...
            if ($this->is_last_slot_in_moeworksheets($moveafterslotnumber) ||
        ...
        if ($slotreorder) {
            update_field_with_unique_index('moeworksheets_slots', 'slot', $slotreorder,
                    array('moeworksheetsid' => $this->get_moeworksheetsid()));
        }

        // Page has changed. Record it.
        if ($movingslot->page != $page) {
            $DB->set_field('moeworksheets_slots', 'page', $page,
        ...
        if ($headingmovedirection == -1) {
            $DB->execute("
                ...
                ", array($headingmovedirection, $this->get_moeworksheetsid(),
        ...
        $emptypages = $DB->get_fieldset_sql("
                SELECT DISTINCT page - 1
                 ...
                 WHERE moeworksheetsid = ?
                   AND page > 1
                   AND NOT EXISTS (SELECT 1 FROM {moeworksheets_slots} WHERE moeworksheetsid = ? AND page = slot.page - 1)
              ORDER BY page - 1 DESC
                ", array($this->get_moeworksheetsid(), $this->get_moeworksheetsid()));

        foreach ($emptypages as $page) {
            $DB->execute("
                    UPDATE {moeworksheets_slots}
                       SET page = page - 1
                     WHERE moeworksheetsid = ?
                       AND page > ?
                    ", array($this->get_moeworksheetsid(), $page));
        }
    ...
    /**
     * Refresh page numbering of moeworksheets slots.
    ...
    public function refresh_page_numbers($slots = array()) {
        ...
        if (!count($slots)) {
            $slots = $DB->get_records('moeworksheets_slots', array('moeworksheetsid' => $this->get_moeworksheetsid()), 'slot, page');
        }
    ...
    /**
     * Refresh page numbering of moeworksheets slots and save to the database.
     * @param \stdClass $moeworksheets the moeworksheets object.
     * @return \stdClass[] array of slot objects.
     */
    public function refresh_page_numbers_and_update_db() {
        ...
        foreach ($slots as $slot) {
            $DB->set_field('moeworksheets_slots', 'page', $slot->page,
                    array('id' => $slot->id));
        }
    ...
    /**
     * Remove a slot from a moeworksheets
    ...
    public function remove_slot($slotnumber) {
        ...
        $slot = $DB->get_record('moeworksheets_slots', array('moeworksheetsid' => $this->get_moeworksheetsid(), 'slot' => $slotnumber));
        if (!$slot) {
            return;
        }
        $maxslot = $DB->get_field_sql('SELECT MAX(slot) FROM {moeworksheets_slots} WHERE moeworksheetsid = ?', array($this->get_moeworksheetsid()));

        $trans = $DB->start_delegated_transaction();
        $DB->delete_records('moeworksheets_slots', array('id' => $slot->id));
        for ($i = $slot->slot + 1; $i <= $maxslot; $i++) {
            $DB->set_field('moeworksheets_slots', 'slot', $i - 1,
                    array('moeworksheetsid' => $this->get_moeworksheetsid(), 'slot' => $i));
        }
        ...
        $DB->execute("
                UPDATE {moeworksheets_sections}
                 ...
                 WHERE moeworksheetsid = ?
                   AND firstslot > ?
                ", array($this->get_moeworksheetsid(), $slotnumber));
     ...
     *
     * Saves changes to the question grades in the moeworksheets_slots table and any
     * corresponding question_attempts.
     * It does not update 'sumgrades' in the moeworksheets table.
     *
     * @param \stdClass $slot row from the moeworksheets_slots table.
    ...
    public function update_slot_maxmark($slot, $maxmark) {
        ...
        $DB->update_record('moeworksheets_slots', $slot);
        \question_engine::set_max_mark_in_attempts(new \qubaids_for_moeworksheets($slot->moeworksheetsid),
                $slot->slot, $maxmark);
        $trans->allow_commit();
    ...
    public function update_question_dependency($slotid, $requireprevious) {
        global $DB;
        $DB->set_field('moeworksheets_slots', 'requireprevious', $requireprevious, array('id' => $slotid));
    }

    /**
     * Add/Remove a pagebreak.
     *
     * Saves changes to the slot page relationship in the moeworksheets_slots table and reorders the paging
    ...
    public function update_page_break($slotid, $type) {
        ...
        $moeworksheetsslots = $DB->get_records('moeworksheets_slots', array('moeworksheetsid' => $this->get_moeworksheetsid()), 'slot');
        $repaginate = new \mod_moeworksheets\repaginate($this->get_moeworksheetsid(), $moeworksheetsslots);
        $repaginate->repaginate_slots($moeworksheetsslots[$slotid]->slot, $type);
    ...
    public function add_section_heading($pagenumber, $heading = 'Section heading ...') {
        ...
        $section->moeworksheetsid = $this->get_moeworksheetsid();
        $slotsonpage = $DB->get_records('moeworksheets_slots', array('moeworksheetsid' => $this->get_moeworksheetsid(), 'page' => $pagenumber), 'slot DESC');
        $section->firstslot = end($slotsonpage)->slot;
        $section->shufflequestions = 0;
        return $DB->insert_record('moeworksheets_sections', $section);
    }
    ...
    public function set_section_heading($id, $newheading) {
        global $DB;
        $section = $DB->get_record('moeworksheets_sections', array('id' => $id), '*', MUST_EXIST);
        $section->heading = $newheading;
        $DB->update_record('moeworksheets_sections', $section);
    }
    ...
    public function remove_section_heading($sectionid) {
        global $DB;
        $section = $DB->get_record('moeworksheets_sections', array('id' => $sectionid), '*', MUST_EXIST);
        if ($section->firstslot == 1) {
            throw new \coding_exception('Cannot remove the first section in a moeworksheets.');
        }
        $DB->delete_records('moeworksheets_sections', array('id' => $sectionid));
    }
}
