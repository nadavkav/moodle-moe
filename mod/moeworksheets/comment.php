...
$PAGE->set_url('/mod/moeworksheets/comment.php', array('attempt' => $attemptid, 'slot' => $slot));
...
if (!$attemptobj->is_finished()) {
    print_error('attemptclosed', 'moeworksheets');
}

// Check login and permissions.
require_login($attemptobj->get_course(), false, $attemptobj->get_cm());
$attemptobj->require_capability('mod/moeworksheets:grade');

// Print the page header.
$PAGE->set_pagelayout('popup');
$PAGE->set_title(get_string('manualgradequestion', 'moeworksheets', array(
        'question' => format_string($attemptobj->get_question_name($slot)),
        'moeworksheets' => format_string($attemptobj->get_moeworksheets_name()), 'user' => fullname($student))));
...
// moeworksheets name.
$summarydata['moeworksheetsname'] = array(
    'title'   => get_string('modulename', 'moeworksheets'),
    'content' => format_string($attemptobj->get_moeworksheets_name()),
);

// Question name.
$summarydata['questionname'] = array(
    'title'   => get_string('question', 'moeworksheets'),
        ...
        <div class="fitem fitem_actionbuttons fitem_fsubmit">
            <fieldset class="felement fsubmit">
                <input id="id_submitbutton" type="submit" name="submit" value="<?php
                        print_string('save', 'moeworksheets'); ?>"/>
            </fieldset>
        </div>
    </div>
</fieldset>
<?php
echo '</form>';
$PAGE->requires->js_init_call('M.mod_moeworksheets.init_comment_popup', null, false, moeworksheets_get_js_module());
