...
$cm = get_coursemodule_from_id('moeworksheets', $id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$moeworksheets = $DB->get_record('moeworksheets', array('id' => $cm->instance), '*', MUST_EXIST);
require_login($course, false, $cm);

$reportlist = moeworksheets_report_list(context_module::instance($cm->id));
if (empty($reportlist) || $userid == $USER->id) {
    ...
    // appear in the gradebook are unlikely to be able to see moeworksheets reports,
    // but it is possible.)
    redirect(new moodle_url('/mod/moeworksheets/view.php', array('id' => $cm->id)));
}
...
if ($userid) {
    ...
    $attempts = moeworksheets_get_user_attempts($moeworksheets->id, $userid, 'finished');
    $attempt = null;
    ...
    if ($attempt) {
        $attemptobj = new moeworksheets_attempt($attempt, $moeworksheets, $cm, $course, false);
...
redirect(new moodle_url('/mod/moeworksheets/report.php', array(
