...
/**
 * Settings form for overrides in the moeworksheets module.
 *
...
require_once($CFG->dirroot . '/mod/moeworksheets/mod_form.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class moeworksheets_override_form extends moodleform {

    /** @var object course module object. */
    protected $cm;

    /** @var object the moeworksheets settings object. */
     ...
     * @param object the moeworksheets settings object.
     ...
     */
    public function __construct($submiturl, $cm, $moeworksheets, $context, $groupmode, $override) {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'moeworksheets'));
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'moeworksheets'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/moeworksheets/overrides.php', array('cmid'=>$cm->id));
                    print_error('groupsnone', 'moeworksheets', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'moeworksheets'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                $users = get_users_by_capability($this->context, 'mod/moeworksheets:attempt',
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/moeworksheets/overrides.php', array('cmid'=>$cm->id));
                    print_error('usersnone', 'moeworksheets', $link);
                }
        ...
        $mform->addElement('passwordunmask', 'password', get_string('requirepassword', 'moeworksheets'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'requirepassword', 'moeworksheets');
        $mform->setDefault('password', $this->moeworksheets->password);

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen',
                get_string('moeworksheetsopen', 'moeworksheets'), mod_moeworksheets_mod_form::$datefieldoptions);
        $mform->setDefault('timeopen', $this->moeworksheets->timeopen);

        $mform->addElement('date_time_selector', 'timeclose',
                get_string('moeworksheetsclose', 'moeworksheets'), mod_moeworksheets_mod_form::$datefieldoptions);
        $mform->setDefault('timeclose', $this->moeworksheets->timeclose);

        // Time limit.
        $mform->addElement('duration', 'timelimit',
                get_string('timelimit', 'moeworksheets'), array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'moeworksheets');
        $mform->setDefault('timelimit', $this->moeworksheets->timelimit);
        ...
        $mform->addElement('select', 'attempts',
                get_string('attemptsallowed', 'moeworksheets'), $attemptoptions);
        $mform->setDefault('attempts', $this->moeworksheets->attempts);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'moeworksheets'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'moeworksheets'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'moeworksheets'));
    ...
    public function validation($data, $files) {
        ...
        $mform =& $this->_form;
        $moeworksheets = $this->moeworksheets;
        ...
        // Ensure that at least one moeworksheets setting was changed.
        $changed = false;
        $keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
        foreach ($keys as $key) {
            if ($data[$key] != $moeworksheets->{$key}) {
                $changed = true;
                break;
            }
        }
        if (!$changed) {
            $errors['timeopen'] = get_string('nooverridedata', 'moeworksheets');
        }
