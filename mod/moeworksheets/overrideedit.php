...
/**
 * This page handles editing and creation of moeworksheets overrides
 *
...
if ($overrideid) {
    ...
    if (! $override = $DB->get_record('moeworksheets_overrides', array('id' => $overrideid))) {
        print_error('invalidoverrideid', 'moeworksheets');
    }
    if (! $moeworksheets = $DB->get_record('moeworksheets', array('id' => $override->moeworksheets))) {
...
$url = new moodle_url('/mod/moeworksheets/overrideedit.php');
if ($action) {
...
// Merge moeworksheets defaults with data.
$keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
foreach ($keys as $key) {
    if (!isset($data->{$key}) || $reset) {
        $data->{$key} = $moeworksheets->{$key};
    }
}
...
$overridelisturl = new moodle_url('/mod/moeworksheets/overrides.php', array('cmid'=>$cm->id));
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new moeworksheets_override_form($url, $cm, $moeworksheets, $context, $groupmode, $override);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    // Process the data.
    $fromform->moeworksheets = $moeworksheets->id;

    // Replace unchanged values with null.
    foreach ($keys as $key) {
        if ($fromform->{$key} == $moeworksheets->{$key}) {
            $fromform->{$key} = null;
        }
    }
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('moeworksheets_overrides', $conditions)) {
            ...
            // Set the course module id before calling moeworksheets_delete_override().
            $moeworksheets->cmid = $cm->id;
            moeworksheets_delete_override($moeworksheets, $oldoverride->id);
        }
    }
    ...
    if (!empty($override->id)) {
        ...
        $DB->update_record('moeworksheets_overrides', $fromform);
        ...
        unset($fromform->id);
        $fromform->id = $DB->insert_record('moeworksheets_overrides', $fromform);
    ...
    moeworksheets_update_events($moeworksheets, $fromform);
...
// Print the form.
$pagetitle = get_string('editoverride', 'moeworksheets');
