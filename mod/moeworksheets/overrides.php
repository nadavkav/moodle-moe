...
/**
 * This page handles listing of moeworksheets overrides
 *
...
require_once($CFG->dirroot.'/mod/moeworksheets/override_form.php');


$cmid = required_param('cmid', PARAM_INT);
$mode = optional_param('mode', '', PARAM_ALPHA); // One of 'user' or 'group', default is 'group'.

list($course, $cm) = get_course_and_cm_from_cmid($cmid, 'moeworksheets');
...
$url = new moodle_url('/mod/moeworksheets/overrides.php', array('cmid'=>$cm->id, 'mode'=>$mode));
...
require_capability('mod/moeworksheets:manageoverrides', $context);

// Display a list of overrides.
$PAGE->set_pagelayout('admin');
$PAGE->set_title(get_string('overrides', 'moeworksheets'));
...
$sql = 'SELECT o.id
            FROM {moeworksheets_overrides} o LEFT JOIN {groups} g
            ON o.groupid = g.id
...
if (!empty($orphaned)) {
    $DB->delete_records_list('moeworksheets_overrides', 'id', array_keys($orphaned));
}
...
if ($groupmode) {
    ...
    $sql = 'SELECT o.*, g.name
                ...
                WHERE o.moeworksheets = :moeworksheetsid
                ORDER BY g.name';
    $params = array('moeworksheetsid' => $moeworksheets->id);
} else {
...
$table->head = array(
        $colname,
        get_string('overrides', 'moeworksheets'),
        get_string('action'),
);
...
$overridedeleteurl = new moodle_url('/mod/moeworksheets/overridedelete.php');
$overrideediturl = new moodle_url('/mod/moeworksheets/overrideedit.php');
...
foreach ($overrides as $override) {
    ...
    if (!$groupmode) {
        if (!has_capability('mod/moeworksheets:attempt', $context, $override->userid)) {
            // User not allowed to take the moeworksheets.
            $active = false;
    ...
    if (isset($override->timeopen)) {
        $fields[] = get_string('moeworksheetsopens', 'moeworksheets');
        $values[] = $override->timeopen > 0 ?
                userdate($override->timeopen) : get_string('noopen', 'moeworksheets');
    }

    // Format timeclose.
    if (isset($override->timeclose)) {
        $fields[] = get_string('moeworksheetscloses', 'moeworksheets');
        $values[] = $override->timeclose > 0 ?
                userdate($override->timeclose) : get_string('noclose', 'moeworksheets');
    }

    // Format timelimit.
    if (isset($override->timelimit)) {
        $fields[] = get_string('timelimit', 'moeworksheets');
        $values[] = $override->timelimit > 0 ?
                format_time($override->timelimit) : get_string('none', 'moeworksheets');
    }

    // Format number of attempts.
    if (isset($override->attempts)) {
        $fields[] = get_string('attempts', 'moeworksheets');
    ...
    if (isset($override->password)) {
        $fields[] = get_string('requirepassword', 'moeworksheets');
        $values[] = $override->password !== '' ?
                get_string('enabled', 'moeworksheets') : get_string('none', 'moeworksheets');
    }
...
echo html_writer::start_tag('div', array('id' => 'moeworksheetsoverrides'));
if (count($table->data)) {
    echo html_writer::table($table);
}
if ($hasinactive) {
    echo $OUTPUT->notification(get_string('inactiveoverridehelp', 'moeworksheets'), 'dimmed_text');
}
...
if ($groupmode) {
    if (empty($groups)) {
        // There are no groups.
        echo $OUTPUT->notification(get_string('groupsnone', 'moeworksheets'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'addgroup', 'cmid' => $cm->id)),
            get_string('addnewgroupoverride', 'moeworksheets'), 'post', $options);
} else {
    $users = array();
    // See if there are any students in the moeworksheets.
    $users = get_users_by_capability($context, 'mod/moeworksheets:attempt', 'u.id');
    $info = new \core_availability\info_module($cm);
    $users = $info->filter_user_list($users);

    if (empty($users)) {
        // There are no students.
        echo $OUTPUT->notification(get_string('usersnone', 'moeworksheets'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'adduser', 'cmid' => $cm->id)),
            get_string('addnewuseroverride', 'moeworksheets'), 'get', $options);
}
