...
/**
 * Base class for moeworksheets reports that are basically a table with one row for each attempt.
 *
 * @copyright 2010 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class moeworksheets_attempts_report extends moeworksheets_default_report {
    ...
    /** @var object the moeworksheets context. */
    protected $context;

    /** @var mod_moeworksheets_attempts_report_form The settings form to use. */
    protected $form;
     ...
     * @param object $course
     */
    protected function init($mode, $formclass, $moeworksheets, $cm, $course) {
        $this->mode = $mode;
        ...
        $this->qmsubselect = moeworksheets_report_qm_filter_select($moeworksheets);

        $this->form = new $formclass($this->get_base_url(),
                array('moeworksheets' => $moeworksheets, 'currentgroup' => $currentgroup, 'context' => $this->context));
    ...
    protected function add_state_column(&$columns, &$headers) {
        $columns[] = 'state';
        $headers[] = get_string('attemptstate', 'moeworksheets');
    }
    ...
    protected function add_time_columns(&$columns, &$headers) {
        $columns[] = 'timestart';
        $headers[] = get_string('startedon', 'moeworksheets');

        $columns[] = 'timefinish';
        $headers[] = get_string('timecompleted', 'moeworksheets');

        $columns[] = 'duration';
        $headers[] = get_string('attemptduration', 'moeworksheets');
    }
     ...
     * @param bool $usercanseegrades whether the user is allowed to see grades for this moeworksheets.
     * @param array $columns the list of columns. Added to.
     * @param array $headers the columns headings. Added to.
     * @param bool $includefeedback whether to include the feedbacktext columns
     */
    protected function add_grade_columns($moeworksheets, $usercanseegrades, &$columns, &$headers, $includefeedback = true) {
        if ($usercanseegrades) {
            $columns[] = 'sumgrades';
            $headers[] = get_string('grade', 'moeworksheets') . '/' .
                    moeworksheets_format_grade($moeworksheets, $moeworksheets->grade);
        }

        if ($includefeedback && moeworksheets_has_feedback($moeworksheets)) {
            $columns[] = 'feedbacktext';
            $headers[] = get_string('feedback', 'moeworksheets');
        }
    }
     ...
     * @param mod_moeworksheets_attempts_report_options $options the display options.
    ...
    protected function set_up_table_columns($table, $columns, $headers, $reporturl,
            mod_moeworksheets_attempts_report_options $options, $collapsible) {
        ...
        if (empty($currentgroup) || $groupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    require_capability('mod/moeworksheets:deleteattempts', $this->context);
                    $this->delete_selected_attempts($moeworksheets, $cm, $attemptids, $allowed);
    ...
    /**
     * Delete the moeworksheets attempts
     * @param object $moeworksheets the moeworksheets settings. Attempts that don't belong to
     * this moeworksheets are not deleted.
     ...
     */
    protected function delete_selected_attempts($moeworksheets, $cm, $attemptids, $allowed) {
        global $DB;

        foreach ($attemptids as $attemptid) {
            $attempt = $DB->get_record('moeworksheets_attempts', array('id' => $attemptid));
            if (!$attempt || $attempt->moeworksheets != $moeworksheets->id || $attempt->preview != 0) {
                // Ensure the attempt exists, and belongs to this moeworksheets. If not skip.
                continue;
            }
            ...
            // Set the course module id before calling moeworksheets_delete_attempt().
