...
/**
 * Base class for the table used by a {@link moeworksheets_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class moeworksheets_attempts_report_table extends table_sql {
    ...
    /** @var object the moeworksheets settings for the moeworksheets we are reporting on. */
    ...
    /** @var object mod_moeworksheets_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this moeworksheets.. */
    protected $questions;
     ...
     * @param mod_moeworksheets_attempts_report_options $options
     * @param array $groupstudents
     * @param array $students
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($uniqueid, $moeworksheets, $context, $qmsubselect,
            mod_moeworksheets_attempts_report_options $options, $groupstudents, $students,
            $questions, $reporturl) {
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/moeworksheets/review.php', array('attempt' => $attempt->attempt)),
                get_string('reviewattempt', 'moeworksheets'), array('class' => 'reviewlink'));
    }
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return moeworksheets_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = moeworksheets_report_feedback_for_grade(
                moeworksheets_rescale_grade($attempt->sumgrades, $this->moeworksheets, false),
                $this->moeworksheets->id, $this->context);
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/moeworksheets/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output,
                new popup_action('click', $url, 'reviewquestion',
                        array('height' => 450, 'width' => 650)),
                array('title' => get_string('reviewresponse', 'moeworksheets')));
    ...
    public function base_sql($reportstudents) {
        ...
        $fields = $DB->sql_concat('u.id', "'#'", 'COALESCE(moeworksheetsa.attempt, 0)') . ' AS uniqueid,';
        ...
        $fields .= '
                moeworksheetsa.uniqueid AS usageid,
                moeworksheetsa.id AS attempt,
                u.id AS userid,
                u.idnumber, ' . $allnames . ',
                u.picture,
                u.imagealt,
                ...
                u.department,
                u.email' . $extrafields . ',
                moeworksheetsa.state,
                moeworksheetsa.sumgrades,
                moeworksheetsa.timefinish,
                moeworksheetsa.timestart,
                CASE WHEN moeworksheetsa.timefinish = 0 THEN null
                     WHEN moeworksheetsa.timefinish > moeworksheetsa.timestart THEN moeworksheetsa.timefinish - moeworksheetsa.timestart
                     ELSE 0 END AS duration';
            // To explain that last bit, timefinish can be non-zero and less
            // than timestart when you have two load-balanced servers with very
            // badly synchronised clocks, and a student does a really quick attempt.

        // This part is the same for all cases. Join the users and moeworksheets_attempts tables.
        $from = "\n{user} u";
        $from .= "\nLEFT JOIN {moeworksheets_attempts} moeworksheetsa ON
                                    moeworksheetsa.userid = u.id AND moeworksheetsa.moeworksheets = :moeworksheetsid";
        $params = array('moeworksheetsid' => $this->moeworksheets->id);

        if ($this->qmsubselect && $this->options->onlygraded) {
            $from .= " AND (moeworksheetsa.state <> :finishedstate OR $this->qmsubselect)";
            $params['finishedstate'] = moeworksheets_attempt::FINISHED;
        }

        switch ($this->options->attempts) {
            case moeworksheets_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'moeworksheetsa.id IS NOT NULL AND moeworksheetsa.preview = 0';
                break;
            case moeworksheets_attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND moeworksheetsa.preview = 0 AND moeworksheetsa.id IS NOT NULL";
                break;
            case moeworksheets_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND moeworksheetsa.id IS NULL";
                break;
            case moeworksheets_attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND (moeworksheetsa.preview = 0 OR moeworksheetsa.preview IS NULL)";
                break;
        }
        ...
        if ($this->options->states) {
            ...
            $params += $stateparams;
            $where .= " AND (moeworksheetsa.state $statesql OR moeworksheetsa.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{moeworksheets_attempts} {$alias}moeworksheetsa", "{$alias}moeworksheetsa.uniqueid",
                "{$alias}moeworksheetsa.moeworksheets = :{$alias}moeworksheetsid", array("{$alias}moeworksheetsid" => $this->sql->params['moeworksheetsid']));
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = moeworksheetsa.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join($this->sql->from, 'moeworksheetsa.uniqueid',
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['moeworksheetsa.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    public function wrap_html_finish() {
        ...
        echo '<a href="javascript:select_all_in(\'DIV\', null, \'tablecontainer\');">' .
                get_string('selectall', 'moeworksheets') . '</a> / ';
        echo '<a href="javascript:deselect_all_in(\'DIV\', null, \'tablecontainer\');">' .
                get_string('selectnone', 'moeworksheets') . '</a> ';
        echo '&nbsp;&nbsp;';
        $this->submit_buttons();
        echo '</div>';
    ...
    protected function submit_buttons() {
        global $PAGE;
        if (has_capability('mod/moeworksheets:deleteattempts', $this->context)) {
            echo '<input type="submit" id="deleteattemptsbutton" name="delete" value="' .
                    get_string('deleteselected', 'moeworksheets_overview') . '"/>';
            $PAGE->requires->event_handler('#deleteattemptsbutton', 'click', 'M.util.show_confirm_dialog',
                    array('message' => get_string('deleteattemptcheck', 'moeworksheets')));
        }
    }
}
