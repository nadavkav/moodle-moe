...
/**
 * This file defines the moeworksheets manual grading report class.
 *
 * @package   moeworksheets_grading
...
require_once($CFG->dirroot . '/mod/moeworksheets/report/grading/gradingsettings_form.php');


/**
 * moeworksheets report to help teachers manually grade questions that need it.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class moeworksheets_grading_report extends moeworksheets_default_report {
    ...
    public function display($moeworksheets, $cm, $course) {
        ...
        require_capability('mod/moeworksheets:grade', $this->context);
        $shownames = has_capability('moeworksheets/grading:viewstudentnames', $this->context);
        $showidnumbers = has_capability('moeworksheets/grading:viewidnumber', $this->context);
        ...
        // Get the list of questions in this moeworksheets.
        $this->questions = moeworksheets_report_get_significant_questions($moeworksheets);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'moeworksheets_grading');
        }
        ...
        if ($slot && $hasquestions) {
            ...
            if (!$counts || $counts->$grade == 0) {
                redirect($this->list_questions_url(), get_string('alldoneredirecting', 'moeworksheets_grading'));
            }
        }

        // Start output.
        $this->print_header_and_tabs($cm, $course, $moeworksheets, 'grading');
    ...
    protected function get_qubaids_condition() {
        ...
        $where = "moeworksheetsa.moeworksheets = :mangrmoeworksheetsid AND
                moeworksheetsa.preview = 0 AND
                moeworksheetsa.state = :statefinished";
        $params = array('mangrmoeworksheetsid' => $this->cm->instance, 'statefinished' => moeworksheets_attempt::FINISHED);

        $currentgroup = groups_get_activity_group($this->cm, true);
        if ($currentgroup) {
            $users = get_users_by_capability($this->context,
                    array('mod/moeworksheets:reviewmyattempts', 'mod/moeworksheets:attempt'), 'u.id, u.id', '', '', '',
                    $currentgroup, '', false);
            if (empty($users)) {
                $where .= ' AND moeworksheetsa.userid = 0';
            } else {
        ...
        return new qubaid_join('{moeworksheets_attempts} moeworksheetsa', 'moeworksheetsa.uniqueid', $where, $params);
    }
    ...
    protected function load_attempts_by_usage_ids($qubaids) {
        ...
        $params[] = moeworksheets_attempt::FINISHED;
        $params[] = $this->moeworksheets->id;

        $fields = 'moeworksheetsa.*, u.idnumber, ';
        $fields .= get_all_user_name_fields(true, 'u');
        $attemptsbyid = $DB->get_records_sql("
                SELECT $fields
                FROM {moeworksheets_attempts} moeworksheetsa
                JOIN {user} u ON u.id = moeworksheetsa.userid
                WHERE moeworksheetsa.uniqueid $asql AND moeworksheetsa.state = ? AND moeworksheetsa.moeworksheets = ?",
                $params);
    ...
    protected function base_url() {
        return new moodle_url('/mod/moeworksheets/report.php',
    ...
    protected function format_count_for_table($counts, $type, $gradestring) {
        ...
        if ($counts->$type > 0) {
            $result .= ' ' . html_writer::link($this->grade_question_url(
                    ...
                    get_string($gradestring, 'moeworksheets_grading'),
    ...
    protected function display_index($includeauto) {
        ...
        echo $OUTPUT->heading(get_string('questionsthatneedgrading', 'moeworksheets_grading'), 3);
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'moeworksheets_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'moeworksheets_grading');
        }
        ...
        if (empty($data)) {
            echo $OUTPUT->notification(get_string('nothingfound', 'moeworksheets_grading'));
            return;
        }
        ...
        $table->head[] = get_string('qno', 'moeworksheets_grading');
        $table->head[] = get_string('questionname', 'moeworksheets_grading');
        $table->head[] = get_string('tograde', 'moeworksheets_grading');
        $table->head[] = get_string('alreadygraded', 'moeworksheets_grading');
        if ($includeauto) {
            $table->head[] = get_string('automaticallygraded', 'moeworksheets_grading');
        }
        $table->head[] = get_string('total', 'moeworksheets_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new moeworksheets_grading_settings_form($hidden, $counts, $shownames, $showidnumbers);
        ...
        echo $OUTPUT->heading(get_string('gradingquestionx', 'moeworksheets_grading', $a), 3);
        echo html_writer::tag('p', html_writer::link($this->list_questions_url(),
                get_string('backtothelistofquestions', 'moeworksheets_grading')),
        ...
        $a->of = $count;
        echo $OUTPUT->heading(get_string('gradingattemptsxtoyofz', 'moeworksheets_grading', $a), 3);
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = moeworksheets_get_review_options($this->moeworksheets, $attempt, $this->context);
        ...
        echo html_writer::tag('div', html_writer::empty_tag('input', array(
                'type' => 'submit', 'value' => get_string('saveandnext', 'moeworksheets_grading'))),
                array('class' => 'mdl-align')) .
    ...
    protected function get_question_heading($attempt, $shownames, $showidnumbers) {
        ...
        if ($shownames && $showidnumbers) {
            return get_string('gradingattemptwithidnumber', 'moeworksheets_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'moeworksheets_grading', $a);
    ...
    protected function process_submitted_data() {
        ...
        foreach ($qubaids as $qubaid) {
            $attempt = $attempts[$qubaid];
            $attemptobj = new moeworksheets_attempt($attempt, $this->moeworksheets, $this->cm, $this->course);
            ...
            $events[] = \mod_moeworksheets\event\question_manually_graded::create($params);
        }
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot,
        ...
        if ($orderby == 'date') {
            ...
            $qubaids->from .= " JOIN {user} u ON moeworksheetsa.userid = u.id ";
