...
/**
 * This file defines the moeworksheets grades table.
 *
...
require_once($CFG->dirroot . '/mod/moeworksheets/report/attemptsreport_table.php');


/**
 * This is a table subclass for displaying the moeworksheets grades report.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class moeworksheets_overview_table extends moeworksheets_attempts_report_table {
     ...
     * @param moeworksheets_overview_options $options
     * @param array $groupstudents
     * @param array $students
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($moeworksheets, $context, $qmsubselect,
            moeworksheets_overview_options $options, $groupstudents, $students, $questions, $reporturl) {
        parent::__construct('mod-moeworksheets-report-overview-report', $moeworksheets , $context,
    ...
    protected function add_average_row($label, $users) {
        ...
        $record = $DB->get_record_sql("
                SELECT AVG(moeworksheetsa.sumgrades) AS grade, COUNT(moeworksheetsa.sumgrades) AS numaveraged
                  FROM $from
                 WHERE $where", $params);
        $record->grade = moeworksheets_rescale_grade($record->grade, $this->moeworksheets, false);
        ...
        $averagerow = array(
            ...
            'feedbacktext'=> strip_tags(moeworksheets_report_feedback_for_grade(
                                        $record->grade, $this->moeworksheets->id, $this->context))
        );

        if ($this->options->slotmarks) {
            $dm = new question_engine_data_mapper();
            $qubaids = new qubaid_join($from, 'moeworksheetsa.uniqueid', $where, $params);
    ...
    protected function format_average_grade_for_questions($gradeaverages) {
        ...
        foreach ($this->questions as $question) {
            if (isset($gradeaverages[$question->slot]) && $question->maxmark > 0) {
                ...
                $record->grade = moeworksheets_rescale_grade(
                        $record->averagefraction * $question->maxmark, $this->moeworksheets, false);
    ...
    protected function format_average($record, $question = false) {
        if (is_null($record->grade)) {
            ...
            $average = moeworksheets_format_question_grade($this->moeworksheets, $record->grade);
        } else {
            $average = moeworksheets_format_grade($this->moeworksheets, $record->grade);
        }
    ...
    protected function submit_buttons() {
        if (has_capability('mod/moeworksheets:regrade', $this->context)) {
            echo '<input type="submit" name="regrade" value="' .
                    get_string('regradeselected', 'moeworksheets_overview') . '"/>';
        }
    ...
    public function col_sumgrades($attempt) {
        ...
        $grade = moeworksheets_rescale_grade($attempt->sumgrades, $this->moeworksheets);
        if ($this->is_downloading()) {
            return $grade;
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid])) {
            ...
            $newsumgrade = moeworksheets_rescale_grade($newsumgrade, $this->moeworksheets);
            $oldsumgrade = moeworksheets_rescale_grade($oldsumgrade, $this->moeworksheets);
            $grade = html_writer::tag('del', $oldsumgrade) . '/' .
                    html_writer::empty_tag('br') . $newsumgrade;
        }
        return html_writer::link(new moodle_url('/mod/moeworksheets/review.php',
                array('attempt' => $attempt->attempt)), $grade,
                array('title' => get_string('reviewattempt', 'moeworksheets')));
    }

    /**
     * @param string $colname the name of the column.
     * @param object $attempt the row of data - see the SQL in display() in
     * mod/moeworksheets/report/overview/report.php to see what fields are present,
    ...
    public function other_cols($colname, $attempt) {
        ...
        if ($question->maxmark == 0) {
            ...
            $grade = moeworksheets_rescale_grade(
                    $stepdata->fraction * $question->maxmark, $this->moeworksheets, 'question');
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid][$slot])) {
            $gradefromdb = $grade;
            $newgrade = moeworksheets_rescale_grade(
                    $this->regradedqs[$attempt->usageid][$slot]->newfraction * $question->maxmark,
                    $this->moeworksheets, 'question');
            $oldgrade = moeworksheets_rescale_grade(
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'moeworksheets_overview');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'moeworksheets_overview');
        }
    }
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if ($this->options->slotmarks && has_capability('mod/moeworksheets:regrade', $this->context)) {
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('moeworksheets_overviewregrad',
                'questionusageid ' . $qubaids->usage_id_in(), $qubaids->usage_id_in_params());
        return moeworksheets_report_index_by_keys($regradedqs, array('questionusageid', 'slot'));
    }
}
