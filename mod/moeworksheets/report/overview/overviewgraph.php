...
/**
 * This file renders the moeworksheets overview graph.
 *
...
$moeworksheetsid = required_param('id', PARAM_INT);
$groupid = optional_param('groupid', 0, PARAM_INT);

$moeworksheets = $DB->get_record('moeworksheets', array('id' => $moeworksheetsid));
$course = $DB->get_record('course', array('id' => $moeworksheets->course));
$cm = get_coursemodule_from_instance('moeworksheets', $moeworksheetsid);

require_login($course, false, $cm);
$modcontext = context_module::instance($cm->id);
require_capability('mod/moeworksheets:viewreports', $modcontext);
...
// Pick a sensible number of bands depending on moeworksheets maximum grade.
$bands = $moeworksheets->grade;
...
$bandwidth = $moeworksheets->grade / $bands;
$bandlabels = array();
for ($i = 1; $i <= $bands; $i++) {
    $bandlabels[] = moeworksheets_format_grade($moeworksheets, ($i - 1) * $bandwidth) . ' - ' .
            moeworksheets_format_grade($moeworksheets, $i * $bandwidth);
}
...
$line->y_data['allusers'] = moeworksheets_report_grade_bands($bandwidth, $bands, $moeworksheetsid, $groupusers);
