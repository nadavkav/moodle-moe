 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class moeworksheets_report_responses_from_steps_testcase extends mod_moeworksheets_attempt_walkthrough_from_csv_testcase {
     ...
     *
     * @param array $moeworksheetssettings settings to override default settings for moeworksheets created by generator. Taken from moeworksheetszes.csv.
    ...
    public function test_walkthrough_from_csv($moeworksheetssettings, $csvdata) {
        ...
        $moeworksheetsattemptids = $this->walkthrough_attempts($csvdata['steps']);
        ...
        for ($rowno = 0; $rowno < $csvdata['responses']->getRowCount(); $rowno++) {
            ...
            if (!isset($moeworksheetsattemptids[$responses['moeworksheetsattempt']])) {
                throw new coding_exception("There is no moeworksheetsattempt {$responses['moeworksheetsattempt']}!");
            }
            $this->assert_response_test($moeworksheetsattemptids[$responses['moeworksheetsattempt']], $responses);
        }
    }

    protected function assert_response_test($moeworksheetsattemptid, $responses) {
        $moeworksheetsattempt = moeworksheets_attempt::create($moeworksheetsattemptid);
        ...
        foreach ($responses['slot'] as $slot => $tests) {
            ...
            $qa = $moeworksheetsattempt->get_question_attempt($slot);
            ...
            if (null === $step) {
                throw new coding_exception("There is no step no {$responses['submittedstepno']} ".
                                           "for slot $slot in moeworksheetsattempt {$responses['moeworksheetsattempt']}!");
            }
            foreach (array('responsesummary', 'fraction', 'state') as $column) {
                if (isset($tests[$column]) && $tests[$column] != '') {
                    ...
                    $failuremessage = "Error in  moeworksheetsattempt {$responses['moeworksheetsattempt']} in $column, slot $slot, ".
