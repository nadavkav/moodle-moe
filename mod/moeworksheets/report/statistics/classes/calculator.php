...
/**
 * Class to calculate and also manage caching of moeworksheets statistics.
 *
...
class calculator {
    ...
    /**
     * Compute the moeworksheets statistics.
     *
     ...
     * @return calculated $moeworksheetsstats The statistics for overall attempt scores.
     */
    public function calculate($moeworksheetsid, $whichattempts, $groupstudents, $p, $sumofmarkvariance) {

        $this->progress->start_progress('', 3);

        $moeworksheetsstats = new calculated($whichattempts);

        $countsandaverages = $this->attempt_counts_and_averages($moeworksheetsid, $groupstudents);
        $this->progress->progress(1);

        foreach ($countsandaverages as $propertyname => $value) {
            $moeworksheetsstats->{$propertyname} = $value;
        }

        $s = $moeworksheetsstats->s();
        if ($s != 0) {

            // Recalculate sql again this time possibly including test for first attempt.
            list($fromqa, $whereqa, $qaparams) =
                moeworksheets_statistics_attempts_sql($moeworksheetsid, $groupstudents, $whichattempts);

            $moeworksheetsstats->median = $this->median($s, $fromqa, $whereqa, $qaparams);
            $this->progress->progress(2);

            if ($s > 1) {

                $powers = $this->sum_of_powers_of_difference_to_mean($moeworksheetsstats->avg(), $fromqa, $whereqa, $qaparams);
                $this->progress->progress(3);

                $moeworksheetsstats->standarddeviation = sqrt($powers->power2 / ($s - 1));

                // Skewness.
                if ($s > 2) {
                    // See http://docs.moodle.org/dev/moeworksheets_item_analysis_calculations_in_practise#Skewness_and_Kurtosis.
                    $m2 = $powers->power2 / $s;
                    $m3 = $powers->power3 / $s;
                    $m4 = $powers->power4 / $s;
                    ...
                    $k3 = $s * $s * $m3 / (($s - 1) * ($s - 2));
                    if ($k2 != 0) {
                        $moeworksheetsstats->skewness = $k3 / (pow($k2, 3 / 2));

                        // Kurtosis.
                        if ($s > 3) {
                            $k4 = $s * $s * ((($s + 1) * $m4) - (3 * ($s - 1) * $m2 * $m2)) / (($s - 1) * ($s - 2) * ($s - 3));
                            $moeworksheetsstats->kurtosis = $k4 / ($k2 * $k2);
                        }

                        if ($p > 1) {
                            $moeworksheetsstats->cic = (100 * $p / ($p - 1)) * (1 - ($sumofmarkvariance / $k2));
                            $moeworksheetsstats->errorratio = 100 * sqrt(1 - ($moeworksheetsstats->cic / 100));
                            $moeworksheetsstats->standarderror = $moeworksheetsstats->errorratio *
                                $moeworksheetsstats->standarddeviation / 100;
                        }
                    }
            ...
            $moeworksheetsstats->cache(moeworksheets_statistics_qubaids_condition($moeworksheetsid, $groupstudents, $whichattempts));
        }
        $this->progress->end_progress();
        return $moeworksheetsstats;
    }
    ...
    public function get_cached($qubaids) {
        ...
        $fromdb = $DB->get_record_select('moeworksheets_statistics', 'hashcode = ? AND timemodified > ?',
    ...
    public function get_last_calculated_time($qubaids) {
        ...
        $timemodified = time() - self::TIME_TO_CACHE;
        return $DB->get_field_select('moeworksheets_statistics', 'timemodified', 'hashcode = ? AND timemodified > ?',
                                         array($qubaids->get_hash_code(), $timemodified));
    }

    /**
     * Given a particular moeworksheets grading method return a lang string describing which attempts contribute to grade.
     *
    ...
    public static function using_attempts_lang_string($whichattempts) {
         return get_string(static::using_attempts_string_id($whichattempts), 'moeworksheets_statistics');
    }

    /**
     * Given a particular moeworksheets grading method return a string id for use as a field name prefix in mdl_moeworksheets_statistics or to
     ...
     *
     * See : http://docs.moodle.org/dev/moeworksheets_item_analysis_calculations_in_practise
     ...
     */
    protected function attempt_counts_and_averages($moeworksheetsid, $groupstudents) {
        global $DB;

        $attempttotals = new \stdClass();
        foreach (array_keys(moeworksheets_get_grading_options()) as $which) {

            list($fromqa, $whereqa, $qaparams) = moeworksheets_statistics_attempts_sql($moeworksheetsid, $groupstudents, $which);
     ...
     * Median mark.
     *
     * http://docs.moodle.org/dev/moeworksheets_statistics_calculations#Median_Score
     *
     ...
     *
     * Explanation here : http://docs.moodle.org/dev/moeworksheets_item_analysis_calculations_in_practise
    ...
    protected function sum_of_powers_of_difference_to_mean($mean, $fromqa, $whereqa, $qaparams) {
        ...
        $sql = "SELECT
                    SUM(POWER((moeworksheetsa.sumgrades - $mean), 2)) AS power2,
                    SUM(POWER((moeworksheetsa.sumgrades - $mean), 3)) AS power3,
                    SUM(POWER((moeworksheetsa.sumgrades - $mean), 4)) AS power4
                    FROM $fromqa
                    WHERE $whereqa";
