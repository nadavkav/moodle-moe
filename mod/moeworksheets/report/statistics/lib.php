...
/**
 * Standard plugin entry points of the moeworksheets statistics report.
 *
 ...
 * @package  moeworksheets_statistics
 * @category files
 * @param context $previewcontext the moeworksheets context
 ...
 */
function moeworksheets_statistics_question_preview_pluginfile($previewcontext, $questionid,
    ...
    require_capability('moeworksheets/statistics:view', $context);
...
/**
 * moeworksheets statistics report cron code. Deletes cached data more than a certain age.
 */
function moeworksheets_statistics_cron() {
    global $DB;

    mtrace("\n  Cleaning up old moeworksheets statistics cache records...", '');

    $expiretime = time() - 5*HOURSECS;
    $DB->delete_records_select('moeworksheets_statistics', 'timemodified < ?', array($expiretime));
