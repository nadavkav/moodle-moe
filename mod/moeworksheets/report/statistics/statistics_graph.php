...
/**
 * This script renders the moeworksheets statistics graph.
 *
 * It takes one parameter, the moeworksheets_statistics.id. This is enough to identify the
 * moeworksheets etc.
 *
...
// Check access.
require_login($moeworksheets->course, false, $cm);
$modcontext = context_module::instance($cm->id);
require_capability('moeworksheets/statistics:view', $modcontext);
...
if (empty($currentgroup)) {
    ...
    $groupstudents = get_users_by_capability($modcontext, array('mod/moeworksheets:reviewmyattempts', 'mod/moeworksheets:attempt'),
                                             '', '', '', '', $currentgroup, '', false);
}
$qubaids = moeworksheets_statistics_qubaids_condition($moeworksheetsid, $groupstudents, $whichattempts);
...
$graph->parameter['x_label'] = get_string('position', 'moeworksheets_statistics');
...
$fieldstoplot = array(
    ...
    'discriminativeefficiency' => get_string('discriminative_efficiency', 'moeworksheets_statistics')
);
...
foreach (array_keys($fieldstoplot) as $fieldtoplot) {
    ...
    $graph->y_format[$fieldtoplot] = array(
        'colour' => moeworksheets_statistics_graph_get_new_colour(),
        'bar' => 'fill',
        'shadow_offset' => 1,
