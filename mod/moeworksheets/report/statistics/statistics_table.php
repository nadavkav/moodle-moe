...
/**
 * moeworksheets statistics report, table for showing statistics of each question in the moeworksheets.
 *
...
/**
 * This table has one row for each question in the moeworksheets, with sub-rows when
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class moeworksheets_statistics_table extends flexible_table {
    /** @var object the moeworksheets settings. */
    protected $moeworksheets;

    /** @var integer the moeworksheets course_module id. */
    protected $cmid;
    ...
    public function __construct() {
        parent::__construct('mod-moeworksheets-report-statistics-report');
    }
     ...
     * @param int $cmid the moeworksheets course_module id
     * @param moodle_url $reporturl the URL to redisplay this report.
     * @param int $s number of attempts included in the statistics.
     */
    public function statistics_setup($moeworksheets, $cmid, $reporturl, $s) {
        ...
        $columns[] = 'number';
        $headers[] = get_string('questionnumber', 'moeworksheets_statistics');
        ...
        if (!$this->is_downloading()) {
            ...
            $columns[] = 'qtype';
            $headers[] = get_string('questiontype', 'moeworksheets_statistics');
        }

        $columns[] = 'name';
        $headers[] = get_string('questionname', 'moeworksheets');

        $columns[] = 's';
        $headers[] = get_string('attempts', 'moeworksheets_statistics');

        if ($s > 1) {
            $columns[] = 'facility';
            $headers[] = get_string('facility', 'moeworksheets_statistics');

            $columns[] = 'sd';
            $headers[] = get_string('standarddeviationq', 'moeworksheets_statistics');
        }

        $columns[] = 'random_guess_score';
        $headers[] = get_string('random_guess_score', 'moeworksheets_statistics');

        $columns[] = 'intended_weight';
        $headers[] = get_string('intended_weight', 'moeworksheets_statistics');

        $columns[] = 'effective_weight';
        $headers[] = get_string('effective_weight', 'moeworksheets_statistics');

        $columns[] = 'discrimination_index';
        $headers[] = get_string('discrimination_index', 'moeworksheets_statistics');

        $columns[] = 'discriminative_efficiency';
        $headers[] = get_string('discriminative_efficiency', 'moeworksheets_statistics');
    ...
    protected function col_actions($questionstat) {
        return moeworksheets_question_action_icons($this->moeworksheets, $this->cmid,
    ...
    protected function col_name($questionstat) {
        ...
        if (!is_null($questionstat->variant)) {
            ...
            $name = get_string('nameforvariant', 'moeworksheets_statistics', $a);
        }
        ...
        if (!is_null($questionstat->variant)) {
            ...
            if ($questionstat->subquestion && !$questionstat->get_variants()) {
                ...
                $name = html_writer::link($url, $name, array('title' => get_string('detailedanalysis', 'moeworksheets_statistics')));
                ...
                if ($questionstat->get_variants() || $questionstat->get_sub_question_ids()) {
                    ...
                    $name = html_writer::link($url,
                                              $name,
                                              array('title' => get_string('slotstructureanalysis', 'moeworksheets_statistics', $number)));
                } else {
                    // Question cannot be broken down into sub-questions or variants. Link will show response analysis page.
                    $name = html_writer::link($url,
                                              $name,
                                              array('title' => get_string('detailedanalysis', 'moeworksheets_statistics')));
                }
            }
        }
        ...
        if (!empty($questionstat->minmedianmaxnotice)) {
            $name = get_string($questionstat->minmedianmaxnotice, 'moeworksheets_statistics') . '<br />' . $name;
        }
     ...
     * of maximum mark for the moeworksheets. That is, the indended influence this question
     * on the student's overall mark.
     * @param \core_question\statistics\questions\calculated $questionstat stats for the question.
     * @return string contents of this table cell.
     */
    protected function col_intended_weight($questionstat) {
        return moeworksheets_report_scale_summarks_as_percentage($questionstat->maxmark, $this->moeworksheets);
    }
    ...
    protected function col_effective_weight($questionstat) {
        ...
        if ($questionstat->negcovar) {
            $negcovar = get_string('negcovar', 'moeworksheets_statistics');

            if (!$this->is_downloading()) {
                $negcovar = html_writer::tag('div',
                        $negcovar . $OUTPUT->help_icon('negcovar', 'moeworksheets_statistics'),
     ...
     * other questions in this moeworksheets.
     * @param \core_question\statistics\questions\calculated $questionstat stats for the question.
     * @return string contents of this table cell.
     */
