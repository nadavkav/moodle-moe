 ...
 *
 * The moeworksheets stats below and the question stats found in qstats00.csv were calculated independently in a spreadsheet which is
 * available in open document or excel format here :
 * https://github.com/jamiepratt/moodle-moeworksheets-tools/tree/master/statsspreadsheet
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/moeworksheets/tests/attempt_walkthrough_from_csv_test.php');
require_once($CFG->dirroot . '/mod/moeworksheets/report/default.php');
require_once($CFG->dirroot . '/mod/moeworksheets/report/statistics/report.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class moeworksheets_report_statistics_from_steps_testcase extends mod_moeworksheets_attempt_walkthrough_from_csv_testcase {

    /**
     * @var moeworksheets_statistics_report object to do stats calculations.
     */
    protected $report;
    ...
    /**
     * Create a moeworksheets add questions to it, walk through moeworksheets attempts and then check results.
     *
    ...
    public function test_walkthrough_from_csv($moeworksheetssettings, $csvdata) {
        ...
        $whichattempts = moeworksheets_GRADEAVERAGE; // All attempts.
        $whichtries = question_attempt::ALL_TRIES;
        $groupstudents = array();
        list($questions, $moeworksheetsstats, $questionstats, $qubaids) =
                    $this->check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries, $groupstudents);
        if ($moeworksheetssettings['testnumber'] === '00') {
            $this->check_variants_count_for_moeworksheets_00($questions, $questionstats, $whichtries, $qubaids);
            $this->check_moeworksheets_stats_for_moeworksheets_00($moeworksheetsstats);
        }
    }
     ...
     * @param $qubaids
     */
    protected function check_variants_count_for_moeworksheets_00($questions, $questionstats, $whichtries, $qubaids) {
    ...
    /**
     * @param $moeworksheetsstats
     */
    protected function check_moeworksheets_stats_for_moeworksheets_00($moeworksheetsstats) {
        $moeworksheetsstatsexpected = array(
            'median'             => 4.5,
        ...
        foreach ($moeworksheetsstatsexpected as $statname => $statvalue) {
            $this->assertEquals($statvalue, $moeworksheetsstats->$statname, $moeworksheetsstats->$statname, abs($statvalue) * 1.5e-5);
        }
    }
     ...
     * @return array with contents 0 => $questions, 1 => $moeworksheetsstats, 2=> $questionstats, 3=> $qubaids Might be needed for further
     *               testing.
     */
    protected function check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries, $groupstudents) {
        $this->report = new moeworksheets_statistics_report();
        $questions = $this->report->load_and_initialise_questions_for_calculations($this->moeworksheets);
        list($moeworksheetsstats, $questionstats) = $this->report->get_all_stats_and_analysis($this->moeworksheets,
                                                                                     $whichattempts,
                                                                                     $whichtries,
                                                                                     $groupstudents,
                                                                                     $questions);

        $qubaids = moeworksheets_statistics_qubaids_condition($this->moeworksheets->id, $groupstudents, $whichattempts);

        // We will create some moeworksheets and question stat calculator instances and some response analyser instances, just in order
        // to check the last analysed time then returned.
        $moeworksheetscalc = new \moeworksheets_statistics\calculator();
        // Should not be a delay of more than one second between the calculation of stats above and here.
        $this->assertTimeCurrent($moeworksheetscalc->get_last_calculated_time($qubaids));
        ...
        if (isset($csvdata['qstats'])) {
            ...
            return array($questions, $moeworksheetsstats, $questionstats, $qubaids);
        }
        return array($questions, $moeworksheetsstats, $questionstats, $qubaids);
    }
