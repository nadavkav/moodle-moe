...
/**
 * Tests for the moeworksheets_attempt class.
 *
...
/**
 * Subclass of moeworksheets_attempt to allow faking of the page layout.
 *
 * @copyright 2014 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_moeworksheets_attempt_testable extends moeworksheets_attempt {
     ...
     * @param string $layout layout to set. Like moeworksheets attempt.layout. E.g. '1,2,0,3,4,0,'.
     * @param array $infos slot numbers which contain 'descriptions', or other non-questions.
     * @return moeworksheets_attempt attempt object for use in unit tests.
     */
    public static function setup_fake_attempt_layout($id, $layout, $infos = array()) {
        ...
        $attemptobj = new self($attempt, $moeworksheets, $cm, $course, false);
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_moeworksheets_attempt_testcase extends basic_testcase {
    /**
     * Test the functions moeworksheets_update_open_attempts() and get_list_of_overdue_attempts()
     */
    public function test_attempt_url() {
        $attempt = mod_moeworksheets_attempt_testable::setup_fake_attempt_layout(
                123, '1,2,0,3,4,0,5,6,0');

        // Attempt pages.
        $this->assertEquals(new moodle_url(
                '/mod/moeworksheets/attempt.php?attempt=123'),
                $attempt->attempt_url());

        $this->assertEquals(new moodle_url(
                '/mod/moeworksheets/attempt.php?attempt=123&page=2'),
                $attempt->attempt_url(null, 2));

        $this->assertEquals(new moodle_url(
                '/mod/moeworksheets/attempt.php?attempt=123&page=1#'),
                $attempt->attempt_url(3));

        $this->assertEquals(new moodle_url(
                '/mod/moeworksheets/attempt.php?attempt=123&page=1#q4'),
        ...
        // Summary page.
        $this->assertEquals(new moodle_url(
                '/mod/moeworksheets/summary.php?attempt=123'),
                $attempt->summary_url());

        // Review page.
        $this->assertEquals(new moodle_url(
                '/mod/moeworksheets/review.php?attempt=123'),
                $attempt->review_url());

        $this->assertEquals(new moodle_url(
                '/mod/moeworksheets/review.php?attempt=123&page=2'),
                $attempt->review_url(null, 2));

        $this->assertEquals(new moodle_url(
                '/mod/moeworksheets/review.php?attempt=123&page=1'),
                $attempt->review_url(3, -1, false));

        $this->assertEquals(new moodle_url(
                '/mod/moeworksheets/review.php?attempt=123&page=1#q4'),
        ...
        $this->assertEquals(new moodle_url(
                '/mod/moeworksheets/review.php?attempt=123&showall=0'),
        ...
        // Review with more than 50 questions in the moeworksheets.
        $attempt = mod_moeworksheets_attempt_testable::setup_fake_attempt_layout(
        ...
        $this->assertEquals(new moodle_url(
                '/mod/moeworksheets/review.php?attempt=124'),
                $attempt->review_url());

        $this->assertEquals(new moodle_url(
                '/mod/moeworksheets/review.php?attempt=124&page=2'),
                $attempt->review_url(null, 2));

        $this->assertEquals(new moodle_url(
                '/mod/moeworksheets/review.php?attempt=124&page=1'),
                $attempt->review_url(11, -1, false));

        $this->assertEquals(new moodle_url(
                '/mod/moeworksheets/review.php?attempt=124&page=1#q12'),
                $attempt->review_url(12, -1, false));

        $this->assertEquals(new moodle_url(
                '/mod/moeworksheets/review.php?attempt=124&showall=1'),
                $attempt->review_url(null, 2, true));
