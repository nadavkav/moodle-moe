...
/**
 * moeworksheets attempt walk through using data from csv file.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_moeworksheets_attempt_walkthrough_from_csv_testcase extends advanced_testcase {

    protected $files = array('questions', 'steps', 'results');

    /**
     * @var stdClass the moeworksheets record we create.
     */
     ...
     * directory.
     *
     * @param array $moeworksheetssettings of settings read from csv file moeworksheetszes.csv
     ...
     */
    public function test_walkthrough_from_csv($moeworksheetssettings, $csvdata) {

        // CSV data files for these tests were generated using :
        // https://github.com/jamiepratt/moodle-moeworksheets-tools/tree/master/responsegenerator

        $this->create_moeworksheets_simulate_attempts_and_check_results($moeworksheetssettings, $csvdata);
    }

    public function create_moeworksheets($moeworksheetssettings, $qs) {
        global $SITE, $DB;
        $this->setAdminUser();
        ...
        $aggregratedsettings = $moeworksheetssettings + array('course' => $SITE->id,
                                                     'questionsperpage' => 0,
                                                     'grade' => 100.0,
                                                     'sumgrades' => $sumofgrades);

        $this->moeworksheets = $moeworksheetsgenerator->create_instance($aggregratedsettings);

        $this->randqids = array();
        foreach ($slots as $slotno => $slotquestion) {
            if ($slotquestion['type'] !== 'random') {
                moeworksheets_add_moeworksheets_question($slotquestion['id'], $this->moeworksheets, 0, $slotquestion['mark']);
            } else {
                moeworksheets_add_random_questions($this->moeworksheets, 0, $slotquestion['catid'], 1, 0);
    ...
    /**
     * Create moeworksheets, simulate attempts and check results (if resultsXX.csv exists).
     *
     * @param array $moeworksheetssettings moeworksheets overrides for this moeworksheets.
     * @param PHPUnit_Extensions_Database_DataSet_ITable[] $csvdata Data loaded from csv files for this test.
     */
    protected function create_moeworksheets_simulate_attempts_and_check_results($moeworksheetssettings, $csvdata) {
        $this->resetAfterTest(true);
        question_bank::get_qtype('random')->clear_caches_before_testing();

        $this->create_moeworksheets($moeworksheetssettings, $csvdata['questions']);
    ...
    public function get_data_for_walkthrough() {
        $moeworksheetszes = $this->load_csv_data_file('moeworksheetszes');
        $datasets = array();
        for ($rowno = 0; $rowno < $moeworksheetszes->getRowCount(); $rowno++) {
            $moeworksheetssettings = $moeworksheetszes->getRow($rowno);
            $dataset = array();
            foreach ($this->files as $file) {
                if (file_exists($this->get_full_path_of_csv_file($file, $moeworksheetssettings['testnumber']))) {
                    $dataset[$file] = $this->load_csv_data_file($file, $moeworksheetssettings['testnumber']);
                }
            }
            $datasets[] = array($moeworksheetssettings, $dataset);
        }
        return $datasets;
    }
     ...
     * @return array attempt no as in csv file => the id of the moeworksheets_attempt as stored in the db.
     */
    protected function walkthrough_attempts($steps) {
        ...
        for ($rowno = 0; $rowno < $steps->getRowCount(); $rowno++) {
            ...
            // Find existing user or make a new user to do the moeworksheets.
            ...
            if (!isset($attemptids[$step['moeworksheetsattempt']])) {
                // Start the attempt.
                $moeworksheetsobj = moeworksheets::create($this->moeworksheets->id, $user->id);
                ...
                $prevattempts = moeworksheets_get_user_attempts($this->moeworksheets->id, $user->id, 'all', true);
                $attemptnumber = count($prevattempts) + 1;
                $timenow = time();
                $attempt = moeworksheets_create_attempt($moeworksheetsobj, $attemptnumber, false, $timenow, false, $user->id);
                ...
                moeworksheets_start_new_attempt($moeworksheetsobj, $quba, $attempt, $attemptnumber, $timenow, $step['randqs'], $step['variants']);
                moeworksheets_attempt_save_started($moeworksheetsobj, $quba, $attempt);
                $attemptid = $attemptids[$step['moeworksheetsattempt']] = $attempt->id;
            } else {
                $attemptid = $attemptids[$step['moeworksheetsattempt']];
            }
     ...
     * @param $attemptids array attempt no as in csv file => the id of the moeworksheets_attempt as stored in the db.
     */
    protected function check_attempts_results($results, $attemptids) {
        for ($rowno = 0; $rowno < $results->getRowCount(); $rowno++) {
            ...
            // Re-load moeworksheets attempt data.
            $attemptobj = moeworksheets_attempt::create($attemptids[$result['moeworksheetsattempt']]);
     ...
     * @param moeworksheets_attempt $attemptobj         the attempt object loaded from db.
        ...
        foreach ($result as $fieldname => $value) {
            ...
            switch ($fieldname) {
                            ...
                            switch ($slotfieldname) {
                                case 'mark' :
                                    $this->assertEquals(round($slotvalue, 2), $attemptobj->get_question_mark($slotno),
                                                        "Mark for slot $slotno of attempt {$result['moeworksheetsattempt']}.");
                                    break;
                                default :
                ...
                case 'finished' :
                    ...
                    break;
                case 'summarks' :
                    $this->assertEquals($value, $attemptobj->get_sum_marks(), "Sum of marks of attempt {$result['moeworksheetsattempt']}.");
                    break;
                case 'moeworksheetsgrade' :
                    // Check moeworksheets grades.
                    $grades = moeworksheets_get_user_grades($attemptobj->get_moeworksheets(), $attemptobj->get_userid());
                    $grade = array_shift($grades);
                    $this->assertEquals($value, $grade->rawgrade, "moeworksheets grade for attempt {$result['moeworksheetsattempt']}.");
                    break;
                case 'gradebookgrade' :
                    // Check grade book.
                    ...
                    $this->assertEquals($value, $gradebookgrade->grade, "Gradebook grade for attempt {$result['moeworksheetsattempt']}.");
                    break;
                default :
