...
/**
 * moeworksheets attempt walk through tests.
 *
...
/**
 * moeworksheets attempt walk through.
 *
 * @package    mod_moeworksheets
 * @category   phpunit
 * @copyright  2013 The Open University
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_moeworksheets_attempt_walkthrough_testcase extends advanced_testcase {

    /**
     * Create a moeworksheets with questions and walk through a moeworksheets attempt.
     */
    public function test_moeworksheets_attempt_walkthrough() {
        global $SITE;
        ...
        $moeworksheets = $moeworksheetsgenerator->create_instance(array('course'=>$SITE->id, 'questionsperpage' => 0, 'grade' => 100.0,
                                                      'sumgrades' => 2));
        ...
        $timenow = time();
        $attempt = moeworksheets_create_attempt($moeworksheetsobj, 1, false, $timenow, false, $user1->id);
        ...
        $grades = moeworksheets_get_user_grades($moeworksheets, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'moeworksheets', $moeworksheets->id, $user1->id);
    ...
    /**
     * Create a moeworksheets with a random as well as other questions and walk through moeworksheets attempts.
     */
    public function test_moeworksheets_with_random_question_attempt_walkthrough() {
        global $SITE;
        ...
        $moeworksheets = $moeworksheetsgenerator->create_instance(array('course' => $SITE->id, 'questionsperpage' => 2, 'grade' => 100.0,
                                                      'sumgrades' => 4));
        ...
        // Add random question to the moeworksheets.
        ...
        moeworksheets_add_moeworksheets_question($match->id, $moeworksheets, 0);

        $multichoicemulti = $questiongenerator->create_question('multichoice', 'two_of_four', array('category' => $cat->id));

        moeworksheets_add_moeworksheets_question($multichoicemulti->id, $moeworksheets, 0);

        $multichoicesingle = $questiongenerator->create_question('multichoice', 'one_of_four', array('category' => $cat->id));

        moeworksheets_add_moeworksheets_question($multichoicesingle->id, $moeworksheets, 0);

        foreach (array($saq->id => 'frog', $numq->id => '3.14') as $randomqidtoselect => $randqanswer) {
            // Make a new user to do the moeworksheets each loop.
            $user1 = $this->getDataGenerator()->create_user();
            $this->setUser($user1);
            ...
            $timenow = time();
            $attempt = moeworksheets_create_attempt($moeworksheetsobj, 1, false, $timenow);

            moeworksheets_start_new_attempt($moeworksheetsobj, $quba, $attempt, 1, $timenow, array(1 => $randomqidtoselect));
    ...
    protected $moeworksheetswithvariants = null;

    /**
     * Create a moeworksheets with a single question with variants and walk through moeworksheets attempts.
     *
     * @dataProvider get_correct_response_for_variants
     */
    public function test_moeworksheets_with_question_with_variants_attempt_walkthrough($variantno, $correctresponse, $done = false) {
        global $SITE;
        ...
        if ($this->moeworksheetswithvariants === null) {
            // Make a moeworksheets.
            $moeworksheetsgenerator = $this->getDataGenerator()->get_plugin_generator('mod_moeworksheets');

            $this->moeworksheetswithvariants = $moeworksheetsgenerator->create_instance(array('course'=>$SITE->id,
                                                                            'questionsperpage' => 0,
                                                                            'grade' => 100.0,
                                                                            'sumgrades' => 1));
            ...
            moeworksheets_add_moeworksheets_question($calc->id, $this->moeworksheetswithvariants, 0);
        }


        // Make a new user to do the moeworksheets.
        $user1 = $this->getDataGenerator()->create_user();
        $this->setUser($user1);
        $moeworksheetsobj = moeworksheets::create($this->moeworksheetswithvariants->id, $user1->id);
        ...
        // Select variant.
        moeworksheets_start_new_attempt($moeworksheetsobj, $quba, $attempt, 1, $timenow, array(), array(1 => $variantno));
        ...
        // Check moeworksheets grades.
        $grades = moeworksheets_get_user_grades($this->moeworksheetswithvariants, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'moeworksheets', $this->moeworksheetswithvariants->id, $user1->id);
        $gradebookitem = array_shift($gradebookgrades->items);
        $gradebookgrade = array_shift($gradebookitem->grades);
        $this->assertEquals(100, $gradebookgrade->grade);
    }
