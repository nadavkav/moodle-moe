...
/**
 * Steps definitions related to mod_moeworksheets.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_mod_moeworksheets extends behat_question_base {

    /**
     * Put the specified questions on the specified pages of a given moeworksheets.
     *
     ...
     *
     * @param string $moeworksheetsname the name of the moeworksheets to add questions to.
     * @param TableNode $data information about the questions to add.
     *
     * @Given /^moeworksheets "([^"]*)" contains the following questions:$/
     */
    public function moeworksheets_contains_the_following_questions($moeworksheetsname, TableNode $data) {
        global $DB;

        $moeworksheets = $DB->get_record('moeworksheets', array('name' => $moeworksheetsname), '*', MUST_EXIST);
        ...
        if (!in_array('question', $firstrow) && !in_array('page', $firstrow)) {
            if (count($firstrow) == 2) {
                ...
                throw new ExpectationException('When adding questions to a moeworksheets, you should give 2 or three 3 things: ' .
        ...
        foreach ($data->getHash() as $questiondata) {
            if (!array_key_exists('question', $questiondata)) {
                throw new ExpectationException('When adding questions to a moeworksheets, ' .
            ...
            // Add the question.
            moeworksheets_add_moeworksheets_question($questionid, $moeworksheets, $page, $maxmark);

            // Require previous.
            if (array_key_exists('requireprevious', $questiondata)) {
                if ($questiondata['requireprevious'] === '1') {
                    $slot = $DB->get_field('moeworksheets_slots', 'MAX(slot)', array('moeworksheetsid' => $moeworksheets->id));
                    $DB->set_field('moeworksheets_slots', 'requireprevious', 1,
                            array('moeworksheetsid' => $moeworksheets->id, 'slot' => $slot));
    ...
    /**
     * Put the specified section headings to start at specified pages of a given moeworksheets.
     *
     * The first row should be column names:
     ...
     *
     * @param string $moeworksheetsname the name of the moeworksheets to add sections to.
     * @param TableNode $data information about the sections to add.
     *
     * @Given /^moeworksheets "([^"]*)" contains the following sections:$/
     */
    public function moeworksheets_contains_the_following_sections($moeworksheetsname, TableNode $data) {
        global $DB;
        ...
        foreach ($data->getHash() as $rownumber => $sectiondata) {
            if (!array_key_exists('heading', $sectiondata)) {
                throw new ExpectationException('When adding sections to a moeworksheets, ' .
            ...
            if ($rownumber == 0) {
                $section = $DB->get_record('moeworksheets_sections', array('moeworksheetsid' => $moeworksheets->id), '*', MUST_EXIST);
            } else {
                $section = new stdClass();
                $section->moeworksheetsid = $moeworksheets->id;
            }
            ...
            if ($rownumber == 0) {
                ...
                $DB->insert_record('moeworksheets_sections', $section);
            }
        }

        if ($section->firstslot > $DB->count_records('moeworksheets_slots', array('moeworksheetsid' => $moeworksheets->id))) {
            throw new ExpectationException('The section firstslot must be less than the total number of slots in the moeworksheets.',
                    $this->getSession());
        }
    }

    /**
     * Adds a question to the existing moeworksheets with filling the form.
     *
     * The form for creating a question should be on one page.
     *
     * @When /^I add a "(?P<question_type_string>(?:[^"]|\\")*)" question to the "(?P<moeworksheets_name_string>(?:[^"]|\\")*)" moeworksheets with:$/
     * @param string $questiontype
     * @param string $moeworksheetsname
     * @param TableNode $questiondata with data for filling the add question form
     */
    public function i_add_question_to_the_moeworksheets_with($questiontype, $moeworksheetsname, TableNode $questiondata) {
        $moeworksheetsname = $this->escape($moeworksheetsname);
        $editmoeworksheets = $this->escape(get_string('editmoeworksheets', 'moeworksheets'));
        $moeworksheetsadmin = $this->escape(get_string('pluginadministration', 'moeworksheets'));
        $addaquestion = $this->escape(get_string('addaquestion', 'moeworksheets'));
        ...
        $this->execute('behat_general::click_link', $moeworksheetsname);

        $this->execute("behat_navigation::i_navigate_to_node_in", array($editmoeworksheets, $moeworksheetsadmin));
    ...
    /**
     * Set the max mark for a question on the Edit moeworksheets page.
     *
     ...
     */
    public function i_set_the_max_mark_for_moeworksheets_question($questionname, $newmark) {
        $this->execute('behat_general::click_link', $this->escape(get_string('editmaxmark', 'moeworksheets')));
    ...
    /**
     * Open the add menu on a given page, or at the end of the Edit moeworksheets page.
     * @Given /^I open the "(?P<page_n_or_last_string>(?:[^"]|\\")*)" add to moeworksheets menu$/
     * @param string $pageorlast either "Page n" or "last".
     */
    public function i_open_the_add_to_moeworksheets_menu_for($pageorlast) {
        ...
        if ($pageorlast == 'last') {
            ...
            throw new ExpectationException("The I open the add to moeworksheets menu step must specify either 'Page N' or 'last'.");
        }
    ...
    /**
     * Check whether a particular question is on a particular page of the moeworksheets on the Edit moeworksheets page.
     * @Given /^I should see "(?P<question_name>(?:[^"]|\\")*)" on moeworksheets page "(?P<page_number>\d+)"$/
     ...
     */
    public function i_should_see_on_moeworksheets_page($questionname, $pagenumber) {
    ...
    /**
     * Check whether a particular question is not on a particular page of the moeworksheets on the Edit moeworksheets page.
     * @Given /^I should not see "(?P<question_name>(?:[^"]|\\")*)" on moeworksheets page "(?P<page_number>\d+)"$/
     * @param string $questionname the name of the question we are looking for.
     * @param number $pagenumber the page it should be found on.
     */
    public function i_should_not_see_on_moeworksheets_page($questionname, $pagenumber) {
        $xpath = "//li[contains(., '" . $this->escape($questionname) .
                "')][./preceding-sibling::li[contains(@class, 'pagenumber')][1][contains(., 'Page " .
                $pagenumber . "')]]";
    ...
    /**
     * Check whether one question comes before another on the Edit moeworksheets page.
     * The two questions must be on the same page.
     * @Given /^I should see "(?P<first_q_name>(?:[^"]|\\")*)" before "(?P<second_q_name>(?:[^"]|\\")*)" on the edit moeworksheets page$/
     ...
     */
    public function i_should_see_before_on_the_edit_moeworksheets_page($firstquestionname, $secondquestionname) {
    ...
    /**
     * Check the number displayed alongside a question on the Edit moeworksheets page.
     * @Given /^"(?P<question_name>(?:[^"]|\\")*)" should have number "(?P<number>(?:[^"]|\\")*)" on the edit moeworksheets page$/
     * @param string $questionname the name of the question we are looking for.
     * @param number $number the number (or 'i') that should be displayed beside that question.
     */
    public function should_have_number_on_the_edit_moeworksheets_page($questionname, $number) {
     ...
     *
     * @Given /^I click on shuffle for section "([^"]*)" on the moeworksheets edit page$/
     */
     ...
     *
     * @Given /^shuffle for section "([^"]*)" should be "(On|Off)" on the moeworksheets edit page$/
     */
    public function shuffle_for_section_should_be($heading, $value) {
        ...
        if ($value == 'On' && !$checkbox->isChecked()) {
            $msg = "Shuffle for section '$heading' is not checked, but you are expecting it to be checked ($value). " .
                    "Check the line with: \nshuffle for section \"$heading\" should be \"$value\" on the moeworksheets edit page" .
    ...
    /**
     * Move a question on the Edit moeworksheets page by first clicking on the Move icon,
     * then clicking one of the "After ..." links.
     * @When /^I move "(?P<question_name>(?:[^"]|\\")*)" to "(?P<target>(?:[^"]|\\")*)" in the moeworksheets by clicking the move icon$/
     * @param string $questionname the name of the question we are looking for.
    ...
    /**
     * Move a question on the Edit moeworksheets page by dragging a given question on top of another item.
     * @When /^I move "(?P<question_name>(?:[^"]|\\")*)" to "(?P<target>(?:[^"]|\\")*)" in the moeworksheets by dragging$/
     * @param string $questionname the name of the question we are looking for.
    ...
    /**
     * Delete a question on the Edit moeworksheets page by first clicking on the Delete icon,
     * then clicking one of the "After ..." links.
     * @When /^I delete "(?P<question_name>(?:[^"]|\\")*)" in the moeworksheets by clicking the delete icon$/
     * @param string $questionname the name of the question we are looking for.
     * @return array of steps.
     */
    ...
    /**
     * Set the section heading for a given section on the Edit moeworksheets page
     *
     * @When /^I change moeworksheets section heading "(?P<section_name_string>(?:[^"]|\\")*)" to "(?P<new_section_heading_string>(?:[^"]|\\")*)"$/
     ...
     * moeworksheets navigation block.
     *
     * @Then /^I should see question "(?P<questionnumber>\d+)" in section "(?P<section_heading_string>(?:[^"]|\\")*)" in the moeworksheets navigation$/
     ...
     */
    public function i_should_see_question_in_section_in_the_moeworksheets_navigation($questionnumber, $sectionheading) {
        ...
        $exception = new ExpectationException('Question "' . $questionnumber . '" is not in section "' .
                $sectionheading . '" in the moeworksheets navigation.', $this->getSession());
        $xpath = "//div[@id = 'mod_moeworksheets_navblock']//*[contains(concat(' ', normalize-space(@class), ' '), ' qnbutton ') and " .
