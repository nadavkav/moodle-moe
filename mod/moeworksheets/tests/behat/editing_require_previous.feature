...
Feature: Edit moeworksheetszes where some questions require the previous one to have been completed
  In order to create moeworksheetszes where later questions can only be seen after earlier ones are answered
  As a teacher
  I need to be able to configure this on the Edit moeworksheets page
  ...
  Scenario: The first question cannot depend on the previous (whatever is in the DB)
    Given the following "activities" exist:
      ...
      | moeworksheets       | moeworksheets 1 | moeworksheets 1 description | C1     | moeworksheets1    | immediatefeedback  |
  ...
  @javascript
  Scenario: Question dependency cannot apply to deferred feedback moeworksheetszes so UI is hidden
    Given the following "activities" exist:
      | activity   | name   | intro              | course | idnumber | preferredbehaviour |
      | moeworksheets       | moeworksheets 1 | moeworksheets 1 description | C1     | moeworksheets1    | deferredfeedback   |
    And the following "questions" exist:
      | questioncategory | qtype       | name                    | questiontext    |
      | Test questions   | truefalse   | TF1                     | First question  |
      | Test questions   | truefalse   | TF2                     | Second question |
  ...
  Scenario: Question dependency cannot apply to questions in a shuffled section so UI is hidden
    Given the following "activities" exist:
      ...
      | moeworksheets       | moeworksheets 1 | moeworksheets 1 description | C1     | moeworksheets1    | immediatefeedback  | 2                |
    And the following "questions" exist:
      | questioncategory | qtype       | name | questiontext    |
      | Test questions   | truefalse   | TF1  | First question  |
      | Test questions   | truefalse   | TF2  | Second question |
  ...
  @javascript
  Scenario: Question dependency cannot apply to moeworksheetszes with sequential navigation so UI is hidden
    Given the following "activities" exist:
      | activity   | name   | intro              | course | idnumber | preferredbehaviour | navmethod  |
      | moeworksheets       | moeworksheets 1 | moeworksheets 1 description | C1     | moeworksheets1    | immediatefeedback  | sequential |
    And the following "questions" exist:
      | questioncategory | qtype       | name | questiontext    |
      | Test questions   | truefalse   | TF1  | First question  |
      | Test questions   | truefalse   | TF2  | Second question |
  ...
  Scenario: When questions are reordered, the dependency icons are updated correctly
    ...
    And I follow "moeworksheets 1"
    And I follow "Edit moeworksheets"
    When I move "Question 1" to "After Question 3" in the moeworksheets by clicking the move icon
    Then "Question 2 cannot be attempted until the previous question 1 has been completed • Click to change" "link" should be visible
    And "No restriction on when question 3 can be attempted • Click to change" "link" should be visible
