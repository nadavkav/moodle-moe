...
/**
 * Unit tests for the {@link \mod_moeworksheets\repaginate} class.
...
require_once($CFG->dirroot . '/mod/moeworksheets/classes/repaginate.php');


/**
 * Testable subclass, giving access to the protected methods of {@link \mod_moeworksheets\repaginate}
 * @copyright 2014 The Open Univsersity
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_moeworksheets_repaginate_testable extends \mod_moeworksheets\repaginate {

    public function __construct($moeworksheetsid = 0, $slots = null) {
        return parent::__construct($moeworksheetsid, $slots);
    }
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_moeworksheets_repaginate_test extends advanced_testcase {

    /** @var array stores the slots. */
    private $moeworksheetsslots;
    /** @var mod_moeworksheets_repaginate_testable the object being tested. */
    private $repaginate = null;

    public function setUp() {
        $this->set_moeworksheets_slots($this->get_moeworksheets_object()->get_slots());
        $this->repaginate = new mod_moeworksheets_repaginate_testable(0, $this->moeworksheetsslots);
    }
    ...
    /**
     * Create a moeworksheets, add five questions to the moeworksheets
     * which are all on one page and return the moeworksheets object.
     */
    private function get_moeworksheets_object() {
        global $SITE;
        ...
        $moeworksheets = $moeworksheetsgenerator->create_instance(array(
                'course' => $SITE->id, 'questionsperpage' => 0, 'grade' => 100.0, 'sumgrades' => 2));
        $cm = get_coursemodule_from_instance('moeworksheets', $moeworksheets->id, $SITE->id);
        ...
        moeworksheets_add_moeworksheets_question($shortanswer->id, $moeworksheets);
        moeworksheets_add_moeworksheets_question($numerical->id, $moeworksheets);
        moeworksheets_add_moeworksheets_question($essay->id, $moeworksheets);
        moeworksheets_add_moeworksheets_question($truefalse->id, $moeworksheets);
        moeworksheets_add_moeworksheets_question($match->id, $moeworksheets);

        // Return the moeworksheets object.
        $moeworksheetsobj = new moeworksheets($moeworksheets, $cm, $SITE);
        return \mod_moeworksheets\structure::create_for_moeworksheets($moeworksheetsobj);
    }

    /**
     * Set the moeworksheets slots
     * @param string $slots
     */
    private function set_moeworksheets_slots($slots = null) {
        if (!$slots) {
            $this->moeworksheetsslots = $this->get_moeworksheets_object()->get_slots();
        } else {
            $this->moeworksheetsslots = $slots;
        }
    }
    ...
    public function test_get_this_slot() {
        $this->set_moeworksheets_slots();
        $actual = array();
        $expected = $this->repaginate->get_slots_by_slot_number();
        $this->assertEquals($expected, $actual);

        $slotsbyno = $this->repaginate->get_slots_by_slot_number($this->moeworksheetsslots);
        $slotnumber = 5;
        $thisslot = $this->repaginate->get_this_slot($this->moeworksheetsslots, $slotnumber);
    ...
    public function test_get_slots_by_slotnumber() {
        ...
        foreach ($this->moeworksheetsslots as $slot) {
            $expected[$slot->slot] = $slot;
        }
        $actual = $this->repaginate->get_slots_by_slot_number($this->moeworksheetsslots);
        $this->assertEquals($expected, $actual);
    }
    ...
    public function test_get_slots_by_slotid() {
        ...
        $this->assertEquals($this->moeworksheetsslots, $actual);
    }
    ...
    public function test_repaginate_n_questions_per_page() {
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->moeworksheetsslots, 2);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->moeworksheetsslots, 3);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->moeworksheetsslots, 5);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->moeworksheetsslots, 10);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->moeworksheetsslots, 1);
        $this->assertEquals($expected, $actual);
    }

    public function test_repaginate_this_slot() {
        $this->set_moeworksheets_slots();
        $slotsbyslotno = $this->repaginate->get_slots_by_slot_number($this->moeworksheetsslots);
        $slotnumber = 3;
        $newpagenumber = 2;
    ...
    public function test_repaginate_the_rest() {
        $this->set_moeworksheets_slots();
        $slotfrom = 1;
        $type = \mod_moeworksheets\repaginate::LINK;
        $expected = array();
        foreach ($this->moeworksheetsslots as $slot) {
        ...
        $actual = $this->repaginate->repaginate_the_rest($this->moeworksheetsslots, $slotfrom, $type, false);
        $this->assertEquals($expected, $actual);

        $slotfrom = 2;
        $newslots = array();
        foreach ($this->moeworksheetsslots as $s) {
            if ($s->slot === $slotfrom) {
                $s->page = $s->page - 1;
            }
            $newslots[$s->id] = $s;
        ...
        $type = \mod_moeworksheets\repaginate::UNLINK;
        $expected = array();
        foreach ($this->moeworksheetsslots as $slot) {
