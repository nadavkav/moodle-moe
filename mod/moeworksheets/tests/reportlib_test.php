...
/**
 * Unit tests for (some of) mod/moeworksheets/report/reportlib.php
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU Public License
 */
class mod_moeworksheets_reportlib_testcase extends advanced_testcase {
    public function test_moeworksheets_report_index_by_keys() {
        $datum = array();
        $object = new stdClass();
        $object->qid = 3;
        $object->aid = 101;
        ...
        $indexed = moeworksheets_report_index_by_keys($datum, array('aid', 'qid'));
        ...
        $indexed = moeworksheets_report_index_by_keys($datum, array('aid', 'qid'), false);
    ...
    public function test_moeworksheets_report_scale_summarks_as_percentage() {
        $moeworksheets = new stdClass();
        $moeworksheets->sumgrades = 10;
        $moeworksheets->decimalpoints = 2;

        $this->assertEquals('12.34567%',
            moeworksheets_report_scale_summarks_as_percentage(1.234567, $moeworksheets, false));
        $this->assertEquals('12.35%',
            moeworksheets_report_scale_summarks_as_percentage(1.234567, $moeworksheets, true));
        $this->assertEquals('-',
            moeworksheets_report_scale_summarks_as_percentage('-', $moeworksheets, true));
    }

    public function test_moeworksheets_report_qm_filter_select_only_one_attempt_allowed() {
        $moeworksheets = new stdClass();
        $moeworksheets->attempts = 1;
        $this->assertSame('', moeworksheets_report_qm_filter_select($moeworksheets));
    }

    public function test_moeworksheets_report_qm_filter_select_average() {
        $moeworksheets = new stdClass();
        $moeworksheets->attempts = 10;
        $moeworksheets->grademethod = moeworksheets_GRADEAVERAGE;
        $this->assertSame('', moeworksheets_report_qm_filter_select($moeworksheets));
    }

    public function test_moeworksheets_report_qm_filter_select_first_last_best() {
        global $DB;
        $this->resetAfterTest();

        $fakeattempt = new stdClass();
        $fakeattempt->userid = 123;
        $fakeattempt->moeworksheets = 456;
        $fakeattempt->layout = '1,2,0,3,4,0,5';
        $fakeattempt->state = moeworksheets_attempt::FINISHED;

        // We intentionally insert these in a funny order, to test the SQL better.
        // The test data is:
        // id | moeworksheetsid | user | attempt | sumgrades | state
        // ---------------------------------------------------
        ...
        $DB->insert_record('moeworksheets_attempts', $fakeattempt);
        ...
        $fakeattempt->state = moeworksheets_attempt::IN_PROGRESS;
        $DB->insert_record('moeworksheets_attempts', $fakeattempt);
        ...
        $moeworksheets->grademethod = moeworksheets_ATTEMPTFIRST;
        $firstattempt = $DB->get_records_sql("
                SELECT * FROM {moeworksheets_attempts} moeworksheetsa WHERE userid = ? AND moeworksheets = ? AND "
                        . moeworksheets_report_qm_filter_select($moeworksheets), array(123, 456));
        $this->assertEquals(1, count($firstattempt));
        $firstattempt = reset($firstattempt);
        $this->assertEquals(1, $firstattempt->attempt);

        $moeworksheets->grademethod = moeworksheets_ATTEMPTLAST;
        ...
        $bestattempt = $DB->get_records_sql("
                SELECT * FROM {moeworksheets_attempts} qa_alias WHERE userid = ? AND moeworksheets = ? AND "
                . moeworksheets_report_qm_filter_select($moeworksheets, 'qa_alias'), array(123, 456));
