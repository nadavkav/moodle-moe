 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_moeworksheets_structure_testcase extends advanced_testcase {

    /**
     * Create a course with an empty moeworksheets.
     * @return array with three elements moeworksheets, cm and course.
     */
    protected function prepare_moeworksheets_data() {
        ...
        return array($moeworksheets, $cm, $course);
    }

    /**
     * Creat a test moeworksheets.
     *
     ...
     * @return moeworksheets the created moeworksheets.
     */
    protected function create_test_moeworksheets($layout) {
        list($moeworksheets, $cm, $course) = $this->prepare_moeworksheets_data();
        ...
        foreach ($layout as $item) {
            if (is_string($item)) {
                ...
                moeworksheets_add_moeworksheets_question($q->id, $moeworksheets, $page);
                $lastpage = $page;
            }
        }

        $moeworksheetsobj = new moeworksheets($moeworksheets, $cm, $course);
        $structure = \mod_moeworksheets\structure::create_for_moeworksheets($moeworksheetsobj);
        if (isset($headings[1])) {
        ...
        return $moeworksheetsobj;
    }

    /**
     * Verify that the given layout matches that expected.
     * @param array $expectedlayout as for $layout in {@link create_test_moeworksheets()}.
     * @param \mod_moeworksheets\structure $structure the structure to test.
     */
    protected function assert_moeworksheets_layout($expectedlayout, \mod_moeworksheets\structure $structure) {
        $sections = $structure->get_sections();
        ...
        foreach ($expectedlayout as $item) {
            if (is_string($item)) {
                ...
                if ($slot > 1 && $section->heading == '' && $section->firstslot == 1) {
                    // The array $expectedlayout did not contain default first moeworksheets section, so skip over it.
                    $section = array_shift($sections);
                }
        ...
        if ($slot - 1 != count($structure->get_slots())) {
            $this->fail('The moeworksheets contains more slots than expected.');
        }

        if (!empty($sections)) {
            $section = array_shift($sections);
            if ($section->heading != '' || $section->firstslot != 1) {
                $this->fail('Unexpected section (' . $section->heading .') found in the moeworksheets.');
            }
        }
    }
     ...
     * used by create_test_moeworksheets as assert_moeworksheets_layout.
    ...
    public function test_get_moeworksheets_slots() {
        $moeworksheetsobj = $this->create_test_moeworksheets(array(
                array('TF1', 1, 'truefalse'),
    ...
    public function test_moeworksheets_has_one_section_by_default() {
        $moeworksheetsobj = $this->create_test_moeworksheets(array(
                array('TF1', 1, 'truefalse'),
            ));
        $structure = \mod_moeworksheets\structure::create_for_moeworksheets($moeworksheetsobj);
    ...
    public function test_remove_section_heading() {
        ...
        $structure = \mod_moeworksheets\structure::create_for_moeworksheets($moeworksheetsobj);
        $this->assert_moeworksheets_layout(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
                array('TF2', 2, 'truefalse'),
            ), $structure);
    ...
    public function test_moeworksheets_remove_slot() {
        $moeworksheetsobj = $this->create_test_moeworksheets(array(
                array('TF1', 1, 'truefalse'),
                array('TF2', 1, 'truefalse'),
                'Heading 2',
    ...
    public function test_moeworksheets_removing_a_random_question_deletes_the_question() {
        global $DB;
        ...
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category();
        moeworksheets_add_random_questions($moeworksheetsobj->get_moeworksheets(), 1, $cat->id, 1, false);
        $structure = \mod_moeworksheets\structure::create_for_moeworksheets($moeworksheetsobj);
    ...
    public function test_can_remove_last_question_in_a_moeworksheets() {
        $moeworksheetsobj = $this->create_test_moeworksheets(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
            ));
        ...
        moeworksheets_add_moeworksheets_question($q->id, $moeworksheetsobj->get_moeworksheets(), 0);
        $structure = \mod_moeworksheets\structure::create_for_moeworksheets($moeworksheetsobj);
    ...
    public function test_add_question_updates_headings() {
        ...
        moeworksheets_add_moeworksheets_question($q->id, $moeworksheetsobj->get_moeworksheets(), 1);
    ...
    public function test_remove_page_break() {
        ...
        $slots = $structure->update_page_break($slotid, \mod_moeworksheets\repaginate::LINK);
    ...
    public function test_add_page_break() {
        ...
        $slotid = $structure->get_question_in_slot(2)->slotid;
        $slots = $structure->update_page_break($slotid, \mod_moeworksheets\repaginate::UNLINK);
