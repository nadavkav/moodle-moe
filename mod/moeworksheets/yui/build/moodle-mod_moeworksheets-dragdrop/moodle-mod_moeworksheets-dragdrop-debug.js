YUI.add('moodle-mod_moeworksheets-dragdrop', function (Y, NAME) {

/**
 * Drag and Drop for moeworksheets sections and slots.
 *
 * @module moodle-mod-moeworksheets-dragdrop
 */
...
var CSS = {
    ...
    CONTENT: 'content',
    COURSECONTENT: 'mod-moeworksheets-edit-content',
 ...
 *
 * @class M.mod_moeworksheets.dragdrop.section
 * @constructor
...
Y.extend(DRAGSECTION, M.core.dragdrop, {
    ...
    initializer: function() {
        ...
        if (this.sectionlistselector) {
            ...
            del.dd.plug(Y.Plugin.DDConstrained, {
                // Keep it inside the .mod-moeworksheets-edit-content
    ...
    drop_hit: function(e) {
        ...
        if (dragnodeid === dropnodeindex) {
            Y.log("Skipping move - same location moving " + dragnodeid + " to " + dropnodeindex,
                  'debug', 'moodle-mod_moeworksheets-dragdrop');
            return;
        }

        Y.log("Moving from position " + dragnodeid + " to position " + dropnodeindex, 'debug', 'moodle-mod_moeworksheets-dragdrop');
        ...
        params.moeworksheetsid = this.get('moeworksheetsid');
            ...
            on: {
                ...
                success: function(tid, response) {
                    ...
                    try {
                        ...
                        M.mod_moeworksheets.edit.process_sections(Y, sectionlist, responsetext, loopstart, loopend);
                    } catch (e) {}
                    ...
                    do {
                        ...
                        for (index = loopstart; index <= loopend; index++) {
                            if (Y.Moodle.core_course.util.section.getId(sectionlist.item(index - 1)) >
                                ...
                                M.mod_moeworksheets.edit.swap_sections(Y, index - 1, index);
    ...
    NAME: 'mod_moeworksheets-dragdrop-section',
    ATTRS: {
        courseid: {
            value: null
        },
...
M.mod_moeworksheets.init_section_dragdrop = function(params) {
...
Y.extend(DRAGRESOURCE, M.core.dragdrop, {
    initializer: function() {
        ...
        M.mod_moeworksheets.dragres = this;
    },
    ...
    setup_for_section: function() {
        Y.Node.all('.mod-moeworksheets-edit-content ul.slots ul.section').each(function(resources) {
    ...
    drop_hit: function(e) {
        ...
        params.field = 'move';
        params.id = Number(Y.Moodle.mod_moeworksheets.util.slot.getId(dragnode));
        ...
        if (previousslot) {
            params.previousid = Number(Y.Moodle.mod_moeworksheets.util.slot.getId(previousslot));
        }

        var previouspage = dragnode.previous(SELECTOR.PAGE);
        if (previouspage) {
            params.page = Number(Y.Moodle.mod_moeworksheets.util.page.getId(previouspage));
        }
        ...
        Y.io(uri, {
            ...
            on: {
                ...
                success: function(tid, response) {
                    ...
                    M.mod_moeworksheets.moeworksheetsbase.invoke_function('set_visibility_resource_ui', params);
                    this.unlock_drag_handle(drag, CSS.EDITINGMOVE);
                    window.setTimeout(function() {
                        spinner.hide();
                    }, 250);
                    M.mod_moeworksheets.resource_toolbox.reorganise_edit_page();
                },
                failure: function(tid, response) {
                    this.ajax_failure(response);
    ...
    NAME: 'mod_moeworksheets-dragdrop-resource',
    ATTRS: {
        courseid: {
            value: null
        },
...
M.mod_moeworksheets = M.mod_moeworksheets || {};
M.mod_moeworksheets.init_resource_dragdrop = function(params) {
    ...
    "requires": [
        ...
        "moodle-mod_moeworksheets-util-base",
        "moodle-mod_moeworksheets-util-page",
