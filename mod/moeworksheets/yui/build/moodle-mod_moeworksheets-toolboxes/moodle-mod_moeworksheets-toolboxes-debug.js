YUI.add('moodle-mod_moeworksheets-toolboxes', function (Y, NAME) {
...
var CSS = {
    ...
    SELECTOR = {
        ...
        SHOW : 'a.' + CSS.SHOW,
        SLOTLI : 'li.slot',
        SUMMARKS : '.mod_moeworksheets_summarks'
    },
...
Y.extend(TOOLBOX, Y.Base, {
    ...
    send_request: function(data, statusspinner, success_callback, optionalconfig) {
        ...
        data.moeworksheetsid = this.get('moeworksheetsid');
                ...
                success: function(tid, response) {
                    ...
                    if (responsetext.hasOwnProperty('newnumquestions')) {
                        Y.one(SELECTOR.NUMQUESTIONS).setHTML(
                                M.util.get_string('numquestionsx', 'moeworksheets', responsetext.newnumquestions)
                            );
                    }
                    if (success_callback) {
...
{
    NAME: 'mod_moeworksheets-toolbox',
    ATTRS: {
        /**
 ...
 *
 * This class is responsible for managing AJAX interactions with activities and resources
 * when viewing a moeworksheets in editing mode.
 *
 * @module mod_moeworksheets-resource-toolbox
 * @namespace M.mod_moeworksheets.resource_toolbox
 */
...
Y.extend(RESOURCETOOLBOX, TOOLBOX, {
    ...
    initializer: function() {
        M.mod_moeworksheets.moeworksheetsbase.register_module(this);
    ...
    delete_with_confirmation: function(ev, button, activity) {
        ...
        confirmstring = M.util.get_string('confirmremovequestion', 'moeworksheets', qtypename);
        ...
        confirm.on('complete-yes', function() {
            ...
            var data = {
                'class': 'resource',
                'action': 'DELETE',
                'id': Y.Moodle.mod_moeworksheets.util.slot.getId(element)
            };
            this.send_request(data, spinner, function(response) {
                if (response.deleted) {
                    // Actually remove the element.
                    Y.Moodle.mod_moeworksheets.util.slot.remove(element);
    ...
    edit_maxmark : function(ev, button, activity) {
        ...
        var instancemaxmark  = activity.one(SELECTOR.INSTANCEMAXMARK),
            ...
            data = {
                'class'   : 'resource',
                'field'   : 'getmaxmark',
                'id'      : Y.Moodle.mod_moeworksheets.util.slot.getId(activity)
            };
    ...
    update_page_break: function(ev, button, activity, action) {
        ...
        var data = {
            ...
            'id':    Y.Moodle.mod_moeworksheets.util.slot.getId(nextactivity),
            'value': value
        };
        ...
        this.send_request(data, spinner, function(response) {
            if (response.slots) {
                if (action === 'addpagebreak') {
                    Y.Moodle.mod_moeworksheets.util.page.add(activity);
                } else {
                    var page = activity.next(Y.Moodle.mod_moeworksheets.util.page.SELECTORS.PAGE);
                    Y.Moodle.mod_moeworksheets.util.page.remove(page, true);
                }
                this.reorganise_edit_page();
            }
        });
    ...
    update_dependency: function(ev, button, activity, action) {
        ...
        this.send_request(data, spinner, function(response) {
            if (response.hasOwnProperty('requireprevious')) {
                Y.Moodle.mod_moeworksheets.util.slot.updateDependencyIcon(activity, response.requireprevious);
            }
        });
    ...
    reorganise_edit_page: function() {
        Y.Moodle.mod_moeworksheets.util.slot.reorderSlots();
        Y.Moodle.mod_moeworksheets.util.slot.reorderPageBreaks();
        Y.Moodle.mod_moeworksheets.util.page.reorderPages();
        Y.Moodle.mod_moeworksheets.util.slot.updateOneSlotSections();
        Y.Moodle.mod_moeworksheets.util.slot.updateAllDependencyIcons();
    },

    NAME : 'mod_moeworksheets-resource-toolbox',
    ATTRS : {
        courseid : {
            'value' : 0
        },
...
M.mod_moeworksheets.resource_toolbox = null;
M.mod_moeworksheets.init_resource_toolbox = function(config) {
    M.mod_moeworksheets.resource_toolbox = new RESOURCETOOLBOX(config);
    return M.mod_moeworksheets.resource_toolbox;
};
/**
 ...
 *
 * @module moodle-mod_moeworksheets-toolboxes
 * @namespace M.mod_moeworksheets.toolboxes
 */
 ...
 *
 * This class is responsible for managing AJAX interactions with sections
 * when adding, editing, removing section headings when editing a moeworksheets.
 *
 * @class section
 * @constructor
 * @extends M.mod_moeworksheets.toolboxes.toolbox
 */
...
Y.extend(SECTIONTOOLBOX, TOOLBOX, {
    ...
    delete_section_with_confirmation: function(ev, button, activity) {
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('confirmremovesectionheading', 'moeworksheets', activity.get('aria-label')),
            modal: true
        });
            ...
            this.send_request(data, spinner, function(response) {
                if (response) {
                    ...
                    activity.one(SELECTOR.EDITSECTIONICON).set('title',
                            M.util.get_string('sectionheadingedit', 'moeworksheets', response.instancesection));
                    activity.one(SELECTOR.EDITSECTIONICON).set('alt',
                            M.util.get_string('sectionheadingedit', 'moeworksheets', response.instancesection));
                    var deleteicon = activity.one(SELECTOR.DELETESECTIONICON);
                    if (deleteicon) {
                        deleteicon.set('title', M.util.get_string('sectionheadingremove', 'moeworksheets', response.instancesection));
                        deleteicon.set('alt', M.util.get_string('sectionheadingremove', 'moeworksheets', response.instancesection));
                    }
                }
            });
        }
    ...
    NAME: 'mod_moeworksheets-section-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_moeworksheets.init_section_toolbox = function(config) {
    ...
    "requires": [
        ...
        "event-key",
        "io",
        "moodle-mod_moeworksheets-moeworksheetsbase",
        "moodle-mod_moeworksheets-util-slot",
