YUI.add('moodle-mod_moeworksheets-util-page', function (Y, NAME) {

/**
 * A collection of utility classes for use with pages.
 *
 * @module moodle-mod_moeworksheets-util
 * @submodule moodle-mod_moeworksheets-util-page
 */

Y.namespace('Moodle.mod_moeworksheets.util.page');

/**
 * A collection of utility classes for use with pages.
 *
 * @class Moodle.mod_moeworksheets.util.page
 * @static
 */
Y.Moodle.mod_moeworksheets.util.page = {
    CSS: {
        PAGE : 'page'
    },
    CONSTANTS: {
     ...
     */
    getPages: function() {
        return Y.all(Y.Moodle.mod_moeworksheets.util.slot.SELECTORS.PAGECONTENT + ' ' +
                     Y.Moodle.mod_moeworksheets.util.slot.SELECTORS.SECTIONUL + ' ' +
                    this.SELECTORS.PAGE);
    },
    ...
    add: function(beforenode) {
        ...
        var pagehtml = M.mod_moeworksheets.resource_toolbox.get('config').pagehtml;
        ...
        YUI().use('dd-drop', function(Y) {
            var drop = new Y.DD.Drop({
                node: page,
                groups: M.mod_moeworksheets.dragres.groups
            });
            page.drop = drop;
        });
    ...
    remove: function(page, keeppagebreak) {
        ...
        var previousslot = page.previous(Y.Moodle.mod_moeworksheets.util.slot.SELECTORS.SLOT);
        if (!keeppagebreak && previousslot) {
            Y.Moodle.mod_moeworksheets.util.slot.removePageBreak(previousslot);
        }
        page.remove();
    },
