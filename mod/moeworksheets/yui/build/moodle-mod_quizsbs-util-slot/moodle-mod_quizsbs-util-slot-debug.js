YUI.add('moodle-mod_moeworksheets-util-slot', function (Y, NAME) {

/**
 * A collection of utility classes for use with slots.
 *
 * @module moodle-mod_moeworksheets-util
 * @submodule moodle-mod_moeworksheets-util-slot
 */

Y.namespace('Moodle.mod_moeworksheets.util.slot');

/**
 * A collection of utility classes for use with slots.
 *
 * @class Moodle.mod_moeworksheets.util.slot
 * @static
 */
Y.Moodle.mod_moeworksheets.util.slot = {
    CSS: {
        SLOT : 'slot',
    ...
    reorderSlots: function() {
        ...
        slots.each(function(slot) {
            ...
            if (!Y.Moodle.mod_moeworksheets.util.page.getPageFromSlot(slot)) {
                // Move the next page to the front.
                var nextpage = slot.next(Y.Moodle.mod_moeworksheets.util.page.SELECTORS.PAGE);
                slot.swap(nextpage);
            }
    ...
    updateOneSlotSections: function() {
        Y.all('.mod-moeworksheets-edit-content ul.slots li.section').each(function(section) {
    ...
    remove: function(slot) {
        var page = Y.Moodle.mod_moeworksheets.util.page.getPageFromSlot(slot);
        slot.remove();
        // Is the page empty.
        if (!Y.Moodle.mod_moeworksheets.util.page.isEmpty(page)) {
            return;
        }
        // If so remove it. Including add menu and page break.
        Y.Moodle.mod_moeworksheets.util.page.remove(page);
    },
    ...
    addPageBreak: function(slot) {
        var nodetext = M.mod_moeworksheets.resource_toolbox.get('config').addpageiconhtml;
    ...
    reorderPageBreaks: function() {
        ...
        slots.each(function(slot, key) {
            ...
            var action = '', iconname = '';
            if (Y.Moodle.mod_moeworksheets.util.page.isPage(nextitem)) {
            ...
            pagebreaklink.set('title', M.util.get_string(action, 'moeworksheets'));
            pagebreaklink.setData('action', action);
            // Update the image title.
            var icon = pagebreaklink.one(this.SELECTORS.ICON);
            icon.set('title', M.util.get_string(action, 'moeworksheets'));
            icon.set('alt', M.util.get_string(action, 'moeworksheets'));
    ...
    updateDependencyIcon: function(slot, requiresprevious) {
        ...
        if (requiresprevious) {
            link.set('title', M.util.get_string('questiondependencyremove', 'moeworksheets', a));
            link.setData('action', 'removedependency');
            icon.set('alt', M.util.get_string('questiondependsonprevious', 'moeworksheets'));
            icon.set('src', M.util.image_url('t/locked', 'moodle'));
        } else {
            link.set('title', M.util.get_string('questiondependencyadd', 'moeworksheets', a));
            link.setData('action', 'adddependency');
            icon.set('alt', M.util.get_string('questiondependencyfree', 'moeworksheets'));
...
}, '@VERSION@', {"requires": ["node", "moodle-mod_moeworksheets-util-base"]});
