...
/**
 * Classes to enforce the various access rules that can apply to a quizsbs.
 *
 * @package   mod_quizsbs
 ...
 * quizsbs, with convinient methods for seeing whether access is allowed.
 *
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     Moodle 2.2
 */
class quizsbs_access_manager {
    /** @var quizsbs the quizsbs settings object. */
    protected $quizsbsobj;
    /** @var int the time to be considered as 'now'. */
    protected $timenow;
    /** @var array of quizsbs_access_rule_base. */
    protected $rules = array();

    /**
     * Create an instance for a particular quizsbs.
     * @param object $quizsbsobj An instance of the class quizsbs from attemptlib.php.
     *      The quizsbs we will be controlling access to.
     * @param int $timenow The time to use as 'now'.
     * @param bool $canignoretimelimits Whether this user is exempt from time
     *      limits (has_capability('mod/quizsbs:ignoretimelimits', ...)).
     */
    public function __construct($quizsbsobj, $timenow, $canignoretimelimits) {
        $this->quizsbsobj = $quizsbsobj;
        $this->timenow = $timenow;
        $this->rules = $this->make_rules($quizsbsobj, $timenow, $canignoretimelimits);
    }

    /**
     * Make all the rules relevant to a particular quizsbs.
     * @param quizsbs $quizsbsobj information about the quizsbs in question.
     * @param int $timenow the time that should be considered as 'now'.
     * @param bool $canignoretimelimits whether the current user is exempt from
     *      time limits by the mod/quizsbs:ignoretimelimits capability.
     * @return array of {@link quizsbs_access_rule_base}s.
     */
    protected function make_rules($quizsbsobj, $timenow, $canignoretimelimits) {

        $rules = array();
        foreach (self::get_rule_classes() as $ruleclass) {
            $rule = $ruleclass::make($quizsbsobj, $timenow, $canignoretimelimits);
            if ($rule) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['quizsbsaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return core_component::get_plugin_list_with_class('quizsbsaccess', '', 'rule.php');
    }
     ...
     * settings are stored in the quizsbs table.
     *
     * @param mod_quizsbs_mod_form $quizsbsform the quizsbs settings form that is being built.
    ...
    public static function add_settings_form_fields(
        ...
        foreach (self::get_rule_classes() as $rule) {
            $rule::add_settings_form_fields($quizsbsform, $mform);
        }
    }
    ...
    public static function get_browser_security_choices() {
        $options = array('-' => get_string('none', 'quizsbs'));
        foreach (self::get_rule_classes() as $rule) {
     ...
     * @param mod_quizsbs_mod_form $quizsbsform the quizsbs form object.
     * @return array $errors the updated $errors array.
     */
    public static function validate_settings_form_fields(array $errors,
            array $data, $files, mod_quizsbs_mod_form $quizsbsform) {

        foreach (self::get_rule_classes() as $rule) {
            $errors = $rule::validate_settings_form_fields($errors, $data, $files, $quizsbsform);
        }
    ...
    /**
     * Save any submitted settings when the quizsbs settings form is submitted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the quizsbs table.
     *
     * @param object $quizsbs the data from the quizsbs form, including $quizsbs->id
     *      which is the id of the quizsbs being saved.
     */
    public static function save_settings($quizsbs) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::save_settings($quizsbs);
        }
    }

    /**
     * Delete any rule-specific settings when the quizsbs is deleted.
     *
     * Note that the standard plugins do not use this mechanism because their
     * settings are stored in the quizsbs table.
     *
     * @param object $quizsbs the data from the database, including $quizsbs->id
     *      which is the id of the quizsbs being deleted.
     * @since Moodle 2.7.1, 2.6.4, 2.5.7
     */
    public static function delete_settings($quizsbs) {

        foreach (self::get_rule_classes() as $rule) {
            $rule::delete_settings($quizsbs);
        }
    }
     ...
     */
    protected static function get_load_sql($quizsbsid, $rules, $basefields) {
        $allfields = $basefields;
        $alljoins = '{quizsbs} quizsbs';
        $allparams = array('quizsbsid' => $quizsbsid);

        foreach ($rules as $rule) {
            list($fields, $joins, $params) = $rule::get_settings_sql($quizsbsid);
            if ($fields) {
                if ($allfields) {
                    $allfields .= ', ';
                }
        ...
        return array("SELECT $allfields FROM $alljoins WHERE quizsbs.id = :quizsbsid", $allparams);
    }
     ...
     */
    public static function load_settings($quizsbsid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($quizsbsid, $rules, '');
        ...
        foreach ($rules as $rule) {
            $data += $rule::get_extra_settings($quizsbsid);
        }
    ...
    /**
     * Load the quizsbs settings and any settings required by the access rules.
     ...
     *
     * @param int $quizsbsid the quizsbs id.
     * @return object mdl_quizsbs row with extra fields.
     */
    public static function load_quizsbs_and_settings($quizsbsid) {
        global $DB;

        $rules = self::get_rule_classes();
        list($sql, $params) = self::get_load_sql($quizsbsid, $rules, 'quizsbs.*');
        $quizsbs = $DB->get_record_sql($sql, $params, MUST_EXIST);

        foreach ($rules as $rule) {
            foreach ($rule::get_extra_settings($quizsbsid) as $name => $value) {
                $quizsbs->$name = $value;
            }
        }
    ...
    /**
     * Provide a description of the rules that apply to this quizsbs, such
     * as is shown at the top of the quizsbs view page. Note that not all
    ...
    /**
     * Whether or not a user should be allowed to start a new attempt at this quizsbs now.
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_quizsbs_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        ...
        $method = 'post';
        if (!empty($_GET['_qf__mod_quizsbs_preflight_check_form'])) {
            $method = 'get';
        }
        return new mod_quizsbs_preflight_check_form($url->out_omit_querystring(),
                array('rules' => $this->rules, 'quizsbsobj' => $this->quizsbsobj,
     ...
     * quizsbs. Used, for example, to change the label by the grade displayed on the view page from
     ...
     *
     * @param object $attempt the data from the relevant quizsbs_attempts row.
    ...
    /**
     * @return bolean if this quizsbs should only be shown to students in a popup window.
     */
    ...
    /**
     * Send the user back to the quizsbs view page. Normally this is just a redirect, but
     * If we were in a secure window, we close this window, and reload the view window we came from.
     *
     * This method does not return;
     *
     * @param mod_quizsbs_renderer $output the quizsbs renderer.
    ...
    public function back_to_view_page($output, $message = '') {
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup($this->quizsbsobj->view_url(), $message);
            die();
        } else {
            redirect($this->quizsbsobj->view_url(), $message);
        }
    }

    /**
     * Make some text into a link to review the quizsbs, if that is appropriate.
     *
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(quizsbs_attempt::IN_PROGRESS, quizsbs_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = quizsbs_attempt_state($this->quizsbsobj->get_quizsbs(), $attempt);
        $reviewoptions = mod_quizsbs_display_options::make_from_quizsbs(
                $this->quizsbsobj->get_quizsbs(), $when);

        if (!$reviewoptions->attempt) {
            return $output->no_review_message($this->quizsbsobj->cannot_review_message($when, true));

        } else {
            return $output->review_link($this->quizsbsobj->review_url($attempt->id),
