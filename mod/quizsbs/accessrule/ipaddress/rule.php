...
/**
 * Implementaton of the quizsbsaccess_ipaddress plugin.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizsbsaccess_ipaddress extends quizsbs_access_rule_base {

    public static function make(quizsbs $quizsbsobj, $timenow, $canignoretimelimits) {
        if (empty($quizsbsobj->get_quizsbs()->subnet)) {
            return null;
        }
    ...
    public function prevent_access() {
        if (address_in_subnet(getremoteaddr(), $this->quizsbs->subnet)) {
            return false;
        } else {
            return get_string('subnetwrong', 'quizsbsaccess_ipaddress');
        }
    }
}
