...
/**
 * Unit tests for the quizsbsaccess_ipaddress plugin.
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/quizsbs/accessrule/ipaddress/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizsbsaccess_ipaddress_testcase extends basic_testcase {
    public function test_ipaddress_access_rule() {
        ...
        $quizsbs->subnet = getremoteaddr(null);
        if (!empty($quizsbs->subnet)) {
            $quizsbsobj = new quizsbs($quizsbs, $cm, null);
            $rule = new quizsbsaccess_ipaddress($quizsbsobj, 0);
