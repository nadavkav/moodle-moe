...
/**
 * Unit tests for the quizsbsaccess_numattempts plugin.
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/quizsbs/accessrule/numattempts/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizsbsaccess_numattempts_testcase extends basic_testcase {
    public function test_num_attempts_access_rule() {
        ...
        $rule = new quizsbsaccess_numattempts($quizsbsobj, 0);
        $attempt = new stdClass();

        $this->assertEquals($rule->description(),
            get_string('attemptsallowedn', 'quizsbsaccess_numattempts', 3));

        $this->assertFalse($rule->prevent_new_attempt(0, $attempt));
        $this->assertFalse($rule->prevent_new_attempt(2, $attempt));
        $this->assertEquals($rule->prevent_new_attempt(3, $attempt),
            get_string('nomoreattempts', 'quizsbs'));
