...
/**
 * Thisscript processes ajax auto-save requests during the quizsbs.
 *
...
if ($attemptobj->is_finished()) {
    throw new moodle_quizsbs_exception($attemptobj->get_quizsbsobj(),
...
if ($finishattempt) {
    ...
    if ($thispage >= 0 && $attemptobj->get_currentpage() != $thispage) {
        $DB->set_field('quizsbs_attempts', 'currentpage', $thispage, array('id' => $attemptid));
    }
    ...
    // Normally, during a quizsbs attempt, every time the student goes to a new page,
    ...
    // student is actively attempting the quizsbs.
    $params = array(
            'objectid' => $attemptid,
