...
$message = "This is a rather longer test message, because, if the message was short, " .
        ...
        "the real data we have to encrypt are all the responses to a quizsbs, including " .
...
$PAGE->set_url('/mod/quizsbs/accessrule/offlinemode/cryptotest.php');
...
Y.use('node', 'json', 'moodle-quizsbsaccess_offlinemode-jsencrypt',
        'moodle-quizsbsaccess_offlinemode-sjcl', function(Y) {
    ...
    Y.one('div.result').append('<h2>Decryption of the data passed back to PHP</h2>' +
            ...
            '/mod/quizsbs/accessrule/offlinemode/cryptotest.php?json=' +
...
function decrypt_json($json, $context) {
    ...
    $PAGE->set_context($context);
    $PAGE->set_url('/mod/quizsbs/accessrule/offlinemode/cryptotest.php', array('json' => $json));
