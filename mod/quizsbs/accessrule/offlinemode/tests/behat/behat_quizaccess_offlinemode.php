...
/**
 * Steps definitions related to quizsbsaccess_offlinemode.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_quizsbsaccess_offlinemode extends behat_question_base {
    ...
    public function i_follow_the_emergency_download_link() {
        ...
        $session->evaluateScript('
                window.behat_quizsbsaccess_offlinemode_click_handler = function(e) {
                    e.preventDefault();
                };
                document.body.addEventListener("click", window.behat_quizsbsaccess_offlinemode_click_handler);');

        $linknode = $this->find_link(get_string('savetheresponses', 'quizsbsaccess_offlinemode'));
        $this->ensure_node_is_visible($linknode);
        $linknode->click();

        $session->evaluateScript('
                document.body.removeEventListener("click", window.behat_quizsbsaccess_offlinemode_click_handler);');
        ...
        $tempdir = make_temp_directory('behat_quizsbsaccess_offlinemode');
    ...
    /**
     * Set the quizsbs's Auto-save period configuration setting to this many seconds.
     *
     * @param int $time time in seconds.
     *
     * @Given /^the quizsbs auto-save period is set to "([^"]*)"$/
     */
    public function the_quizsbs_auto_save_period_is_set_to($time) {
        set_config('autosaveperiod', $time, 'quizsbs');
    }
    ...
    public function i_simulate_losing_the_network_by_changing_the_submit_url() {
        ...
        $session->evaluateScript('
                M.quizsbsaccess_offlinemode.autosave.AUTOSAVE_HANDLER = "' .
                        $CFG->wwwroot . '/mod/quizsbs/accessrule/offlinemode/does_not_exist.ajax.php";');
    }
}
