...
Feature: Download responses, encrypted, on the client-side, and re-upload.
  In order to attempt quizsbszes with dodgy wifi that dies and never comes back
  As a student or invigilator
  I need to be able to download my responses on the client-side, and later upload them to the quizsbs.
  ...
  Background:
    ...
    And quizsbs "quizsbs fault-tolerant" contains the following questions:
      ...
      | Question B | 1 |
      | Question C | 1 |
    And the quizsbs auto-save period is set to "2"
  ...
  Scenario: Download the responses so far, then re-upload them without finishing the attempt.
    ...
    And I should not see "Completed on" in the "quizsbsreviewsummary" "table"
  ...
  Scenario: Download the responses so far, then re-upload them finishing the attempt.
    ...
    And I should see "Completed on" in the "quizsbsreviewsummary" "table"
    And I should see "Answer to the third question"
