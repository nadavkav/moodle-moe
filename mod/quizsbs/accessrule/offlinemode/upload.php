...
$cm = get_coursemodule_from_id('quizsbs', $cmid, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$quizsbs = $DB->get_record('quizsbs', array('id' => $cm->instance), '*', MUST_EXIST);
$quizsbsurl = new moodle_url('/mod/quizsbs/view.php', array('id' => $cm->id));
$context = context_module::instance($cm->id);

$PAGE->set_url('/mod/quizsbs/accessrule/offlinemode/upload.php', array('id' => $cmid));
require_login($course, false, $cm);
require_capability('quizsbsaccess/offlinemode:uploadresponses', $context);

$form = new \quizsbsaccess_offlinemode\form\upload_responses($PAGE->url);
if ($form->is_cancelled()) {
    ...
    // Process submission.
    $title = get_string('uploadingresponsesfor', 'quizsbsaccess_offlinemode',
            format_string($quizsbs->name, true, array('context' => $context)));
    ...
    $privatekey = null;
    $privatekeystring = get_config('quizsbsaccess_offlinemode', 'privatekey');
    if ($privatekeystring) {
    ...
    foreach ($files as $file) {
        ...
        echo $OUTPUT->heading(get_string('processingfile', 'quizsbsaccess_offlinemode', s($file->get_filename())), 3);
        ...
        try {
            ...
            // Load the attempt.
            $attemptobj = quizsbs_attempt::create($postdata['attempt']);
            if ($attemptobj->get_cmid() != $cmid) {
                throw new coding_exception('The uploaded data does not belong to this quizsbs.');
            }
            ...
            echo $OUTPUT->notification(get_string('dataprocessedsuccessfully', 'quizsbsaccess_offlinemode',
                    html_writer::link($attemptobj->review_url(), get_string('reviewthisattempt', 'quizsbsaccess_offlinemode'))),
                    'notifysuccess');
            ...
            echo $OUTPUT->heading(get_string('uploadfailed', 'quizsbsaccess_offlinemode'), 4);
    ...
    echo $OUTPUT->confirm(get_string('processingcomplete', 'quizsbsaccess_offlinemode', 3),
            new single_button($PAGE->url, get_string('uploadmoreresponses', 'quizsbsaccess_offlinemode'), 'get'),
            new single_button($quizsbsurl, get_string('backtothequizsbs', 'quizsbsaccess_offlinemode'), 'get'));
    echo $OUTPUT->footer();

} else {

    // Show the form.
    $title = get_string('uploadresponsesfor', 'quizsbsaccess_offlinemode',
            format_string($quizsbs->name, true, array('context' => $context)));
    $PAGE->navbar->add($title);
    $PAGE->set_pagelayout('admin');
    $PAGE->set_title($title);
