YUI.add('moodle-quizsbsaccess_offlinemode-navigation', function (Y, NAME) {
...
/**
 * fault-tolerant mode for quizsbs attempts.
 *
 * @module moodle-quizsbsaccess_offlinemode-navigation
 */

/**
 * Auto-save functionality for during quizsbs attempts.
 *
 * @class M.quizsbsaccess_offlinemode.navigation
 */

M.quizsbsaccess_offlinemode = M.quizsbsaccess_offlinemode || {};
M.quizsbsaccess_offlinemode.navigation = {
    /**
    ...
    SELECTORS: {
        ...
        NAV_BLOCK:            '#mod_quizsbs_navblock',
        NAV_BUTTON:           '.qnbutton',
        FINISH_LINK:          '.endtestlink',
        NEXT_BUTTON:          'input[name=next]',
        SUMMARY_TABLE:        '.quizsbssummaryofattempt',
        ...
        SUMMARY_PAGE_BUTTON:  '#quizsbsaccess_offlinemode-attempt_page--1 .submitbtns input[type=submit]',
        PAGE_DIV_ROOT:        '#quizsbsaccess_offlinemode-attempt_page-',
        ALL_PAGE_DIVS:        'div[id|=quizsbsaccess_offlinemode-attempt_page]',
    ...
    /**
     * A Node reference to the main quizsbs form.
     *
     * @property form
     * @type Node
     * @default null
    ...
    init: function(currentpage) {
        ...
        if (!this.form) {
            Y.log('Response form not found.', 'debug', 'moodle-quizsbsaccess_offlinemode-navigation');
            return;
        }
        ...
        Y.all(this.SELECTORS.ALL_PAGE_DIVS).each(function(element) {
                    var pageno;
                    var matches = element.get('id').match(/quizsbsaccess_offlinemode-attempt_page-(\d+)/);
                    if (matches) {
                        pageno = +matches[1];
        ...
        Y.all(this.SELECTORS.ALL_PAGE_DIVS).removeClass('quizsbs-loading-hide');
        ...
        Y.log('Initialised fault-tolerant quizsbs mode.', 'debug', 'moodle-quizsbsaccess_offlinemode-navigation');
    },
    ...
    nav_button_click: function(e) {
        // Prevent the quizsbs's own event handler running.
        e.halt();
    ...
    page_number_from_link: function(anchor) {
        var dataValue = anchor.getData('quizsbs-page');
    ...
    navigate_to_page: function(pageno) {
        ...
        if (pageno === -1) {
            Y.one(this.SELECTORS.quizsbs_FORM).addClass('hidden');
        } else {
            Y.one(this.SELECTORS.quizsbs_FORM).removeClass('hidden');
        }
        ...
        if (window.history.replaceState) {
            ...
            window.history.replaceState(null, '', M.cfg.wwwroot + '/mod/quizsbs/accessrule/offlinemode/attempt.php' + queryString);
        }
