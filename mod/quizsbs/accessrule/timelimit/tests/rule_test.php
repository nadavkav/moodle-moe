...
/**
 * Unit tests for the quizsbsaccess_timelimit plugin.
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/quizsbs/accessrule/timelimit/rule.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizsbsaccess_timelimit_testcase extends basic_testcase {
    public function test_time_limit_access_rule() {
        $quizsbs = new stdClass();
        $quizsbs->timelimit = 3600;
        $cm = new stdClass();
        $cm->id = 0;
        $quizsbsobj = new quizsbs($quizsbs, $cm, null);
        $rule = new quizsbsaccess_timelimit($quizsbsobj, 10000);
        $attempt = new stdClass();

        $this->assertEquals($rule->description(),
            get_string('quizsbstimelimit', 'quizsbsaccess_timelimit', format_time(3600)));
