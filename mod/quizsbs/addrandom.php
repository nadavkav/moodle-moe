...
/**
 * Fallback page of /mod/quizsbs/edit.php add random question dialog,
...
list($thispageurl, $contexts, $cmid, $cm, $quizsbs, $pagevars) =
        question_edit_setup('editq', '/mod/quizsbs/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/quizsbs/edit.php', array('cmid' => $cmid));
}
if ($scrollpos) {
...
$mform = new quizsbs_add_random_form(new moodle_url('/mod/quizsbs/addrandom.php'),
...
if ($data = $mform->get_data()) {
    ...
    quizsbs_add_random_questions($quizsbs, $addonpage, $categoryid, $data->numbertoadd, $includesubcategories);
...
// Setup $PAGE.
$streditingquizsbs = get_string('editinga', 'moodle', get_string('modulename', 'quizsbs'));
$PAGE->navbar->add($streditingquizsbs);
$PAGE->set_title($streditingquizsbs);
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();

if (!$quizsbsname = $DB->get_field($cm->modname, 'name', array('id' => $cm->instance))) {
            print_error('invalidcoursemodule');
}

echo $OUTPUT->heading(get_string('addrandomquestiontoquizsbs', 'quizsbs', $quizsbsname), 2);
$mform->display();
echo $OUTPUT->footer();
