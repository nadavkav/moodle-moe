 ...
 *
 * This is also responsible for opening the pop-up window, if the quizsbs requires to be in one.
 *
...
define(['jquery', 'core/yui'], function($, Y) {
    ...
    /**
     * @alias module:mod_quizsbs/preflightcheck
     */
    var t = {
         ...
         * @param {String} popupoptions If not null, the quizsbs should be launced in a pop-up.
         */
            ...
            Y.use('moodle-core-notification', 'moodle-core-formchangechecker', 'io-form', function () {
                if (Y.one(confirmationForm)) {
                    t.confirmDialogue = new M.core.dialogue({
                        ...
                        modal: true,
                        width: null,
                        extraClasses: ['mod_quizsbs_preflight_popup']
                    });
                ...
                if (popupoptions) {
                    Y.one(finalStartButton).on('click', t.launchquizsbsPopup, t, popupoptions);
                }
            });
        },
        ...
        /**
         * Event handler for the quizsbs start attempt button.
         */
        launchquizsbsPopup: function(e, popupoptions) {
            e.halt();
            M.core_formchangechecker.reset_form_dirty_state();
            var form = e.target.ancestor('form');
            window.openpopup(e, {
                url: form.get('action') + '?' + Y.IO.stringify(form).replace(/\bcancel=/, 'x='),
                windowname: 'quizsbspopup',
