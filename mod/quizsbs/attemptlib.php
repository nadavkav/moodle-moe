...
/**
 * Back-end code for handling data about quizsbszes and the current user's attempt.
 *
 * There are classes for loading all the information about a quizsbs and attempts,
...
/**
 * Class for quizsbs exceptions. Just saves a couple of arguments on the
 ...
 * @since     Moodle 2.0
 */
class moodle_quizsbs_exception extends moodle_exception {
    public function __construct($quizsbsobj, $errorcode, $a = null, $link = '', $debuginfo = null) {
        if (!$link) {
            $link = $quizsbsobj->view_url();
        }
        parent::__construct($errorcode, 'quizsbs', $link, $a, $debuginfo);
    }
}


/**
 * A class encapsulating a quizsbs and the questions it contains, and making the
...
class quizsbs {
    ...
    protected $cm;
    /** @var stdClass the quizsbs settings from the database. */
    protected $quizsbs;
    /** @var context the quizsbs context. */
    protected $context;

    /** @var array of questions augmented with slot information. */
    protected $questions = null;
    /** @var array of quizsbs_section rows. */
    protected $sections = null;
    /** @var quizsbs_access_manager the access manager for this quizsbs. */
    protected $accessmanager = null;
    /** @var bool whether the current user has capability mod/quizsbs:preview. */
     ...
     *
     * @param object $quizsbs the row from the quizsbs table.
     * @param object $cm the course_module object for this quizsbs.
     * @param object $course the row from the course table for the course we belong to.
     * @param bool $getcontext intended for testing - stops the constructor getting the context.
     */
    public function __construct($quizsbs, $cm, $course, $getcontext = true) {
        $this->quizsbs = $quizsbs;
        $this->cm = $cm;
        $this->quizsbs->cmid = $this->cm->id;
        $this->course = $course;
    ...
    /**
     * Static function to create a new quizsbs object for a specific user.
     *
     * @param int $quizsbsid the the quizsbs id.
     * @param int $userid the the userid.
     * @return quizsbs the new quizsbs object
     */
    public static function create($quizsbsid, $userid = null) {
        global $DB;

        $quizsbs = quizsbs_access_manager::load_quizsbs_and_settings($quizsbsid);
        $course = $DB->get_record('course', array('id' => $quizsbs->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('quizsbs', $quizsbs->id, $course->id, false, MUST_EXIST);

        // Update quizsbs with override information.
        if ($userid) {
            $quizsbs = quizsbs_update_effective_access($quizsbs, $userid);
        }

        return new quizsbs($quizsbs, $cm, $course);
    }

    /**
     * Create a {@link quizsbs_attempt} for an attempt at this quizsbs.
     * @param object $attemptdata row from the quizsbs_attempts table.
     * @return quizsbs_attempt the new quizsbs_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new quizsbs_attempt($attemptdata, $this->quizsbs, $this->cm, $this->course);
    }

    // Functions for loading more data =========================================

    /**
     * Load just basic information about all the questions in this quizsbs.
     */
    public function preload_questions() {
        $this->questions = question_preload_questions(null,
                'slot.maxmark, slot.id AS slotid, slot.slot, slot.page',
                '{quizsbs_slots} slot ON slot.quizsbsid = :quizsbsid AND q.id = slot.questionid',
                array('quizsbsid' => $this->quizsbs->id), 'slot.slot');
    }

    /**
     * Fully load some or all of the questions for this quizsbs. You must call
    ...
    /**
     * Get an instance of the {@link \mod_quizsbs\structure} class for this quizsbs.
     * @return \mod_quizsbs\structure describes the questions in the quizsbs.
     */
    public function get_structure() {
        return \mod_quizsbs\structure::create_for_quizsbs($this);
    }
    ...
    /** @return int the quizsbs id. */
    public function get_quizsbsid() {
        return $this->quizsbs->id;
    }

    /** @return object the row of the quizsbs table. */
    public function get_quizsbs() {
        return $this->quizsbs;
    }

    /** @return string the name of this quizsbs. */
    public function get_quizsbs_name() {
        return $this->quizsbs->name;
    }

    /** @return int the quizsbs navigation method. */
    public function get_navigation_method() {
        return $this->quizsbs->navmethod;
    }

    /** @return int the number of attempts allowed at this quizsbs (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->quizsbs->attempts;
    }
    ...
    /** @return object the module context for this quizsbs. */
    public function get_context() {
        return $this->context;
    }

    /**
     * @return bool wether the current user is someone who previews the quizsbs,
     * rather than attempting it.
     */
    public function is_preview_user() {
        if (is_null($this->ispreviewuser)) {
            $this->ispreviewuser = has_capability('mod/quizsbs:preview', $this->context);
        }
        return $this->ispreviewuser;
    }

    /**
     * @return whether any questions have been added to this quizsbs.
     */
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'quizsbs', $this->view_url());
            }
    ...
    /**
     * Get all the sections in this quizsbs.
     * @return array 0, 1, 2, ... => quizsbs_sections row from the database.
     */
    public function get_sections() {
        global $DB;
        if ($this->sections === null) {
            $this->sections = array_values($DB->get_records('quizsbs_sections',
                    array('quizsbsid' => $this->get_quizsbsid()), 'firstslot'));
        }
        return $this->sections;
    }

    /**
     * Return quizsbs_access_manager and instance of the quizsbs_access_manager class
     * for this quizsbs at this time.
    ...
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new quizsbs_access_manager($this, $timenow,
                    has_capability('mod/quizsbs:ignoretimelimits', $this->context, null, false));
        }
        return $this->accessmanager;
    }

    /**
     * Wrapper round the has_capability funciton that automatically passes in the quizsbs context.
     */
    ...
    /**
     * Wrapper round the require_capability funciton that automatically passes in the quizsbs context.
     */
    ...
    /**
     * @return string the URL of this quizsbs's view page.
     */
    public function view_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/quizsbs/view.php?id=' . $this->cm->id;
    }

    /**
     * @return string the URL of this quizsbs's edit page.
     */
    public function edit_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/quizsbs/edit.php?cmid=' . $this->cm->id;
    }
    ...
    public function attempt_url($attemptid, $page = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/quizsbs/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
        return $url;
    ...
    /**
     * @return string the URL of this quizsbs's edit page. Needs to be POSTed to with a cmid parameter.
     */
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/quizsbs/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new moodle_url('/mod/quizsbs/review.php', array('attempt' => $attemptid));
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/quizsbs/summary.php', array('attempt' => $attemptid));
    }
     ...
     * @deprecated since 3.1. This sort of functionality is now entirely handled by quizsbs access rules.
     */
    public function confirm_start_attempt_message($notused) {
        debugging('confirm_start_attempt_message is deprecated. ' .
                'This sort of functionality is now entirely handled by quizsbs access rules.');
        return '';
    }
     ...
     *
     * @param int $when One of the mod_quizsbs_display_options::DURING,
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($when == mod_quizsbs_display_options::DURING ||
                $when == mod_quizsbs_display_options::IMMEDIATELY_AFTER) {
            return '';
        } else if ($when == mod_quizsbs_display_options::LATER_WHILE_OPEN && $this->quizsbs->timeclose &&
                $this->quizsbs->reviewattempt & mod_quizsbs_display_options::AFTER_CLOSE) {
            return get_string('noreviewuntil' . $langstrsuffix, 'quizsbs',
                    userdate($this->quizsbs->timeclose, $dateformat));
        } else {
            return get_string('noreview' . $langstrsuffix, 'quizsbs');
        }
    }

    /**
     * @param string $title the name of this particular quizsbs page.
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_quizsbs_exception($this, 'questionnotloaded', $id);
        }
    }

    /**
     * Return all the question types used in this quizsbs.
     *
     * @param  boolean $includepotential if the quizsbs include random questions, setting this flag to true will make the function to
    ...
    public function get_all_question_types_used($includepotential = false) {
        ...
        // We must be careful with random questions, if we find a random question we must assume that the quizsbs may content
...
/**
 * This class extends the quizsbs class to hold data about the state of a particular attempt,
 * in addition to the data about the quizsbs.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class quizsbs_attempt {
    ...
    /** @var int maximum number of slots in the quizsbs for the review page to default to show all. */
    const MAX_SLOTS_FOR_DEFAULT_REVIEW_SHOW_ALL = 50;

    /** @var quizsbs object containing the quizsbs settings. */
    protected $quizsbsobj;

    /** @var stdClass the quizsbs_attempts row. */
    protected $attempt;

    /** @var question_usage_by_activity the question usage for this quizsbs attempt. */
    protected $quba;
    ...
    /** @var array of quizsbs_sections rows, with a ->lastslot field added. */
    protected $sections;
    ...
    /** @var mod_quizsbs_display_options cache for the appropriate review options. */
     ...
     * Constructor assuming we already have the necessary data loaded.
     *
     * @param object $attempt the row of the quizsbs_attempts table.
     * @param object $quizsbs the quizsbs object for this attempt and user.
     * @param object $cm the course_module object for this quizsbs.
     * @param object $course the row from the course table for the course we belong to.
     ...
     */
    public function __construct($attempt, $quizsbs, $cm, $course, $loadquestions = true) {
        global $DB;

        $this->attempt = $attempt;
        $this->quizsbsobj = new quizsbs($quizsbs, $cm, $course);
        ...
        $this->slots = $DB->get_records('quizsbs_slots',
                array('quizsbsid' => $this->get_quizsbsid()), 'slot',
     ...
     * @param array $conditions passed to $DB->get_record('quizsbs_attempts', $conditions).
     */
    protected static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('quizsbs_attempts', $conditions, '*', MUST_EXIST);
        $quizsbs = quizsbs_access_manager::load_quizsbs_and_settings($attempt->quizsbs);
        $course = $DB->get_record('course', array('id' => $quizsbs->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('quizsbs', $quizsbs->id, $course->id, false, MUST_EXIST);

        // Update quizsbs with override information.
        $quizsbs = quizsbs_update_effective_access($quizsbs, $attempt->userid);

        return new quizsbs_attempt($attempt, $quizsbs, $cm, $course);
    }

    /**
     * Static function to create a new quizsbs_attempt object given an attemptid.
     *
    ...
    /**
     * Static function to create a new quizsbs_attempt object given a usage id.
     *
    ...
    public static function state_name($state) {
        return quizsbs_attempt_state_name($state);
    }
        ...
        foreach ($this->pagelayout as $page => $slots) {
            foreach ($slots as $slot) {
                if ($length = $this->is_real_question($slot)) {
                    ...
                    $this->questionnumbers[$slot] = get_string('infoshort', 'quizsbs');
                }
    ...
    public function get_quizsbsobj() {
        return $this->quizsbsobj;
    }
    ...
    /** @return int the course id. */
    public function get_course() {
        return $this->quizsbsobj->get_course();
    }
    ...
    /** @return string the name of this quizsbs. */
    public function get_quizsbs_name() {
        return $this->quizsbsobj->get_quizsbs_name();
    }

    /** @return int the quizsbs navigation method. */
    public function get_navigation_method() {
        return $this->quizsbsobj->get_navigation_method();
    }

    /** @return object the course_module object. */
    public function get_cm() {
        return $this->quizsbsobj->get_cm();
    }
    ...
    /** @return int the number of attempts allowed at this quizsbs (0 = infinite). */
    public function get_num_attempts_allowed() {
        return $this->quizsbsobj->get_num_attempts_allowed();
    }

    /** @return int number fo pages in this quizsbs. */
     ...
     */
    public function get_access_manager($timenow) {
        return $this->quizsbsobj->get_access_manager($timenow);
    }
    ...
    /** @return object the row from the quizsbs_attempts table. */
    ...
    /** @return string one of the quizsbs_attempt::IN_PROGRESS, FINISHED, OVERDUE or ABANDONED constants. */
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/quizsbs:viewreports')) {
            return false;
        }
    ...
    /**
     * Has the student, in this attempt, engaged with the quizsbs in a non-trivial way?
     ...
     * To see how this works, try setting a quizsbs to one of the CBM behaviours,
     * and then look at the extra information displayed at the top of the quizsbs
     ...
     *
     * @param question_display_options $options the display options for this quizsbs attempt at this time.
    ...
    public function get_overall_feedback($grade) {
        return quizsbs_feedback_for_grade($grade, $this->get_quizsbs(),
                $this->quizsbsobj->get_context());
    }
     ...
     */
    public function has_capability($capability, $userid = null, $doanything = true) {
        return $this->quizsbsobj->has_capability($capability, $userid, $doanything);
    }
     ...
     */
    public function require_capability($capability, $userid = null, $doanything = true) {
        return $this->quizsbsobj->require_capability($capability, $userid, $doanything);
    }
    ...
    public function check_review_capability() {
        if ($this->get_attempt_state() == mod_quizsbs_display_options::IMMEDIATELY_AFTER) {
            $capability = 'mod/quizsbs:attempt';
        } else {
            $capability = 'mod/quizsbs:reviewmyattempts';
        }

        // These next tests are in a slighly funny order. The point is that the
        // common and most performance-critical case is students attempting a quizsbs
        // so we want to check that permisison first.

        if ($this->has_capability($capability)) {
            // User has the permission that lets you do the quizsbs as a student. Fine.
            return;
        }

        if ($this->has_capability('mod/quizsbs:viewreports') ||
                $this->has_capability('mod/quizsbs:preview')) {
    ...
    public function can_navigate_to($slot) {
        switch ($this->get_navigation_method()) {
            case quizsbs_NAVMETHOD_FREE:
                return true;
                break;
            case quizsbs_NAVMETHOD_SEQ:
                return false;
                break;
        }
        return true;
    ...
    public function get_attempt_state() {
        return quizsbs_attempt_state($this->get_quizsbs(), $this->attempt);
    }

    /**
     * Wrapper that the correct mod_quizsbs_display_options for this quizsbs at the
     * moment.
     *
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = quizsbs_get_review_options($this->get_quizsbs(),
                        $this->attempt, $this->quizsbsobj->get_context());
            ...
            $options = mod_quizsbs_display_options::make_from_quizsbs($this->get_quizsbs(),
                    mod_quizsbs_display_options::DURING);
            $options->flags = quizsbs_get_flag_option($this->attempt, $this->quizsbsobj->get_context());
            return $options;
        }
    }
     ...
     * @return bool true if this is the last page of the quizsbs.
     */
    ...
    /**
     * Return the list of slot numbers for either a given page of the quizsbs, or for the
     * whole quizsbs.
     *
    ...
    public function is_blocked_by_previous_question($slot) {
        return $slot > 1 && isset($this->slots[$slot]) && $this->slots[$slot]->requireprevious &&
                ...
                $this->get_navigation_method() != quizsbs_NAVMETHOD_SEQ &&
    ...
    public function can_question_be_redone_now($slot) {
        return $this->get_quizsbs()->canredoquestions && !$this->is_finished() &&
    ...
    /**
     * Return the page of the quizsbs where this question appears.
     * @param int $slot the number used to identify this question within this attempt.
     * @return int the page of the quizsbs this question appears on.
     */
     ...
     *
     * @param int $slot the number used to identify this question within this attempt.
     * @return string the formatted grade, to the number of decimal places specified by the quizsbs.
     */
    public function get_question_mark($slot) {
        return quizsbs_format_question_grade($this->get_quizsbs(), $this->quba->get_question_mark($slot));
    }
    ...
    public function get_due_date() {
        $deadlines = array();
        if ($this->quizsbsobj->get_quizsbs()->timelimit) {
            $deadlines[] = $this->attempt->timestart + $this->quizsbsobj->get_quizsbs()->timelimit;
        }
        if ($this->quizsbsobj->get_quizsbs()->timeclose) {
            $deadlines[] = $this->quizsbsobj->get_quizsbs()->timeclose;
        }
        if ($deadlines) {
        ...
        switch ($this->attempt->state) {
            ...
            case self::OVERDUE:
                return $duedate + $this->quizsbsobj->get_quizsbs()->graceperiod;
    ...
    // URLs related to this attempt ============================================
    /**
     * @return string quizsbs view url.
     */
    public function view_url() {
        return $this->quizsbsobj->view_url();
    }
    ...
    public function start_attempt_url($slot = null, $page = -1) {
        ...
        return $this->quizsbsobj->start_attempt_url($page);
    }
    ...
    /**
     * @return string the URL of this quizsbs's summary page.
     */
    public function summary_url() {
        return new moodle_url('/mod/quizsbs/summary.php', array('attempt' => $this->attempt->id));
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/quizsbs/processattempt.php');
    }
    ...
    public function cannot_review_message($short = false) {
        return $this->quizsbsobj->cannot_review_message(
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new moodle_url(
                    ...
                    get_string('startnewpreview', 'quizsbs'));
        } else {
            return '';
        }
    }
     ...
     * @param mod_quizsbs_renderer $renderer the quizsbs renderer.
     ...
     */
    public function render_question($slot, $reviewing, mod_quizsbs_renderer $renderer, $thispageurl = null) {
        if ($this->is_blocked_by_previous_question($slot)) {
            ...
            return html_writer::div($placeholderqa->render($displayoptions,
                    ...
                    'mod_quizsbs-blocked_question_warning');
        }
     ...
     */
    protected function render_question_helper($slot, $reviewing, $thispageurl, mod_quizsbs_renderer $renderer, $seq) {
        ...
        if ($displayoptions->history && $displayoptions->questionreviewlink) {
            ...
            if ($links) {
                $displayoptions->extrahistorycontent = html_writer::tag('p',
                        get_string('redoesofthisquestion', 'quizsbs', $renderer->render($links)));
            }
        }
    ...
    protected function make_blocked_question_placeholder($slot) {
        ...
        $question->name = '';
        $question->questiontext = get_string('questiondependsonprevious', 'quizsbs');
        ...
        $placeholderqa->start($this->get_quizsbs()->preferredbehaviour, 1);
     ...
     *
     * @param int $id the id of a question in this quizsbs attempt.
     ...
     * @return string HTML for the question in its current state.
     */
    public function render_question_at_step($slot, $seq, $reviewing, mod_quizsbs_renderer $renderer, $thispageurl = '') {
     ...
     *
     * @param $panelclass The type of panel, quizsbs_attempt_nav_panel or quizsbs_review_nav_panel
     * @param $page the current page number.
     * @param $showall whether we are showing the whole quizsbs on one page. (Used by review.php)
     * @return quizsbs_nav_panel_base the requested object.
     */
    public function get_navigation_panel(mod_quizsbs_renderer $output,
             $panelclass, $page, $showall = false) {
        $panel = new $panelclass($this, $this->get_display_options(true), $page, $showall);

        $bc = new block_contents();
        $bc->attributes['id'] = 'mod_quizsbs_navblock';
    ...
    /**
     * Return an array of variant URLs to other attempts at this quizsbs.
     *
     * The $url passed in must contain an attempt parameter.
     *
     * The {@link mod_quizsbs_links_to_other_attempts} object returned contains an
     ...
     * @return mod_quizsbs_links_to_other_attempts containing array int => null|moodle_url.
     */
    public function links_to_other_attempts(moodle_url $url) {
        $attempts = quizsbs_get_user_attempts($this->get_quizsbs()->id, $this->attempt->userid, 'all');
        if (count($attempts) <= 1) {
            return false;
        }

        $links = new mod_quizsbs_links_to_other_attempts();
     ...
     * @return mod_quizsbs_links_to_other_attempts|null containing array int => null|moodle_url,
    ...
    public function handle_if_time_expired($timestamp, $studentisonline) {
        ...
        if ($this->attempt->state == self::OVERDUE) {
            ...
            $graceperiod = $this->quizsbsobj->get_quizsbs()->graceperiod;
        ...
        // Otherwise, we were in quizsbs_attempt::IN_PROGRESS, and time has now expired.
        // Transition to the appropriate state.
        switch ($this->quizsbsobj->get_quizsbs()->overduehandling) {
            case 'autosubmit':
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false, $simulatedresponses = null) {
        ...
        if ($becomingoverdue) {
            ...
            $DB->update_record('quizsbs_attempts', $this->attempt);
        }

        if (!$this->is_preview() && $this->attempt->state == self::FINISHED) {
            quizsbs_save_best_grade($this->get_quizsbs(), $this->get_userid());
        }
    ...
    public function process_redo_question($slot, $timestamp) {
        ...
        $qubaids = new \mod_quizsbs\question\qubaids_for_users_attempts(
                $this->get_quizsbsid(), $this->get_userid());
    ...
    public function process_finish($timestamp, $processsubmitted) {
        ...
        if (!$this->is_preview()) {
            quizsbs_save_best_grade($this->get_quizsbs(), $this->attempt->userid);

            // Trigger event.
            $this->fire_state_transition_event('\mod_quizsbs\event\attempt_submitted', $timestamp);
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('quizsbs_attempts', 'timecheckstate', $time, array('id' => $this->attempt->id));
        }
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_quizsbs\event\attempt_becameoverdue', $timestamp);

        $transaction->allow_commit();

        quizsbs_send_overdue_message($this);
    }
    ...
    public function process_abandon($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('\mod_quizsbs\event\attempt_abandoned', $timestamp);
    ...
    protected function fire_state_transition_event($eventclass, $timestamp) {
        global $USER;
        $quizsbsrecord = $this->get_quizsbs();
        $params = array(
            'context' => $this->get_quizsbsobj()->get_context(),
            'courseid' => $this->get_courseid(),
            'objectid' => $this->attempt->id,
            'relateduserid' => $this->attempt->userid,
            'other' => array(
                'submitterid' => CLI_SCRIPT ? null : $USER->id,
                'quizsbsid' => $quizsbsrecord->id
            )
        );

        $event = $eventclass::create($params);
        $event->add_record_snapshot('quizsbs', $this->get_quizsbs());
        $event->add_record_snapshot('quizsbs_attempts', $this->get_attempt());
        $event->trigger();
    }
    ...
    /**
     * Get a URL for a particular question on a particular page of the quizsbs.
     * Used by {@link attempt_url()} and {@link review_url()}.
     *
     * @param string $script. Used in the URL like /mod/quizsbs/$script.php
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/quizsbs/' . $script . '.php' . $fragment,
    ...
    /**
     * Process responses during an attempt at a quizsbs.
     *
    ...
    public function process_attempt($timenow, $finishattempt, $timeup, $thispage) {
        ...
        // to show the student another page of the quizsbs. Just finish now.
        $graceperiodmin = null;
        ...
        $toolate = false;
        if ($timeclose !== false && $timenow > $timeclose - quizsbs_MIN_TIME_TO_CONTINUE) {
            $timeup = true;
            $graceperiodmin = get_config('quizsbs', 'graceperiodmin');
        ...
        if ($timeup) {
            if ($this->get_quizsbs()->overduehandling == 'graceperiod') {
                if (is_null($graceperiodmin)) {
                    $graceperiodmin = get_config('quizsbs', 'graceperiodmin');
                }
                if ($timenow > $timeclose + $this->get_quizsbs()->graceperiod + $graceperiodmin) {
        ...
        // Update the quizsbs attempt record.
        try {
    ...
    public function check_page_access($page) {
        ...
        if ($this->get_currentpage() != $page) {
            if ($this->get_navigation_method() == quizsbs_NAVMETHOD_SEQ && $this->get_currentpage() > $page) {
                return false;
            }
        }
        return true;
    ...
    public function set_currentpage($page) {
        ...
        if ($this->check_page_access($page)) {
            $DB->set_field('quizsbs_attempts', 'currentpage', $page, array('id' => $this->get_attemptid()));
            return true;
        }
        return false;
    }
 ...
 * @since      Moodle 2.9
 */
class quizsbs_nav_section_heading implements renderable {
 ...
 * @since      Moodle 2.1
 */
class quizsbs_nav_question_button implements renderable {
 ...
 * @since      Moodle 2.0
 */
abstract class quizsbs_nav_panel_base {
    /** @var quizsbs_attempt */
    ...
    public function __construct(quizsbs_attempt $attemptobj,
    ...
    /**
     * Get the buttons and section headings to go in the quizsbs navigation block.
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            if ($heading = $this->attemptobj->get_heading_before_slot($slot)) {
                $buttons[] = new quizsbs_nav_section_heading(format_string($heading));
            }

            $qa = $this->attemptobj->get_question_attempt($slot);
            $showcorrectness = $this->options->correctness && $qa->has_marks();

            $button = new quizsbs_nav_question_button();
            $button->id          = 'quizsbsnavbutton' . $slot;
            ...
            if ($this->attemptobj->is_blocked_by_previous_question($slot)) {
                ...
                $button->statestring = get_string('questiondependsonprevious', 'quizsbs');
            }
            $buttons[] = $button;
        }
        $pages = array();
    ...
    protected function get_state_string(question_attempt $qa, $showcorrectness) {
        ...
        if ($qa->get_state() == question_state::$todo) {
            return get_string('notyetviewed', 'quizsbs');
        } else {
            return get_string('viewed', 'quizsbs');
        }
    }

    public function render_before_button_bits(mod_quizsbs_renderer $output) {
        return '';
    }

    abstract public function render_end_bits(mod_quizsbs_renderer $output);
    ...
    public function user_picture() {
        global $DB;
        if ($this->attemptobj->get_quizsbs()->showuserpicture == quizsbs_SHOWIMAGE_NONE) {
            return null;
        }
        $user = $DB->get_record('user', array('id' => $this->attemptobj->get_userid()));
        $userpicture = new user_picture($user);
        $userpicture->courseid = $this->attemptobj->get_courseid();
        if ($this->attemptobj->get_quizsbs()->showuserpicture == quizsbs_SHOWIMAGE_LARGE) {
    ...
    public function get_button_container_class() {
        // quizsbs navigation is set on 'Show all questions on one page'.
        if ($this->showall) {
            return 'allquestionsononepage';
        }
        // quizsbs navigation is set on 'Show one page at a time'.
        return 'multipages';
    }
}
...
/**
 * Specialisation of {@link quizsbs_nav_panel_base} for the attempt quizsbs page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class quizsbs_attempt_nav_panel extends quizsbs_nav_panel_base {
    ...
    public function render_before_button_bits(mod_quizsbs_renderer $output) {
        return html_writer::tag('div', get_string('navnojswarning', 'quizsbs'),
                array('id' => 'quizsbsnojswarning'));
    }
...
/**
 * Specialisation of {@link quizsbs_nav_panel_base} for the review quizsbs page.
 *
 * @copyright  2008 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since      Moodle 2.0
 */
class quizsbs_review_nav_panel extends quizsbs_nav_panel_base {
    public function get_question_url($slot) {
