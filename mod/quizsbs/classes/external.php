 ...
 * @since      Moodle 3.1
 */
class mod_quizsbs_external extends external_api {

    /**
     * Describes the parameters for get_quizsbszes_by_courses.
     *
     * @return external_external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_quizsbszes_by_courses_parameters() {
    ...
    /**
     * Returns a list of quizsbszes in a provided list of courses,
     * if no list is provided all quizsbszes that the user can view will be returned.
     *
     * @param array $courseids Array of course ids
     * @return array of quizsbszes details
     * @since Moodle 3.1
     */
    public static function get_quizsbszes_by_courses($courseids = array()) {
        global $USER;

        $warnings = array();
        $returnedquizsbszes = array();

        $params = array(
            'courseids' => $courseids,
        );
        $params = self::validate_parameters(self::get_quizsbszes_by_courses_parameters(), $params);
        ...
        if (!empty($params['courseids'])) {
            ...
            // Get the quizsbszes in this course, this function checks users visibility permissions.
            // We can avoid then additional validate_context calls.
            $quizsbszes = get_all_instances_in_courses("quizsbs", $courses);
            foreach ($quizsbszes as $quizsbs) {
                ...
                $quizsbs = quizsbs_update_effective_access($quizsbs, $USER->id);

                // Entry to return.
                $quizsbsdetails = array();
                // First, we return information that any user can see in the web interface.
                $quizsbsdetails['id'] = $quizsbs->id;
                $quizsbsdetails['coursemodule']      = $quizsbs->coursemodule;
                $quizsbsdetails['course']            = $quizsbs->course;
                $quizsbsdetails['name']              = external_format_string($quizsbs->name, $context->id);

                if (has_capability('mod/quizsbs:view', $context)) {
                    // Format intro.
                    list($quizsbsdetails['intro'], $quizsbsdetails['introformat']) = external_format_text($quizsbs->intro,
                                                                    $quizsbs->introformat, $context->id, 'mod_quizsbs', 'intro', null);

                    $viewablefields = array('timeopen', 'timeclose', 'grademethod', 'section', 'visible', 'groupmode',
                                            'groupingid');

                    $timenow = time();
                    $quizsbsobj = quizsbs::create($quizsbs->id, $USER->id);
                    $accessmanager = new quizsbs_access_manager($quizsbsobj, $timenow, has_capability('mod/quizsbs:ignoretimelimits',
                                                                $context, null, false));

                    // Fields the user could see if have access to the quizsbs.
                    if (!$accessmanager->prevent_access()) {
                        // Some times this function returns just empty.
                        $hasfeedback = quizsbs_has_feedback($quizsbs);
                        $quizsbsdetails['hasfeedback'] = (!empty($hasfeedback)) ? 1 : 0;
                        $quizsbsdetails['hasquestions'] = (int) $quizsbsobj->has_questions();
                        $quizsbsdetails['autosaveperiod'] = get_config('quizsbs', 'autosaveperiod');
                    ...
                    foreach ($viewablefields as $field) {
                        $quizsbsdetails[$field] = $quizsbs->{$field};
                    }
                }
                $returnedquizsbszes[] = $quizsbsdetails;
            }
        }
        $result = array();
        $result['quizsbszes'] = $returnedquizsbszes;
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_quizsbszes_by_courses return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_quizsbszes_by_courses_returns() {
        return new external_single_structure(
            array(
                'quizsbszes' => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            'id' => new external_value(PARAM_INT, 'Standard Moodle primary key.'),
                            'course' => new external_value(PARAM_INT, 'Foreign key reference to the course this quizsbs is part of.'),
                            'coursemodule' => new external_value(PARAM_INT, 'Course module id.'),
                            'name' => new external_value(PARAM_RAW, 'quizsbs name.'),
                            'intro' => new external_value(PARAM_RAW, 'quizsbs introduction text.', VALUE_OPTIONAL),
                            'introformat' => new external_format_value('intro', VALUE_OPTIONAL),
                            'timeopen' => new external_value(PARAM_INT, 'The time when this quizsbs opens. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timeclose' => new external_value(PARAM_INT, 'The time when this quizsbs closes. (0 = no restriction.)',
                                                                VALUE_OPTIONAL),
                            'timelimit' => new external_value(PARAM_INT, 'The time limit for quizsbs attempts, in seconds.',
                                                                VALUE_OPTIONAL),
                            ...
                            'canredoquestions' => new external_value(PARAM_INT, 'Allows students to redo any completed question
                                                                        within a quizsbs attempt.', VALUE_OPTIONAL),
                            ...
                            'attemptonlast' => new external_value(PARAM_INT, 'Whether subsequent attempts start from the answer
                                                                    ...
                                                                    VALUE_OPTIONAL),
                            'grademethod' => new external_value(PARAM_INT, 'One of the values quizsbs_GRADEHIGHEST, quizsbs_GRADEAVERAGE,
                                                                    quizsbs_ATTEMPTFIRST or quizsbs_ATTEMPTLAST.', VALUE_OPTIONAL),
                            ...
                            'reviewattempt' => new external_value(PARAM_INT, 'Whether users are allowed to review their quizsbs
                                                                    attempts at various times. This is a bit field, decoded by the
                                                                    mod_quizsbs_display_options class. It is formed by ORing together
                                                                    the constants defined there.', VALUE_OPTIONAL),
                            'reviewcorrectness' => new external_value(PARAM_INT, 'Whether users are allowed to review their quizsbs
                                                                        attempts at various times.
                                                                        A bit field, like reviewattempt.', VALUE_OPTIONAL),
                            'reviewmarks' => new external_value(PARAM_INT, 'Whether users are allowed to review their quizsbs attempts
                            ...
                            'sumgrades' => new external_value(PARAM_FLOAT, 'The total of all the question instance maxmarks.',
                                                                VALUE_OPTIONAL),
                            'grade' => new external_value(PARAM_FLOAT, 'The total that the quizsbs overall grade is scaled to be
                                                            out of.', VALUE_OPTIONAL),
                            'timecreated' => new external_value(PARAM_INT, 'The time when the quizsbs was added to the course.',
                                                                VALUE_OPTIONAL),
                            ...
                            'password' => new external_value(PARAM_RAW, 'A password that the student must enter before starting or
                                                                continuing a quizsbs attempt.', VALUE_OPTIONAL),
                            'subnet' => new external_value(PARAM_RAW, 'Used to restrict the IP addresses from which this quizsbs can
                            ...
                            'completionattemptsexhausted' => new external_value(PARAM_INT, 'Mark quizsbs complete when the student has
                            ...
                            'hasfeedback' => new external_value(PARAM_INT, 'Whether the quizsbs has any non-blank feedback text',
                                                                VALUE_OPTIONAL),
                            'hasquestions' => new external_value(PARAM_INT, 'Whether the quizsbs has questions', VALUE_OPTIONAL),
    ...
    /**
     * Utility function for validating a quizsbs.
     *
     * @param int $quizsbsid quizsbs instance id
     * @return array array containing the quizsbs, course, context and course module objects
     * @since  Moodle 3.1
     */
    protected static function validate_quizsbs($quizsbsid) {
        global $DB;

        // Request and permission validation.
        $quizsbs = $DB->get_record('quizsbs', array('id' => $quizsbsid), '*', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($quizsbs, 'quizsbs');

        $context = context_module::instance($cm->id);
        self::validate_context($context);

        return array($quizsbs, $course, $cm, $context);
    }

    /**
     * Describes the parameters for view_quizsbs.
     *
     * @return external_external_function_parameters
     * @since Moodle 3.1
     */
    public static function view_quizsbs_parameters() {
        return new external_function_parameters (
            array(
                'quizsbsid' => new external_value(PARAM_INT, 'quizsbs instance id'),
            )
        );
    }
     ...
     */
    public static function view_quizsbs($quizsbsid) {
        global $DB;

        $params = self::validate_parameters(self::view_quizsbs_parameters(), array('quizsbsid' => $quizsbsid));
        $warnings = array();

        list($quizsbs, $course, $cm, $context) = self::validate_quizsbs($params['quizsbsid']);
    ...
    /**
     * Describes the view_quizsbs return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function view_quizsbs_returns() {
        return new external_single_structure(
            array(
    ...
    public static function get_user_attempts_parameters() {
        return new external_function_parameters (
            array(
                ...
                'status' => new external_value(PARAM_ALPHA, 'quizsbs status: all, finished or unfinished', VALUE_DEFAULT, 'finished'),
    ...
    /**
     * Return a list of attempts for the given quizsbs and user.
     *
     * @param int $quizsbsid quizsbs instance id
     * @param int $userid user id
     * @param string $status quizsbs status: all, finished or unfinished
     * @param bool $includepreviews whether to include previews or not
     * @return array of warnings and the list of attempts
     * @since Moodle 3.1
     * @throws invalid_parameter_exception
     */
    public static function get_user_attempts($quizsbsid, $userid = 0, $status = 'finished', $includepreviews = false) {
        global $DB, $USER;
        ...
        if ($USER->id != $user->id) {
            require_capability('mod/quizsbs:viewreports', $context);
        }

        $attempts = quizsbs_get_user_attempts($quizsbs->id, $user->id, $params['status'], $params['includepreviews']);
    ...
    private static function attempt_structure() {
        return new external_single_structure(
            array(
                ...
                'quizsbs' => new external_value(PARAM_INT, 'Foreign key reference to the quizsbs that was attempted.',
                                                VALUE_OPTIONAL),
                'userid' => new external_value(PARAM_INT, 'Foreign key reference to the user whose attempt this is.',
                                                VALUE_OPTIONAL),
                'attempt' => new external_value(PARAM_INT, 'Sequentially numbers this students attempts at this quizsbs.',
                                                VALUE_OPTIONAL),
                'uniqueid' => new external_value(PARAM_INT, 'Foreign key reference to the question_usage that holds the
                                                    details of the the question_attempts that make up this quizsbs
                ...
                'timecheckstate' => new external_value(PARAM_INT, 'Next time quizsbs cron should check attempt for
    ...
    /**
     * Get the best current grade for the given user on a quizsbs.
     *
     * @param int $quizsbsid quizsbs instance id
     * @param int $userid user id
     ...
     * @since Moodle 3.1
     */
    public static function get_user_best_grade($quizsbsid, $userid = 0) {
        global $DB, $USER;
        ...
        $result = array();
        $grade = quizsbs_get_best_grade($quizsbs, $user->id);
    ...
    public static function get_user_best_grade_returns() {
        return new external_single_structure(
            array(
                'hasgrade' => new external_value(PARAM_BOOL, 'Whether the user has a grade on the given quizsbs.'),
     ...
     * @since Moodle 3.1
     */
    public static function get_combined_review_options($quizsbsid, $userid = 0) {
        global $DB, $USER;
        ...
        $attempts = quizsbs_get_user_attempts($quizsbs->id, $user->id, 'all', true);
    ...
    /**
     * Starts a new attempt at a quizsbs.
     *
     * @param int $quizsbsid quizsbs instance id
     ...
     */
    public static function start_attempt($quizsbsid, $preflightdata = array(), $forcenew = false) {
        global $DB, $USER;
        ...
        if (!$quizsbsobj->has_questions()) {
            throw new moodle_quizsbs_exception($quizsbsobj, 'noquestionsfound');
        }
        ...
        list($currentattemptid, $attemptnumber, $lastattempt, $messages, $page) =
            quizsbs_validate_new_attempt($quizsbsobj, $accessmanager, $forcenew, -1, false);
        ...
        if (!$quizsbsobj->is_preview_user() && $messages) {
            ...
            if ($accessmanager->is_preflight_check_required($currentattemptid)) {
                ...
                if (!empty($errors)) {
                    throw new moodle_quizsbs_exception($quizsbsobj, array_shift($errors));
                }
            ...
            if ($currentattemptid) {
                if ($lastattempt->state == quizsbs_attempt::OVERDUE) {
                    throw new moodle_quizsbs_exception($quizsbsobj, 'stateoverdue');
                } else {
                    throw new moodle_quizsbs_exception($quizsbsobj, 'attemptstillinprogress');
                }
            }
     ...
     * @param  bool $checkaccessrules whether to check the quizsbs access rules or not
    ...
    protected static function validate_attempt($params, $checkaccessrules = true, $failifoverdue = true) {
        ...
        $attemptobj = quizsbs_attempt::create($params['attemptid']);
        ...
        if ($checkaccessrules) {
            ...
            if (!$ispreviewuser && $messages) {
                throw new moodle_quizsbs_exception($attemptobj->get_quizsbsobj(), 'attempterror');
            }
        }

        // Attempt closed?.
        if ($attemptobj->is_finished()) {
            throw new moodle_quizsbs_exception($attemptobj->get_quizsbsobj(), 'attemptalreadyclosed');
        } else if ($failifoverdue && $attemptobj->get_state() == quizsbs_attempt::OVERDUE) {
            throw new moodle_quizsbs_exception($attemptobj->get_quizsbsobj(), 'stateoverdue');
        }

        // User submitted data (like the quizsbs password).
        if ($accessmanager->is_preflight_check_required($attemptobj->get_attemptid())) {
            ...
            if (!empty($errors)) {
                throw new moodle_quizsbs_exception($attemptobj->get_quizsbsobj(), array_shift($errors));
            }
            // Pre-flight check passed.
        ...
        if (isset($params['page'])) {
            ...
            if ($params['page'] != $attemptobj->force_page_number_into_range($params['page'])) {
                throw new moodle_quizsbs_exception($attemptobj->get_quizsbsobj(), 'Invalid page number');
            }

            // Prevent out of sequence access.
            if (!$attemptobj->check_page_access($params['page'])) {
                throw new moodle_quizsbs_exception($attemptobj->get_quizsbsobj(), 'Out of sequence access');
            }
    ...
    private static function question_structure() {
        return new external_single_structure(
            array(
                ...
                'page' => new external_value(PARAM_INT, 'page of the quizsbs this question appears on'),
                ...
                'number' => new external_value(PARAM_INT, 'question ordering number in the quizsbs', VALUE_OPTIONAL),
     ...
     *
     * @param  quizsbs_attempt  $attemptobj  the quizsbs attempt object
     * @param  bool  $review  whether if we are in review mode or not
     * @param  mixed  $page  string 'all' or integer page number
     * @return array array of questions including data
     */
    private static function get_attempt_questions_data(quizsbs_attempt $attemptobj, $review, $page = 'all') {
        global $PAGE;

        $questions = array();
        $contextid = $attemptobj->get_quizsbsobj()->get_context()->id;
        $displayoptions = $attemptobj->get_display_options($review);
        $renderer = $PAGE->get_renderer('mod_quizsbs');
    ...
    /**
     * Returns information for the given attempt page for a quizsbs attempt in progress.
     *
     ...
     * @since Moodle 3.1
     * @throws moodle_quizsbs_exceptions
     */
        ...
        return new external_single_structure(
            array(
                ...
                'messages' => new external_multiple_structure(
                    ...
                    'access messages, will only be returned for users with mod/quizsbs:preview capability,
    ...
    /**
     * Returns a summary of a quizsbs attempt before it is submitted.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Processes save requests during the quizsbs. This function is intended for the quizsbs auto-save feature.
     *
     * @param int $attemptid attempt id
    ...
    /**
     * Process responses during an attempt at a quizsbs and also deals with attempts finishing.
     *
     * @param int $attemptid attempt id
     * @param array $data the data to be saved
    ...
    protected static function validate_attempt_review($params) {
        ...
        if ($attemptobj->is_own_attempt()) {
            if (!$attemptobj->is_finished()) {
                throw new moodle_quizsbs_exception($attemptobj->get_quizsbsobj(), 'attemptclosed');
            } else if (!$displayoptions->attempt) {
                throw new moodle_exception($attemptobj->cannot_review_message());
            }
        } else if (!$attemptobj->is_review_allowed()) {
            throw new moodle_quizsbs_exception($attemptobj->get_quizsbsobj(), 'noreviewattempt');
        }
    ...
    public static function get_attempt_review($attemptid, $page = -1) {
        ...
        foreach ($summarydata as $key => $data) {
            ...
            $result['additionaldata'][] = array(
                'id' => $key,
                'title' => $data['title'], $attemptobj->get_quizsbsobj()->get_context()->id,
                'content' => $data['content'],
            );
        }

        // Feedback if there is any, and the user is allowed to see it now.
        $grade = quizsbs_rescale_grade($attemptobj->get_attempt()->sumgrades, $attemptobj->get_quizsbs(), false);

        $feedback = $attemptobj->get_overall_feedback($grade);
        if ($displayoptions->overallfeedback && $feedback) {
            $result['additionaldata'][] = array(
                'id' => 'feedback',
                'title' => get_string('feedback', 'quizsbs'),
                'content' => $feedback,
            );
        }
    ...
    public static function get_attempt_review_returns() {
        return new external_single_structure(
            array(
                'grade' => new external_value(PARAM_RAW, 'grade for the quizsbs (or empty or "notyetgraded")'),
                'attempt' => self::attempt_structure(),
     ...
     * @since Moodle 3.1
     */
    public static function get_quizsbs_feedback_for_grade_parameters() {
        return new external_function_parameters (
            array(
                'quizsbsid' => new external_value(PARAM_INT, 'quizsbs instance id'),
    ...
    /**
     * Get the feedback text that should be show to a student who got the given grade in the given quizsbs.
     *
     * @param int $quizsbsid quizsbs instance id
     ...
     * @throws moodle_exception
     */
    public static function get_quizsbs_feedback_for_grade($quizsbsid, $grade) {
        global $DB;
        ...
        $params = self::validate_parameters(self::get_quizsbs_feedback_for_grade_parameters(), $params);
        $warnings = array();
    ...
    /**
     * Describes the get_quizsbs_feedback_for_grade return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_quizsbs_feedback_for_grade_returns() {
        return new external_single_structure(
            array(
    ...
    /**
     * Describes the parameters for get_quizsbs_access_information.
     *
     * @return external_external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_quizsbs_access_information_parameters() {
        return new external_function_parameters (
            array(
                'quizsbsid' => new external_value(PARAM_INT, 'quizsbs instance id')
            )
    ...
    /**
     * Return access information for a given quizsbs.
     *
     * @param int $quizsbsid quizsbs instance id
     ...
     * @throws  moodle_quizsbs_exception
     */
    public static function get_quizsbs_access_information($quizsbsid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_quizsbs_access_information_parameters(), $params);

        list($quizsbs, $course, $cm, $context) = self::validate_quizsbs($params['quizsbsid']);

        $result = array();
        // Capabilities first.
        $result['canattempt'] = has_capability('mod/quizsbs:attempt', $context);;
        $result['canmanage'] = has_capability('mod/quizsbs:manage', $context);;
        $result['canpreview'] = has_capability('mod/quizsbs:preview', $context);;
        $result['canreviewmyattempts'] = has_capability('mod/quizsbs:reviewmyattempts', $context);;
        $result['canviewreports'] = has_capability('mod/quizsbs:viewreports', $context);;

        // Access manager now.
        $quizsbsobj = quizsbs::create($cm->instance, $USER->id);
        $ignoretimelimits = has_capability('mod/quizsbs:ignoretimelimits', $context, null, false);
        $timenow = time();
        $accessmanager = new quizsbs_access_manager($quizsbsobj, $timenow, $ignoretimelimits);
    ...
    /**
     * Describes the get_quizsbs_access_information return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_quizsbs_access_information_returns() {
        return new external_single_structure(
            array(
                'canattempt' => new external_value(PARAM_BOOL, 'Whether the user can do the quizsbs or not.'),
                'canmanage' => new external_value(PARAM_BOOL, 'Whether the user can edit the quizsbs settings or not.'),
                'canpreview' => new external_value(PARAM_BOOL, 'Whether the user can preview the quizsbs or not.'),
                'canreviewmyattempts' => new external_value(PARAM_BOOL, 'Whether the users can review their previous attempts
                                                                or not.'),
                'canviewreports' => new external_value(PARAM_BOOL, 'Whether the user can view the quizsbs reports or not.'),
    ...
    /**
     * Return access information for a given attempt in a quizsbs.
     *
     * @param int $quizsbsid quizsbs instance id
     ...
     * @throws  moodle_quizsbs_exception
     */
    public static function get_attempt_access_information($quizsbsid, $attemptid = 0) {
        global $DB, $USER;
        ...
        if ($unfinishedattempt = quizsbs_get_user_attempt_unfinished($quizsbs->id, $USER->id)) {
            ...
            if ($unfinishedattempt->state != quizsbs_attempt::IN_PROGRESS and $unfinishedattempt->state != quizsbs_attempt::OVERDUE) {
    ...
    /**
     * Describes the parameters for get_quizsbs_required_qtypes.
     *
     * @return external_external_function_parameters
     * @since Moodle 3.1
     */
    public static function get_quizsbs_required_qtypes_parameters() {
        return new external_function_parameters (
            array(
                'quizsbsid' => new external_value(PARAM_INT, 'quizsbs instance id')
            )
    ...
    /**
     * Return the potential question types that would be required for a given quizsbs.
     ...
     * @throws  moodle_quizsbs_exception
     */
    public static function get_quizsbs_required_qtypes($quizsbsid) {
        global $DB, $USER;
        ...
        $params = self::validate_parameters(self::get_quizsbs_required_qtypes_parameters(), $params);
        ...
        // Question types used.
        $result = array();
        $result['questiontypes'] = $quizsbsobj->get_all_question_types_used(true);
        $result['warnings'] = $warnings;
        return $result;
    }

    /**
     * Describes the get_quizsbs_required_qtypes return value.
     *
     * @return external_single_structure
     * @since Moodle 3.1
     */
    public static function get_quizsbs_required_qtypes_returns() {
        return new external_single_structure(
            array(
                'questiontypes' => new external_multiple_structure(
                                    new external_value(PARAM_PLUGIN, 'question type'), 'list of question types used in the quizsbs'),
                'warnings' => new external_warnings(),
            )
        );
    }
    ...
    public static function load_connected_content_parameters(){
        return new external_function_parameters(array(
            'id' => new external_value(PARAM_INT, 'additional content id'),
        ));
    }

    public static function load_connected_content($id) {
        global $DB;

        $contents = array();
        $additionalcontent = new additional_content($id);
        $subject = $DB->get_records('quizsbs_subject', array('quizsbsid' => $additionalcontent->get_quizsbsid()));
        $contents['subjects'] = array_values($subject);
        $contents['question'] = array_values($additionalcontent->get_avilable_questions());
        foreach ($contents['question'] as $key => $slot) {
            $question = $DB->get_record('question', array('id' => $slot->questionid));
            $contents['question'][$key]->name = $question->name;
            $contents['question'][$key]->id = $question->id;
        }
        $contents['additionalcontent'] = new stdClass();
        $contents['additionalcontent']->subjectid = $additionalcontent->get_subjectid();
        return $contents;
    }

    public static function load_connected_content_returns() {
        return new external_single_structure(array(
            'subjects' => new external_multiple_structure(new external_single_structure(array(
                'id' => new external_value(PARAM_INT),
                'name' => new external_value(PARAM_TEXT),
                'quizsbsid' => new external_value(PARAM_INT, 'quizsbs id', false, null, true),
            ))),
            'question' => new external_multiple_structure(new external_single_structure(array(
                'id' => new external_value(PARAM_INT),
                'maxmark' => new external_value(PARAM_FLOAT, 'Max question grade', false, 0, true),
                'page' => new external_value(PARAM_INT, 'Page to show question', false, 0, true),
                'additionalcontentid' => new external_value(PARAM_INT, 'additional content id', false, null, true),
                'questionid' => new external_value(PARAM_INT),
                'quizsbsid' => new external_value(PARAM_INT),
                'requireprevious' => new external_value(PARAM_INT, 'is there is previous question', false, 0, true),
                'slot' => new external_value(PARAM_INT, 'the slot number', false, 0, true),
                'name' => new external_value(PARAM_TEXT),
            ))),
            'additionalcontent' => new external_single_structure(array(
                'subjectid' => new external_value(PARAM_INT, 'the connected subject', false, null, true),
            )),
        ));
    }
