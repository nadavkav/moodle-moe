...
class edit_renderer extends \plugin_renderer_base {
     ...
     *
     * @param \quizsbs $quizsbsobj object containing all the quizsbs settings information.
     * @param structure $structure object containing the structure of the quizsbs.
     * @param \question_edit_contexts $contexts the relevant question bank contexts.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @param array $pagevars the variables from {@link question_edit_setup()}.
     * @return string HTML to output.
     */
    public function edit_page(\quizsbs $quizsbsobj, structure $structure,
            \question_edit_contexts $contexts, \moodle_url $pageurl, array $pagevars) {
        $output = '';

        // Page title.
        $output .= $this->heading_with_help(get_string('editingquizsbsx', 'quizsbs',
                format_string($quizsbsobj->get_quizsbs_name())), 'editingquizsbs', 'quizsbs', '',
                get_string('basicideasofquizsbs', 'quizsbs'), 2);

        // Information at the top.
        $output .= $this->quizsbs_state_warnings($structure);
        $output .= $this->quizsbs_information($structure);
        $output .= $this->maximum_grade_input($structure, $pageurl);
        $output .= $this->repaginate_button($structure, $pageurl);
        $output .= $this->total_marks($quizsbsobj->get_quizsbs());
        ...
        if ($structure->can_be_edited()) {
            ...
            $this->page->requires->yui_module('moodle-mod_quizsbs-quizsbsquestionbank',
                    'M.mod_quizsbs.quizsbsquestionbank.init',
                    array('class' => 'questionbank', 'cmid' => $structure->get_cmid()));

            $popups .= $this->random_question_form($pageurl, $contexts, $pagevars);
            $this->page->requires->yui_module('moodle-mod_quizsbs-randomquestion',
                    'M.mod_quizsbs.randomquestion.init');

            $output .= html_writer::div($popups, 'mod_quizsbs_edit_forms');

            // Include the question chooser.
            $output .= $this->question_chooser();
            $this->page->requires->yui_module('moodle-mod_quizsbs-questionchooser', 'M.mod_quizsbs.init_questionchooser');
        }
    ...
    /**
     * Render any warnings that might be required about the state of the quizsbs,
     * e.g. if it has been attempted, or if the shuffle questions option is
     * turned on.
     *
     * @param structure $structure the quizsbs structure.
     * @return string HTML to output.
     */
    public function quizsbs_state_warnings(structure $structure) {
     ...
     * @return string HTML to output.
     */
    public function quizsbs_information(structure $structure) {
        list($currentstatus, $explanation) = $structure->get_dates_summary();

        $output = html_writer::span(
                    get_string('numquestionsx', 'quizsbs', $structure->get_question_count()),
                    'numberofquestions') . ' | ' .
                html_writer::span($currentstatus, 'quizsbsopeningstatus',
    ...
    /**
     * Render the form for setting a quizsbs' overall grade
     *
     * @param structure $structure the quizsbs structure.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
     ...
     * @param structure $structure the structure of the quizsbs being edited.
     * @param \moodle_url $pageurl the canonical URL of this page.
     * @return string HTML to output.
     */
    protected function repaginate_button(structure $structure, \moodle_url $pageurl) {

        $header = html_writer::tag('span', get_string('repaginatecommand', 'quizsbs'), array('class' => 'repaginatecommand'));
        ...
        $buttonoptions = array(
            ...
            'value' => get_string('repaginatecommand', 'quizsbs'),
        );
        if (!$structure->can_be_repaginated()) {
            $buttonoptions['disabled'] = 'disabled';
        } else {
            $this->page->requires->yui_module('moodle-mod_quizsbs-repaginate', 'M.mod_quizsbs.repaginate.init');
        }
    ...
    protected function repaginate_form(structure $structure, \moodle_url $pageurl) {
        $perpage = array();
        $perpage[0] = get_string('allinone', 'quizsbs');
        for ($i = 1; $i <= 50; ++$i) {
            $perpage[$i] = $i;
        }
        ...
        $formcontent = html_writer::tag('form', html_writer::div(
                    ...
                    get_string('repaginate', 'quizsbs', $select) .
    ...
    /**
     * Render the total marks available for the quizsbs.
     *
     * @param \stdClass $quizsbs the quizsbs settings from the database.
     * @return string HTML to output.
     */
    public function total_marks($quizsbs) {
        $totalmark = html_writer::span(quizsbs_format_grade($quizsbs, $quizsbs->sumgrades), 'mod_quizsbs_summarks');
        return html_writer::tag('span',
                get_string('totalmarksx', 'quizsbs', $totalmark),
     ...
     *
     * @param structure $structure the structure of the quizsbs being edited.
     * @param \stdClass $section The quizsbs_section entry from DB
     * @return string HTML to output.
     */
    protected function start_section($structure, $section) {
        ...
        if (!$structure->can_be_edited()) {
            ...
            $editsectionheadingicon = html_writer::link(new \moodle_url('#'),
                $this->pix_icon('t/editstring', get_string('sectionheadingedit', 'quizsbs', $section->heading),
     ...
     *
     * @param structure $structure object containing the structure of the quizsbs.
     * @param \stdClass $section data from the quizsbs_section table.
     * @return string HTML to output.
     */
    public function section_shuffle_questions(structure $structure, $section) {
        ...
        if ($structure->is_first_section($section)) {
            $help = $this->help_icon('shufflequestions', 'quizsbs');
        } else {
            $help = '';
        }
        ...
        $label = html_writer::label(get_string('shufflequestions', 'quizsbs') . ' ' . $help,
    ...
    /**
     * Render an icon to remove a section from the quizsbs.
     *
     * @param object $section the section to be removed.
     * @return string HTML to output.
     */
    public function section_remove_icon($section) {
        $title = get_string('sectionheadingremove', 'quizsbs', $section->heading);
        $url = new \moodle_url('/mod/quizsbs/edit.php',
    ...
    /**
     * Renders HTML to display the questions in a section of the quizsbs.
     *
     * This function calls {@link core_course_renderer::quizsbs_section_question()}
     *
     * @param structure $structure object containing the structure of the quizsbs.
    ...
    public function question_row(structure $structure, $slot, $contexts, $pagevars, $pageurl) {
        ...
        $joinhtml = '';
        if ($structure->can_be_edited() && !$structure->is_last_slot_in_quizsbs($slot) &&
    ...
    public function add_menu_actions(structure $structure, $page, \moodle_url $pageurl,
        ...
        $menu->set_constraint('.mod-quizsbs-edit-content');
        $trigger = html_writer::tag('span', get_string('add', 'quizsbs'), array('class' => 'add-menu'));
        ...
        // Disable the link if quizsbs has attempts.
        if (!$structure->can_be_edited()) {
    ...
    public function edit_menu_actions(structure $structure, $page,
        ...
        if (!isset($str)) {
            $str = get_strings(array('addasection', 'addaquestion', 'addarandomquestion',
                    'addarandomselectedquestion', 'questionbank'), 'quizsbs');
        }
        ...
        // Add a new question to the quizsbs.
        ...
        if ($page) {
            $title = get_string('addquestionfrombanktopage', 'quizsbs', $page);
        } else {
            $title = get_string('addquestionfrombankatend', 'quizsbs');
        }
        ...
        $returnurl = new \moodle_url('/mod/quizsbs/edit.php', array('cmid' => $structure->get_cmid(), 'data-addonpage' => $page));
        $params = array('returnurl' => $returnurl, 'cmid' => $structure->get_cmid(), 'appendqnumstring' => 'addarandomquestion');
        $url = new \moodle_url('/mod/quizsbs/addrandom.php', $params);
        ...
        if ($page) {
            $title = get_string('addrandomquestiontopage', 'quizsbs', $page);
        } else {
            $title = get_string('addrandomquestionatend', 'quizsbs');
        }
    ...
    /**
     * Render the form that contains the data for adding a new question to the quizsbs.
     *
     * @param structure $structure object containing the structure of the quizsbs.
     * @param int $page the page number that this menu will add to.
     * @param \moodle_url $pageurl the canonical URL of this page.
    ...
    public function question(structure $structure, $slot, \moodle_url $pageurl) {
        ...
        // Action icons.
        $questionicons = '';
        $questionicons .= $this->question_preview_icon($structure->get_quizsbs(), $structure->get_question_in_slot($slot));
        if ($structure->can_be_edited()) {
     ...
     *
     * @param \stdClass $quizsbs the quizsbs settings from the database.
     * @param \stdClass $question data from the question and quizsbs_slots tables.
     * @param bool $label if true, show the preview question label after the icon
     * @param int $variant which question variant to preview (optional).
     * @return string HTML to output.
     */
    public function question_preview_icon($quizsbs, $question, $label = null, $variant = null) {
        $url = quizsbs_question_preview_url($quizsbs, $question, $variant);

        // Do we want a label?
        $strpreviewlabel = '';
        if ($label) {
            $strpreviewlabel = ' ' . get_string('preview', 'quizsbs');
        }

        // Build the icon.
        $strpreviewquestion = get_string('previewquestion', 'quizsbs');
    ...
    /**
     * Render an icon to remove a question from the quizsbs.
     *
     * @param structure $structure object containing the structure of the quizsbs.
     * @param int $slot the first slot on the page we are outputting.
    ...
    /**
     * Display an icon to split or join two pages of the quizsbs.
     *
     * @param structure $structure object containing the structure of the quizsbs.
     * @param int $slot the first slot on the page we are outputting.
     * @return string HTML to output.
    ...
    public function page_split_join_button($structure, $slot) {
        ...
        $url = new \moodle_url('repaginate.php', array('quizsbsid' => $structure->get_quizsbsid(),
                'slot' => $slot, 'repag' => $insertpagebreak ? 2 : 1, 'sesskey' => sesskey()));

        if ($insertpagebreak) {
            $title = get_string('addpagebreak', 'quizsbs');
            $image = $this->pix_icon('e/insert_page_break', $title);
            $action = 'addpagebreak';
        } else {
            $title = get_string('removepagebreak', 'quizsbs');
    ...
    public function question_dependency_icon($structure, $slot) {
        ...
        if ($structure->is_question_dependent_on_previous_slot($slot)) {
            $title = get_string('questiondependencyremove', 'quizsbs', $a);
            $image = $this->pix_icon('t/locked', get_string('questiondependsonprevious', 'quizsbs'),
                    'moodle', array('title' => ''));
            $action = 'removedependency';
        } else {
            $title = get_string('questiondependencyadd', 'quizsbs', $a);
            $image = $this->pix_icon('t/unlocked', get_string('questiondependencyfree', 'quizsbs'),
                    'moodle', array('title' => ''));
    ...
    /**
     * Renders html to display a name with the link to the question on a quizsbs edit page
     *
    ...
    public function question_name(structure $structure, $slot, $pageurl) {
        ...
        $instancename = quizsbs_question_tostring($question);
        ...
        $output .= html_writer::link($editurl, $activitylink,
                array('title' => get_string('editquestion', 'quizsbs').' '.$title));
    ...
    public function random_question(structure $structure, $slot, $pageurl) {
        ...
        $temp = clone($question);
        $temp->questiontext = '';
        $instancename = quizsbs_question_tostring($temp);

        $configuretitle = get_string('configurerandomquestion', 'quizsbs');
        $qtype = \question_bank::get_qtype($question->qtype, false);
        $namestr = $qtype->local_name();
        $icon = $this->pix_icon('icon', $namestr, $qtype->plugin_name(), array('title' => $namestr,
                'class' => 'icon activityicon', 'alt' => ' ', 'role' => 'presentation'));
        ...
        $qbanklink = ' ' . \html_writer::link($qbankurl,
                get_string('seequestions', 'quizsbs'), array('class' => 'mod_quizsbs_random_qbank_link'));
    ...
    public function marked_out_of_field(structure $structure, $slot) {
        ...
        $output = html_writer::span($structure->formatted_question_grade($slot),
                'instancemaxmark decimalplaces_' . $structure->get_decimal_places_for_question_marks(),
                array('title' => get_string('maxmark', 'quizsbs')));
        ...
        $output .= html_writer::span(
            html_writer::link(
                ...
                array(
                    ...
                    'title' => get_string('editmaxmark', 'quizsbs'),
                )
            )
        );
    ...
    protected function random_question_form(\moodle_url $thispageurl, \question_edit_contexts $contexts, array $pagevars) {
        ...
        $randomform = new \quizsbs_add_random_form(new \moodle_url('/mod/quizsbs/addrandom.php'),
    ...
    protected function initialise_editing_javascript(structure $structure,
        ...
        $config = new \stdClass();
        $config->resourceurl = '/mod/quizsbs/edit_rest.php';
        $config->sectionurl = '/mod/quizsbs/edit_rest.php';
        ...
        $this->page->requires->yui_module('moodle-mod_quizsbs-toolboxes',
                'M.mod_quizsbs.init_resource_toolbox',
                array(array(
                        'courseid' => $structure->get_courseid(),
                        'quizsbsid' => $structure->get_quizsbsid(),
        ...
        $this->page->requires->yui_module('moodle-mod_quizsbs-toolboxes',
                'M.mod_quizsbs.init_section_toolbox',
                array(array(
                        'courseid' => $structure,
                        'quizsbsid' => $structure->get_quizsbsid(),
        ...
        $this->page->requires->yui_module('moodle-mod_quizsbs-dragdrop', 'M.mod_quizsbs.init_section_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'quizsbsid' => $structure->get_quizsbsid(),
                        'ajaxurl' => $config->sectionurl,
        ...
        $this->page->requires->yui_module('moodle-mod_quizsbs-dragdrop', 'M.mod_quizsbs.init_resource_dragdrop',
                array(array(
                        'courseid' => $structure,
                        'quizsbsid' => $structure->get_quizsbsid(),
                        'ajaxurl' => $config->resourceurl,
    ...
    protected function new_page_template(structure $structure,
        ...
        $strcontexts[] = 'addonpage="';
        $strcontexts[] = get_string('addquestionfrombanktopage', 'quizsbs', '');
     ...
     *
     * @param \mod_quizsbs\question\bank\custom_view $questionbank the question bank view object.
     * @param array $pagevars the variables from {@link \question_edit_setup()}.
     * @return string HTML to output / send back in response to an AJAX request.
     */
    public function question_bank_contents(\mod_quizsbs\question\bank\custom_view $questionbank, array $pagevars) {
