...
/**
 * Defines the custom question bank view used on the Edit quizsbs page.
 *
 * @package   mod_quizsbs
 * @category  question
...
namespace mod_quizsbs\question\bank;
defined('MOODLE_INTERNAL') || die();


/**
 * Subclass to customise the view of the question bank for the quizsbs editing screen.
 *
 * @copyright  2009 Tim Hunt
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class custom_view extends \core_question\bank\view {
    /** @var bool whether the quizsbs this is used by has been attemptd. */
    protected $quizsbshasattempts = false;
    /** @var \stdClass the quizsbs settings. */
    protected $quizsbs = false;
     ...
     * @param \stdClass $quizsbs quizsbs settings.
     */
    public function __construct($contexts, $pageurl, $course, $cm, $quizsbs) {
    ...
    protected function wanted_columns() {
        ...
        if (empty($CFG->quizsbsquestionbankcolumns)) {
            $quizsbsquestionbankcolumns = array(
                'add_action_column',
                'checkbox_column',
            ...
            $quizsbsquestionbankcolumns = explode(',', $CFG->quizsbsquestionbankcolumns);
        }

        foreach ($quizsbsquestionbankcolumns as $fullname) {
            if (!class_exists($fullname)) {
                if (class_exists('mod_quizsbs\\question\\bank\\' . $fullname)) {
                    $fullname = 'mod_quizsbs\\question\\bank\\' . $fullname;
                    ...
                    debugging('Legacy question bank column class question_bank_' .
                            $fullname . ' should be renamed to mod_quizsbs\\question\\bank\\' .
    ...
    protected function heading_column() {
        return 'mod_quizsbs\\question\\bank\\question_name_text_column';
    }

    protected function default_sort() {
        return array(
            'core_question\\bank\\question_type_column' => 1,
            'mod_quizsbs\\question\\bank\\question_name_text_column' => 1,
        );
    }

    /**
     * Let the question bank display know whether the quizsbs has been attempted,
     * hence whether some bits of UI, like the add this question to the quizsbs icon,
     * should be displayed.
     * @param bool $quizsbshasattempts whether the quizsbs has attempts.
     */
    public function set_quizsbs_has_attempts($quizsbshasattempts) {
        $this->quizsbshasattempts = $quizsbshasattempts;
        if ($quizsbshasattempts && isset($this->visiblecolumns['addtoquizsbsaction'])) {
            unset($this->visiblecolumns['addtoquizsbsaction']);
        }
    }

    public function preview_question_url($question) {
        return quizsbs_question_preview_url($this->quizsbs, $question);
    }

    public function add_to_quizsbs_url($questionid) {
        global $CFG;
        $params = $this->baseurl->params();
        $params['addquestion'] = $questionid;
        $params['sesskey'] = sesskey();
        return new \moodle_url('/mod/quizsbs/edit.php', $params);
    }
    ...
    protected function display_bottom_controls($totalnumber, $recurse, $category, \context $catcontext, array $addcontexts) {
        ...
        $cmoptions->hasattempts = !empty($this->quizsbshasattempts);
        ...
        if ($canuseall) {
            ...
            // Add selected questions to the quizsbs.
            $params = array(
                    'type' => 'submit',
                    'name' => 'add',
                    'value' => get_string('addselectedquestionstoquizsbs', 'quizsbs'),
            );
    ...
    protected function display_options_form($showquestiontext, $scriptpath = '/mod/quizsbs/edit.php',
    ...
    protected function print_category_info($category) {
        ...
        $strcategory = get_string('category', 'quizsbs');
