...
/**
 * Defines the quizsbs repaginate class.
 *
 * @package   mod_quizsbs
 ...
 *
 * The quizsbs setting allows users to write quizsbszes with one question per page,
...
class repaginate {
    ...
    /** @var int the id of the quizsbs being manipulated. */
    private $quizsbsid;
    /** @var array the quizsbs_slots for that quizsbs. */
    private $slots;

    /**
     * Constructor.
     * @param int $quizsbsid the id of the quizsbs being manipulated.
     * @param stdClass[] $slots the quizsbs_slots for that quizsbs.
     */
    public function __construct($quizsbsid = 0, $slots = null) {
        ...
        if (!$slots) {
            $this->slots = $DB->get_records('quizsbs_slots', array('quizsbsid' => $this->quizsbsid), 'slot');
        } else {
            $this->slots = $slots;
        }
    }
    ...
    public function repaginate_slots($nextslotnumber, $type) {
        ...
        foreach ($this->slots as $slot) {
            if ($slot->slot < $nextslotnumber) {
                ...
                // Update DB.
                $DB->update_record('quizsbs_slots', $nextslot, true);
    ...
    /**
     * Repaginate the rest.
     * @param stdClass[] $quizsbsslots
     * @param int $slotfrom
     * @param int $type
     * @param bool $dbupdate
     * @return stdClass[]
     */
    public function repaginate_the_rest($quizsbsslots, $slotfrom, $type, $dbupdate = true) {
        global $DB;
        if (!$quizsbsslots) {
            return null;
        }
        $newslots = array();
        foreach ($quizsbsslots as $slot) {
            if ($type == self::LINK) {
