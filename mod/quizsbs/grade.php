...
$cm = get_coursemodule_from_id('quizsbs', $id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$quizsbs = $DB->get_record('quizsbs', array('id' => $cm->instance), '*', MUST_EXIST);
require_login($course, false, $cm);

$reportlist = quizsbs_report_list(context_module::instance($cm->id));
if (empty($reportlist) || $userid == $USER->id) {
    ...
    // appear in the gradebook are unlikely to be able to see quizsbs reports,
    // but it is possible.)
    redirect(new moodle_url('/mod/quizsbs/view.php', array('id' => $cm->id)));
}
...
if ($userid) {
    ...
    $attempts = quizsbs_get_user_attempts($quizsbs->id, $userid, 'finished');
    $attempt = null;
    ...
    if ($attempt) {
        $attemptobj = new quizsbs_attempt($attempt, $quizsbs, $cm, $course, false);
...
redirect(new moodle_url('/mod/quizsbs/report.php', array(
