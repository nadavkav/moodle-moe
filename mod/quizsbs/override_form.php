...
/**
 * Settings form for overrides in the quizsbs module.
 *
 * @package    mod_quizsbs
...
require_once($CFG->dirroot . '/mod/quizsbs/mod_form.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizsbs_override_form extends moodleform {

    /** @var object course module object. */
    protected $cm;

    /** @var object the quizsbs settings object. */
    protected $quizsbs;
     ...
     * @param object the quizsbs settings object.
     ...
     */
    public function __construct($submiturl, $cm, $quizsbs, $context, $groupmode, $override) {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'quizsbs'));
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'quizsbs'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/quizsbs/overrides.php', array('cmid'=>$cm->id));
                    print_error('groupsnone', 'quizsbs', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'quizsbs'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                $users = get_users_by_capability($this->context, 'mod/quizsbs:attempt',
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/quizsbs/overrides.php', array('cmid'=>$cm->id));
                    print_error('usersnone', 'quizsbs', $link);
                }
        ...
        $mform->addElement('passwordunmask', 'password', get_string('requirepassword', 'quizsbs'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'requirepassword', 'quizsbs');
        $mform->setDefault('password', $this->quizsbs->password);

        // Open and close dates.
        $mform->addElement('date_time_selector', 'timeopen',
                get_string('quizsbsopen', 'quizsbs'), mod_quizsbs_mod_form::$datefieldoptions);
        $mform->setDefault('timeopen', $this->quizsbs->timeopen);

        $mform->addElement('date_time_selector', 'timeclose',
                get_string('quizsbsclose', 'quizsbs'), mod_quizsbs_mod_form::$datefieldoptions);
        $mform->setDefault('timeclose', $this->quizsbs->timeclose);

        // Time limit.
        $mform->addElement('duration', 'timelimit',
                get_string('timelimit', 'quizsbs'), array('optional' => true));
        $mform->addHelpButton('timelimit', 'timelimit', 'quizsbs');
        $mform->setDefault('timelimit', $this->quizsbs->timelimit);
        ...
        $mform->addElement('select', 'attempts',
                get_string('attemptsallowed', 'quizsbs'), $attemptoptions);
        $mform->setDefault('attempts', $this->quizsbs->attempts);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'quizsbs'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'quizsbs'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'quizsbs'));
    ...
    public function validation($data, $files) {
        ...
        // Ensure that at least one quizsbs setting was changed.
        $changed = false;
        $keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
        foreach ($keys as $key) {
            if ($data[$key] != $quizsbs->{$key}) {
                $changed = true;
                break;
            }
        }
        if (!$changed) {
            $errors['timeopen'] = get_string('nooverridedata', 'quizsbs');
        }
