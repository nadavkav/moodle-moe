...
/**
 * This page handles editing and creation of quizsbs overrides
 *
 * @package   mod_quizsbs
...
if ($overrideid) {
    ...
    if (! $override = $DB->get_record('quizsbs_overrides', array('id' => $overrideid))) {
        print_error('invalidoverrideid', 'quizsbs');
    }
    if (! $quizsbs = $DB->get_record('quizsbs', array('id' => $override->quizsbs))) {
...
$url = new moodle_url('/mod/quizsbs/overrideedit.php');
if ($action) {
...
// Merge quizsbs defaults with data.
$keys = array('timeopen', 'timeclose', 'timelimit', 'attempts', 'password');
foreach ($keys as $key) {
    if (!isset($data->{$key}) || $reset) {
        $data->{$key} = $quizsbs->{$key};
    }
}
...
$overridelisturl = new moodle_url('/mod/quizsbs/overrides.php', array('cmid'=>$cm->id));
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new quizsbs_override_form($url, $cm, $quizsbs, $context, $groupmode, $override);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    // Process the data.
    $fromform->quizsbs = $quizsbs->id;

    // Replace unchanged values with null.
    foreach ($keys as $key) {
        if ($fromform->{$key} == $quizsbs->{$key}) {
            $fromform->{$key} = null;
        }
    }
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('quizsbs_overrides', $conditions)) {
            ...
            // Set the course module id before calling quizsbs_delete_override().
            $quizsbs->cmid = $cm->id;
            quizsbs_delete_override($quizsbs, $oldoverride->id);
        }
    }
    ...
    if (!empty($override->id)) {
        ...
        $DB->update_record('quizsbs_overrides', $fromform);
        ...
        unset($fromform->id);
        $fromform->id = $DB->insert_record('quizsbs_overrides', $fromform);
    ...
    quizsbs_update_events($quizsbs, $fromform);
...
// Print the form.
$pagetitle = get_string('editoverride', 'quizsbs');
