...
/**
 * This page handles listing of quizsbs overrides
 *
 * @package    mod_quizsbs
...
require_once($CFG->dirroot.'/mod/quizsbs/override_form.php');


$cmid = required_param('cmid', PARAM_INT);
$mode = optional_param('mode', '', PARAM_ALPHA); // One of 'user' or 'group', default is 'group'.

list($course, $cm) = get_course_and_cm_from_cmid($cmid, 'quizsbs');
...
$url = new moodle_url('/mod/quizsbs/overrides.php', array('cmid'=>$cm->id, 'mode'=>$mode));
...
require_capability('mod/quizsbs:manageoverrides', $context);

// Display a list of overrides.
$PAGE->set_pagelayout('admin');
$PAGE->set_title(get_string('overrides', 'quizsbs'));
$PAGE->set_heading($course->fullname);
echo $OUTPUT->header();
echo $OUTPUT->heading(format_string($quizsbs->name, true, array('context' => $context)));

// Delete orphaned group overrides.
$sql = 'SELECT o.id
            FROM {quizsbs_overrides} o LEFT JOIN {groups} g
            ON o.groupid = g.id
...
if (!empty($orphaned)) {
    $DB->delete_records_list('quizsbs_overrides', 'id', array_keys($orphaned));
}
...
if ($groupmode) {
    ...
    $sql = 'SELECT o.*, g.name
                ...
                WHERE o.quizsbs = :quizsbsid
                ORDER BY g.name';
    $params = array('quizsbsid' => $quizsbs->id);
} else {
...
$overridedeleteurl = new moodle_url('/mod/quizsbs/overridedelete.php');
$overrideediturl = new moodle_url('/mod/quizsbs/overrideedit.php');
...
foreach ($overrides as $override) {
    ...
    if (!$groupmode) {
        if (!has_capability('mod/quizsbs:attempt', $context, $override->userid)) {
            // User not allowed to take the quizsbs.
            $active = false;
    ...
    if (isset($override->timeopen)) {
        $fields[] = get_string('quizsbsopens', 'quizsbs');
        $values[] = $override->timeopen > 0 ?
                userdate($override->timeopen) : get_string('noopen', 'quizsbs');
    }

    // Format timeclose.
    if (isset($override->timeclose)) {
        $fields[] = get_string('quizsbscloses', 'quizsbs');
        $values[] = $override->timeclose > 0 ?
                userdate($override->timeclose) : get_string('noclose', 'quizsbs');
    }

    // Format timelimit.
    if (isset($override->timelimit)) {
        $fields[] = get_string('timelimit', 'quizsbs');
        $values[] = $override->timelimit > 0 ?
                format_time($override->timelimit) : get_string('none', 'quizsbs');
    }

    // Format number of attempts.
    if (isset($override->attempts)) {
        $fields[] = get_string('attempts', 'quizsbs');
    ...
    if (isset($override->password)) {
        $fields[] = get_string('requirepassword', 'quizsbs');
        $values[] = $override->password !== '' ?
                get_string('enabled', 'quizsbs') : get_string('none', 'quizsbs');
    }
...
echo html_writer::start_tag('div', array('id' => 'quizsbsoverrides'));
if (count($table->data)) {
    echo html_writer::table($table);
}
if ($hasinactive) {
    echo $OUTPUT->notification(get_string('inactiveoverridehelp', 'quizsbs'), 'dimmed_text');
}
...
if ($groupmode) {
    if (empty($groups)) {
        // There are no groups.
        echo $OUTPUT->notification(get_string('groupsnone', 'quizsbs'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'addgroup', 'cmid' => $cm->id)),
            get_string('addnewgroupoverride', 'quizsbs'), 'post', $options);
} else {
    $users = array();
    // See if there are any students in the quizsbs.
    $users = get_users_by_capability($context, 'mod/quizsbs:attempt', 'u.id');
    $info = new \core_availability\info_module($cm);
    $users = $info->filter_user_list($users);

    if (empty($users)) {
        // There are no students.
        echo $OUTPUT->notification(get_string('usersnone', 'quizsbs'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'adduser', 'cmid' => $cm->id)),
            get_string('addnewuseroverride', 'quizsbs'), 'get', $options);
}
