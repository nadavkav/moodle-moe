...
/**
 * Defines the renderer for the quizsbs module.
 *
 * @package   mod_quizsbs
...
/**
 * The renderer for the quizsbs module.
 *
 * @copyright  2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_quizsbs_renderer extends plugin_renderer_base {
    /**
     * Builds the review page
     *
     * @param quizsbs_attempt $attemptobj an instance of quizsbs_attempt.
     ...
     * @param mod_quizsbs_display_options $displayoptions instance of mod_quizsbs_display_options.
     * @param array $summarydata contains all table data
     * @return $output containing html data.
     */
    public function review_page(quizsbs_attempt $attemptobj, $slots, $page, $showall,
                                $lastpage, mod_quizsbs_display_options $displayoptions,
                                $summarydata) {
     ...
     * @return $output containing html data.
     */
    public function review_question_page(quizsbs_attempt $attemptobj, $slot, $seq,
            mod_quizsbs_display_options $displayoptions, $summarydata) {
     ...
     */
    public function review_question_not_allowed(quizsbs_attempt $attemptobj, $message) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_quizsbs_name(), true,
                                  array("context" => $attemptobj->get_quizsbsobj()->get_context())));
    ...
    public function review_summary_table($summarydata, $page) {
        ...
        $output .= html_writer::start_tag('table', array(
                'class' => 'generaltable generalbox quizsbsreviewsummary'));
     ...
     *
     * @param quizsbs_attempt $attemptobj instance of quizsbs_attempt
     ...
     * @param mod_quizsbs_display_options $displayoptions instance of mod_quizsbs_display_options
     */
    public function questions(quizsbs_attempt $attemptobj, $reviewing, $slots, $page, $showall,
                              mod_quizsbs_display_options $displayoptions) {
        $output = '';
    ...
    public function review_form($page, $showall, $displayoptions, $content, $attemptobj) {
        ...
        $this->page->requires->js_init_call('M.mod_quizsbs.init_review_form', null, false,
     ...
     * @param quizsbs_attempt $attemptobj instance of quizsbs_attempt
     */
    public function finish_review_link(quizsbs_attempt $attemptobj) {
        $url = $attemptobj->view_url();

        if ($attemptobj->get_access_manager(time())->attempt_must_be_in_popup()) {
            $this->page->requires->js_init_call('M.mod_quizsbs.secure_window.init_close_button',
                    array($url), false, quizsbs_get_js_module());
            return html_writer::empty_tag('input', array('type' => 'button',
                    'value' => get_string('finishreview', 'quizsbs'),
                    'id' => 'secureclosebutton',
                    'class' => 'mod_quizsbs-next-nav'));

        } else {
            return html_writer::link($url, get_string('finishreview', 'quizsbs'),
                    array('class' => 'mod_quizsbs-next-nav'));
        }
    }
     ...
     */
    public function review_next_navigation(quizsbs_attempt $attemptobj, $page, $lastpage, $showall = null) {
        $nav = '';
        if ($page > 0) {
            $nav .= link_arrow_left(get_string('navigateprevious', 'quizsbs'),
                    $attemptobj->review_url(null, $page - 1, $showall), false, 'mod_quizsbs-prev-nav');
        }
        if ($lastpage) {
            $nav .= $this->finish_review_link($attemptobj);
        } else {
            $nav .= link_arrow_right(get_string('navigatenext', 'quizsbs'),
                    $attemptobj->review_url(null, $page + 1, $showall), false, 'mod_quizsbs-next-nav');
        }
        return html_writer::tag('div', $nav, array('class' => 'submitbtns'));
    }

    /**
     * Return the HTML of the quizsbs timer.
     * @return string HTML content.
     */
    public function countdown_timer(quizsbs_attempt $attemptobj, $timenow) {
        ...
        if ($timeleft !== false) {
            ...
            if (!$ispreview) {
                ...
                // this will just have the effect of causing the quizsbs to be submitted immediately.
    ...
    public function restart_preview_button($url) {
        return $this->single_button($url, get_string('startnewpreview', 'quizsbs'));
    }
    ...
    /**
     * Display a quizsbs navigation button.
     *
     * @param quizsbs_nav_question_button $button
     * @return string HTML fragment.
     */
    protected function render_quizsbs_nav_question_button(quizsbs_nav_question_button $button) {
        $classes = array('qnbutton', $button->stateclass, $button->navmethod);
        $extrainfo = array();

        if ($button->currentpage) {
            $classes[] = 'thispage';
            $extrainfo[] = get_string('onthispage', 'quizsbs');
        }
        ...
        $tagcontents = html_writer::tag('span', '', array('class' => 'thispageholder')) .
                        ...
                        get_string($qnostring, 'quizsbs', $a);
        $tagattributes = array('class' => implode(' ', $classes), 'id' => $button->id,
                                  'title' => $button->statestring, 'data-quizsbs-page' => $button->page);
    ...
    /**
     * Display a quizsbs navigation heading.
     *
     * @param quizsbs_nav_section_heading $heading the heading.
     * @return string HTML fragment.
     */
     ...
     *
     * @param mod_quizsbs_links_to_other_attempts $links
     */
    protected function render_mod_quizsbs_links_to_other_attempts(
            mod_quizsbs_links_to_other_attempts $links) {
        $attemptlinks = array();
    ...
    public function start_attempt_page(quizsbs $quizsbsobj, mod_quizsbs_preflight_check_form $mform) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($quizsbsobj->get_quizsbs_name(), true,
                                  array("context" => $quizsbsobj->get_context())));
        $output .= $this->quizsbs_intro($quizsbsobj->get_quizsbs(), $quizsbsobj->get_cm());
        $output .= $mform->render();
        $output .= $this->footer();
        return $output;
    }
     ...
     * @param quizsbs_attempt $attemptobj Instance of quizsbs_attempt
     * @param int $page Current page number
     * @param quizsbs_access_manager $accessmanager Instance of quizsbs_access_manager
     ...
     */
    public function quizsbs_notices($messages) {
        if (!$messages) {
            return '';
        }
        return $this->box($this->heading(get_string('accessnoticesheader', 'quizsbs'), 3) .
                $this->access_messages($messages), 'quizsbsaccessnotices');
    }

    /**
     * Ouputs the form for making an attempt
     *
     * @param quizsbs_attempt $attemptobj
     * @param int $page Current page number
     ...
     * @param bool $lastpage is this the last page in the quizsbs?
     * @return string HTML fragment.
     */
    ...
    public function redo_question_button($slot, $disabled) {
        $attributes = array('type' => 'submit',  'name' => 'redoslot' . $slot,
                'value' => get_string('redoquestion', 'quizsbs'), 'class' => 'mod_quizsbs-redo_question_button');
        if ($disabled) {
    ...
    public function initialise_timer($timerstartvalue, $ispreview) {
        ...
        $this->page->requires->js_init_call('M.mod_quizsbs.timer.init', $options, false, quizsbs_get_js_module());
    }
    ...
    public function close_attempt_popup($url, $message = '') {
        ...
        if ($message) {
            ...
            $output .= html_writer::tag('p', get_string('windowclosing', 'quizsbs'));
            $delay = 5;
        } else {
            $output .= html_writer::tag('p', get_string('pleaseclose', 'quizsbs'));
            $delay = 0;
        }
        $this->page->requires->js_init_call('M.mod_quizsbs.secure_window.close',
                array($url, $delay), false, quizsbs_get_js_module());
     ...
     *
     * @param quizsbs_attempt $attemptobj
     * @param mod_quizsbs_display_options $displayoptions
     */
    public function summary_page($attemptobj, $displayoptions) {
        $output = '';
        $output .= $this->header();
        $output .= $this->heading(format_string($attemptobj->get_quizsbs_name()));
        $output .= $this->heading(get_string('summaryofattempt', 'quizsbs'), 3);
    ...
    public function summary_table($attemptobj, $displayoptions) {
        ...
        $table->attributes['class'] = 'generaltable quizsbssummaryofattempt boxaligncenter';
        $table->head = array(get_string('question', 'quizsbs'), get_string('status', 'quizsbs'));
        $table->align = array('left', 'left');
        $table->size = array('', '');
        $markscolumn = $displayoptions->marks >= question_display_options::MARK_AND_MAX;
        if ($markscolumn) {
            $table->head[] = get_string('marks', 'quizsbs');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($slots as $slot) {
            ...
            if ($heading) {
                ...
                $table->rowclasses[] = 'quizsbssummaryheading';
            }
            ...
            $table->data[] = $row;
            $table->rowclasses[] = 'quizsbssummary' . $slot . ' ' . $attemptobj->get_question_state_class(
    ...
    public function summary_page_controls($attemptobj) {
        ...
        if ($attemptobj->get_state() == quizsbs_attempt::IN_PROGRESS) {
            $button = new single_button(
                    new moodle_url($attemptobj->attempt_url(null, $attemptobj->get_currentpage())),
                    get_string('returnattempt', 'quizsbs'));
        ...
        $button = new single_button(
                ...
                get_string('submitallandfinish', 'quizsbs'));
        $button->id = 'responseform';
        if ($attemptobj->get_state() == quizsbs_attempt::IN_PROGRESS) {
            $button->add_action(new confirm_action(get_string('confirmclose', 'quizsbs'), null,
                    get_string('submitallandfinish', 'quizsbs')));
        }

        $duedate = $attemptobj->get_due_date();
        $message = '';
        if ($attemptobj->get_state() == quizsbs_attempt::OVERDUE) {
            $message = get_string('overduemustbesubmittedby', 'quizsbs', userdate($duedate));

        } else if ($duedate) {
            $message = get_string('mustbesubmittedby', 'quizsbs', userdate($duedate));
        }
     ...
     * @param array $quizsbs Array conting quizsbs data
     * @param int $cm Course Module ID
     * @param int $context The page context ID
     * @param array $infomessages information about this quizsbs
     * @param mod_quizsbs_view_object $viewobj
     ...
     *      attempt this quizsbs now, if appicable this quizsbs
     */
    public function view_page($course, $quizsbs, $cm, $context, $viewobj) {
        $output = '';
        $output .= $this->view_information($quizsbs, $cm, $context, $viewobj->infomessages);
        $output .= $this->view_table($quizsbs, $context, $viewobj);
        $output .= $this->view_result_info($quizsbs, $context, $cm, $viewobj);
        $output .= $this->box($this->view_page_buttons($viewobj), 'quizsbsattempt');
        return $output;
    }
     ...
     * @param mod_quizsbs_view_object $viewobj the information required to display
     * the view page.
     * @return string HTML to output.
     */
    public function view_page_buttons(mod_quizsbs_view_object $viewobj) {
        global $CFG;
        $output = '';
        ...
        if ($viewobj->showbacktocourse) {
            $output .= $this->single_button($viewobj->backtocourseurl,
                    get_string('backtocourse', 'quizsbs'), 'get',
     ...
     * @param mod_quizsbs_preflight_check_form $preflightcheckform deprecated.
    ...
    public function start_attempt_button($buttontext, moodle_url $url,
            mod_quizsbs_preflight_check_form $preflightcheckform = null,
        ...
        if (is_string($preflightcheckform)) {
            ...
            debugging('The third argument to start_attempt_button should now be the ' .
                    'mod_quizsbs_preflight_check_form from ' .
                    'quizsbs_access_manager::get_preflight_check_form, not a warning message string.');
        }

        $button = new single_button($url, $buttontext);
        $button->class .= ' quizsbsstartbuttondiv';
        ...
        $this->page->requires->js_call_amd('mod_quizsbs/preflightcheck', 'init',
                array('.quizsbsstartbuttondiv input[type=submit]', get_string('startattempt', 'quizsbs'),
                       '#mod_quizsbs_preflight_form', $popupjsoptions));

        return $this->render($button) . $checkform;
    }

    /**
     * Generate a message saying that this quizsbs has no questions, with a button to
    ...
    public function no_questions_message($canedit, $editurl) {
        $output = '';
        $output .= $this->notification(get_string('noquestions', 'quizsbs'));
        if ($canedit) {
            $output .= $this->single_button($editurl, get_string('editquizsbs', 'quizsbs'), 'get');
        }
    ...
    /**
     * Outputs an error message for any guests accessing the quizsbs
     *
     * @param int $course The course ID
     * @param array $quizsbs Array contingin quizsbs data
     * @param int $cm Course Module ID
     * @param int $context The page contect ID
     * @param array $messages Array containing any messages
     */
    public function view_page_guest($course, $quizsbs, $cm, $context, $messages) {
        $output = '';
        $output .= $this->view_information($quizsbs, $cm, $context, $messages);
        $guestno = html_writer::tag('p', get_string('guestsno', 'quizsbs'));
     ...
     * @param array $messages Array containing any messages
     */
    public function view_page_notenrolled($course, $quizsbs, $cm, $context, $messages) {
        global $CFG;
        $output = '';
        $output .= $this->view_information($quizsbs, $cm, $context, $messages);
        $youneedtoenrol = html_writer::tag('p', get_string('youneedtoenrol', 'quizsbs'));
     ...
     * @return string HTML to output.
     */
    public function view_information($quizsbs, $cm, $context, $messages) {
        global $CFG;

        $output = '';

        // Print quizsbs name and description.
        $output .= $this->heading(format_string($quizsbs->name));
        $output .= $this->quizsbs_intro($quizsbs, $cm);

        // Output any access messages.
        if ($messages) {
            $output .= $this->box($this->access_messages($messages), 'quizsbsinfo');
        }

        // Show number of attempts summary to those who can view reports.
        if (has_capability('mod/quizsbs:viewreports', $context)) {
            if ($strattemptnum = $this->quizsbs_attempt_summary_link_to_reports($quizsbs, $cm,
                    $context)) {
                $output .= html_writer::tag('div', $strattemptnum,
                        array('class' => 'quizsbsattemptcounts'));
            }
        }
        return $output;
    }
    ...
    /**
     * Output the quizsbs intro.
     * @param object $quizsbs the quizsbs settings.
     * @param object $cm the course_module object.
     * @return string HTML to output.
     */
    public function quizsbs_intro($quizsbs, $cm) {
        if (html_is_blank($quizsbs->intro)) {
            return '';
        }

        return $this->box(format_module_intro('quizsbs', $quizsbs, $cm->id), 'generalbox', 'intro');
    }
    ...
    public function view_table_heading() {
        return $this->heading(get_string('summaryofattempts', 'quizsbs'), 3);
    }

    /**
     * Generates the table of data
     *
     * @param array $quizsbs Array contining quizsbs data
     * @param int $context The page context ID
     * @param mod_quizsbs_view_object $viewobj
     */
    public function view_table($quizsbs, $context, $viewobj) {
        if (!$viewobj->attempts) {
            return '';
        }
        ...
        $table = new html_table();
        $table->attributes['class'] = 'generaltable quizsbsattemptsummary';
        $table->head = array();
        $table->align = array();
        $table->size = array();
        if ($viewobj->attemptcolumn) {
            $table->head[] = get_string('attemptnumber', 'quizsbs');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        $table->head[] = get_string('attemptstate', 'quizsbs');
        $table->align[] = 'left';
        $table->size[] = '';
        ...
        if ($viewobj->gradecolumn) {
            $table->head[] = get_string('grade') . ' / ' .
                    quizsbs_format_grade($quizsbs, $quizsbs->grade);
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->canreviewmine) {
            $table->head[] = get_string('review', 'quizsbs');
            $table->align[] = 'center';
            $table->size[] = '';
        }
        if ($viewobj->feedbackcolumn) {
            $table->head[] = get_string('feedback', 'quizsbs');
            $table->align[] = 'left';
            $table->size[] = '';
        }
        ...
        foreach ($viewobj->attemptobjs as $attemptobj) {
            ...
            if ($viewobj->attemptcolumn) {
                if ($attemptobj->is_preview()) {
                    $row[] = get_string('preview', 'quizsbs');
                } else {
            ...
            if ($viewobj->markcolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    $row[] = quizsbs_format_grade($quizsbs, $attemptobj->get_sum_marks());
                } else {
                    $row[] = '';
                }
            }
            ...
            $attemptgrade = quizsbs_rescale_grade($attemptobj->get_sum_marks(), $quizsbs, false);
            ...
            if ($viewobj->gradecolumn) {
                if ($attemptoptions->marks >= question_display_options::MARK_AND_MAX &&
                    ...
                    if ($viewobj->overallstats && !$attemptobj->is_preview()
                            ...
                            && $attemptobj->get_state() == quizsbs_attempt::FINISHED
                            && $attemptgrade == $viewobj->mygrade
                            && $quizsbs->grademethod == quizsbs_GRADEHIGHEST) {
                        $table->rowclasses[$attemptobj->get_attempt_number()] = 'bestrow';
                    }

                    $row[] = quizsbs_format_grade($quizsbs, $attemptgrade);
                } else {
                    $row[] = '';
                }
            }
            ...
            if ($viewobj->feedbackcolumn && $attemptobj->is_finished()) {
                if ($attemptoptions->overallfeedback) {
                    $row[] = quizsbs_feedback_for_grade($attemptgrade, $quizsbs, $context);
                } else {
                    $row[] = '';
                }
            }
     ...
     * @param quizsbs_attempt $attemptobj the attempt
    ...
    public function attempt_state($attemptobj) {
        switch ($attemptobj->get_state()) {
            ...
            case quizsbs_attempt::OVERDUE:
                return get_string('stateoverdue', 'quizsbs') . html_writer::tag('span',
                        get_string('stateoverduedetails', 'quizsbs',
                                userdate($attemptobj->get_due_date())),
                        array('class' => 'statedetails'));

            case quizsbs_attempt::FINISHED:
                return get_string('statefinished', 'quizsbs') . html_writer::tag('span',
                        get_string('statefinisheddetails', 'quizsbs',
    ...
    /**
     * Generates data pertaining to quizsbs results
     *
     * @param array $quizsbs Array containing quizsbs data
     * @param int $context The page context ID
     * @param int $cm The Course Module Id
     * @param mod_quizsbs_view_object $viewobj
     */
    public function view_result_info($quizsbs, $context, $cm, $viewobj) {
        $output = '';
        ...
        if ($viewobj->overallstats) {
            if ($viewobj->moreattempts) {
                $a = new stdClass();
                $a->method = quizsbs_get_grading_option_name($quizsbs->grademethod);
                $a->mygrade = quizsbs_format_grade($quizsbs, $viewobj->mygrade);
                $a->quizsbsgrade = quizsbs_format_grade($quizsbs, $quizsbs->grade);
                $resultinfo .= $this->heading(get_string('gradesofar', 'quizsbs', $a), 3);
            } else {
                $a = new stdClass();
                $a->grade = quizsbs_format_grade($quizsbs, $viewobj->mygrade);
                $a->maxgrade = quizsbs_format_grade($quizsbs, $quizsbs->grade);
                $a = get_string('outofshort', 'quizsbs', $a);
                $resultinfo .= $this->heading(get_string('yourfinalgradeis', 'quizsbs', $a), 3);
            }
        }
        ...
        if ($viewobj->gradebookfeedback) {
            $resultinfo .= $this->heading(get_string('comment', 'quizsbs'), 3);
            $resultinfo .= html_writer::div($viewobj->gradebookfeedback, 'quizsbsteacherfeedback') . "\n";
        }
        if ($viewobj->feedbackcolumn) {
            $resultinfo .= $this->heading(get_string('overallfeedback', 'quizsbs'), 3);
            $resultinfo .= html_writer::div(
                    quizsbs_feedback_for_grade($viewobj->mygrade, $quizsbs, $context),
                    'quizsbsgradefeedback') . "\n";
        }
    ...
    public function review_link($url, $reviewinpopup, $popupoptions) {
        if ($reviewinpopup) {
            $button = new single_button($url, get_string('review', 'quizsbs'));
            $button->add_action(new popup_action('click', $url, 'quizsbspopup', $popupoptions));
            return $this->render($button);

        } else {
            return html_writer::link($url, get_string('review', 'quizsbs'),
                    array('title' => get_string('reviewthisattempt', 'quizsbs')));
        }
    }
     ...
     */
    public function quizsbs_attempt_summary_link_to_reports($quizsbs, $cm, $context,
    ...
    public function connection_warning() {
        ...
        $warning = format_text(get_string('connectionerror', 'quizsbs'), FORMAT_MARKDOWN, $options);
        $ok = format_text(get_string('connectionok', 'quizsbs'), FORMAT_MARKDOWN, $options);
...
class mod_quizsbs_links_to_other_attempts implements renderable {
    /**
...
class mod_quizsbs_view_object {
    /** @var array $infomessages of messages with information to display about the quizsbs. */
    public $infomessages;
    /** @var array $attempts contains all the user's attempts at this quizsbs. */
    public $attempts;
    /** @var array $attemptobjs quizsbs_attempt objects corresponding to $attempts. */
    public $attemptobjs;
    /** @var quizsbs_access_manager $accessmanager contains various access rules. */
    public $accessmanager;
    /** @var bool $canreviewmine whether the current user has the capability to
     *       review their own attempts. */
    public $canreviewmine;
    /** @var bool $canedit whether the current user has the capability to edit the quizsbs. */
    public $canedit;
    /** @var moodle_url $editurl the URL for editing this quizsbs. */
    public $editurl;
    ...
    public $numattempts;
    /** @var float $mygrade contains the user's final grade for a quizsbs. */
    public $mygrade;
    ...
    public $popupoptions;
    /** @var bool $quizsbshasquestions whether the quizsbs has any questions. */
    public $quizsbshasquestions;
