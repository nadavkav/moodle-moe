...
/**
 * This script controls the display of the quizsbs reports.
 *
 * @package   mod_quizsbs
...
$url = new moodle_url('/mod/quizsbs/report.php', array('id' => $cm->id));
if ($mode !== '') {
    $url->param('mode', $mode);
}
$PAGE->set_url($url);
...
$reportlist = quizsbs_report_list($context);
if (empty($reportlist)) {
    print_error('erroraccessingreport', 'quizsbs');
}
...
if (!is_readable("report/$mode/report.php")) {
    print_error('reportnotfound', 'quizsbs', '', $mode);
}

// Open the selected quizsbs report and display it.
$file = $CFG->dirroot . '/mod/quizsbs/report/' . $mode . '/report.php';
if (is_readable($file)) {
    include_once($file);
}
$reportclassname = 'quizsbs_' . $mode . '_report';
if (!class_exists($reportclassname)) {
    print_error('preprocesserror', 'quizsbs');
}

$report = new $reportclassname();
$report->display($quizsbs, $cm, $course);
