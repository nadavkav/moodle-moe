...
/**
 * Base class for quizsbs reports that are basically a table with one row for each attempt.
 *
 * @copyright 2010 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class quizsbs_attempts_report extends quizsbs_default_report {
    ...
    /** @var object the quizsbs context. */
    protected $context;

    /** @var mod_quizsbs_attempts_report_form The settings form to use. */
    protected $form;
     ...
     * @param object $course
     */
    protected function init($mode, $formclass, $quizsbs, $cm, $course) {
        $this->mode = $mode;
        ...
        $this->qmsubselect = quizsbs_report_qm_filter_select($quizsbs);

        $this->form = new $formclass($this->get_base_url(),
                array('quizsbs' => $quizsbs, 'currentgroup' => $currentgroup, 'context' => $this->context));
    ...
    protected function add_state_column(&$columns, &$headers) {
        $columns[] = 'state';
        $headers[] = get_string('attemptstate', 'quizsbs');
    }
    ...
    protected function add_time_columns(&$columns, &$headers) {
        $columns[] = 'timestart';
        $headers[] = get_string('startedon', 'quizsbs');

        $columns[] = 'timefinish';
        $headers[] = get_string('timecompleted', 'quizsbs');

        $columns[] = 'duration';
        $headers[] = get_string('attemptduration', 'quizsbs');
    }
     ...
     * @param bool $usercanseegrades whether the user is allowed to see grades for this quizsbs.
     * @param array $columns the list of columns. Added to.
     * @param array $headers the columns headings. Added to.
     * @param bool $includefeedback whether to include the feedbacktext columns
     */
    protected function add_grade_columns($quizsbs, $usercanseegrades, &$columns, &$headers, $includefeedback = true) {
        if ($usercanseegrades) {
            $columns[] = 'sumgrades';
            $headers[] = get_string('grade', 'quizsbs') . '/' .
                    quizsbs_format_grade($quizsbs, $quizsbs->grade);
        }

        if ($includefeedback && quizsbs_has_feedback($quizsbs)) {
            $columns[] = 'feedbacktext';
            $headers[] = get_string('feedback', 'quizsbs');
        }
    }
     ...
     * @param mod_quizsbs_attempts_report_options $options the display options.
    ...
    protected function set_up_table_columns($table, $columns, $headers, $reporturl,
            mod_quizsbs_attempts_report_options $options, $collapsible) {
        ...
        if (empty($currentgroup) || $groupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    require_capability('mod/quizsbs:deleteattempts', $this->context);
                    $this->delete_selected_attempts($quizsbs, $cm, $attemptids, $allowed);
    ...
    /**
     * Delete the quizsbs attempts
     * @param object $quizsbs the quizsbs settings. Attempts that don't belong to
     * this quizsbs are not deleted.
     ...
     */
    protected function delete_selected_attempts($quizsbs, $cm, $attemptids, $allowed) {
        global $DB;

        foreach ($attemptids as $attemptid) {
            $attempt = $DB->get_record('quizsbs_attempts', array('id' => $attemptid));
            if (!$attempt || $attempt->quizsbs != $quizsbs->id || $attempt->preview != 0) {
                // Ensure the attempt exists, and belongs to this quizsbs. If not skip.
                continue;
            }
            ...
            // Set the course module id before calling quizsbs_delete_attempt().
            $quizsbs->cmid = $cm->id;
