...
/**
 * Base class for the options that control what is visible in an {@link quizsbs_attempts_report}.
 *
 * @package   mod_quizsbs
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_quizsbs_attempts_report_options {

    /** @var string the report mode. */
    public $mode;

    /** @var object the settings for the quizsbs being reported on. */
    public $quizsbs;

    /** @var object the course module objects for the quizsbs being reported on. */
    public $cm;

    /** @var object the course settings for the course the quizsbs is in. */
    public $course;

    /**
     * @var array form field name => corresponding quizsbs_attempt:: state constant.
     */
    protected static $statefields = array(
        'stateinprogress' => quizsbs_attempt::IN_PROGRESS,
        'stateoverdue'    => quizsbs_attempt::OVERDUE,
        'statefinished'   => quizsbs_attempt::FINISHED,
        'stateabandoned'  => quizsbs_attempt::ABANDONED,
    );

    /**
     * @var string quizsbs_attempts_report::ALL_WITH or quizsbs_attempts_report::ENROLLED_WITH
     *      quizsbs_attempts_report::ENROLLED_WITHOUT or quizsbs_attempts_report::ENROLLED_ALL
     */
    public $attempts = quizsbs_attempts_report::ENROLLED_WITH;

    /** @var int the currently selected group. 0 if no group is selected. */
    public $group = 0;

    /**
     * @var array|null of quizsbs_attempt::IN_PROGRESS, etc. constants. null means
     *      no restriction.
     */
    public $states = array(quizsbs_attempt::IN_PROGRESS, quizsbs_attempt::OVERDUE,
            quizsbs_attempt::FINISHED, quizsbs_attempt::ABANDONED);
    ...
    public $pagesize = quizsbs_attempts_report::DEFAULT_PAGE_SIZE;
     ...
     * @param object $quizsbs the settings for the quizsbs being reported on.
     * @param object $cm the course module objects for the quizsbs being reported on.
     * @param object $coures the course settings for the coures this quizsbs is in.
     */
    public function __construct($mode, $quizsbs, $cm, $course) {
        $this->mode   = $mode;
        $this->quizsbs   = $quizsbs;
        $this->cm     = $cm;
        $this->course = $course;

        $this->usercanseegrades = quizsbs_report_should_show_grades($quizsbs, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/quizsbs/report.php', $this->get_url_params());
    }
    ...
    public function setup_from_user_preferences() {
        $this->pagesize = get_user_preferences('quizsbs_report_pagesize', $this->pagesize);
    }
    ...
    public function update_user_preferences() {
        set_user_preference('quizsbs_report_pagesize', $this->pagesize);
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == quizsbs_attempts_report::ALL_WITH) {
                $this->attempts = quizsbs_attempts_report::ENROLLED_WITH;
            }

        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = quizsbs_attempts_report::ALL_WITH;

        } else if (!in_array($this->attempts, array(quizsbs_attempts_report::ALL_WITH, quizsbs_attempts_report::ENROLLED_WITH,
                quizsbs_attempts_report::ENROLLED_WITHOUT, quizsbs_attempts_report::ENROLLED_ALL))) {
            $this->attempts = quizsbs_attempts_report::ENROLLED_WITH;
        }
        ...
        if (!quizsbs_report_can_filter_only_graded($this->quizsbs)) {
        ...
        if ($this->attempts == quizsbs_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }
        ...
        if ($this->pagesize < 1) {
            $this->pagesize = quizsbs_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
    ...
    protected function is_showing_finished_attempts() {
        return $this->states === null || in_array(quizsbs_attempt::FINISHED, $this->states);
    }
}
