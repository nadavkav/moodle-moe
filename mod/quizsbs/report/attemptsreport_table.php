...
/**
 * Base class for the table used by a {@link quizsbs_attempts_report}.
 *
 * @package   mod_quizsbs
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class quizsbs_attempts_report_table extends table_sql {
    ...
    /** @var object the quizsbs settings for the quizsbs we are reporting on. */
    protected $quizsbs;
    ...
    /** @var object mod_quizsbs_attempts_report_options the options affecting this report. */
    protected $options;
    ...
    /** @var object the questions that comprise this quizsbs.. */
    protected $questions;
     ...
     * @param mod_quizsbs_attempts_report_options $options
     * @param array $groupstudents
     * @param array $students
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($uniqueid, $quizsbs, $context, $qmsubselect,
            mod_quizsbs_attempts_report_options $options, $groupstudents, $students,
            $questions, $reporturl) {
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/quizsbs/review.php', array('attempt' => $attempt->attempt)),
                get_string('reviewattempt', 'quizsbs'), array('class' => 'reviewlink'));
    }
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return quizsbs_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = quizsbs_report_feedback_for_grade(
                quizsbs_rescale_grade($attempt->sumgrades, $this->quizsbs, false),
                $this->quizsbs->id, $this->context);
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/quizsbs/reviewquestion.php', $reviewparams);
        $output = $OUTPUT->action_link($url, $output,
                new popup_action('click', $url, 'reviewquestion',
                        array('height' => 450, 'width' => 650)),
                array('title' => get_string('reviewresponse', 'quizsbs')));
    ...
    public function base_sql($reportstudents) {
        ...
        $fields = $DB->sql_concat('u.id', "'#'", 'COALESCE(quizsbsa.attempt, 0)') . ' AS uniqueid,';
        ...
        $fields .= '
                quizsbsa.uniqueid AS usageid,
                quizsbsa.id AS attempt,
                u.id AS userid,
                u.idnumber, ' . $allnames . ',
                u.picture,
                u.imagealt,
                ...
                quizsbsa.timefinish,
                quizsbsa.timestart,
                CASE WHEN quizsbsa.timefinish = 0 THEN null
                     WHEN quizsbsa.timefinish > quizsbsa.timestart THEN quizsbsa.timefinish - quizsbsa.timestart
                     ELSE 0 END AS duration';
            // To explain that last bit, timefinish can be non-zero and less
            // than timestart when you have two load-balanced servers with very
            // badly synchronised clocks, and a student does a really quick attempt.

        // This part is the same for all cases. Join the users and quizsbs_attempts tables.
        $from = "\n{user} u";
        $from .= "\nLEFT JOIN {quizsbs_attempts} quizsbsa ON
                                    quizsbsa.userid = u.id AND quizsbsa.quizsbs = :quizsbsid";
        $params = array('quizsbsid' => $this->quizsbs->id);

        if ($this->qmsubselect && $this->options->onlygraded) {
            $from .= " AND (quizsbsa.state <> :finishedstate OR $this->qmsubselect)";
            $params['finishedstate'] = quizsbs_attempt::FINISHED;
        }

        switch ($this->options->attempts) {
            case quizsbs_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'quizsbsa.id IS NOT NULL AND quizsbsa.preview = 0';
                break;
            case quizsbs_attempts_report::ENROLLED_WITH:
                // Show only students with attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND quizsbsa.preview = 0 AND quizsbsa.id IS NOT NULL";
                break;
            case quizsbs_attempts_report::ENROLLED_WITHOUT:
                // Show only students without attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND quizsbsa.id IS NULL";
                break;
            case quizsbs_attempts_report::ENROLLED_ALL:
                // Show all students with or without attempts.
                list($usql, $uparams) = $DB->get_in_or_equal(
                        $reportstudents, SQL_PARAMS_NAMED, 'u');
                $params += $uparams;
                $where = "u.id $usql AND (quizsbsa.preview = 0 OR quizsbsa.preview IS NULL)";
                break;
        }
        ...
        if ($this->options->states) {
            ...
            $params += $stateparams;
            $where .= " AND (quizsbsa.state $statesql OR quizsbsa.state IS NULL)";
        }
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{quizsbs_attempts} {$alias}quizsbsa", "{$alias}quizsbsa.uniqueid",
                "{$alias}quizsbsa.quizsbs = :{$alias}quizsbsid", array("{$alias}quizsbsid" => $this->sql->params['quizsbsid']));
        ...
        $this->sql->from .= "\nLEFT JOIN $inlineview ON " .
                "$alias.questionusageid = quizsbsa.uniqueid AND $alias.slot = :{$alias}slot";
    ...
    protected function get_qubaids_condition() {
        ...
        if ($this->is_downloading()) {
            ...
            return new qubaid_join($this->sql->from, 'quizsbsa.uniqueid',
    ...
    public function get_sort_columns() {
        ...
        $sortcolumns['quizsbsa.id'] = SORT_ASC;
        return $sortcolumns;
    }
    ...
    public function wrap_html_finish() {
        ...
        echo '<a href="javascript:select_all_in(\'DIV\', null, \'tablecontainer\');">' .
                get_string('selectall', 'quizsbs') . '</a> / ';
        echo '<a href="javascript:deselect_all_in(\'DIV\', null, \'tablecontainer\');">' .
                get_string('selectnone', 'quizsbs') . '</a> ';
        echo '&nbsp;&nbsp;';
        $this->submit_buttons();
        echo '</div>';
    ...
    protected function submit_buttons() {
        global $PAGE;
        if (has_capability('mod/quizsbs:deleteattempts', $this->context)) {
            echo '<input type="submit" id="deleteattemptsbutton" name="delete" value="' .
                    get_string('deleteselected', 'quizsbs_overview') . '"/>';
            $PAGE->requires->event_handler('#deleteattemptsbutton', 'click', 'M.util.show_confirm_dialog',
                    array('message' => get_string('deleteattemptcheck', 'quizsbs')));
        }
    }
}
