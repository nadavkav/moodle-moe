...
/**
 * This file defines the quizsbs manual grading report class.
 *
 * @package   quizsbs_grading
...
require_once($CFG->dirroot . '/mod/quizsbs/report/grading/gradingsettings_form.php');


/**
 * quizsbs report to help teachers manually grade questions that need it.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizsbs_grading_report extends quizsbs_default_report {
    ...
    public function display($quizsbs, $cm, $course) {
        ...
        require_capability('mod/quizsbs:grade', $this->context);
        $shownames = has_capability('quizsbs/grading:viewstudentnames', $this->context);
        $showidnumbers = has_capability('quizsbs/grading:viewidnumber', $this->context);
        ...
        // Get the list of questions in this quizsbs.
        $this->questions = quizsbs_report_get_significant_questions($quizsbs);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'quizsbs_grading');
        }
        ...
        if ($slot && $hasquestions) {
            ...
            if (!$counts || $counts->$grade == 0) {
                redirect($this->list_questions_url(), get_string('alldoneredirecting', 'quizsbs_grading'));
            }
        }

        // Start output.
        $this->print_header_and_tabs($cm, $course, $quizsbs, 'grading');
    ...
    protected function get_qubaids_condition() {
        ...
        $where = "quizsbsa.quizsbs = :mangrquizsbsid AND
                quizsbsa.preview = 0 AND
                quizsbsa.state = :statefinished";
        $params = array('mangrquizsbsid' => $this->cm->instance, 'statefinished' => quizsbs_attempt::FINISHED);

        $currentgroup = groups_get_activity_group($this->cm, true);
        if ($currentgroup) {
            $users = get_users_by_capability($this->context,
                    array('mod/quizsbs:reviewmyattempts', 'mod/quizsbs:attempt'), 'u.id, u.id', '', '', '',
                    $currentgroup, '', false);
            if (empty($users)) {
                $where .= ' AND quizsbsa.userid = 0';
            } else {
        ...
        return new qubaid_join('{quizsbs_attempts} quizsbsa', 'quizsbsa.uniqueid', $where, $params);
    }
    ...
    protected function load_attempts_by_usage_ids($qubaids) {
        ...
        $params[] = quizsbs_attempt::FINISHED;
        $params[] = $this->quizsbs->id;

        $fields = 'quizsbsa.*, u.idnumber, ';
        $fields .= get_all_user_name_fields(true, 'u');
        $attemptsbyid = $DB->get_records_sql("
                SELECT $fields
                FROM {quizsbs_attempts} quizsbsa
                JOIN {user} u ON u.id = quizsbsa.userid
                WHERE quizsbsa.uniqueid $asql AND quizsbsa.state = ? AND quizsbsa.quizsbs = ?",
                $params);
    ...
    protected function base_url() {
        return new moodle_url('/mod/quizsbs/report.php',
    ...
    protected function format_count_for_table($counts, $type, $gradestring) {
        ...
        if ($counts->$type > 0) {
            $result .= ' ' . html_writer::link($this->grade_question_url(
                    ...
                    get_string($gradestring, 'quizsbs_grading'),
    ...
    protected function display_index($includeauto) {
        ...
        echo $OUTPUT->heading(get_string('questionsthatneedgrading', 'quizsbs_grading'), 3);
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'quizsbs_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'quizsbs_grading');
        }
        ...
        if (empty($data)) {
            echo $OUTPUT->notification(get_string('nothingfound', 'quizsbs_grading'));
            return;
        }
        ...
        $table->head[] = get_string('qno', 'quizsbs_grading');
        $table->head[] = get_string('questionname', 'quizsbs_grading');
        $table->head[] = get_string('tograde', 'quizsbs_grading');
        $table->head[] = get_string('alreadygraded', 'quizsbs_grading');
        if ($includeauto) {
            $table->head[] = get_string('automaticallygraded', 'quizsbs_grading');
        }
        $table->head[] = get_string('total', 'quizsbs_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new quizsbs_grading_settings_form($hidden, $counts, $shownames, $showidnumbers);
        ...
        echo $OUTPUT->heading(get_string('gradingquestionx', 'quizsbs_grading', $a), 3);
        echo html_writer::tag('p', html_writer::link($this->list_questions_url(),
                get_string('backtothelistofquestions', 'quizsbs_grading')),
        ...
        $a->of = $count;
        echo $OUTPUT->heading(get_string('gradingattemptsxtoyofz', 'quizsbs_grading', $a), 3);
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = quizsbs_get_review_options($this->quizsbs, $attempt, $this->context);
        ...
        echo html_writer::tag('div', html_writer::empty_tag('input', array(
                'type' => 'submit', 'value' => get_string('saveandnext', 'quizsbs_grading'))),
                array('class' => 'mdl-align')) .
    ...
    protected function get_question_heading($attempt, $shownames, $showidnumbers) {
        ...
        if ($shownames && $showidnumbers) {
            return get_string('gradingattemptwithidnumber', 'quizsbs_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'quizsbs_grading', $a);
    ...
    protected function process_submitted_data() {
        ...
        foreach ($qubaids as $qubaid) {
            $attempt = $attempts[$qubaid];
            $attemptobj = new quizsbs_attempt($attempt, $this->quizsbs, $this->cm, $this->course);
            ...
            $events[] = \mod_quizsbs\event\question_manually_graded::create($params);
        }
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot,
        ...
        if ($orderby == 'date') {
            ...
            $qubaids->from .= " JOIN {user} u ON quizsbsa.userid = u.id ";
