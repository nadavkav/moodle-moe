...
/**
 * This file defines the quizsbs grades table.
 *
...
require_once($CFG->dirroot . '/mod/quizsbs/report/attemptsreport_table.php');


/**
 * This is a table subclass for displaying the quizsbs grades report.
 *
 * @copyright 2008 Jamie Pratt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizsbs_overview_table extends quizsbs_attempts_report_table {
     ...
     * @param quizsbs_overview_options $options
     * @param array $groupstudents
     * @param array $students
     * @param array $questions
     * @param moodle_url $reporturl
     */
    public function __construct($quizsbs, $context, $qmsubselect,
            quizsbs_overview_options $options, $groupstudents, $students, $questions, $reporturl) {
        parent::__construct('mod-quizsbs-report-overview-report', $quizsbs , $context,
    ...
    protected function add_average_row($label, $users) {
        ...
        $record = $DB->get_record_sql("
                SELECT AVG(quizsbsa.sumgrades) AS grade, COUNT(quizsbsa.sumgrades) AS numaveraged
                  FROM $from
                 WHERE $where", $params);
        $record->grade = quizsbs_rescale_grade($record->grade, $this->quizsbs, false);
        ...
        $averagerow = array(
            ...
            'feedbacktext'=> strip_tags(quizsbs_report_feedback_for_grade(
                                        $record->grade, $this->quizsbs->id, $this->context))
        );

        if ($this->options->slotmarks) {
            $dm = new question_engine_data_mapper();
            $qubaids = new qubaid_join($from, 'quizsbsa.uniqueid', $where, $params);
    ...
    protected function format_average_grade_for_questions($gradeaverages) {
        ...
        foreach ($this->questions as $question) {
            if (isset($gradeaverages[$question->slot]) && $question->maxmark > 0) {
                ...
                $record->grade = quizsbs_rescale_grade(
                        $record->averagefraction * $question->maxmark, $this->quizsbs, false);
    ...
    protected function format_average($record, $question = false) {
        if (is_null($record->grade)) {
            ...
            $average = quizsbs_format_question_grade($this->quizsbs, $record->grade);
        } else {
            $average = quizsbs_format_grade($this->quizsbs, $record->grade);
        }
    ...
    protected function submit_buttons() {
        if (has_capability('mod/quizsbs:regrade', $this->context)) {
            echo '<input type="submit" name="regrade" value="' .
                    get_string('regradeselected', 'quizsbs_overview') . '"/>';
        }
    ...
    public function col_sumgrades($attempt) {
        ...
        $grade = quizsbs_rescale_grade($attempt->sumgrades, $this->quizsbs);
        if ($this->is_downloading()) {
            return $grade;
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid])) {
            ...
            $newsumgrade = quizsbs_rescale_grade($newsumgrade, $this->quizsbs);
            $oldsumgrade = quizsbs_rescale_grade($oldsumgrade, $this->quizsbs);
            $grade = html_writer::tag('del', $oldsumgrade) . '/' .
                    html_writer::empty_tag('br') . $newsumgrade;
        }
        return html_writer::link(new moodle_url('/mod/quizsbs/review.php',
                array('attempt' => $attempt->attempt)), $grade,
                array('title' => get_string('reviewattempt', 'quizsbs')));
    }

    /**
     * @param string $colname the name of the column.
     * @param object $attempt the row of data - see the SQL in display() in
     * mod/quizsbs/report/overview/report.php to see what fields are present,
    ...
    public function other_cols($colname, $attempt) {
        ...
        if ($question->maxmark == 0) {
            ...
            $grade = quizsbs_rescale_grade(
                    $stepdata->fraction * $question->maxmark, $this->quizsbs, 'question');
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid][$slot])) {
            $gradefromdb = $grade;
            $newgrade = quizsbs_rescale_grade(
                    $this->regradedqs[$attempt->usageid][$slot]->newfraction * $question->maxmark,
                    $this->quizsbs, 'question');
            $oldgrade = quizsbs_rescale_grade(
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'quizsbs_overview');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'quizsbs_overview');
        }
    }
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if ($this->options->slotmarks && has_capability('mod/quizsbs:regrade', $this->context)) {
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('quizsbs_overview_regrades',
                'questionusageid ' . $qubaids->usage_id_in(), $qubaids->usage_id_in_params());
        return quizsbs_report_index_by_keys($regradedqs, array('questionusageid', 'slot'));
    }
}
