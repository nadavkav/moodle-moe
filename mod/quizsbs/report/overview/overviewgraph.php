...
/**
 * This file renders the quizsbs overview graph.
 *
...
$quizsbsid = required_param('id', PARAM_INT);
$groupid = optional_param('groupid', 0, PARAM_INT);

$quizsbs = $DB->get_record('quizsbs', array('id' => $quizsbsid));
$course = $DB->get_record('course', array('id' => $quizsbs->course));
$cm = get_coursemodule_from_instance('quizsbs', $quizsbsid);

require_login($course, false, $cm);
$modcontext = context_module::instance($cm->id);
require_capability('mod/quizsbs:viewreports', $modcontext);
...
// Pick a sensible number of bands depending on quizsbs maximum grade.
$bands = $quizsbs->grade;
...
$bandwidth = $quizsbs->grade / $bands;
$bandlabels = array();
for ($i = 1; $i <= $bands; $i++) {
    $bandlabels[] = quizsbs_format_grade($quizsbs, ($i - 1) * $bandwidth) . ' - ' .
            quizsbs_format_grade($quizsbs, $i * $bandwidth);
}
...
$line->y_data['allusers'] = quizsbs_report_grade_bands($bandwidth, $bands, $quizsbsid, $groupusers);
