 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizsbs_report_responses_from_steps_testcase extends mod_quizsbs_attempt_walkthrough_from_csv_testcase {
     ...
     *
     * @param array $quizsbssettings settings to override default settings for quizsbs created by generator. Taken from quizsbszes.csv.
    ...
    public function test_walkthrough_from_csv($quizsbssettings, $csvdata) {
        ...
        $quizsbsattemptids = $this->walkthrough_attempts($csvdata['steps']);
        ...
        for ($rowno = 0; $rowno < $csvdata['responses']->getRowCount(); $rowno++) {
            ...
            if (!isset($quizsbsattemptids[$responses['quizsbsattempt']])) {
                throw new coding_exception("There is no quizsbsattempt {$responses['quizsbsattempt']}!");
            }
            $this->assert_response_test($quizsbsattemptids[$responses['quizsbsattempt']], $responses);
        }
    }

    protected function assert_response_test($quizsbsattemptid, $responses) {
        $quizsbsattempt = quizsbs_attempt::create($quizsbsattemptid);
        ...
        foreach ($responses['slot'] as $slot => $tests) {
            ...
            $qa = $quizsbsattempt->get_question_attempt($slot);
            ...
            if (null === $step) {
                throw new coding_exception("There is no step no {$responses['submittedstepno']} ".
                                           "for slot $slot in quizsbsattempt {$responses['quizsbsattempt']}!");
            }
            foreach (array('responsesummary', 'fraction', 'state') as $column) {
                if (isset($tests[$column]) && $tests[$column] != '') {
                    ...
                    $failuremessage = "Error in  quizsbsattempt {$responses['quizsbsattempt']} in $column, slot $slot, ".
