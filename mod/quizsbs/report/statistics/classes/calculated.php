...
class calculated {
    ...
    /* Following stats all described here : http://docs.moodle.org/dev/quizsbs_statistics_calculations#Test_statistics  */
     ...
     */
    public function get_formatted_quizsbs_info_data($course, $cm, $quizsbs) {
        ...
        // General information about the quizsbs.
        $quizsbsinfo = array();
        $quizsbsinfo[get_string('quizsbsname', 'quizsbs_statistics')] = format_string($quizsbs->name);
        $quizsbsinfo[get_string('coursename', 'quizsbs_statistics')] = format_string($course->fullname);
        if ($cm->idnumber) {
            $quizsbsinfo[get_string('idnumbermod')] = $cm->idnumber;
        }
        if ($quizsbs->timeopen) {
            $quizsbsinfo[get_string('quizsbsopen', 'quizsbs')] = userdate($quizsbs->timeopen);
        }
        if ($quizsbs->timeclose) {
            $quizsbsinfo[get_string('quizsbsclose', 'quizsbs')] = userdate($quizsbs->timeclose);
        }
        if ($quizsbs->timeopen && $quizsbs->timeclose) {
            $quizsbsinfo[get_string('duration', 'quizsbs_statistics')] =
                format_time($quizsbs->timeclose - $quizsbs->timeopen);
        }
        ...
        foreach ($todisplay as $property => $format) {
            ...
            switch ($format) {
                case 'summarks_as_percentage':
                    $formattedvalue = quizsbs_report_scale_summarks_as_percentage($value, $quizsbs);
                    break;
                case 'number_format_percent':
                    $formattedvalue = quizsbs_format_grade($quizsbs, $value) . '%';
                    break;
                case 'number_format':
                    // 2 extra decimal places, since not a percentage,
                    // and we want the same number of sig figs.
                    $formattedvalue = format_float($value, $quizsbs->decimalpoints + 2);
                    break;
                case 'number':
            ...
            $quizsbsinfo[get_string($property, 'quizsbs_statistics',
    ...
    public function cache($qubaids) {
        ...
        // Store the data.
        $DB->insert_record('quizsbs_statistics', $toinsert);

    }

    /**
     * Given a record from 'quizsbs_statistics' table load the data into the properties of this class.
     *
