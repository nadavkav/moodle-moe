...
/**
 * Standard plugin entry points of the quizsbs statistics report.
 *
 ...
 * @package  quizsbs_statistics
 * @category files
 * @param context $previewcontext the quizsbs context
 ...
 */
function quizsbs_statistics_question_preview_pluginfile($previewcontext, $questionid,
    ...
    require_capability('quizsbs/statistics:view', $context);
...
/**
 * quizsbs statistics report cron code. Deletes cached data more than a certain age.
 */
function quizsbs_statistics_cron() {
    global $DB;

    mtrace("\n  Cleaning up old quizsbs statistics cache records...", '');

    $expiretime = time() - 5*HOURSECS;
    $DB->delete_records_select('quizsbs_statistics', 'timemodified < ?', array($expiretime));
