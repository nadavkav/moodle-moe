...
/**
 * This script renders the quizsbs statistics graph.
 *
 * It takes one parameter, the quizsbs_statistics.id. This is enough to identify the
 * quizsbs etc.
 *
...
// Check access.
require_login($quizsbs->course, false, $cm);
$modcontext = context_module::instance($cm->id);
require_capability('quizsbs/statistics:view', $modcontext);
...
if (empty($currentgroup)) {
    ...
    $groupstudents = get_users_by_capability($modcontext, array('mod/quizsbs:reviewmyattempts', 'mod/quizsbs:attempt'),
                                             '', '', '', '', $currentgroup, '', false);
}
$qubaids = quizsbs_statistics_qubaids_condition($quizsbsid, $groupstudents, $whichattempts);
...
$graph->parameter['x_label'] = get_string('position', 'quizsbs_statistics');
...
$fieldstoplot = array(
    ...
    'discriminativeefficiency' => get_string('discriminative_efficiency', 'quizsbs_statistics')
);
...
foreach (array_keys($fieldstoplot) as $fieldtoplot) {
    ...
    $graph->y_format[$fieldtoplot] = array(
        'colour' => quizsbs_statistics_graph_get_new_colour(),
        'bar' => 'fill',
        'shadow_offset' => 1,
