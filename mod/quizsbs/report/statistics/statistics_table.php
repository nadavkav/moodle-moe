...
/**
 * quizsbs statistics report, table for showing statistics of each question in the quizsbs.
 *
...
/**
 * This table has one row for each question in the quizsbs, with sub-rows when
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizsbs_statistics_table extends flexible_table {
    /** @var object the quizsbs settings. */
    protected $quizsbs;

    /** @var integer the quizsbs course_module id. */
    protected $cmid;
    ...
    public function __construct() {
        parent::__construct('mod-quizsbs-report-statistics-report');
    }
     ...
     * @param int $cmid the quizsbs course_module id
     * @param moodle_url $reporturl the URL to redisplay this report.
     * @param int $s number of attempts included in the statistics.
     */
    public function statistics_setup($quizsbs, $cmid, $reporturl, $s) {
        ...
        $columns[] = 'number';
        $headers[] = get_string('questionnumber', 'quizsbs_statistics');
        ...
        if (!$this->is_downloading()) {
            ...
            $columns[] = 'qtype';
            $headers[] = get_string('questiontype', 'quizsbs_statistics');
        }

        $columns[] = 'name';
        $headers[] = get_string('questionname', 'quizsbs');

        $columns[] = 's';
        $headers[] = get_string('attempts', 'quizsbs_statistics');

        if ($s > 1) {
            $columns[] = 'facility';
            $headers[] = get_string('facility', 'quizsbs_statistics');

            $columns[] = 'sd';
            $headers[] = get_string('standarddeviationq', 'quizsbs_statistics');
        }

        $columns[] = 'random_guess_score';
        $headers[] = get_string('random_guess_score', 'quizsbs_statistics');

        $columns[] = 'intended_weight';
        $headers[] = get_string('intended_weight', 'quizsbs_statistics');

        $columns[] = 'effective_weight';
        $headers[] = get_string('effective_weight', 'quizsbs_statistics');

        $columns[] = 'discrimination_index';
        $headers[] = get_string('discrimination_index', 'quizsbs_statistics');

        $columns[] = 'discriminative_efficiency';
        $headers[] = get_string('discriminative_efficiency', 'quizsbs_statistics');
    ...
    protected function col_actions($questionstat) {
        return quizsbs_question_action_icons($this->quizsbs, $this->cmid,
    ...
    protected function col_name($questionstat) {
        ...
        if (!is_null($questionstat->variant)) {
            ...
            $name = get_string('nameforvariant', 'quizsbs_statistics', $a);
        }
        ...
        if (!is_null($questionstat->variant)) {
            ...
            if ($questionstat->subquestion && !$questionstat->get_variants()) {
                ...
                $name = html_writer::link($url, $name, array('title' => get_string('detailedanalysis', 'quizsbs_statistics')));
                ...
                if ($questionstat->get_variants() || $questionstat->get_sub_question_ids()) {
                    ...
                    $name = html_writer::link($url,
                                              $name,
                                              array('title' => get_string('slotstructureanalysis', 'quizsbs_statistics', $number)));
                } else {
                    // Question cannot be broken down into sub-questions or variants. Link will show response analysis page.
                    $name = html_writer::link($url,
                                              $name,
                                              array('title' => get_string('detailedanalysis', 'quizsbs_statistics')));
                }
            }
        }
        ...
        if (!empty($questionstat->minmedianmaxnotice)) {
            $name = get_string($questionstat->minmedianmaxnotice, 'quizsbs_statistics') . '<br />' . $name;
        }
     ...
     * of maximum mark for the quizsbs. That is, the indended influence this question
     * on the student's overall mark.
     * @param \core_question\statistics\questions\calculated $questionstat stats for the question.
     * @return string contents of this table cell.
     */
    protected function col_intended_weight($questionstat) {
        return quizsbs_report_scale_summarks_as_percentage($questionstat->maxmark, $this->quizsbs);
    }
    ...
    protected function col_effective_weight($questionstat) {
        ...
        if ($questionstat->negcovar) {
            $negcovar = get_string('negcovar', 'quizsbs_statistics');

            if (!$this->is_downloading()) {
                $negcovar = html_writer::tag('div',
                        $negcovar . $OUTPUT->help_icon('negcovar', 'quizsbs_statistics'),
     ...
     * other questions in this quizsbs.
     * @param \core_question\statistics\questions\calculated $questionstat stats for the question.
     * @return string contents of this table cell.
     */
