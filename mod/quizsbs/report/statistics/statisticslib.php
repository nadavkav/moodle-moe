...
/**
 * Common functions for the quizsbs statistics report.
 *
...
/**
 * SQL to fetch relevant 'quizsbs_attempts' records.
 *
 * @param int    $quizsbsid        quizsbs id to get attempts for
 ...
 */
function quizsbs_statistics_attempts_sql($quizsbsid, $groupstudents, $whichattempts = quizsbs_GRADEAVERAGE, $includeungraded = false) {
    global $DB;

    $fromqa = '{quizsbs_attempts} quizsbsa ';

    $whereqa = 'quizsbsa.quizsbs = :quizsbsid AND quizsbsa.preview = 0 AND quizsbsa.state = :quizsbsstatefinished';
    $qaparams = array('quizsbsid' => (int)$quizsbsid, 'quizsbsstatefinished' => quizsbs_attempt::FINISHED);

    if ($groupstudents) {
        ksort($groupstudents);
        list($grpsql, $grpparams) = $DB->get_in_or_equal(array_keys($groupstudents),
                SQL_PARAMS_NAMED, 'statsuser');
        list($grpsql, $grpparams) = quizsbs_statistics_renumber_placeholders(
                $grpsql, $grpparams, 'statsuser');
        $whereqa .= " AND quizsbsa.userid $grpsql";
        $qaparams += $grpparams;
    }

    $whichattemptsql = quizsbs_report_grade_method_sql($whichattempts);
    if ($whichattemptsql) {
        $whereqa .= ' AND '.$whichattemptsql;
    }

    if (!$includeungraded) {
        $whereqa .= ' AND quizsbsa.sumgrades IS NOT NULL';
    }
 ...
 */
function quizsbs_statistics_renumber_placeholders($sql, $params, $paramprefix) {
    $basenumber = null;
    $newparams = array();
...
/**
 * Return a {@link qubaid_condition} from the values returned by {@link quizsbs_statistics_attempts_sql}.
 *
 * @param int     $quizsbsid
 ...
 * @return        \qubaid_join
 */
function quizsbs_statistics_qubaids_condition($quizsbsid, $groupstudents, $whichattempts = quizsbs_GRADEAVERAGE, $includeungraded = false) {
    list($fromqa, $whereqa, $qaparams) = quizsbs_statistics_attempts_sql($quizsbsid, $groupstudents, $whichattempts, $includeungraded);
    return new qubaid_join($fromqa, 'quizsbsa.uniqueid', $whereqa, $qaparams);
}
 ...
 */
function quizsbs_statistics_graph_get_new_colour() {
