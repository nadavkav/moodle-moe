 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizsbs_statistics_statisticslib_testcase extends basic_testcase {

    public function test_quizsbs_statistics_renumber_placeholders_no_op() {
        list($sql, $params) = quizsbs_statistics_renumber_placeholders(
                ' IN (:u1, :u2)', array('u1' => 1, 'u2' => 2), 'u');
        $this->assertEquals(' IN (:u1, :u2)', $sql);
        $this->assertEquals(array('u1' => 1, 'u2' => 2), $params);
    }

    public function test_quizsbs_statistics_renumber_placeholders_work_to_do() {
        list($sql, $params) = quizsbs_statistics_renumber_placeholders(
