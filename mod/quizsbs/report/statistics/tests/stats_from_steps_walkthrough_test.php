 ...
 *
 * The quizsbs stats below and the question stats found in qstats00.csv were calculated independently in a spreadsheet which is
 * available in open document or excel format here :
 * https://github.com/jamiepratt/moodle-quizsbs-tools/tree/master/statsspreadsheet
 *
...
global $CFG;
require_once($CFG->dirroot . '/mod/quizsbs/tests/attempt_walkthrough_from_csv_test.php');
require_once($CFG->dirroot . '/mod/quizsbs/report/default.php');
require_once($CFG->dirroot . '/mod/quizsbs/report/statistics/report.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class quizsbs_report_statistics_from_steps_testcase extends mod_quizsbs_attempt_walkthrough_from_csv_testcase {

    /**
     * @var quizsbs_statistics_report object to do stats calculations.
     */
    protected $report;
    ...
    /**
     * Create a quizsbs add questions to it, walk through quizsbs attempts and then check results.
     *
    ...
    public function test_walkthrough_from_csv($quizsbssettings, $csvdata) {
        ...
        $whichattempts = quizsbs_GRADEAVERAGE; // All attempts.
        $whichtries = question_attempt::ALL_TRIES;
        $groupstudents = array();
        list($questions, $quizsbsstats, $questionstats, $qubaids) =
                    $this->check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries, $groupstudents);
        if ($quizsbssettings['testnumber'] === '00') {
            $this->check_variants_count_for_quizsbs_00($questions, $questionstats, $whichtries, $qubaids);
            $this->check_quizsbs_stats_for_quizsbs_00($quizsbsstats);
        }
    }
     ...
     * @param $qubaids
     */
    protected function check_variants_count_for_quizsbs_00($questions, $questionstats, $whichtries, $qubaids) {
     ...
     * @param $quizsbsstats
     */
    protected function check_quizsbs_stats_for_quizsbs_00($quizsbsstats) {
        $quizsbsstatsexpected = array(
            'median'             => 4.5,
        ...
        foreach ($quizsbsstatsexpected as $statname => $statvalue) {
            $this->assertEquals($statvalue, $quizsbsstats->$statname, $quizsbsstats->$statname, abs($statvalue) * 1.5e-5);
        }
    }
     ...
     * @return array with contents 0 => $questions, 1 => $quizsbsstats, 2=> $questionstats, 3=> $qubaids Might be needed for further
     *               testing.
     */
    protected function check_stats_calculations_and_response_analysis($csvdata, $whichattempts, $whichtries, $groupstudents) {
        $this->report = new quizsbs_statistics_report();
        $questions = $this->report->load_and_initialise_questions_for_calculations($this->quizsbs);
        list($quizsbsstats, $questionstats) = $this->report->get_all_stats_and_analysis($this->quizsbs,
                                                                                     $whichattempts,
                                                                                     $whichtries,
                                                                                     $groupstudents,
                                                                                     $questions);

        $qubaids = quizsbs_statistics_qubaids_condition($this->quizsbs->id, $groupstudents, $whichattempts);

        // We will create some quizsbs and question stat calculator instances and some response analyser instances, just in order
        // to check the last analysed time then returned.
        $quizsbscalc = new \quizsbs_statistics\calculator();
        // Should not be a delay of more than one second between the calculation of stats above and here.
        $this->assertTimeCurrent($quizsbscalc->get_last_calculated_time($qubaids));
        ...
        if (isset($csvdata['qstats'])) {
            ...
            return array($questions, $quizsbsstats, $questionstats, $qubaids);
        }
        return array($questions, $quizsbsstats, $questionstats, $qubaids);
    }
