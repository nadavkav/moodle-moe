...
$baseurl = new moodle_url('/mod/quizsbs/reviewquestion.php',
...
$PAGE->set_title(get_string('reviewofquestion', 'quizsbs', array(
        'question' => format_string($attemptobj->get_question_name($slot)),
        'quizsbs' => format_string($attemptobj->get_quizsbs_name()), 'user' => fullname($student))));
...
if ($attemptobj->is_own_attempt()) {
    if (!$attemptobj->is_finished()) {
        echo $output->review_question_not_allowed($attemptobj, get_string('cannotreviewopen', 'quizsbs'));
        die();
...
// quizsbs name.
$summarydata['quizsbsname'] = array(
    'title'   => get_string('modulename', 'quizsbs'),
    'content' => format_string($attemptobj->get_quizsbs_name()),
);

// Question name.
$summarydata['questionname'] = array(
    'title'   => get_string('question', 'quizsbs'),
    'content' => $attemptobj->get_question_name($slot),
);

// Other attempts at the quizsbs.
