...
/**
 * Tests for the quizsbs_attempt class.
 *
 * @package   mod_quizsbs
 * @category  test
...
/**
 * Subclass of quizsbs_attempt to allow faking of the page layout.
 *
 * @copyright 2014 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_quizsbs_attempt_testable extends quizsbs_attempt {
     ...
     * @param string $layout layout to set. Like quizsbs attempt.layout. E.g. '1,2,0,3,4,0,'.
     * @param array $infos slot numbers which contain 'descriptions', or other non-questions.
     * @return quizsbs_attempt attempt object for use in unit tests.
     */
    public static function setup_fake_attempt_layout($id, $layout, $infos = array()) {
        ...
        $attemptobj = new self($attempt, $quizsbs, $cm, $course, false);
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_quizsbs_attempt_testcase extends basic_testcase {
    /**
     * Test the functions quizsbs_update_open_attempts() and get_list_of_overdue_attempts()
     */
    public function test_attempt_url() {
        $attempt = mod_quizsbs_attempt_testable::setup_fake_attempt_layout(
                123, '1,2,0,3,4,0,5,6,0');

        // Attempt pages.
        $this->assertEquals(new moodle_url(
                '/mod/quizsbs/attempt.php?attempt=123'),
                $attempt->attempt_url());

        $this->assertEquals(new moodle_url(
                '/mod/quizsbs/attempt.php?attempt=123&page=2'),
                $attempt->attempt_url(null, 2));

        $this->assertEquals(new moodle_url(
                '/mod/quizsbs/attempt.php?attempt=123&page=1#'),
                $attempt->attempt_url(3));

        $this->assertEquals(new moodle_url(
                '/mod/quizsbs/attempt.php?attempt=123&page=1#q4'),
        ...
        // Summary page.
        $this->assertEquals(new moodle_url(
                '/mod/quizsbs/summary.php?attempt=123'),
                $attempt->summary_url());

        // Review page.
        $this->assertEquals(new moodle_url(
                '/mod/quizsbs/review.php?attempt=123'),
                $attempt->review_url());

        $this->assertEquals(new moodle_url(
                '/mod/quizsbs/review.php?attempt=123&page=2'),
                $attempt->review_url(null, 2));

        $this->assertEquals(new moodle_url(
                '/mod/quizsbs/review.php?attempt=123&page=1'),
                $attempt->review_url(3, -1, false));

        $this->assertEquals(new moodle_url(
                '/mod/quizsbs/review.php?attempt=123&page=1#q4'),
        ...
        $this->assertEquals(new moodle_url(
                '/mod/quizsbs/review.php?attempt=123&showall=0'),
        ...
        // Review with more than 50 questions in the quizsbs.
        $attempt = mod_quizsbs_attempt_testable::setup_fake_attempt_layout(
        ...
        $this->assertEquals(new moodle_url(
                '/mod/quizsbs/review.php?attempt=124'),
                $attempt->review_url());

        $this->assertEquals(new moodle_url(
                '/mod/quizsbs/review.php?attempt=124&page=2'),
                $attempt->review_url(null, 2));

        $this->assertEquals(new moodle_url(
                '/mod/quizsbs/review.php?attempt=124&page=1'),
                $attempt->review_url(11, -1, false));

        $this->assertEquals(new moodle_url(
                '/mod/quizsbs/review.php?attempt=124&page=1#q12'),
                $attempt->review_url(12, -1, false));

        $this->assertEquals(new moodle_url(
                '/mod/quizsbs/review.php?attempt=124&showall=1'),
                $attempt->review_url(null, 2, true));
