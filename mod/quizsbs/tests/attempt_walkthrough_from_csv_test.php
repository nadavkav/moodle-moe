...
/**
 * quizsbs attempt walk through using data from csv file.
 *
 * @package    mod_quizsbs
 * @category   phpunit
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_quizsbs_attempt_walkthrough_from_csv_testcase extends advanced_testcase {

    protected $files = array('questions', 'steps', 'results');

    /**
     * @var stdClass the quizsbs record we create.
     */
    protected $quizsbs;
     ...
     * directory.
     *
     * @param array $quizsbssettings of settings read from csv file quizsbszes.csv
     ...
     */
    public function test_walkthrough_from_csv($quizsbssettings, $csvdata) {

        // CSV data files for these tests were generated using :
        // https://github.com/jamiepratt/moodle-quizsbs-tools/tree/master/responsegenerator

        $this->create_quizsbs_simulate_attempts_and_check_results($quizsbssettings, $csvdata);
    }

    public function create_quizsbs($quizsbssettings, $qs) {
        global $SITE, $DB;
        $this->setAdminUser();
        ...
        $aggregratedsettings = $quizsbssettings + array('course' => $SITE->id,
                                                     'questionsperpage' => 0,
                                                     'grade' => 100.0,
                                                     'sumgrades' => $sumofgrades);

        $this->quizsbs = $quizsbsgenerator->create_instance($aggregratedsettings);

        $this->randqids = array();
        foreach ($slots as $slotno => $slotquestion) {
            if ($slotquestion['type'] !== 'random') {
                quizsbs_add_quizsbs_question($slotquestion['id'], $this->quizsbs, 0, $slotquestion['mark']);
            } else {
                quizsbs_add_random_questions($this->quizsbs, 0, $slotquestion['catid'], 1, 0);
    ...
    /**
     * Create quizsbs, simulate attempts and check results (if resultsXX.csv exists).
     *
     * @param array $quizsbssettings quizsbs overrides for this quizsbs.
     * @param PHPUnit_Extensions_Database_DataSet_ITable[] $csvdata Data loaded from csv files for this test.
     */
    protected function create_quizsbs_simulate_attempts_and_check_results($quizsbssettings, $csvdata) {
        $this->resetAfterTest(true);
        question_bank::get_qtype('random')->clear_caches_before_testing();

        $this->create_quizsbs($quizsbssettings, $csvdata['questions']);
    ...
    public function get_data_for_walkthrough() {
        $quizsbszes = $this->load_csv_data_file('quizsbszes');
        $datasets = array();
        for ($rowno = 0; $rowno < $quizsbszes->getRowCount(); $rowno++) {
            $quizsbssettings = $quizsbszes->getRow($rowno);
            $dataset = array();
            foreach ($this->files as $file) {
                if (file_exists($this->get_full_path_of_csv_file($file, $quizsbssettings['testnumber']))) {
                    $dataset[$file] = $this->load_csv_data_file($file, $quizsbssettings['testnumber']);
                }
            }
            $datasets[] = array($quizsbssettings, $dataset);
        }
        return $datasets;
    }
     ...
     * @return array attempt no as in csv file => the id of the quizsbs_attempt as stored in the db.
     */
    protected function walkthrough_attempts($steps) {
        ...
        for ($rowno = 0; $rowno < $steps->getRowCount(); $rowno++) {
            ...
            // Find existing user or make a new user to do the quizsbs.
            ...
            if (!isset($attemptids[$step['quizsbsattempt']])) {
                // Start the attempt.
                $quizsbsobj = quizsbs::create($this->quizsbs->id, $user->id);
                ...
                $prevattempts = quizsbs_get_user_attempts($this->quizsbs->id, $user->id, 'all', true);
                $attemptnumber = count($prevattempts) + 1;
                $timenow = time();
                $attempt = quizsbs_create_attempt($quizsbsobj, $attemptnumber, false, $timenow, false, $user->id);
                ...
                quizsbs_start_new_attempt($quizsbsobj, $quba, $attempt, $attemptnumber, $timenow, $step['randqs'], $step['variants']);
                quizsbs_attempt_save_started($quizsbsobj, $quba, $attempt);
                $attemptid = $attemptids[$step['quizsbsattempt']] = $attempt->id;
            } else {
                $attemptid = $attemptids[$step['quizsbsattempt']];
            }
     ...
     * @param $attemptids array attempt no as in csv file => the id of the quizsbs_attempt as stored in the db.
     */
    protected function check_attempts_results($results, $attemptids) {
        for ($rowno = 0; $rowno < $results->getRowCount(); $rowno++) {
            ...
            // Re-load quizsbs attempt data.
            $attemptobj = quizsbs_attempt::create($attemptids[$result['quizsbsattempt']]);
     ...
     * @param quizsbs_attempt $attemptobj         the attempt object loaded from db.
        ...
        foreach ($result as $fieldname => $value) {
            ...
            switch ($fieldname) {
                            ...
                            switch ($slotfieldname) {
                                case 'mark' :
                                    $this->assertEquals(round($slotvalue, 2), $attemptobj->get_question_mark($slotno),
                                                        "Mark for slot $slotno of attempt {$result['quizsbsattempt']}.");
                                    break;
                                default :
                ...
                case 'finished' :
                    ...
                    break;
                case 'summarks' :
                    $this->assertEquals($value, $attemptobj->get_sum_marks(), "Sum of marks of attempt {$result['quizsbsattempt']}.");
                    break;
                case 'quizsbsgrade' :
                    // Check quizsbs grades.
                    $grades = quizsbs_get_user_grades($attemptobj->get_quizsbs(), $attemptobj->get_userid());
                    $grade = array_shift($grades);
                    $this->assertEquals($value, $grade->rawgrade, "quizsbs grade for attempt {$result['quizsbsattempt']}.");
                    break;
                case 'gradebookgrade' :
                    // Check grade book.
                    ...
                    $this->assertEquals($value, $gradebookgrade->grade, "Gradebook grade for attempt {$result['quizsbsattempt']}.");
                    break;
                default :
