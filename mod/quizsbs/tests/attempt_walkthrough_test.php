...
/**
 * quizsbs attempt walk through tests.
 *
 * @package    mod_quizsbs
 * @category   phpunit
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_quizsbs_attempt_walkthrough_testcase extends advanced_testcase {

    /**
     * Create a quizsbs with questions and walk through a quizsbs attempt.
     */
    public function test_quizsbs_attempt_walkthrough() {
        global $SITE;
        ...
        $quizsbs = $quizsbsgenerator->create_instance(array('course'=>$SITE->id, 'questionsperpage' => 0, 'grade' => 100.0,
                                                      'sumgrades' => 2));
        ...
        $timenow = time();
        $attempt = quizsbs_create_attempt($quizsbsobj, 1, false, $timenow, false, $user1->id);
        ...
        $grades = quizsbs_get_user_grades($quizsbs, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'quizsbs', $quizsbs->id, $user1->id);
    ...
    /**
     * Create a quizsbs with a random as well as other questions and walk through quizsbs attempts.
     */
    public function test_quizsbs_with_random_question_attempt_walkthrough() {
        global $SITE;
        ...
        $quizsbs = $quizsbsgenerator->create_instance(array('course' => $SITE->id, 'questionsperpage' => 2, 'grade' => 100.0,
                                                      'sumgrades' => 4));
        ...
        // Add random question to the quizsbs.
        ...
        foreach (array($saq->id => 'frog', $numq->id => '3.14') as $randomqidtoselect => $randqanswer) {
            ...
            $timenow = time();
            $attempt = quizsbs_create_attempt($quizsbsobj, 1, false, $timenow);

            quizsbs_start_new_attempt($quizsbsobj, $quba, $attempt, 1, $timenow, array(1 => $randomqidtoselect));
    ...
    protected $quizsbswithvariants = null;

    /**
     * Create a quizsbs with a single question with variants and walk through quizsbs attempts.
     *
     * @dataProvider get_correct_response_for_variants
     */
    public function test_quizsbs_with_question_with_variants_attempt_walkthrough($variantno, $correctresponse, $done = false) {
        global $SITE;
        ...
        if ($this->quizsbswithvariants === null) {
            // Make a quizsbs.
            $quizsbsgenerator = $this->getDataGenerator()->get_plugin_generator('mod_quizsbs');

            $this->quizsbswithvariants = $quizsbsgenerator->create_instance(array('course'=>$SITE->id,
                                                                            'questionsperpage' => 0,
                                                                            'grade' => 100.0,
                                                                            'sumgrades' => 1));
            ...
            quizsbs_add_quizsbs_question($calc->id, $this->quizsbswithvariants, 0);
        }


        // Make a new user to do the quizsbs.
        $user1 = $this->getDataGenerator()->create_user();
        $this->setUser($user1);
        $quizsbsobj = quizsbs::create($this->quizsbswithvariants->id, $user1->id);
        ...
        // Select variant.
        quizsbs_start_new_attempt($quizsbsobj, $quba, $attempt, 1, $timenow, array(), array(1 => $variantno));
        ...
        // Check quizsbs grades.
        $grades = quizsbs_get_user_grades($this->quizsbswithvariants, $user1->id);
        $grade = array_shift($grades);
        $this->assertEquals(100.0, $grade->rawgrade);

        // Check grade book.
        $gradebookgrades = grade_get_grades($SITE->id, 'mod', 'quizsbs', $this->quizsbswithvariants->id, $user1->id);
        $gradebookitem = array_shift($gradebookgrades->items);
        $gradebookgrade = array_shift($gradebookitem->grades);
        $this->assertEquals(100, $gradebookgrade->grade);
    }
