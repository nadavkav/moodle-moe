@mod @mod_quizsbs
Feature: Allow students to redo questions in a practice quizsbs, without starting a whole new attempt
  In order to practice particular skills I am struggling with
  As a student
  I need to be able to redo each question in a quizsbs as often as necessary without starting a whole new attempt, if my teacher allows it.
  ...
  Background:
    ...
    And the following "activities" exist:
      ...
      | quizsbs       | quizsbs 1 | quizsbs 1 description | C1     | quizsbs1    | immediatefeedback  | 1                |
  ...
  Scenario: Teachers reviewing can see all the qestions attempted in a slot
    ...
    And I navigate to "Statistics" node in "quizsbs administration > Results"
    And I follow "TF1"
    And "False" row "Frequency" column of "quizsbsresponseanalysis" table should contain "100.00%"
    And "True" row "Frequency" column of "quizsbsresponseanalysis" table should contain "0.00%"
    And "[No response]" row "Frequency" column of "quizsbsresponseanalysis" table should contain "100.00%"
