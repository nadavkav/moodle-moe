...
/**
 * Steps definitions related to mod_quizsbs.
 *
 * @package   mod_quizsbs
 * @category  test
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_mod_quizsbs extends behat_question_base {

    /**
     * Put the specified questions on the specified pages of a given quizsbs.
     *
     ...
     *
     * @param string $quizsbsname the name of the quizsbs to add questions to.
     * @param TableNode $data information about the questions to add.
     *
     * @Given /^quizsbs "([^"]*)" contains the following questions:$/
     */
    public function quizsbs_contains_the_following_questions($quizsbsname, TableNode $data) {
        global $DB;

        $quizsbs = $DB->get_record('quizsbs', array('name' => $quizsbsname), '*', MUST_EXIST);
        ...
        if (!in_array('question', $firstrow) && !in_array('page', $firstrow)) {
            if (count($firstrow) == 2) {
                ...
                throw new ExpectationException('When adding questions to a quizsbs, you should give 2 or three 3 things: ' .
        ...
        foreach ($data->getHash() as $questiondata) {
            if (!array_key_exists('question', $questiondata)) {
                throw new ExpectationException('When adding questions to a quizsbs, ' .
            ...
            // Add the question.
            quizsbs_add_quizsbs_question($questionid, $quizsbs, $page, $maxmark);

            // Require previous.
            if (array_key_exists('requireprevious', $questiondata)) {
                if ($questiondata['requireprevious'] === '1') {
                    $slot = $DB->get_field('quizsbs_slots', 'MAX(slot)', array('quizsbsid' => $quizsbs->id));
                    $DB->set_field('quizsbs_slots', 'requireprevious', 1,
                            array('quizsbsid' => $quizsbs->id, 'slot' => $slot));
    ...
    /**
     * Put the specified section headings to start at specified pages of a given quizsbs.
     *
     * The first row should be column names:
     ...
     *
     * @param string $quizsbsname the name of the quizsbs to add sections to.
     * @param TableNode $data information about the sections to add.
     *
     * @Given /^quizsbs "([^"]*)" contains the following sections:$/
     */
    public function quizsbs_contains_the_following_sections($quizsbsname, TableNode $data) {
        global $DB;
        ...
        foreach ($data->getHash() as $rownumber => $sectiondata) {
            if (!array_key_exists('heading', $sectiondata)) {
                throw new ExpectationException('When adding sections to a quizsbs, ' .
            ...
            if ($rownumber == 0) {
                $section = $DB->get_record('quizsbs_sections', array('quizsbsid' => $quizsbs->id), '*', MUST_EXIST);
            } else {
                $section = new stdClass();
                $section->quizsbsid = $quizsbs->id;
            }
            ...
            if ($rownumber == 0) {
                ...
                $DB->insert_record('quizsbs_sections', $section);
            }
        }

        if ($section->firstslot > $DB->count_records('quizsbs_slots', array('quizsbsid' => $quizsbs->id))) {
            throw new ExpectationException('The section firstslot must be less than the total number of slots in the quizsbs.',
                    $this->getSession());
        }
    }

    /**
     * Adds a question to the existing quizsbs with filling the form.
     *
     * The form for creating a question should be on one page.
     *
     * @When /^I add a "(?P<question_type_string>(?:[^"]|\\")*)" question to the "(?P<quizsbs_name_string>(?:[^"]|\\")*)" quizsbs with:$/
     * @param string $questiontype
     * @param string $quizsbsname
     * @param TableNode $questiondata with data for filling the add question form
     */
    public function i_add_question_to_the_quizsbs_with($questiontype, $quizsbsname, TableNode $questiondata) {
        $quizsbsname = $this->escape($quizsbsname);
        $editquizsbs = $this->escape(get_string('editquizsbs', 'quizsbs'));
        $quizsbsadmin = $this->escape(get_string('pluginadministration', 'quizsbs'));
        $addaquestion = $this->escape(get_string('addaquestion', 'quizsbs'));
        ...
        $this->execute('behat_general::click_link', $quizsbsname);

        $this->execute("behat_navigation::i_navigate_to_node_in", array($editquizsbs, $quizsbsadmin));
    ...
    /**
     * Set the max mark for a question on the Edit quizsbs page.
     *
     ...
     */
    public function i_set_the_max_mark_for_quizsbs_question($questionname, $newmark) {
        $this->execute('behat_general::click_link', $this->escape(get_string('editmaxmark', 'quizsbs')));
    ...
    /**
     * Open the add menu on a given page, or at the end of the Edit quizsbs page.
     * @Given /^I open the "(?P<page_n_or_last_string>(?:[^"]|\\")*)" add to quizsbs menu$/
     * @param string $pageorlast either "Page n" or "last".
     */
    public function i_open_the_add_to_quizsbs_menu_for($pageorlast) {
        ...
        if ($pageorlast == 'last') {
            ...
            throw new ExpectationException("The I open the add to quizsbs menu step must specify either 'Page N' or 'last'.");
        }
    ...
    /**
     * Check whether a particular question is on a particular page of the quizsbs on the Edit quizsbs page.
     * @Given /^I should see "(?P<question_name>(?:[^"]|\\")*)" on quizsbs page "(?P<page_number>\d+)"$/
     ...
     */
    public function i_should_see_on_quizsbs_page($questionname, $pagenumber) {
    ...
    /**
     * Check whether a particular question is not on a particular page of the quizsbs on the Edit quizsbs page.
     * @Given /^I should not see "(?P<question_name>(?:[^"]|\\")*)" on quizsbs page "(?P<page_number>\d+)"$/
     * @param string $questionname the name of the question we are looking for.
     * @param number $pagenumber the page it should be found on.
     */
    public function i_should_not_see_on_quizsbs_page($questionname, $pagenumber) {
        $xpath = "//li[contains(., '" . $this->escape($questionname) .
                "')][./preceding-sibling::li[contains(@class, 'pagenumber')][1][contains(., 'Page " .
                $pagenumber . "')]]";
    ...
    /**
     * Check whether one question comes before another on the Edit quizsbs page.
     * The two questions must be on the same page.
     * @Given /^I should see "(?P<first_q_name>(?:[^"]|\\")*)" before "(?P<second_q_name>(?:[^"]|\\")*)" on the edit quizsbs page$/
     ...
     */
    public function i_should_see_before_on_the_edit_quizsbs_page($firstquestionname, $secondquestionname) {
    ...
    /**
     * Check the number displayed alongside a question on the Edit quizsbs page.
     * @Given /^"(?P<question_name>(?:[^"]|\\")*)" should have number "(?P<number>(?:[^"]|\\")*)" on the edit quizsbs page$/
     * @param string $questionname the name of the question we are looking for.
     * @param number $number the number (or 'i') that should be displayed beside that question.
     */
    public function should_have_number_on_the_edit_quizsbs_page($questionname, $number) {
     ...
     *
     * @Given /^I click on shuffle for section "([^"]*)" on the quizsbs edit page$/
     */
     ...
     *
     * @Given /^shuffle for section "([^"]*)" should be "(On|Off)" on the quizsbs edit page$/
     */
    public function shuffle_for_section_should_be($heading, $value) {
        ...
        if ($value == 'On' && !$checkbox->isChecked()) {
            $msg = "Shuffle for section '$heading' is not checked, but you are expecting it to be checked ($value). " .
                    "Check the line with: \nshuffle for section \"$heading\" should be \"$value\" on the quizsbs edit page" .
    ...
    /**
     * Move a question on the Edit quizsbs page by first clicking on the Move icon,
     * then clicking one of the "After ..." links.
     * @When /^I move "(?P<question_name>(?:[^"]|\\")*)" to "(?P<target>(?:[^"]|\\")*)" in the quizsbs by clicking the move icon$/
     * @param string $questionname the name of the question we are looking for.
    ...
    /**
     * Move a question on the Edit quizsbs page by dragging a given question on top of another item.
     * @When /^I move "(?P<question_name>(?:[^"]|\\")*)" to "(?P<target>(?:[^"]|\\")*)" in the quizsbs by dragging$/
     * @param string $questionname the name of the question we are looking for.
    ...
    /**
     * Delete a question on the Edit quizsbs page by first clicking on the Delete icon,
     * then clicking one of the "After ..." links.
     * @When /^I delete "(?P<question_name>(?:[^"]|\\")*)" in the quizsbs by clicking the delete icon$/
     * @param string $questionname the name of the question we are looking for.
     * @return array of steps.
     */
    ...
    /**
     * Set the section heading for a given section on the Edit quizsbs page
     *
     * @When /^I change quizsbs section heading "(?P<section_name_string>(?:[^"]|\\")*)" to "(?P<new_section_heading_string>(?:[^"]|\\")*)"$/
     ...
     * quizsbs navigation block.
     *
     * @Then /^I should see question "(?P<questionnumber>\d+)" in section "(?P<section_heading_string>(?:[^"]|\\")*)" in the quizsbs navigation$/
     ...
     */
    public function i_should_see_question_in_section_in_the_quizsbs_navigation($questionnumber, $sectionheading) {
        ...
        $exception = new ExpectationException('Question "' . $questionnumber . '" is not in section "' .
                $sectionheading . '" in the quizsbs navigation.', $this->getSession());
        $xpath = "//div[@id = 'mod_quizsbs_navblock']//*[contains(concat(' ', normalize-space(@class), ' '), ' qnbutton ') and " .
