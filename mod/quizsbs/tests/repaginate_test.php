...
/**
 * Unit tests for the {@link \mod_quizsbs\repaginate} class.
 * @package   mod_quizsbs
 * @category  test
...
require_once($CFG->dirroot . '/mod/quizsbs/classes/repaginate.php');


/**
 * Testable subclass, giving access to the protected methods of {@link \mod_quizsbs\repaginate}
 * @copyright 2014 The Open Univsersity
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_quizsbs_repaginate_testable extends \mod_quizsbs\repaginate {

    public function __construct($quizsbsid = 0, $slots = null) {
        return parent::__construct($quizsbsid, $slots);
    }
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_quizsbs_repaginate_test extends advanced_testcase {

    /** @var array stores the slots. */
    private $quizsbsslots;
    /** @var mod_quizsbs_repaginate_testable the object being tested. */
    private $repaginate = null;

    public function setUp() {
        $this->set_quizsbs_slots($this->get_quizsbs_object()->get_slots());
        $this->repaginate = new mod_quizsbs_repaginate_testable(0, $this->quizsbsslots);
    }
    ...
    /**
     * Create a quizsbs, add five questions to the quizsbs
     * which are all on one page and return the quizsbs object.
     */
    private function get_quizsbs_object() {
        global $SITE;
        ...
        $quizsbs = $quizsbsgenerator->create_instance(array(
                'course' => $SITE->id, 'questionsperpage' => 0, 'grade' => 100.0, 'sumgrades' => 2));
        $cm = get_coursemodule_from_instance('quizsbs', $quizsbs->id, $SITE->id);
        ...
        quizsbs_add_quizsbs_question($shortanswer->id, $quizsbs);
        quizsbs_add_quizsbs_question($numerical->id, $quizsbs);
        quizsbs_add_quizsbs_question($essay->id, $quizsbs);
        quizsbs_add_quizsbs_question($truefalse->id, $quizsbs);
        quizsbs_add_quizsbs_question($match->id, $quizsbs);

        // Return the quizsbs object.
        $quizsbsobj = new quizsbs($quizsbs, $cm, $SITE);
        return \mod_quizsbs\structure::create_for_quizsbs($quizsbsobj);
    }

    /**
     * Set the quizsbs slots
     * @param string $slots
     */
    private function set_quizsbs_slots($slots = null) {
        if (!$slots) {
            $this->quizsbsslots = $this->get_quizsbs_object()->get_slots();
        } else {
            $this->quizsbsslots = $slots;
        }
    }
    ...
    public function test_get_this_slot() {
        $this->set_quizsbs_slots();
        $actual = array();
        $expected = $this->repaginate->get_slots_by_slot_number();
        $this->assertEquals($expected, $actual);

        $slotsbyno = $this->repaginate->get_slots_by_slot_number($this->quizsbsslots);
        $slotnumber = 5;
        $thisslot = $this->repaginate->get_this_slot($this->quizsbsslots, $slotnumber);
    ...
    public function test_get_slots_by_slotnumber() {
        ...
        foreach ($this->quizsbsslots as $slot) {
            $expected[$slot->slot] = $slot;
        }
        $actual = $this->repaginate->get_slots_by_slot_number($this->quizsbsslots);
        $this->assertEquals($expected, $actual);
    }
    ...
    public function test_get_slots_by_slotid() {
        ...
        $this->assertEquals($this->quizsbsslots, $actual);
    }
    ...
    public function test_repaginate_n_questions_per_page() {
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->quizsbsslots, 2);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->quizsbsslots, 3);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->quizsbsslots, 5);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->quizsbsslots, 10);
        $this->assertEquals($expected, $actual);
        ...
        $actual = $this->repaginate->repaginate_n_question_per_page($this->quizsbsslots, 1);
        $this->assertEquals($expected, $actual);
    }

    public function test_repaginate_this_slot() {
        $this->set_quizsbs_slots();
        $slotsbyslotno = $this->repaginate->get_slots_by_slot_number($this->quizsbsslots);
        $slotnumber = 3;
        $newpagenumber = 2;
    ...
    public function test_repaginate_the_rest() {
        $this->set_quizsbs_slots();
        $slotfrom = 1;
        $type = \mod_quizsbs\repaginate::LINK;
        $expected = array();
        foreach ($this->quizsbsslots as $slot) {
        ...
        $actual = $this->repaginate->repaginate_the_rest($this->quizsbsslots, $slotfrom, $type, false);
        $this->assertEquals($expected, $actual);

        $slotfrom = 2;
        $newslots = array();
        foreach ($this->quizsbsslots as $s) {
            if ($s->slot === $slotfrom) {
                $s->page = $s->page - 1;
            }
            $newslots[$s->id] = $s;
        ...
        $type = \mod_quizsbs\repaginate::UNLINK;
        $expected = array();
        foreach ($this->quizsbsslots as $slot) {
