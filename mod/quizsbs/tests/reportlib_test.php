...
/**
 * Unit tests for (some of) mod/quizsbs/report/reportlib.php
 *
 * @package   mod_quizsbs
 * @category  phpunit
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU Public License
 */
class mod_quizsbs_reportlib_testcase extends advanced_testcase {
    public function test_quizsbs_report_index_by_keys() {
        $datum = array();
        $object = new stdClass();
        $object->qid = 3;
        $object->aid = 101;
        ...
        $indexed = quizsbs_report_index_by_keys($datum, array('aid', 'qid'));
        ...
        $indexed = quizsbs_report_index_by_keys($datum, array('aid', 'qid'), false);
    ...
    public function test_quizsbs_report_scale_summarks_as_percentage() {
        $quizsbs = new stdClass();
        $quizsbs->sumgrades = 10;
        $quizsbs->decimalpoints = 2;

        $this->assertEquals('12.34567%',
            quizsbs_report_scale_summarks_as_percentage(1.234567, $quizsbs, false));
        $this->assertEquals('12.35%',
            quizsbs_report_scale_summarks_as_percentage(1.234567, $quizsbs, true));
        $this->assertEquals('-',
            quizsbs_report_scale_summarks_as_percentage('-', $quizsbs, true));
    }

    public function test_quizsbs_report_qm_filter_select_only_one_attempt_allowed() {
        $quizsbs = new stdClass();
        $quizsbs->attempts = 1;
        $this->assertSame('', quizsbs_report_qm_filter_select($quizsbs));
    }

    public function test_quizsbs_report_qm_filter_select_average() {
        $quizsbs = new stdClass();
        $quizsbs->attempts = 10;
        $quizsbs->grademethod = quizsbs_GRADEAVERAGE;
        $this->assertSame('', quizsbs_report_qm_filter_select($quizsbs));
    }

    public function test_quizsbs_report_qm_filter_select_first_last_best() {
        global $DB;
        $this->resetAfterTest();

        $fakeattempt = new stdClass();
        $fakeattempt->userid = 123;
        $fakeattempt->quizsbs = 456;
        $fakeattempt->layout = '1,2,0,3,4,0,5';
        $fakeattempt->state = quizsbs_attempt::FINISHED;

        // We intentionally insert these in a funny order, to test the SQL better.
        // The test data is:
        // id | quizsbsid | user | attempt | sumgrades | state
        // ---------------------------------------------------
        ...
        $DB->insert_record('quizsbs_attempts', $fakeattempt);
        ...
        $fakeattempt->state = quizsbs_attempt::IN_PROGRESS;
        $DB->insert_record('quizsbs_attempts', $fakeattempt);
        ...
        $quizsbs->grademethod = quizsbs_ATTEMPTFIRST;
        $firstattempt = $DB->get_records_sql("
                SELECT * FROM {quizsbs_attempts} quizsbsa WHERE userid = ? AND quizsbs = ? AND "
                        . quizsbs_report_qm_filter_select($quizsbs), array(123, 456));
        $this->assertEquals(1, count($firstattempt));
        $firstattempt = reset($firstattempt);
        $this->assertEquals(1, $firstattempt->attempt);

        $quizsbs->grademethod = quizsbs_ATTEMPTLAST;
        ...
        $bestattempt = $DB->get_records_sql("
                SELECT * FROM {quizsbs_attempts} qa_alias WHERE userid = ? AND quizsbs = ? AND "
                . quizsbs_report_qm_filter_select($quizsbs, 'qa_alias'), array(123, 456));
