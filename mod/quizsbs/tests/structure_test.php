 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_quizsbs_structure_testcase extends advanced_testcase {

    /**
     * Create a course with an empty quizsbs.
     * @return array with three elements quizsbs, cm and course.
     */
    protected function prepare_quizsbs_data() {
        ...
        return array($quizsbs, $cm, $course);
    }

    /**
     * Creat a test quizsbs.
     *
     ...
     * @return quizsbs the created quizsbs.
     */
    protected function create_test_quizsbs($layout) {
        list($quizsbs, $cm, $course) = $this->prepare_quizsbs_data();
        ...
        foreach ($layout as $item) {
            if (is_string($item)) {
                ...
                quizsbs_add_quizsbs_question($q->id, $quizsbs, $page);
                $lastpage = $page;
            }
        }

        $quizsbsobj = new quizsbs($quizsbs, $cm, $course);
        $structure = \mod_quizsbs\structure::create_for_quizsbs($quizsbsobj);
        if (isset($headings[1])) {
     ...
     * @param array $expectedlayout as for $layout in {@link create_test_quizsbs()}.
     * @param \mod_quizsbs\structure $structure the structure to test.
     */
    protected function assert_quizsbs_layout($expectedlayout, \mod_quizsbs\structure $structure) {
        $sections = $structure->get_sections();
        ...
        foreach ($expectedlayout as $item) {
            if (is_string($item)) {
                ...
                if ($slot > 1 && $section->heading == '' && $section->firstslot == 1) {
                    // The array $expectedlayout did not contain default first quizsbs section, so skip over it.
                    $section = array_shift($sections);
                }
        ...
        if ($slot - 1 != count($structure->get_slots())) {
            $this->fail('The quizsbs contains more slots than expected.');
        }

        if (!empty($sections)) {
            $section = array_shift($sections);
            if ($section->heading != '' || $section->firstslot != 1) {
                $this->fail('Unexpected section (' . $section->heading .') found in the quizsbs.');
            }
        }
    }
     ...
     * used by create_test_quizsbs as assert_quizsbs_layout.
    ...
    public function test_get_quizsbs_slots() {
        $quizsbsobj = $this->create_test_quizsbs(array(
                array('TF1', 1, 'truefalse'),
    ...
    public function test_quizsbs_has_one_section_by_default() {
        $quizsbsobj = $this->create_test_quizsbs(array(
                array('TF1', 1, 'truefalse'),
            ));
        $structure = \mod_quizsbs\structure::create_for_quizsbs($quizsbsobj);
    ...
    public function test_remove_section_heading() {
        ...
        $structure = \mod_quizsbs\structure::create_for_quizsbs($quizsbsobj);
        $this->assert_quizsbs_layout(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
                array('TF2', 2, 'truefalse'),
            ), $structure);
    ...
    public function test_quizsbs_remove_slot() {
        $quizsbsobj = $this->create_test_quizsbs(array(
                array('TF1', 1, 'truefalse'),
                array('TF2', 1, 'truefalse'),
                'Heading 2',
    ...
    public function test_quizsbs_removing_a_random_question_deletes_the_question() {
        global $DB;
        ...
        $questiongenerator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $questiongenerator->create_question_category();
        quizsbs_add_random_questions($quizsbsobj->get_quizsbs(), 1, $cat->id, 1, false);
        $structure = \mod_quizsbs\structure::create_for_quizsbs($quizsbsobj);
    ...
    public function test_can_remove_last_question_in_a_quizsbs() {
        $quizsbsobj = $this->create_test_quizsbs(array(
                'Heading 1',
                array('TF1', 1, 'truefalse'),
            ));
        ...
        quizsbs_add_quizsbs_question($q->id, $quizsbsobj->get_quizsbs(), 0);
        $structure = \mod_quizsbs\structure::create_for_quizsbs($quizsbsobj);
    ...
    public function test_add_question_updates_headings() {
        ...
        quizsbs_add_quizsbs_question($q->id, $quizsbsobj->get_quizsbs(), 1);
    ...
    public function test_remove_page_break() {
        ...
        $slots = $structure->update_page_break($slotid, \mod_quizsbs\repaginate::LINK);
    ...
    public function test_add_page_break() {
        ...
        $slotid = $structure->get_question_in_slot(2)->slotid;
        $slots = $structure->update_page_break($slotid, \mod_quizsbs\repaginate::UNLINK);
