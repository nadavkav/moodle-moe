YUI.add('moodle-mod_quizsbs-modform', function (Y, NAME) {

/**
 * The modform class has all the JavaScript specific to mod/quizsbs/mod_form.php.
 *
 * @module moodle-mod_quizsbs-modform
 */
...
Y.extend(MODFORM, Y.Base, {
    ...
    initializer: function () {
        ...
        if (!this.repaginateCheckbox) {
            // The checkbox only appears when editing an existing quizsbs.
            return;
        }
...
// Ensure that M.mod_quizsbs exists and that coursebase is initialised correctly
M.mod_quizsbs = M.mod_quizsbs || {};
M.mod_quizsbs.modform = M.mod_quizsbs.modform || new MODFORM();
M.mod_quizsbs.modform.init = function() {
    return new MODFORM();
};
