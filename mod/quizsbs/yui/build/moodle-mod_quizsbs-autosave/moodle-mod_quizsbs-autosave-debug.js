YUI.add('moodle-mod_quizsbs-autosave', function (Y, NAME) {
 ...
 *
 * @module moodle-mod_quizsbs-autosave
 */

/**
 * Auto-save functionality for during quizsbs attempts.
 *
 * @class M.mod_quizsbs.autosave
 */

M.mod_quizsbs = M.mod_quizsbs || {};
M.mod_quizsbs.autosave = {
    /**
     ...
     * @private
     */
    AUTOSAVE_HANDLER: M.cfg.wwwroot + '/mod/quizsbs/autosave.ajax.php',
    ...
    init: function(delay) {
        ...
        if (!this.form) {
            Y.log('No response form found. Why did you try to set up autosave?', 'debug', 'moodle-mod_quizsbs-autosave');
            return;
        }
    ...
    init_tinymce: function(repeatcount) {
        if (typeof window.tinyMCE === 'undefined') {
            if (repeatcount > 0) {
                ...
                Y.log('Gave up looking for TinyMCE.', 'debug', 'moodle-mod_quizsbs-autosave');
            }
            return;
        }

        Y.log('Found TinyMCE.', 'debug', 'moodle-mod_quizsbs-autosave');
    ...
    init_tinymce_editor: function(e, editor) {
        Y.log('Found TinyMCE editor ' + editor.id + '.', 'debug', 'moodle-mod_quizsbs-autosave');
    ...
    value_changed: function(e) {
        ...
        Y.log('Detected a value change in element ' + name + '.', 'debug', 'moodle-mod_quizsbs-autosave');
        this.start_save_timer_if_necessary();
    },

    editor_changed: function(editor) {
        Y.log('Detected a value change in editor ' + editor.id + '.', 'debug', 'moodle-mod_quizsbs-autosave');
        this.start_save_timer_if_necessary();
    },
    ...
    save_changes: function() {
        ...
        if (this.is_time_nearly_over()) {
            Y.log('No more saving, time is nearly over.', 'debug', 'moodle-mod_quizsbs-autosave');
            this.stop_autosaving();
            return;
        }

        Y.log('Doing a save.', 'debug', 'moodle-mod_quizsbs-autosave');
        if (typeof window.tinyMCE !== 'undefined') {
    ...
    save_done: function(transactionid, response) {
        ...
        Y.log('Save completed.', 'debug', 'moodle-mod_quizsbs-autosave');
        this.save_transaction = null;

        if (this.dirty) {
            Y.log('Dirty after save.', 'debug', 'moodle-mod_quizsbs-autosave');
            this.start_save_timer();
        }
    ...
    save_failed: function() {
        Y.log('Save failed.', 'debug', 'moodle-mod_quizsbs-autosave');
        this.save_transaction = null;
