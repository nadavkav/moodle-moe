YUI.add('moodle-mod_quizsbs-toolboxes', function (Y, NAME) {
...
Y.extend(TOOLBOX, Y.Base, {
    ...
    send_request: function(data, statusspinner, success_callback, optionalconfig) {
        ...
        data.quizsbsid = this.get('quizsbsid');
                ...
                success: function(tid, response) {
                    ...
                    if (responsetext.hasOwnProperty('newnumquestions')) {
                        Y.one(SELECTOR.NUMQUESTIONS).setHTML(
                                M.util.get_string('numquestionsx', 'quizsbs', responsetext.newnumquestions)
                            );
                    }
                    if (success_callback) {
 ...
 *
 * This class is responsible for managing AJAX interactions with activities and resources
 * when viewing a quizsbs in editing mode.
 *
 * @module mod_quizsbs-resource-toolbox
 * @namespace M.mod_quizsbs.resource_toolbox
 */
...
Y.extend(RESOURCETOOLBOX, TOOLBOX, {
    ...
    initializer: function() {
        M.mod_quizsbs.quizsbsbase.register_module(this);
    ...
    delete_with_confirmation: function(ev, button, activity) {
        ...
        confirmstring = M.util.get_string('confirmremovequestion', 'quizsbs', qtypename);
        ...
        confirm.on('complete-yes', function() {
            ...
            var data = {
                'class': 'resource',
                'action': 'DELETE',
                'id': Y.Moodle.mod_quizsbs.util.slot.getId(element)
            };
            this.send_request(data, spinner, function(response) {
                if (response.deleted) {
                    // Actually remove the element.
                    Y.Moodle.mod_quizsbs.util.slot.remove(element);
    ...
    edit_maxmark : function(ev, button, activity) {
        ...
        var instancemaxmark  = activity.one(SELECTOR.INSTANCEMAXMARK),
            ...
            data = {
                'class'   : 'resource',
                'field'   : 'getmaxmark',
                'id'      : Y.Moodle.mod_quizsbs.util.slot.getId(activity)
            };
    ...
    update_page_break: function(ev, button, activity, action) {
        ...
        var data = {
            ...
            'id':    Y.Moodle.mod_quizsbs.util.slot.getId(nextactivity),
            'value': value
        };
        ...
        this.send_request(data, spinner, function(response) {
            if (response.slots) {
                if (action === 'addpagebreak') {
                    Y.Moodle.mod_quizsbs.util.page.add(activity);
                } else {
                    var page = activity.next(Y.Moodle.mod_quizsbs.util.page.SELECTORS.PAGE);
                    Y.Moodle.mod_quizsbs.util.page.remove(page, true);
                }
                this.reorganise_edit_page();
            }
        });
    ...
    update_dependency: function(ev, button, activity, action) {
        ...
        this.send_request(data, spinner, function(response) {
            if (response.hasOwnProperty('requireprevious')) {
                Y.Moodle.mod_quizsbs.util.slot.updateDependencyIcon(activity, response.requireprevious);
            }
        });
    ...
    reorganise_edit_page: function() {
        Y.Moodle.mod_quizsbs.util.slot.reorderSlots();
        Y.Moodle.mod_quizsbs.util.slot.reorderPageBreaks();
        Y.Moodle.mod_quizsbs.util.page.reorderPages();
        Y.Moodle.mod_quizsbs.util.slot.updateOneSlotSections();
        Y.Moodle.mod_quizsbs.util.slot.updateAllDependencyIcons();
    },

    NAME : 'mod_quizsbs-resource-toolbox',
    ATTRS : {
        courseid : {
            'value' : 0
        },
...
M.mod_quizsbs.resource_toolbox = null;
M.mod_quizsbs.init_resource_toolbox = function(config) {
    M.mod_quizsbs.resource_toolbox = new RESOURCETOOLBOX(config);
    return M.mod_quizsbs.resource_toolbox;
};
/**
 ...
 *
 * @module moodle-mod_quizsbs-toolboxes
 * @namespace M.mod_quizsbs.toolboxes
 */
 ...
 *
 * This class is responsible for managing AJAX interactions with sections
 * when adding, editing, removing section headings when editing a quizsbs.
 *
 * @class section
 * @constructor
 * @extends M.mod_quizsbs.toolboxes.toolbox
 */
...
Y.extend(SECTIONTOOLBOX, TOOLBOX, {
    ...
    delete_section_with_confirmation: function(ev, button, activity) {
        ...
        // Create the confirmation dialogue.
        var confirm = new M.core.confirm({
            question: M.util.get_string('confirmremovesectionheading', 'quizsbs', activity.get('aria-label')),
            modal: true
        });
            ...
            this.send_request(data, spinner, function(response) {
                if (response) {
                    ...
                    activity.one(SELECTOR.EDITSECTIONICON).set('title',
                            M.util.get_string('sectionheadingedit', 'quizsbs', response.instancesection));
                    activity.one(SELECTOR.EDITSECTIONICON).set('alt',
                            M.util.get_string('sectionheadingedit', 'quizsbs', response.instancesection));
                    var deleteicon = activity.one(SELECTOR.DELETESECTIONICON);
                    if (deleteicon) {
                        deleteicon.set('title', M.util.get_string('sectionheadingremove', 'quizsbs', response.instancesection));
                        deleteicon.set('alt', M.util.get_string('sectionheadingremove', 'quizsbs', response.instancesection));
                    }
                }
            });
        }
    ...
    NAME: 'mod_quizsbs-section-toolbox',
    ATTRS: {
        courseid: {
            'value': 0
        },
...
M.mod_quizsbs.init_section_toolbox = function(config) {
    ...
    "requires": [
        ...
        "event-key",
        "io",
        "moodle-mod_quizsbs-quizsbsbase",
        "moodle-mod_quizsbs-util-slot",
