YUI.add('moodle-mod_quizsbs-util-slot', function (Y, NAME) {

/**
 * A collection of utility classes for use with slots.
 *
 * @module moodle-mod_quizsbs-util
 * @submodule moodle-mod_quizsbs-util-slot
 */

Y.namespace('Moodle.mod_quizsbs.util.slot');

/**
 * A collection of utility classes for use with slots.
 *
 * @class Moodle.mod_quizsbs.util.slot
 * @static
 */
Y.Moodle.mod_quizsbs.util.slot = {
    CSS: {
        SLOT : 'slot',
    ...
    reorderSlots: function() {
        ...
        slots.each(function(slot) {
            ...
            if (!Y.Moodle.mod_quizsbs.util.page.getPageFromSlot(slot)) {
                // Move the next page to the front.
                var nextpage = slot.next(Y.Moodle.mod_quizsbs.util.page.SELECTORS.PAGE);
                slot.swap(nextpage);
            }
    ...
    updateOneSlotSections: function() {
        Y.all('.mod-quizsbs-edit-content ul.slots li.section').each(function(section) {
    ...
    remove: function(slot) {
        var page = Y.Moodle.mod_quizsbs.util.page.getPageFromSlot(slot);
        slot.remove();
        // Is the page empty.
        if (!Y.Moodle.mod_quizsbs.util.page.isEmpty(page)) {
            return;
        }
        // If so remove it. Including add menu and page break.
        Y.Moodle.mod_quizsbs.util.page.remove(page);
    },
    ...
    addPageBreak: function(slot) {
        var nodetext = M.mod_quizsbs.resource_toolbox.get('config').addpageiconhtml;
    ...
    reorderPageBreaks: function() {
        ...
        slots.each(function(slot, key) {
            ...
            var action = '', iconname = '';
            if (Y.Moodle.mod_quizsbs.util.page.isPage(nextitem)) {
            ...
            pagebreaklink.set('title', M.util.get_string(action, 'quizsbs'));
            pagebreaklink.setData('action', action);
            // Update the image title.
            var icon = pagebreaklink.one(this.SELECTORS.ICON);
            icon.set('title', M.util.get_string(action, 'quizsbs'));
            icon.set('alt', M.util.get_string(action, 'quizsbs'));
    ...
    updateDependencyIcon: function(slot, requiresprevious) {
        ...
        if (requiresprevious) {
            link.set('title', M.util.get_string('questiondependencyremove', 'quizsbs', a));
            link.setData('action', 'removedependency');
            icon.set('alt', M.util.get_string('questiondependsonprevious', 'quizsbs'));
            icon.set('src', M.util.image_url('t/locked', 'moodle'));
        } else {
            link.set('title', M.util.get_string('questiondependencyadd', 'quizsbs', a));
            link.setData('action', 'adddependency');
            icon.set('alt', M.util.get_string('questiondependencyfree', 'quizsbs'));
...
}, '@VERSION@', {"requires": ["node", "moodle-mod_quizsbs-util-base"]});
