...
define(['report_moereports/handsontable', 'jquery', 'core/ajax', 'core/notification'],function(Handsontable, $, ajax, notification){
	...
	Report.prototype.init = function(report,schools) {
		...
		var hot = new Handsontable($('#reportstable')[0], {
		    ...
		    colHeaders: ['ID',
		        ...
		        M.util.get_string('class', 'report_moereports'),
		        M.util.get_string('studentsnumber', 'report_moereports'),
		    ],
		    contextMenu: true,
		    columns: [
		              {
	...
	$("#savereporttable").click(function () {
		...
		hot.validateCells(function (valid) {
			if(valid) {
				var call = ajax.call([
						{methodname: 'report_moereports_classes', args: { classes: hot.getData(), del: deleted }}]
				);
